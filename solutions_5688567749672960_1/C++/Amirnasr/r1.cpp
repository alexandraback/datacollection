...
int R, C, N;
int dp[(1<<16) + 5];
int get(int msk, int i, int j)
{
	if (i < 0 || i >= R || j < 0 || j >= C) return 0;
	int in = i*C+j;
	if (msk&(1<<in)) return 1;
	else return 0;
}
int sol(int msk)
{
	...
	for (int i = 0; i < R; i++)
	{
		for (int j = 0; j < C; j++)
		{
			if (get(msk, i, j))
			{
				val += get(msk, i+1, j) + get(msk, i, j-1) + get(msk, i, j+1) + get(msk, i-1, j);
			}
		}
	}
	val /=2;
...
long long sol(long long N)
{
	...
	int dig2 = dig-(dig/2);
	long long pw = pow(10ll, dig2);
	long long r2 = rev(N/pw);
	long long r1 = N%pw + r2;
	//cout<<N<<" "<<pw<<" "<<N%pw<<" "<<N/pw<<endl;
	if (r2 <= 1) r1--;
	return r1 + sol(pow(10, dig-1));
}
