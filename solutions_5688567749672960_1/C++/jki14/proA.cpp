...
inline lld reve(const lld x){
	char buf[32];
	sprintf(buf,"%lld",x);
	int len=strlen(buf);
	for(int i=len-1;i>len-i-1;i--)SWAP(buf[i],buf[len-i-1]);
	if(buf[0]=='0')return inf;
	lld y;sscanf(buf,"%lld",&y);
	return y;
}

inline lld fund(const lld x){
	char buf[32];
	sprintf(buf,"%lld",x);
	int len=strlen(buf);
	int hlf=len>>1;
	buf[len-1]='1';
	for(int i=len-2;i>=hlf;i--)buf[i]='0';
	lld y;sscanf(buf,"%lld",&y);
	return y;
}

inline lld call(const lld x){
	if(x==1LL)return 1LL;
	lld buf_fund=fund(x);
	if(buf_fund<x){
		//printf("f[%lld]=%lld\n",x,call(buf_fund)+x-buf_fund);
		return call(buf_fund)+x-buf_fund;
	}
	lld buf_reve=reve(x);
	if(buf_reve<x){
		//printf("f[%lld]=%lld\n",x,call(buf_fund)+1LL;
		return call(buf_reve)+1LL;
	}
	return call(x-1LL)+1LL;
}
