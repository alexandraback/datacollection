...
int main(int argc, char **argv)
{
	...
	for(int it_t = 1; it_t <= in_t; it_t++)
	{
		...
		long long in_c = readHugeInt();
		long long in_d = readHugeInt();
        long long in_v = readHugeInt();
        long long d_arr[200];
        for(int it_i = 0; it_i < in_d; it_i++)
        {
            d_arr[it_i] = readHugeInt();
        }
        long long o = 0;
        long long r = 0;
        auto range_update = [&](long long n_d){
            r = in_c * n_d + r;
        };
        sort(d_arr, d_arr+in_d);
        for(int it_i = 0; it_i < in_d; it_i++)
        {
            if(r >= in_v)
            {
