	...
	public static void main(String[] args) {
		...
		try {
			...
			for(int testCase=1; testCase<=numTestCase; testCase++)
			{
				...
				if(!dList.isEmpty() && dList.get(0) == 1)
				{
					newList.add(dList.remove(0));
				}
				else
				{
					newList.add((long)1);
				...
				long sumFromAll = sum(newList)*c;
				if(!dList.isEmpty())
				{
					Long minFromDx = dList.get(0);
					while(sumFromAll+1 >= minFromDx){
						Long minTemp = dList.remove(0);
						newList.add(minTemp);
						sumFromAll += minTemp*c;
						if(dList.isEmpty())
						{
							break;
						}
						...
						{
							minFromDx = dList.get(0);
						}
					}
				}
				while(sumFromAll < v)
				{
					long newD = sumFromAll+1;
					newList.add(newD);
					ansl++;
					sumFromAll += newD*c;
					if(!dList.isEmpty())
