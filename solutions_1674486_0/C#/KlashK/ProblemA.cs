...
namespace CodeJam
{
    class ProblemA
    {
        static void Main(string[] args)
        {
            ...
            for (var i = 0; i < cases; i++)
            {
                ...
                for (var j = 0; j < classes; j++)
                {
                    ...
                    GetInherits(j, graph, inherits, classes);

                    if(HasDiamond(inherits))
                    {
                        hasDiam = true;
                        break;
                    }
        ...
        private static void GetInherits(int klass, bool[,] graph, Dictionary<int, int> inherits, int classes)
        {
            for (var i = 0; i < classes; i++)
            {
                if (graph[klass, i])
                {
                    if (inherits.ContainsKey(i))
                        inherits[i] = inherits[i] + 1;
                    else
                        inherits.Add(i, 1);
                    GetInherits(i, graph, inherits, classes);
                }
            }
        }
    }
