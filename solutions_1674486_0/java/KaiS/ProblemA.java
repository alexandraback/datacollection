...
public class ProblemA {
	...
	public static void main(String[] args) {
		...
		problemA.processFile("in/round1c/A-small-attempt2.in", "out/round1c/A-small.txt");
	...
	private boolean calcInheritance(Set<Node> nodes) {
		...
		for (Node n : nodes) {
			if(n.edges.size() > 1) {
				List<Node>[] paths = new List[n.edges.size()];
				i = 0;
				for (Node p : n.edges) {
					paths[i] = new Vector<Node>();
	...
	private List<Node> calcPath(List<Node> path, Node n) {
