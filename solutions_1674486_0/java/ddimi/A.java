...
public class A {
	...
	private static void solveCase(int caseNum) throws IOException {
		...
		List<Integer>[] parents = (List<Integer>[]) new List[n];
		for (int i=0; i<n; i++) {
			tokens = io.readNextLine();
			int m = Integer.parseInt(tokens[0]);
			parents[i] = new ArrayList<Integer>(m);
			for (int j=0; j<m; j++) {
				parents[i].add(Integer.parseInt(tokens[j+1]) - 1);
			}
		}
		...
		for (Integer leaf : leaves) {
			if (hasDiamond = hasDiamond(parents, leaf)) {
				break;
			}
		}
		
		// Print results
		io.printCase(caseNum).println(hasDiamond ? "Yes" : "No");
	}

	private static boolean hasChildren(List<Integer>[] parents, int node) {
	...
	private static boolean hasDiamond(List<Integer>[] parents, int leaf) {
		...
		while (!toCheck.isEmpty()) {
			int next = toCheck.remove(0);
//			io.debug("Checking: " + next);
			if (visited.contains(next)) {
				hasDiamond = true;
				break;
			}
			visited.add(next);
			for (Integer parent : parents[next]) {
				toCheck.add(parent);
			}
		}
