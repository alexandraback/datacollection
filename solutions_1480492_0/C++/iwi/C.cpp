...
void check() {
  // rep (i, G) {rep (j, G) debug("%d", adj[i][j]);debug("\n"); }

  memset(col, -1, sizeof(col));
  rep (i, G) if (col[i] == -1) search(i, 0);
}

void join(int a, int b) {  // a -> b
  adj[a][b] = adj[b][a] = 2;
}
...
void solve() {
  ...
  rep (i, N) rep (j, i) {
    ...
    // P[a] <= P[b]
    if (P[b] < P[a] + 5) {
      // if (P[b] + P[a] == 5 && S[a] <= S[b]) continue;  // T=0 special!!
      join(a, b);
    }
  }
  ...
  rep (i, N) rep (j, N) {
    ...
    rep (k, 2) {
      ...
      assert(t >= -EPS);
      if (fabs(t) < EPS && k == 0) continue;  // T=0 special!!
      // debug("%d -> %d\n", i, j);
      evt.pb(mp(mp(t + (k == 0 ? -EPS : +EPS), k),
                k == 1 ? mp(i, j) : mp(j, i)));
    }
  }
  sort(all(evt));
