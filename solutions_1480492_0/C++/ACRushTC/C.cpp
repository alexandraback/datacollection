...
bool refine(bool f[],double t)
{
	double x[6];
	bool g[6][6];
	for (int i=0;i<n;i++) x[i]=p[i]+t*v[i];
	memset(g,false,sizeof(g));
	for (int i=0;i<n;i++) for (int j=0;j<n;j++) if (i!=j && abs(x[i]-x[j])<W-eps) g[i][j]=true;
	for (int set=0;set<two(n);set++) if (f[set])
		for (int i=0;i<n;i++) for (int j=i+1;j<n;j++) if (g[i][j] && contain(set,i)==contain(set,j)) 
			f[set]=false;
	for (int set=0;set<two(n);set++) if (f[set]) return true;
	return false;
}
void calc(bool f[],double t)
{
	double x[6];
	bool g[6][6];
	for (int i=0;i<n;i++) x[i]=p[i]+t*v[i];
	...
	while (1)
	{
		bool ok=true;
		for (int set=0;set<two(n);set++) if (f[set])
			for (int i=0;i<n;i++) if (!f[set^two(i)])
			{
				bool flag=true;
				for (int j=0;j<n;j++) if (i!=j && g[i][j] && contain(set,i)!=contain(set,j)) flag=false;
				if (flag) { f[set^two(i)]=true; ok=false; }
			}
		if (ok) break;
	}	
}
double solve()
{
	...
	pts.resize(pts_cnt);
	int set=0;
	for (int i=0;i<n;i++) if (s[i][0]=='R') set|=two(i);
	memset(f,false,sizeof(f));
	f[set]=true;
	for (int i=0;i<SIZE(pts);i++)
	{
		calc(f,pts[i]);
		if (i+1<SIZE(pts) && !refine(f,(pts[i]+pts[i+1])/2)) return pts[i];
	}
	return 1e100;
}
int main()
