...
def GenerateLargeInput(Tmax, Nmax):
	print str(Tmax)
	for c in xrange(Tmax):
		print str(Nmax)
		n = [random.random() for _ in xrange(Nmax)]
		k = [random.random() for _ in xrange(Nmax)]
		print " ".join(map(str, n))
		print " ".join(map(str, k))


def main(argv):
	#GenerateLargeInput(50,1000)
	#return 0
	# Wow, python can actually solve the large input, I am surprised
