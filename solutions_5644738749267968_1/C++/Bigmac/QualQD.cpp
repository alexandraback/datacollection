...
int main(int argc, char **argv)
{
	...
	for(int it_t = 1; it_t <= in_t; it_t++)
	{
		...
		for(int i =0; i < N; i++)
		{
			Naomi.push_back(readDouble());
		}
        for(int i =0; i < N; i++)
		{
			Ken.push_back(readDouble());
		}
		sort(Naomi.begin(), Naomi.end());
		sort(Ken.begin(), Ken.end());
		
        //===========================================solve function begin================================//
		auto solve = [&](deque<double> _N, deque<double> _K) -> int{
			int ret = 0;
			for(int i = 0; i < N; i++)
			{
				if(_K.front() > _N.front())
				{
					_K.pop_back();
					_N.pop_front();
				}
        ...
        //===========================================solve function  end ================================//
		int y = solve(Naomi,Ken);
		int z = N - solve(Ken,Naomi);
		fprintf(fo, "Case #%d: %d %d", it_t, y, z);
		fprintf(fo, "\n");
	}
	closeIOFile();
}