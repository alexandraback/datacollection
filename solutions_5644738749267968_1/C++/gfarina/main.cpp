...
bool finito(ss a, ss b) {
    dbg_assert(a.size() == b.size());
    
    while (a.size() && *a.begin() > *b.begin()) {
        a.erase(a.begin());
        b.erase(b.begin());
    }
...
void testcase(int testnum) {
    ...
    cout << "Case #" << testnum << ": " << sleale(a, b) << " " << leale(a, b) << "\n";
