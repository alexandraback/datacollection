...
int BFB2(const std::vector<double>& aN, const std::vector<double>& aK) {
  std::vector<double> aW = aN;
  std::vector<int> NI, KI;
  NI.resize(aN.size());
  KI.resize(aN.size());
  aW.insert(aW.end(), aK.begin(), aK.end());
  std::sort(aW.begin(), aW.end());
  for (int i=0; i<(int)aN.size(); i++) {
    NI[i] = std::distance(aW.begin(), std::find(aW.begin(), aW.end(), aN[i]));
    KI[i] = std::distance(aW.begin(), std::find(aW.begin(), aW.end(), aK[i]));
  }
  int kLow = 0;
  int kHi = (int)aK.size()-1;
  int p = 0;
  for (int i=0; i<(int)aN.size(); i++) {
    if (NI[i]>KI[kLow]) {
      p++;
...
//  std::cout << "\n";
//  return 0;
  const std::string KFileName = "D-large.in";
