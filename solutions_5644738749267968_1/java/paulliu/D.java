...
class Main {
	...
	for (int t=0; t<T; t++) {
		...
		try {
		    Naomi[i] = Double.parseDouble(com1);
		} catch (Exception e) {
		    return ret;
		}
	    }
     ...
     */
    private void solve(double[] Naomi, double[] Ken) {
	Arrays.sort(Naomi);
	Arrays.sort(Ken);
	int Naomi_left=0;
	int Naomi_right=Naomi.length-1;
	int Ken_left=0;
	int Ken_right=Ken.length-1;
	int d_point=0;
	...
	/* Deceitful War */
	while (Naomi_left <= Naomi_right) {
	    if (Naomi[Naomi_left] < Ken[Ken_left]) {
		Naomi_left++;
		Ken_right--;
	    } else {
		d_point++;
	...
	Naomi_left=0;
	Naomi_right=Naomi.length-1;
	Ken_left=0;
	Ken_right=Ken.length-1;
	int point=0;
	/* War */
	while (Naomi_left <= Naomi_right) {
	    if (Naomi[Naomi_right] > Ken[Ken_right]) {
		point++;
		Naomi_right--;
		Ken_left++;
	    } else {
     ...
     * Output the results
     */
    private void output(int d_point, int point) {
	output_N++;
	System.out.format("Case #%1$d: %2$d %3$d%n",output_N, d_point, point);
    }
	...
	if (logger != null) {
		...
		sb1.append(Double.toString(a[i]));
	    }
