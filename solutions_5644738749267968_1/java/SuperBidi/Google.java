...
public class Google
{
    ...
    {
        Vector<Float> naomiV = new Vector<Float>();
        Vector<Float> kenV = new Vector<Float>();

        for (int i = 0; i < Integer.parseInt(input[0][0]); i ++)
        {
            naomiV.add(Float.parseFloat(input[1][i]));
            kenV.add(Float.parseFloat(input[2][i]));
        }
        Collections.sort(naomiV);
        Collections.sort(kenV);

        Float[] naomi = new Float[Integer.parseInt(input[0][0])];
        Float[] ken = new Float[Integer.parseInt(input[0][0])];
        naomiV.toArray(naomi);
        kenV.toArray(ken);

        int deceitfulWarPoints = 0;
        int minimumPlayedByKen = 0;
        for (int i = 0; i < Integer.parseInt(input[0][0]); i ++)
        {
            if (naomi[i] > ken[minimumPlayedByKen])
            {
                minimumPlayedByKen++;
                deceitfulWarPoints++;
            }
        ...
        int warPoints = 0;
        minimumPlayedByKen = 0;
        for (int i = 0; i < Integer.parseInt(input[0][0]); i ++)
        {
            for (int j = minimumPlayedByKen; j < Integer.parseInt(input[0][0]); j ++)
            {
                minimumPlayedByKen++;
                if (naomi[i] < ken[j])
                    break;
        ...
        return outputCase(output, deceitfulWarPoints + " " + warPoints);
    }
