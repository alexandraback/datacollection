    ...
    class Program
    {
        static void Main(string[] args)
        {
            ...
            for (int i = 0; i < T; i++)
            {
                ...
                var result = GetResult(A, sortedMotes);
                // Write Output
        ...
        static long GetResult(long A, List<long> sortedMotes)
        {
            ...
            if (A <= sortedMotes[0])
            {
                var result = 1 + GetResult(A + A - 1, sortedMotes);
            ...
            {
                var result = GetResult(A + sortedMotes[0], sortedMotes.GetRange(1, sortedMotes.Count - 1));
                if (result < bestResult)
                {
                    bestResult = result;
                }
