...
namespace C
{
    class Program
    {
        ...
        static ulong IsNoPrime(ulong u)
        {
        ...
        static ulong[] IsJamCoin(ulong coin)
        {
            ulong proof2 = IsNoPrime(coin);
            if (proof2 == 0) return null;

            ulong[] proof = new ulong[10];
            proof[1] = proof2;
            ...
            {
                coin = bits.Aggregate((a,b) => i * a + b);
                proof[i - 1] = IsNoPrime(coin);
                if (proof[i - 1] == 0) return null;
                if (i == 10) proof[0] = coin;
            }
