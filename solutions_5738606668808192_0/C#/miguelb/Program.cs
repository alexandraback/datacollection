...
namespace CodeJam
{
    ...
    class Problem : CodeJamProblemC
    {
        public Problem () : base()
        {
            primes = PrimesLowerThan(1000000);
        }
        ...
        BigInteger ConvertFromBase ( string number , int b)
        {
            BigInteger n = 0;
            BigInteger aux = 1;
        ...
        string[] Divisors ( string number)
        {
            List<string> divisors = new List<string>();
            for ( int b = 2 ; b<= 10; b++)
            {
                BigInteger n = ConvertFromBase(number, b);
                bool found = false;
        ...
        protected override string SolveSmall()
        {
            ...
            {
                var s = "1" + ConvertToBase(i, 2).PadLeft ( N - 2 , '0' ) + "1";
            
                
                var divisors = Divisors(s);
                if ( divisors != null)
                {
                    result.AppendFormat("\r\n{0}", s);
                    foreach (var divisor in divisors)
                        result.AppendFormat(" {0}", divisor);
                    count++;
                    if (count == J)
