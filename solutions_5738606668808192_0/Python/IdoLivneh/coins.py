...
def calc_result(case):
	...
	count = 0
	for prod in itertools.product(*([(0,1)] * (N-2))):
		flag = True
		print prod, 
		for b in range(2, 11):
			if is_prime(prod_to_value(prod, b)):
				flag = False
				break
		if not flag:
			continue
		print 'yey'
		count += 1
		prod_string = '1%s1' % ''.join([str(i) for i in prod])
		denominators = []
		for b in range(2, 11):
			denominators.append(str(get_denominator(prod_to_value(prod, b))))
