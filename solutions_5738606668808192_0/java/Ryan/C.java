...
public class C{
  ...
  private static int generate(int digits){
    int ret=1,total=2;
  ...
  private static boolean isFactor(int coin,int base,int factor){
    int sum=0,digit=1;
    while(coin>0){
      if(coin%2==1){
  ...
  private static int factorize(int coin,int base){
  ...
  private static int[] produceJamcoin(int digits){
    while(true){
      int[] jamcoin=new int[11];
      int coin=generate(digits);
      ...
      for(int base=2;base<jamcoin.length;base++){
        if(jamcoin[base]!=0){
          if(!isFactor(coin,base,jamcoin[base]))
  ...
  private static String print(int coin){
    String s="";
    while(coin>0){
      s=(coin%2==0?"0":"1")+s;
      coin/=2;
  ...
  private static void solve(int digits,int count){
    for(int i=0;i<count;i++){
      int[] jamcoin=produceJamcoin(digits);
