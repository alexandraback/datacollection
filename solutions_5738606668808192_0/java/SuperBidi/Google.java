...
public class Google
{
    ...
    {
        long digits = Long.parseLong(input[0][0]);
        number = Long.parseLong(input[0][1]);

        outputCase(output, "");

        String s = "1001";
        for (int i = 8; i < digits; i ++)
            s = s + "0";
        s = s + "1001";

        recursiveFunction(s, 1, output);

        return true;
    }

    public static void recursiveFunction(String s, int position, PrintWriter output)
    {
        if (position >= s.length() - 4)
            return;

        if (s.charAt(position) == '0' && s.charAt(position + 3) == '0')
        {
            number --;
            if (number >= 0)
            {
                String S2 = s.substring(0, position) + "1" + s.substring(position + 1, position + 3) + "1" + s.substring(position + 4);
                output.println(S2 + " 3 7 5 6 31 8 27 5 77");
                recursiveFunction(S2, position + 1, output);
            }
        }

        recursiveFunction(s, position + 1, output);
    }
