...
public class C {
	...
	// making this table larger allows find more jamcoins
	...
	Optional<Long> getDivisor(long number) {
		Optional<Long> ret = null;
		List<Long> primes = this.table.getTable();
		long maxPrime = (long) Math.sqrt((double) number);
		long lastPrime = primes.get(0);
		boolean searching = true;
		Iterator<Long> it = primes.iterator();
		for (int k = 0; searching && (k < primes.size()) && (lastPrime < maxPrime); k++) {
			long prime = it.next();
			if ((number % prime) == 0) {
	...
	Optional<List<Long>> getDivisors(String str) {
		...
		for (int base = this.firstBase; isCandidate && base < (this.lastBase + 1); base += 1) {
			Long number = Long.valueOf(str, base);
