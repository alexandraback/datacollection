		...
		try {
			...
			while(found < J) {
				...
				if(divisors != null) {
					...
					//	long value = expandCandidate(candidate, radix);
					//	long divisor = divisors[radix - 2];
					//	boolean isOK = value % divisor == 0l;
					//	debug((isOK ? "[ OK ]" : "[fail]") + " radix=" + radix + " value=" + value + " divisor=" + divisor);
					//	if(!isOK) {
