...
public class MainProgram {
	...
	public static void solve(int k, PrintWriter writer) {
		...
		if (d < j) {
			...
			if (f == false) {
				writer.print("1");
				for (int i = 0; i < value.length; i++) writer.printf("%d", value[i]);
				writer.print("1 ");
				for (int i = 2; i <= 10; i++) writer.printf("%d ", devide[i]);
				writer.println();
				d++;
			}
		}
...
String newstr = "";
for (int u = 0; u< str.length();) {
	int v = u+1;
	while (v < str.length() && str.charAt(v) == str.charAt(u)) v++;
	newstr = newstr + str.charAt(u);
	u = v;
}

int[] l = new int[newstr.length()];
if (newstr.charAt(0) == '-') l[0] = 1;
else l[0] = 0;
for (int u = 1; u < newstr.length(); u++) {
	if (newstr.charAt(u) == '-') l[u] = l[u-1] +2;
	else l[u] = l[u-1];
}*/
