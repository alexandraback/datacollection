...
#endif
const size_t j_size = 20814;

unsigned long* get_jams();

uint32_t** get_args();

unsigned long get_num_with_len(int n){
     return 1uL<<(n-1);
}

size_t get_first_index(int len){
    unsigned long* jams = get_jams();
    unsigned long min = get_num_with_len(len);
    return find_if(jams, jams+j_size,[&min](unsigned long & i){ return i>=min;})-jams;
}

string get_bits(unsigned long v){
    string res="";
    while (v>0){
        if (v&1){
            res = "1"+res;
...
int main()
{
    ...
    for (int i = 0; i < test_count; ++i)
    {
        ...
        results <<"Case #1:\n"
                         ...
                         "1000000010111001 31 2 3 2 1373 2 11 2 3\n"
                         "1000000010111011 3 7 13 3 31 5 3 73 7\n"
                         "1000000011000001 3 2 5 2 7 2 3 2 11";
            break;
        }
        size_t begin = get_first_index(N), end = begin + J;
        for(size_t i = begin;i<end;i++){
            results<<get_bits(get_jams()[i]);
            for (int j = 0; j< 9; j++){
                results<<" "<<get_args()[i][j];
            }
            results<<"\n";
        }
    }
