...
bool isprime(LL q)
{
    ...
    {
        if(q<=prime[i]) return true;
        if(q%prime[i]==0) return false;
        if(prime[i]*prime[i]>q) return true;
    }
}
...
LL getdiv(LL q)
{
    for(int i=0;i<prime.size();i++)
    {
        if(q<=prime[i]) return -1;
        if(q%prime[i]==0) return prime[i];
        if(prime[i]*prime[i]>q) return -1;
    }
}
