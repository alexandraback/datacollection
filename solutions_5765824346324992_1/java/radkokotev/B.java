...
public class B {
	...
	public static long binsearch(long arr[], long n) {
		...
		while (lo < hi - 1) {
			long mid = lo + (hi - lo) / 2;
			val = 0;
			for (long l : arr) val += Math.ceil((mid* 1.0) / l);
//			System.out.println("bin search val = " + val + " (" + lo + "," + mid + "," + hi + ")");
			if (val < n) {
				lo = mid;
			} else {
				hi = mid;
...
//		System.out.println("bin search val = " + val);
		return lo;
	}
	
	public static int getIndex(long arr[], long n, int b) {
//		System.out.println("About to bin search");
		long t = binsearch(arr, n);
//		System.out.println("Bin search complete " + t);
		for (long l : arr) n -= Math.ceil(t * 1.0 / l);
		long newarr[] = new long[b];
		for (int i = 0; i < newarr.length; i++) {
			newarr[i] = arr[i] - t % arr[i];
			if (newarr[i] == arr[i]) {
				newarr[i] = 0;
			}
		}
...
//			System.out.println(Arrays.toString(newarr) + " " + i);
	...
	public static void main(String[] args) throws IOException {
		BufferedReader in = new BufferedReader(new FileReader("B-large (1).in"));
