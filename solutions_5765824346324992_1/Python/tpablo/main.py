...
class Test(object):
  ...
  def read(self, reader):
    self.B = reader.get(int)
    self.N = reader.get(int)
    self.M = [reader.get(int) for _ in range(self.B)]
  
  def t2n(self, t):
    return sum(t // b + 1 for b in self.M)
  
  def ok(self, t):
    return self.t2n(t) >= self.N and (t == 0 or self.t2n(t-1) < self.N)
  
  def solve(self, handle):
    handle.write('Case #%d: ' % self.case)
    
    tMin = 0
    tMax = max(self.M) * self.N
    
    t = None
    if self.ok(tMin): t = tMin
    if self.ok(tMax): t = tMax
    
    while t == None:
      th = (tMin + tMax) // 2
      if self.ok(th): t = th
      if self.t2n(th) >= self.N: tMax = th
      else: tMin = th
