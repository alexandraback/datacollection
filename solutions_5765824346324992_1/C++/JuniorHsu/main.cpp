...
int main()
{
	...
	input>>total_case; 
	ll I,H,K, lb, ub, B, N, m[1000],done, ava, ans;
	for(case_id=1;case_id<=total_case;case_id++)
	{   
        input>>B>>N;
        for(I=0;I<B;I++) input>>m[I];
        ...
        while(ub-lb>0) {
           ...
           // cout<<"mid="<<mid<<" "<<done<<" "<<ava<<endl;
            if(N>done && N<=done + ava) {
                ll c = N-done;
                for(I=0;I<B;I++) {
                    if((mid%m[I]) ==0) {
                        c--;
                ...
                cout<<"error: "<<case_id<<endl;
            }
            
            N>done ?  lb = mid : ub = mid ;
        }
        cout<<"not found:"<<case_id<<endl;
ANS:
		output<<"Case #"<<case_id<<": ";
		output<<ans;
		output<<endl;
