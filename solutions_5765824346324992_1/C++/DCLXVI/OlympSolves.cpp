...
ll addinf(ll x, ll y)
{
	if (x + y >= 1000000000000000000LL)
		return 1000000000000000000LL;
	return x + y;
}

ll getProcess(vi &v, ll t)
{
	ll sum = 0;
	foriv sum = addinf(sum, divup(t, v[i]));
	return sum;
}

int solve(vi v, int number)
{
	ll step = (1LL << 60);
	ll cur = 0;
	while (step)
	{
		ll test = cur + step;
		if (getProcess(v, test) < number)
			cur = test;
		step /= 2;
	}

	vi free_barbers;
	foriv if (divup(cur+1, v[i]) != divup(cur, v[i])) free_barbers.push_back(i + 1);
	number -= getProcess(v, cur);
	return free_barbers[number - 1];
