    ...
    class Program
    {
        ...
        private void Go()
        {
            ...
            for (int t = 0; t < T; t++)
            {
                int A = m_io.ReadInt();
                int B = m_io.ReadInt();
                double[] p = Enumerable.Range(0, A).Select(i => m_io.ReadDouble()).ToArray();

                double evEnterRightAway = 2 + B;

                double minEv = evEnterRightAway;
                double pCompletelyCorrect = 1.0;
                for (int i = 0; i < A; i++)
                {
                    double backSpaceCost = A - i;
                    double typedToEndFromHere = (backSpaceCost*2 + B-A + 1);
                    minEv = Math.Min(minEv, typedToEndFromHere * pCompletelyCorrect + (typedToEndFromHere + B + 1) * (1 - pCompletelyCorrect));

                    pCompletelyCorrect *= p[i];
                }

                minEv = Math.Min(minEv, (B-A + 1) * pCompletelyCorrect + ((B-A) + B + 2) * (1 - pCompletelyCorrect));

                m_io.WriteCase(minEv.ToString());
            }
        }
    }
