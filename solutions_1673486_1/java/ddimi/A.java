...
public class A {
	...
	private static void solveCase(int caseNum) throws IOException {
		...
		int charsTyped = Integer.parseInt(tokens[0]);
		int charsTotal = Integer.parseInt(tokens[1]);
		double[] p = new double[charsTyped];
		...
		// Do the work
		double[] pCorrect = new double[charsTyped+1];
		pCorrect[0] = 1;
		for (int i=1; i<=charsTyped; i++) {
			pCorrect[i] = pCorrect[i-1] * p[i-1];
		}
		
		double minKeystrokes = Double.MAX_VALUE;
		for (int i=0; i<=charsTyped; i++) {
			minKeystrokes = Math.min(
					minKeystrokes,
					(i + charsTotal - (charsTyped-i) + 1) + (charsTotal+1) * (1-pCorrect[charsTyped-i]));
		}
		if (charsTyped == charsTotal) {
			minKeystrokes = Math.min(minKeystrokes, 1 + (charsTotal+1) * (1-pCorrect[charsTyped]));
		} else {
			minKeystrokes = Math.min(minKeystrokes, charsTotal+2);
		}
		
		// Print results
		io.printCase(caseNum).println(String.format("%.6f", minKeystrokes));

	}

	/**
	 * Input and Output A style
	 *
	 */
