...
int T, A, B;
double p1[MaxA], p2[MaxA], Cnt[MaxA];
...
inline void Solve ( void )
{
    ...
    for ( int TestCase = 0; TestCase < T; TestCase++ )
    {
        ...
        for ( int i = 0; i < A; i++ ) P0 *= p1[i];

        Cnt[0] = P0 * ( B - A + 1 ) + ( 1.000000 - P0 ) * ( B - A + 1 + B + 1 );
        ...
        for ( int i = 0; i < A; i++ )
        {
            ...
            Cnt[i + 1] = A - i + P * ( B - i + 1 ) + ( 1.000000 - P ) * ( B - i + 1 + B + 1 );
        ...
        for ( int i = 0; i < A + 2; i++ ) if ( Cnt[i] < Sol ) Sol = Cnt[i];
