...
for ( int i = 1 ; i <= got ; i ++ )
   {
   pgood[i] = pgood[i-1] * p[i-1] ;
   }

double best = total+2 ; // enter, retype, enter

for ( int bs = 0 ; bs <= got ; bs ++ )
   {
   double exp = ( bs+bs+(total-got) + 1 ) ;
   exp += (total+1) * (1.0-pgood[got-bs]) ;
