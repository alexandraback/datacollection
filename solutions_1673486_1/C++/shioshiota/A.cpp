...
public:
  ...
  //次合うべき場所、長さ、次打つべき場所
  Data(int cPos, int len, int pos, long double prob, vector<long double> &tP){
    //すべてあってる場合
    if(cPos == pos){
	...
	BS.push_back((prob-tP[newPos]) * (long double)(len-newPos+1+i) + (tP[newPos]) * (long double)(len+1+len-newPos+1 + i));
      }
      enter = prob * (long double)(len+2);
    }
  }
...
int main(){
  ...
  rep(tc, T){
    ...
    int pos, len;
    cin >> pos >> len;
    vector<long double> P(pos), tP;
    rep(i, pos)cin >> P[i];
    Data res = Data(pos);
    long double CP = 0, ICP = 0, pre = 0;
    tP.push_back(0);
    rep(i, pos){
      
      //correct
      if(i+1 == pos){
	//res = res + Data(i+1, len, pos, prob*(P[i]));
	CP = prob*P[i];
      }
      //incorrect
      //res = res + Data(i, len, pos, prob*(1.0 - P[i]));
      ICP += prob*(1.0 - P[i]);
      tP.push_back(pre + prob*(1.0-P[i]));
      pre = tP.back();
      prob *= P[i];
    }
    res = res + Data(pos, len, pos, CP, tP);
    res = res + Data(-1, len, pos, ICP, tP);
    printf("Case #%d: %.9Lf\n",tc+1, res.getMin());
  }
