...
void resuelva(){
	...
	*/
	double enter = B + 2.0;
	//keep
	double prob_sinfallo = 1.0;
	double sinfallo[A + 1 ];
	sinfallo[0] = 1.0;
	for(int i=1;i<=A;i++){
		sinfallo[i] = sinfallo[i-1] * P[i];
	}
	double keep = sinfallo[A] * (B - A + 1);
	keep += (1.0 - sinfallo[A]) * (B - A + 1.0 + B + 1.0);

	//DBG(keep);

	double res = min(enter,keep);
	double prob,tmp;	
	double siempre = B - A + 1.0;
	for(int i=1;i<=A;i++){
		prob = sinfallo[A-i];
		tmp = prob * (i + i + siempre);
		tmp += (1.0 - prob) * (i + i + siempre + B + 1.0);
