...
def calc_result_after_one_run(case, results):
	#global results
	#global intervals
	...
	B_sqrt_max = int(math.floor(math.sqrt(float(100000000000000))))
	if B_sqrt > B_sqrt_max:
	...
	count = 0
	for res in results:
		if res >= A_sqrt and res <= B_sqrt:
			count +=1
			print "\tfound: %d, %d" % (res, res * res)
...
def main(filepath):
	...
	with file('output.txt', 'wb') as f_out:
		...
		for case_index, case in iterate_cases_1lpc(filepath):
			...
			result = calc_result_after_one_run(case, results)
