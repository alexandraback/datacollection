...
int main(){
    ...
    /*for (; i<=100000000; i++) {
     ...
     long long int a,b;
     fscanf(f,"%lld %lld",&a,&b);
     long double as=sqrtl(a),bs=sqrtl(b);
     a=ceil(as);
     b=floor(bs);
     fprintf(r,"Case #%d: %lld\n",u,arr[b]-arr[a-1]);
     u++;
     }
     return 0;*/
}/*
