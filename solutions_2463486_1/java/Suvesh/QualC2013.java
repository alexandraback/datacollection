...
public class QualC2013 {
  ...
  {
    File file = new File("./suveshp-2013-qual-C-large-1.out");
    BufferedReader br;
      try {
        if (!file.exists()) {
          file.createNewFile();
        ...
        br = new BufferedReader(new FileReader("./C-large-1.in"));
        String line = br.readLine();
        int testCases = Integer.parseInt(line);
        for (int i = 0; i < testCases; i++) {
          line = br.readLine();
          String[] tokens = line.split(" ");
          bw.write("Case #" + (i+1) + ": " + numberOfFairAndSquares(new BigInteger(tokens[0]), new BigInteger(tokens[1])) + "\n");
        }
        bw.close();
        br.close();
      } catch (IOException e) {
