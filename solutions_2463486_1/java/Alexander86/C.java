...
public class C{
	static Scanner scan;
	static BigInteger upper = BigInteger.TEN.pow(10);
	...
	public static void main(String[] args) throws Exception{
		...
		for(int i = 1; i < 10; i++)bruteForce(BigInteger.valueOf(i), BigInteger.valueOf(i), BigInteger.TEN, 1);
	...
	private static void bruteForce(BigInteger front, BigInteger back,
		...
		for(int i = 0; i < 10; i++){
				bruteForce(front.multiply(BigInteger.TEN).add(BigInteger.valueOf(i)), 
							...
							mul.multiply(BigInteger.TEN), digits + 1);
		}
	}
}
