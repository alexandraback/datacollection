...
namespace TicTacToe
{
    class Program
    {
        ...
        {
            long range = (int) Math.Sqrt(100000000000000);
            ...
            IList<string> resultsOfCases = GetInputsFromFile("C-large-1.in");
        ...
        private static IList<string> GetInputsFromFile(string fileName)
        {
            ...
            {
                long lowerLimit = Int64.Parse(lines[i].Split(' ')[0]);
                long upperLimit = Int64.Parse(lines[i].Split(' ')[1]);
        ...
        private static int GetFairPalindrom(long lowerLimit, long upperLimit)
        {
