...
string fairp[1000000],sqrf[1000000];

int main(){
    int i,j,k,len;
    fairp[0] = "1", fairp[1] = "2", fairp[2] = "3", fairp[3] = "11", fairp[4] = "22";
    fairp[5] = "101", fairp[6] = "111",fairp[7] ="121", fairp[8] = "202",fairp[9] = "212";
    BigInt b;
    for(len = 4,i = 10;len <=51;len++){
        string now ;
        k = i;

        now = "0";
        while(now.length()<len - 2){
            now+="0";
        }
        now += "1";
        now = "1" + now;
     ...
     //   cout<<now<<endl;
        for(j = 0;j<k;j++){
            now = fairp[j];
            if(j == 2 || now.length()%2 != len%2 || (now.length()>1 && now[0]=='2')) continue;
            while(now.length()<len - 2){
                now+="0";
                now = "0" + now;
            }
    ...
    freopen("C-large-1.ans","w",stdout);
    //cout<<k<<endl;
    int t,cs = 1;
    string A,B;
    scanf("%d",&t);
    while(t--){
        ...
        j = upper_bound(sqrf,sqrf+k,B) - sqrf;
        i = lower_bound(sqrf,sqrf+k,A) - sqrf;
        printf("Case #%d: %d\n",cs++,j - i);
    }
