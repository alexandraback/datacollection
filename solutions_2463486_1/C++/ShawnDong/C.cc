...
bool check(LL x){
    ...
    for (int i = 0 ; i * 2 <= dig ; ++i) if (b[i] != b[dig - 1 - i]) return false;
    return true;
}
void init(){
    hash.clear();
    for (LL i = 1 ; i <= 2e7 ; ++i) if(check(i) && check(i * i)) hash.PB(i * i);
}
void solve(){
    LL l , r;
    RD(l , r);
    LL rr = r;
    r = lower_bound(ALL(hash) , r) - hash.begin();
    if (hash[r] > rr) r--;
    l = lower_bound(ALL(hash) , l) - hash.begin() - 1;
    OT(r - l);
