...
void dfs(int now) {
	if (now > n) {
		...
		//For(i,1,n / 2) swap(cur.a[i],cur.a[n + 1 - i]);
		For(i,1,n - 1) cur.a[n + n - i] = cur.a[i];
		cur.a[0] = n + n - 1;
		Big t = cur * cur;
		if (check(t)) cache[++Tot] = t; else {
			cerr << "FAIL" << endl;
		}
		if (rem < cur.a[n] * cur.a[n]) return ;
		For(i,n + 1,n + n) cur.a[i] = cur.a[n - (i - n) + 1];
		cur.a[0] = n + n;
		Big t2 = cur * cur;
		if (check(t2)) cache[++Tot] = t2; else {
			cerr << "FAIL" << endl;
		}
		return ;
	}
	For(i,(now == 1),9) {
		int t = i * i * (2 - (now == n));
		cur.a[now] = i; rem -= t; if (rem < 0) { rem += t; continue ; }
		dfs(now + 1); rem += t;
	}
}
...
int main() {
	...
	For(i,1,25) { //cerr << i << ' ' << Tot << endl;
		n = i; rem = 9;
		Fill(cur.a,0);
		dfs(1);
	}
	sort(cache + 1,cache + Tot + 1); //For(i,1,Tot) cache[i].Print();
	int T; cin >> T;
