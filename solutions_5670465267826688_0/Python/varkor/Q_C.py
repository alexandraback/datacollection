...
def quaternionMultiply (units, wanted):
	value = 1
...
def searchFor_ijk (string):
	split_1r = ""
	for split_1 in range(1, len(string) - 1):
		split_1r = split_1r + string[split_1 - 1]
		gah = reducem(split_1r)
		split_1r = gah[0] + ("-" if gah[1] else "")
		if quaternionMultiply(split_1r, 2) == 2:
			for split_2 in range(split_1 + 1, len(string)):
				if quaternionMultiply(string[split_1:split_2], 3) == 3 and quaternionMultiply(string[split_2:], 4) == 4:
					return True
	return False
...
for (number, line) in enumerate(inputData.read().splitlines()[1:]):
	...
	foundSol = True
	if quaternionMultiply(tstring, -1) != -1:
		foundSol = False
	else:
		foundSol = searchFor_ijk(string)
