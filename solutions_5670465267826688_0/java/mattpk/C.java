 ...
 Scanner in = new Scanner(new File("C-small-attempt1.in"));
   //Scanner in = new Scanner(System.in);
    ...
    for(int t =0; t<n; t++)
    {
      ...
      while(usedOnA<Math.min(len*12,len*repeat)) // we'll do upper limit 12 to try and find solution
      {
        if(current == 'i') break;
      ...
      if (current =='i')
      {
        ...
        usedAfter = current+"";
        while(usedOnB<Math.min(len*12,len*repeat))
        {
          if (current == 'k') break;
      ...
      if (worked)
      {
        ...
        if (midLen > 0)
        {
         // System.out.println("midLength: " +midLen+ " " + usedOnA + " " + usedOnB);
          ...
          if (newUsedAfter.length() >(int)(len*repeat-used.length()-newUsed.length()-usedAfter.length()))
