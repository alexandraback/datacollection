...
namespace Dijkstra
{
    ...
    internal class ProblemInstance {
        ...
        private readonly int replicationCount;

        public ProblemInstance(string snip, int replicationCount) {
            this.snip = snip;
            this.replicationCount = replicationCount;
        }

        public static ProblemInstance Parse(string xl, string snip) {
            return new ProblemInstance(snip, int.Parse(xl.Split(' ')[1]));
        }
        ...
        public bool ComputeSolution() {
            int remainder = replicationCount%4;
        ...
        public bool BruteForce() {
            ...
            var rollProd = ComputeRollingProduct(s);

            if (rollProd.Last() != Quaternion.MinusOne) {
                return false;
            }

            int firstI = rollProd.IndexOf(Quaternion.I);

            if (firstI == -1) {
                return false;
            }

            int firstJ = rollProd.IndexOf(Quaternion.K, firstI);
