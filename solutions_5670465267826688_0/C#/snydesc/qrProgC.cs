...
namespace qr2015ProgC
{
   ...
   class qrProgC
   {
      public const string cInputFile = "C:\\Dev\\2015\\Qualify\\qrC\\C-small-attempt1.in";
            ...
            using (StreamWriter writer = new StreamWriter(cOutputFile))
            {
               using (StreamReader reader = new StreamReader(cInputFile))
               {
                  ...
                  for (int testCaseNum = 0; testCaseNum < numberOfTestCases; testCaseNum++)
                  {
                     ...
                     int xValue = int.Parse(row1.Split(' ')[1]);
                     ...
                     {
                        sInputString = new String(Enumerable.Range(0, xValue).SelectMany(o => row2).ToArray());
