    ...
    public class Dijkstra
    {
        public void Process()
        {
            ...
            using (var textReader = File.OpenText("..\\..\\CodeJam\\2015Qualification\\inC.txt"))
            {
                ...
                {
                    var number = 0;
                    var p = textReader.ReadLine().Split(' ').Select(int.Parse).ToArray();
                    var L = p[0];
                    var X = p[1];
                    var str = textReader.ReadLine();

                    result.AppendLine(string.Format("Case #{0}: {1}", t, IsPossible(L, X, str) ? "YES" : "NO"));
                }
            }
        ...
        private bool IsPossible(int L, long X, string str)
        {
            bool strPositive;
