...
void solve(int ca){
  ...
  quaternion<int> ii(0,1,0,0);
  quaternion<int> jj(0,0,1,0);
  quaternion<int> kk(0,0,0,1);
  int idx = 0;
  quaternion<int> hoge[] = {ii, jj, quaternion<int>()};
  rep(i, x){
    rep(j, l){
      q *= quaternion<int>(0, in[j]=='i', in[j] == 'j', in[j] == 'k');
      if(hoge[idx] == q){
        ++idx;
