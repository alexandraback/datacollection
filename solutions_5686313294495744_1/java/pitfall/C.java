...
public class C implements Runnable{
	...
	// PLEASE!!!
	void solve() {
		IdMap<String> first = new IdMap<>();
		IdMap<String> second = new IdMap<>();
		int n = readInt();
		...
		Khun khun = new Khun(first.size(), second.size());
		for (int i=0;i<n;i++) {
			khun.add(first.getId(a[i]), second.getId(b[i]));
		}
		
		for (int i=0;i<first.size();i++) {
			Arrays.fill(khun.used, false);
			khun.work(i);
		}
		int match = 0;
		for (int i=0;i<khun.mt.length;i++) {
			if (khun.mt[i] != -1) match++;
		}
				
		int x = first.size() + second.size() - match;
		out.println(n - x);
