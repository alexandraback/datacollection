...
void cntDistinct() {
	...
	bool skip[1005];
	int totalUni = 0;
	FR(i, 0, n) skip[i] = false;
	FR(i, 0, n) {
		int first = 0;
		int second = 0;
		FR(j, 0, n) if (i != j) {
			if (s[0][i] == s[0][j]) first++;
			if (s[1][i] == s[1][j]) second++;
		}
	...
	FR(i, 0, n) if(!skip[i]){
		bool uni = true;
		FR(j, 0, n) if(j<i || skip[j]) if (s[0][i] == s[0][j]) uni = false;
		if (uni) dis[0]++;
		uni = true;
		FR(j, 0, n) if (j<i || skip[j]) if (s[1][i] == s[1][j]) uni = false;
		if (uni) dis[1]++;
	}
	printf(" %d", n - totalUni- max(dis[0], dis[1]));
	//printf("!!! %d %d %d %d", n, totalUni, dis[0], dis[1]); ////
}
void run(){
	int n;
	cin >> n;
