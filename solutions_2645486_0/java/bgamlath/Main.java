...
public class Main {
    ...
    static long get(int start, int end, long startEnergy, long minEndEnergy, long regain) {
        if (end < start) {
            return 0L;
        }
        if (start == end) {
            if (regain >= minEndEnergy) {
                ...
                return v[start] * startEnergy;
            } else {
                long t = minEndEnergy - regain;
                //System.out.printf("of activity %3d - %5d joules\n", start, (startEnergy - t));
                return v[start] * (startEnergy - t);
            }
        }
        ...
        if (R >= ss[maxIndex]) {
            ...
            res = maxGain * tt[maxIndex];
            res += get(start, maxIndex - 1, startEnergy, tt[maxIndex], R);
            res += get(maxIndex + 1, end, R, minEndEnergy, R);
        } else {
            //System.out.printf("of activity %3d - %5d joules\n", maxIndex, tt[maxIndex] - (ss[maxIndex] - R));
            res = maxGain * (tt[maxIndex] - (ss[maxIndex] - R));
            res += get(start, maxIndex - 1, startEnergy, tt[maxIndex], R);
            res += get(maxIndex + 1, end, ss[maxIndex], minEndEnergy, R);
        }
        return res;
    }
