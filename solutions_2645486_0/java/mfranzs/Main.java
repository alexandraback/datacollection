...
public class Main {
	...
	public static void main(String[] args) throws Exception {
		...
		f = new BufferedReader(new FileReader("in.in"));
		...
		for (int test = 0; test < Tests; test++) {
			...
			for(int i=0;i<N;i++){
				...
				if(distance==0){
					gain+=E*a[i];//Spend all
					E=R;
				}else{
					int spend=E-max+R*distance;
					System.out.println(E+" "+R+" "+spend+" "+distance);
					if(spend>0){
						if(spend<E){
							gain+=spend*a[i];
							E-=spend;
