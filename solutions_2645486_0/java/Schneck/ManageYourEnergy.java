...
public class ManageYourEnergy {
    ...
    long solve(int startEnergy, int start, int end, int endEnergy, long tot) {
        long max = 0;
        int maxIndex = 0;
        ...
        if(maxIndex==start) {
            if(start==end) return tot + v[maxIndex] * maxSpend;
            return tot + v[maxIndex] * maxSpend + solve(minAfter, start+1, end, endEnergy,0);
        }
        if(maxIndex==end) {
            return tot + v[maxIndex] * maxSpend + solve(startEnergy, start, end - 1, maxBefore,0);
        }
        return tot + v[maxIndex] * maxSpend + solve(startEnergy,start,maxIndex - 1,maxBefore,0)
