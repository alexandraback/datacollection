...
public class A12013B {
	...
	public static void main(String[] args) throws IOException {
		...
		while (testCases -- > 0){
			...
			r = nextInt();
			s = new int[nextInt()];
			for (int i = 0; i < s.length; i++) s[i]=nextInt();
			memo = new int[s.length][e+1];
			for (int i = 0; i < memo.length; i++) 
				Arrays.fill(memo[i], -1);
			writeln("Case #"+counter+": "+solve(0,e));
		}
		//-------------------------------------------------------------------------
		//closing up
		end();
	...
	static int e;
	static int[][] memo;
	public static int solve(int index , int en){
		if(en>e)en=e;
		if(index == s.length)return 0;
		if (memo[index][en]!=-1)return memo[index][en];
		int ret = -1;
		for (int i = 0; i <= en; i++) {
			ret = Math.max(ret, solve(index+1,en-i+r)+s[index]*i);
		}
		return memo[index][en]=ret;
	}
	
	
	public A12013B() throws IOException {
...
//		in = new BufferedReader(new FileReader("Template.in"));
		out = new PrintWriter("A12013B.out");
		//-------------------------------------------------------------------------
