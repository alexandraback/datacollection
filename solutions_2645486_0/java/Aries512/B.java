...
public class B {
	...
	static String inFile = B.class.getSimpleName()+"-small-attempt0.in";
	...
	public void solve() throws Exception {
		...
		nextCase: for(int testCase=1; testCase<=testCasesCnt; testCase++) {
			print("Case #"+testCase+": ");
			maxE = nextInt(); reg = nextInt(); int n = nextInt();
			v = nextArr(n);
			res = 0;
			test(maxE, 0, 0);
			println(res);
	...
	void test(int e, int pos, int resTmp) {
//		debug(e,pos,resTmp);
		res = max(res, resTmp);
		if (pos>=v.length) return;
		e = min(maxE, e+reg);
		for (int i=0; i<=e; ++i) {
			test(e-i, pos+1, resTmp+v[pos]*i);
		}
	}
	...
	public static void main(String[] args) throws Exception {
		...
		if (!toFile) {
			...
			String outPath = filesPath + B.class.getSimpleName()+".out";
