...
public class Round1A2013Q2 {
	...
	public static void main(String[] args) throws Exception {
		Scanner reader = new Scanner(new File("B-small-attempt0.in.txt"));
		PrintWriter pw = new PrintWriter(new FileWriter("Bsmall.txt"));
			...
			for(int i = 0; i < activities.length; i++) {
				if(i == 0) {
					...
					while(pointer < group) {
						...
						if(pointer < group - 1)
							maxgain += maxvalue * gain;
						else {
							int remain = max - ((group - 1) * gain);
							maxgain += remain * maxvalue;
						}
						pointer++;
					}
				}
