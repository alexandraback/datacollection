...
static int T,E,R,N,v[10005];
static vector<int> gains;

inline void recurse(int have, int index, int gain)
{
	if (have<0) return;
	if (index==N)
	{
//		printf("gain = %d\n", gain);
	...
	FOREQ(i,0,have)
	{
//		printf("recurse(%d,%d,%d) idx:%d;v:%d\n", MIN(E,have-i+R),index+1,gain+i*v[index],index,v[index]);
		recurse(MIN(E,have-i+R),index+1,gain+i*v[index]);
	}
}
