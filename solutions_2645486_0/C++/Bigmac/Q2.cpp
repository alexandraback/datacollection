...
long long in_r;
long long in_n;
vector<long long> in_vi;
long long solve(int start, long long energy);
std::map<std::pair<int, long long>, long long> dp_table;
int main(int argc, char **argv)
{
	openIOFile(argc,argv);
	in_t = readInt();
	for(it_t = 1; it_t <= in_t; it_t++)
	{
		//init input
		dp_table.clear();
		in_vi.clear();
		...
		REP(it_n, in_n)
		{
			in_vi[it_n] = readHugeInt();
		}
		long long sol = solve(0, in_e);
...
//sol
long long _solve(int start, long long energy);
long long solve(int start, long long energy)
{
	if(dp_table.find(std::pair<int, long long>(start, energy)) != dp_table.end())
	{
		return dp_table[std::pair<int, long long>(start, energy)];
	}
	else
	{
		long long val = _solve(start, energy);
		dp_table[std::pair<int, long long>(start, energy)] = val;
		return val;
	}
}
long long _solve(int start, long long energy)
{
	...
	for(long long i = 0; i <= energy; i++)
	{
		long long re = energy;
		re -= i;
		re += in_r;
		if(re > in_e){re = in_e;}
		long long gain_cur = i * in_vi[start] + solve(start + 1, re);
		if(gain_cur > max_gain)
		{
			max_gain = gain_cur;
		}
	}
