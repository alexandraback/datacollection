...
* Creation Date : 27-04-2013

* Last Modified : Sat 27 Apr 2013 08:12:35 AM IST
...
int solution(int ind, int money){
    money = min(money, e);
    int &reg = store[ind][money];
    if( ind == n-1)
        return reg = money*N[n-1];
    if(reg != -1)
        return reg;
    int maxval = 0;
    for(int i = 0; i <= money; ++i){
        maxval = max(maxval, (i*N[ind]) + solution(ind+1, money-i+r));
    }
    return reg = maxval;
}
