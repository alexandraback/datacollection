...
long long solve_the_problem(int start, int end, int energystart, int energyend){
	if(start == end){
		return 0;
	}
	...
	long long b = solve_the_problem(start, best, energystart, M) + solve_the_problem(best+1, end, min(E, m + R), energyend);
	return b + (M - m) * ((long long)value[best]);
}
int main(){
	int T;
	cin >> T;
