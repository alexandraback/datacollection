...
void track(int E, int R, int N, const vector<int> &v, int level, int score, int e)
{
  assert(e>=0); assert(e<=E);
  if(level==N) {
    if(score>max_score) {
      max_score = score;
    }
...
#if 0
  for(int i=MAX(0, e-(E-R));i<=e;i++) {
    int newe = e-i+R;
    //newe = MIN(newe, E);
    track(E, R, N, v, level+1, score+v[level]*i, newe);
  }
#else
  for(int i=0;i<=e;i++) {
    int newe = e-i+R;
