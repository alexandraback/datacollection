...
namespace jam
{
    ...
    class CodeJamMain
    {
        ...
        static string inputFile = @"C:\Users\a.gilotte\Downloads\C-small-attempt0.in";
        //static string inputFile = @"C:\Users\a.gilotte\Downloads\B-large.in";

        public static void Main()
        {
            AJam jamClass = new JamExoC();
            jamClass.Test();
    ...
    class JamExoC : AJam
    {
        ...
        {
            int n = test.Lines.Count;
            var topics = test.Lines.Select(line => line.Split(' ')).ToList();
            var wordsA = topics.Select(t => t[0]).ToList();
            var wordsB = topics.Select(t => t[1]).ToList();

            var longest = SetAsLong.Build( n);
            var max = 0;
            for (long set = 0; set < longest; set++)
            {
                var fakeIndexs = SetAsLong.GetValues(set).ToList();
                if (max >= fakeIndexs.Count)
                    continue;
                var realWordA = new HashSet<string>();
                var realWordB = new HashSet<string>();
                for (int i = 0; i < n; i++)
                {
                    if (SetAsLong.IsInSet(set, i))
                        continue;
                    realWordA.Add(wordsA[i]);
                    realWordB.Add(wordsB[i]);
                }

                if (fakeIndexs.All(k => realWordA.Contains(wordsA[k]) && realWordB.Contains(wordsB[k])))
                    max = fakeIndexs.Count;
            }
    ...
    static class SetAsLong
    {
        public static bool IsInSet(long set, int k)
        {
            return (set >> k) % 2 == 1;
        }
        public static int SetSize(long set)
        {
            int count = 0;
            while (set > 0)
            {
        ...
        public static IEnumerable<int> GetValues(long set)
        {
            int i = 0;
            while (set > 0)
            {
        ...
        public static long Build(int index)
        {
            long set = 1L << index;
            return set;
        }
        public static long Build(IEnumerable<int> indexes)
        {
            long set = 0;
        ...
        static public void TestSetAsLong()
        {
            var indexes = new HashSet<int>(new int[] { 0, 3, 5, 42, 50, 60, 62 });
            var set = Build(indexes);
            for (int i = 0; i < 64; i++)
            {
                bool inSet1 = indexes.Contains(i);
                bool inSet2 = IsInSet(set, i);
                if (inSet1 != inSet2)
                    throw new Exception();
            }
            if (SetSize(set) != indexes.Count)
                throw new Exception();
            var hashset = new HashSet<int>(GetValues(set));
            if (hashset.Count != SetSize(set))
                throw new Exception();
            if (indexes.Any(k => !hashset.Contains(k)))
                throw new Exception();
        }
