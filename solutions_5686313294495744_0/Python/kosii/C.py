...
def main(input_filename, output_filename):
    with open(input_filename) as input, open(output_filename, 'w') as output:
        ...
        for t in range(T):
            ...
            for n in range(N):
                a, b = sis(input)
                if a in A: A[a].append(b)
                else: A[a] = [b]
                if b in B: B[b].append(a)
                else: B[b] = [a]
                elems.add((a, b))
            # print A, B
            lonersA = []
            lonersB = []
            for a, neigh in A.items():
                if len(neigh) == 1:
                    elems.remove((a, neigh[0]))
                    lonersA.append(a)
            for a, neigh in B.items():
                if len(neigh) == 1:
                    if (neigh[0], a) in elems:
                        elems.remove((neigh[0], a))
                    lonersB.append(a)
            # print "LONERS", lonersA, lonersB
            # print "ELEMS", elems, len(elems)
            # for a in lonersA:
