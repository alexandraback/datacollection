...
int main() {
    ...
    string line="cccc";
    ifstream myfile("c-small-attempt1.in.txt");
        ...
        for(int i=0;i<cases; i++)
        {
            ...
            for (int i=0;i<first.size();i++)
            {
                for (int j=0; j<first.size();j++)
                {
                    ...
                    if(find(pairs.begin(), pairs.end(), make_pair(first[i],second[j])) == pairs.end()&&find(pairs.begin(), pairs.end(), make_pair(second[j],first[i])) == pairs.end())
                    {
                        if(first[i]!=second[j])
                        {
                            result+=1;
                            pairs.push_back(make_pair(first[i],second[j]));
