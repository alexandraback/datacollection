...
pair<string, string> st[N];
pair<pii, int> d[N];
set<string> s1, s2;
set<string> s3, s4;
map<string, int> ms1, ms2;
int dfs(int id, int n, int sum) {
	if (id == n) {
		if (s3.size() == s1.size() && s4.size() == s2.size()) {
			return sum;
		}
		return INF;
	}

	string x = st[id].x, y = st[id].y;
	int mn = dfs(id + 1, n, sum);

	ms1[x]++; ms2[y]++;
	s3.insert(x); s4.insert(y);
	int mn2 = dfs(id + 1, n, sum+1);
	ms1[x]--; ms2[y]--;
	if (ms1[x] == 0) {
		s3.erase(x);
	}
...
int solve() {
	...
	FI(n) {
		FJ(n) {
			d[j] = {pii(min(tf[st[j].x], ts[st[j].y]), max(tf[st[j].x], ts[st[j].y])), j};
		}
		sort(d, d + n);
		int y = d[0].y;
		//DB(y); DB(st[y].x); DBN(st[y].y);
		if (u1.count(st[y].x) + u2.count(st[y].y) < 2) {
			u1.insert(st[y].x);
			u2.insert(st[y].y);
			tf[st[y].x] = n+2;
			ts[st[y].y] = n+2;
