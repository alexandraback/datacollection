...
int main(int argc, char **argv)
{
	...
	{
        map<string, int, lessstr> ma;
        map<string, int, lessstr> mb;
        int in_n = readInt();
        vector< pair<string, string> > ps;
        for(int i = 0; i < in_n; i++) {
            string a = readString();
            string b = readString();
            ps.push_back(pair<string, string>(a, b));
            ma[a]++;
            mb[b]++;
        }
        int cnt = 0;
        map<string, int, lessstr> keys_a;
        map<string, int, lessstr> keys_b;
        vector<bool> mark;
        mark.resize(ps.size());
        ...
        for(int i = 0; i < ps.size(); i++) {
            ...
            if(ma[ps[i].first] == 1) {
                ...
                mark[i] = true;
                keys_a[ps[i].first] = 1;
                keys_b[ps[i].second] = 1;
            } else if(mb[ps[i].second] == 1) {
                cnt++;
                mark[i] = true;
                keys_a[ps[i].first] = 1;
                keys_b[ps[i].second] = 1;
        ...
        for(int i = 0; i < ps.size(); i++) {
            ...
            if(keys_a[ps[i].first] != 1 && keys_b[ps[i].second] != 1) {
                cnt++;
                mark[i] = true;
                keys_a[ps[i].first] = 1;
                keys_b[ps[i].second] = 1;
