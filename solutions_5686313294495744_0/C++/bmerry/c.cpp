...
static void solve_case(int cas, ostream &cout, unique_ptr<promise<void> > &&input_done)
{
    ...
    vector<pair<int, int>> prior(N);
    for (int i = 0; i < N; i++)
        for (int j = 0; j < N; j++)
        {
            if (i == j)
                continue;
            if (topics[i].first == topics[j].first)
                prior[i].first |= 1 << j;
            if (topics[i].second == topics[j].second)
                prior[i].second |= 1 << j;
        }
    ...
    for (int mask = 1; mask < (1 << N); mask++)
    {
        ...
        for (int i = 0; i < N; i++)
        {
            if (mask & (1 << i))
            {
                int extra = (mask & prior[i].first) && (mask & prior[i].second);
                v = max(v, extra + dp[mask ^ (1 << i)]);
            }
        }
        dp[mask] = v;
    }

    cout << "Case #" << cas + 1 << ": " << dp.back() << "\n";
}
