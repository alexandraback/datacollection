...
void solve() {
  ...
  rep (j, 2) {
    D[j] = S[j];
    sort(all(D[j]));
    D[j].erase(unique(all(D[j])), D[j].end());
  }

  tie(L, R) = mp(D[0].size(), D[1].size());
  rep (i, N) {
    int l = lower_bound(all(D[0]), S[0][i]) - D[0].begin();
    int r = lower_bound(all(D[1]), S[1][i]) - D[1].begin();
    adj[l].pb(r);
    // printf("%d--%d\n", l, r);
  }

  printf("%d\n", N - (L + R - bipartite_matching()));
}
