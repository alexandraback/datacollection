...
bool nincsbenn(vector <string>& V, string s)
{
...
int hanyadik(vector <string>& V, string s)
{
    for (int i=0; i<V.size(); i++)
    {
        if (V[i].compare(s)==0)
...
int main(){
    ...
    for (int i=1; i<=t; i++)
    {
        ...
        f>>k;
        vector<string> V;
        vector<pair<string,string> > Jatek;
        for (int sm=0; sm<k; sm++)
        {
            ...
            Jatek.push_back(P);
            if (nincsbenn(V,s1))//vagyis nincs benne
            {
                V.push_back(s1);
            }
             if (nincsbenn(V,s2))//vagyis nincs benne
            {
                V.push_back(s2);
            }
    ...
    initGraph(n);
    int e=Jatek.size();
    for (int l=0; l<e; l++){    //are made for better memory usage.
         int a, b;
         a=hanyadik(V,Jatek[l].first);
         b=hanyadik(V,Jatek[l].second);
        g[a][b]=g[b][a]=unmatched;
     }
