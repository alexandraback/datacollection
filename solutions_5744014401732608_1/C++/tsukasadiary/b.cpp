...
void solveL() {
	...
	if ((1LL << (B - 1)) - 1 < M) {
		...
		for_rev(i,B-2,0) if (M >> i & 1) use.push_back(i + 1);
		
		Matrix< int > mat(B, Vec< int >(B, 0));
		
		int L = use[0];
		for_(i,0,L+1) for_(j,i+1,L+1) mat[i][j] = 1;
		
		int U = use.size();
		for_(i,0,U) if (use[i] < B - 1) mat[use[i]][B-1] = 1;
		
		Vec< lint > p(B, 0); p[0] = 1;
		for_(i,0,B) for_(j,i+1,B) if (mat[i][j]) p[j] += p[i];
