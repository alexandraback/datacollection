...
ll numRoots(const vector <vector <ll> >& vvi )
{
	...
	while(1)
	{
		...
		for(int a=0;a<N;++a)
			for(int b=0;b<N;++b)
			{
				if(vvi[b][a]>0 && freq[b] >= TOOBIG / vvi[b][a])
				{
					tmp[a] = TOOBIG;
				}
				else
...
vector <vector <ll> > large(ll B, ll M)
{
	vector <vector <ll> > ret(B, vector <ll>(B));
	for(int y=0;y<B;y++)
		for(int x=0;x<B;x++)
		{
			if(x>y)
...
int main(){
	...
	for (int testcase = 0; testcase < T; ++testcase)
	{
		ll B,M;
		cin >> B >> M;
		vector <vector <ll> > ansvvi = large(B,M);
