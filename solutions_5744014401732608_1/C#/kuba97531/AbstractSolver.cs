...
namespace Solvers
{
    public abstract class AbstractSolver
    {
        ...
        {
            var typ = Type.GetType(args[0]);
            string input = args[1];
            string output = args[2];

            var solver = Activator.CreateInstance(typ) as AbstractSolver;
        ...
        public void Solve(string inputFile, string outputFile)
        {
            ...
            using (input = new StreamReader(inputFile))
            {
                using (var output = new StreamWriter(outputFile, false))
                {
                    ...
                    {
                        var obj = Solve(input);
                        var result = (obj != null) ? obj.ToString() : "IMPOSSIBLE";
                        
                        output.WriteLine(ResponseFormat, i, result);
                        Console.WriteLine(ResponseFormat, i, result);
                    }
                }
            }
