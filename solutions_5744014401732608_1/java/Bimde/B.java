...
public class B {
	...
	public static void main(String[] args) throws IOException {
		...
		BufferedWriter out = new BufferedWriter(new FileWriter(new File("B-large.out")));

		for (int c = 1; c <= noCases; c++) {
			int noBuildings = in.nextInt();
			long noPaths = in.nextLong();
			if (noPaths > Math.pow(2, noBuildings - 2)) {
				// System.out.println("Case #" + c + ": IMPOSSIBLE\n");
				out.write("Case #" + c + ": IMPOSSIBLE\n");
			} else {
				boolean[][] paths = new boolean[noBuildings][noBuildings];
				long[] multiplier = new long[noBuildings];
	...
	static class InputReader {
		...
		public InputReader() {
			...
			try {
				reader = new BufferedReader(new FileReader(new File("B-large.in")));
