...
int C_main()
{
	...
	string strInput = strPath + "C-large.in";
	string strOutput = strPath + "C-large.out";
...
void solution(ifstream &in, ofstream &out)
{
	...
	unsigned int Wall[1000000*2];
	for(int i=0;i<1000000*2;i++)
		Wall[i]=0;
	int nTribes;
	in>>nTribes;
	std::multimap<long long,Attack> Attacks;
	...
	int TSA =0;
	for(set<long long>::iterator a =keys.begin();a!= keys.end();a++)
	{
		int Count = Attacks.count(*a);
		std::multimap<long long,Attack>::iterator start = Attacks.find(*a);
		for(int i=0;i<Count;i++)
		{
			Attack attack = (*start).second;
			for(long long j=attack.W;j<attack.E;j++)
			{
