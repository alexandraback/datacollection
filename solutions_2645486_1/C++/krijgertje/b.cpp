...
void run(int casenr) {
	int E,R,n; scanf("%d%d%d",&E,&R,&n); if(R>E) R=E;
...
//			printf("\tassigning %d to %d (%d)\n",i,cur,rem);
			...
			while(rem>0) {
				if(cap==0) {
					if(q.empty()) printf("rem %d\n",rem);
					assert(!q.empty());
					if(R*(cur-q.front())>E) printf("! %d %d %d (%d vs %d)",i,cur,q.front(),R,E);
					cap=(int)min((ll)E,(ll)R*(cur-q.front())); cur=q.front(); q.pop_front();
				}
				int now=min(rem,cap);
