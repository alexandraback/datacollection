...
void doit(bool print = true) {
  ...
  for (int i = 0; i < N; i++) no[i] = make_pair(vi[i], i);
  sort(no.rbegin(), no.rend());

  vector<int> have(N, E), need(N, 0);

  long long tot = 0;
  for (int i = 0; i < N; i++) {
    int id = no[i].second;
    tot += (have[id] - need[id]) * (long long)vi[id];
    for (int j = 1; j+id < N; j++) {
      int val = need[id] + j*R;
      if (val > have[id+j]) break;
      have[id+j] = val;
    }
    for (int j = 1; id-j >= 0; j++) {
      int val = have[id] - j*R;
      if (val < need[id-j]) break;
      need[id-j] = val;
    }
  }
