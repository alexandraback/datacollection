...
int main() {
	...
	for(int cs = 1; cs <= T; cs++) {
		...
		if(R == 1 || C == 1 && R!=C) {
			...
			if(N > R*C/2) {
				int perim = (2*R + 2*C - 4)/2 - 2;
				N -= R*C/2;
				ans += max(0, min(4, 2*N));
				N -= 2;
				ans += max(0, min(3*perim, 3*N));
				N -= perim;
				ans += max(0, 4*N);
			}
		}
			...
			if(N > R*C/2 + 1) {
				int ans1 = 0;
				int ans2 = 0;
				int perim = (R*2 + C*2 - 4)/2;
				int N1 = N, N2;
				N1 -= R*C/2 + 1;
				N2 = N1+1;
				ans2 += max(0, min(8, N2*2));
				ans1 += max(0, min(perim*3, N1*3));
				N2 -= 4;
				N1 -= perim;
				ans2 += max(0, 4*N2);
				ans1 += max(0, 4*N1);
...
int main() {
	memset(ans, -1, sizeof(int)*1000000);
	ans[1] = 1;
	ans[1000000] = 3435;
