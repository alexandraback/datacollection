...
int main()
{
	...
	{
		scanf("%d%d%d",&R,&C,&N);
		rep(i,2)rep(j,260)rep(k,1<<C)dp[i][j][k]=INF;
		int cur=0,nxt=1;
		dp[nxt][0][0]=0;
		rep(x,R)rep(y,C)
		{
			swap(cur,nxt);
			rep(i,260)rep(b,1<<C)dp[nxt][i][b]=INF;
			rep(i,260)rep(b,1<<C)if(dp[cur][i][b]!=INF)rep(f,2)
			{
				int nb=(b<<1)&((1<<C)-1);
				nb+=f;
				int ni=i+f;
				...
				if(y>0&&b%2==1)nc++;
				if(b>>(C-1)&1)nc++;
				if(f==0)nc=dp[cur][i][b];
				MIN(dp[nxt][ni][nb],nc);
			}
		}
		int res=INF;
		rep(b,1<<C)MIN(res,dp[nxt][N][b]);
		printf("Case #%d: %d\n",__,res);
	}
}