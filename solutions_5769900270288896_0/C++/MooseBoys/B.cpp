...
inline bool occupied(const vector<bool>& rooms, int r, int c, int R, int C)
{
    return rooms[r * C + c];
}

int GetUnhappiness(const vector<bool>& rooms, int R, int C)
{
    int unhappiness = 0;
    for (int r = 0; r < R; r++)
    {
        for (int c = 0; c < C; c++)
        {
            if (occupied(rooms, r, c, R, C))
            {
                if (r > 0 && occupied(rooms, r - 1, c, R, C)) unhappiness++;
                if (c > 0 && occupied(rooms, r, c - 1, R, C)) unhappiness++;
            }
        }
    }
    return unhappiness;
...
int CodeJamMain()
{
    ...
    {
        // Small input is easy - only on order of 2^16 possible layouts - just check each one
        int R;
        input >> R;
        int C;
        input >> C;
        ...
        for (int p = 0; p < (1 << R*C); p++)
        {
            if (CountSet(p) == N)
            {
                ...
                int score = GetUnhappiness(rooms, R, C);
                if (score < bestScore) bestScore = score;
            }
        }

        int answer = bestScore;
