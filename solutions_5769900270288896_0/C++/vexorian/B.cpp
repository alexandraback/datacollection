...
struct solver
{
    ...
    // maximum cells 16, maximum cost 4*16 = 64, fits in a char, dimensions are
    // quite large, better use a char ...
    char dp[17][17][17][1<<16];
    
    int f(int n, int x, int y, int mask)
    {
        char & res = dp[n][x][y][mask];
        if (res == -1) {
            if (y == C) {
                // the end
                if (n != N) {
                ...
                char a = f(n, x+1,y, nmask);
                
                // put
                char b = INF;
                if (n != N) {
                    b = f(n+1, x+1,y, mask|(1<<x) );
                    if (b != INF) {
                        if ( (x > 0) && (mask & (1 << (x-1)) ) ) {
                            b++;
                        }
    ...
    int solve()
    {
        // oh dear deity I wish I knew B-small was a standard dp problem before
        // investing so much time in A-large,
