...
int solve(int n, int r, int c){
	...
	if (r == 1) {
		...
		int two = max((r % 2 == 0) * 2, (c % 2 == 0) * 2);
		int three = (2 * r + 2 * c - 4) / 2 - two;
		int four = r * c - h - two - three;
		//DBN(two, three, four);
		n -= h;
		int ans = 0;
		int t;
...
int main()
{
	...
	for (int test = 0; test < t; ++test){
		...
		cin >> r >> c >> n;
		cout << "Case #" << test + 1 << ": ";
		cout << min(solve2(n,r,c),solve(n, r, c)) << endl;
	}
