...
ll inf=1000000007;
int row[]={1,0};int col[]={0,1}; //2 Direction
    ...
    for(i=0;i<r;i++)
    {
        for(j=0;j<c;j++)
        {
            for(k=0;k<2;k++)
            {
                ...
                if(x<0||y<0||x==r||y==c)
                  continue;
                v[dist[i][j]].push_back(dist[x][y]);
                v[dist[x][y]].push_back(dist[i][j]);
            }
        }
    }
}
int fitness(int state)
{
    ...
    for(int i=0;i<r*c;i++)
    {
        if(check_bit(state,i)==0)
          continue;
        for(int j=0;j<v[i].size();j++)
        {
            int k=v[i][j];
            if(check_bit(state,k)==0)
              continue;
            int x=i,y=k;
            if(x>y)
              swap(x,y);
...
int solve(int state,int ind,int rem)
{
    if(rem==0)
    {
    ...
    if((r*c-ind)<rem)
      return inf;
    return min(solve(on_bit(state,ind),ind+1,rem-1),solve(state,ind+1,rem));
}
int main()
{
