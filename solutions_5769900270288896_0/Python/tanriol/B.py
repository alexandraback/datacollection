...
class Solver(ProblemSolver):
    ...
    def process(self, infile, ncase):
        ## User code here
        rows, cols, cells = readints(infile)
        odds = []
        evens = []
        for i in range(rows):
            for j in range(cols):
                wall_count = (
                    (0 if i == 0 else 1) +
                    (0 if i == rows - 1 else 1) +
                    (0 if j == 0 else 1) +
                    (0 if j == cols - 1 else 1))
                if (i + j) % 2:
                    odds.append(wall_count)
                else:
                    evens.append(wall_count)
        odds.sort()
        evens.sort()
        if cells <= max(len(odds), len(evens)):
            return 0
        unhappiness_odd = sum(odds[:cells - len(evens)])
        unhappiness_even = sum(evens[:cells - len(odds)])
        return min(unhappiness_odd, unhappiness_even)
