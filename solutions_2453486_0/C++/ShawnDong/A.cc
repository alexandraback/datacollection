...
void update(int &x , int y , int z){
    if (y == INF) return;
    if (x == INF) x = y;
    else if(z == 1) checkMax(x , y);
    else checkMin(x , y);
}
int dfs(int n , int ca , int cb){
    if (n == 0) return 0;
    if (ca == 0 && cb == 0) return 0;
//    if (ca == 0) return -min(cb , n);
//    if (cb < 2) {
//        int res = min(n , ca);
//        return res + dfs(n - res , ca - res , cb);
//    }
    int res = INF;
    for (int i = 1 ; i <= 50 ; ++i){
        if (i & 1){
            if (i > ca || i - 1 > cb) break;
            int res1 = dfs(n - 1 , ca - i , cb) + 1 , res2 = INF;
            if (i + 1 <= cb) res2 = dfs(n - 1 , ca , cb - i - 1) - 1;
            update(res1 , res2 , 0);
            update(res ,res1 , 1);
        }
        else{
            if (i > cb || i - 1 > ca) break;
            int res1 = dfs(n - 1 , ca , cb - i) - 1 , res2 = INF;
            if (i + 1 <= ca) res2 = dfs(n - 1 , ca - i - 1 , cb) + 1;
            update(res1 , res2 , 1);
            update(res , res1 , 0);
        }
    }
...
int main(){
    Case = 0;
//    for (n = 1 ; n <= 10 ; ++n) for (ca = 1 ; ca <= 10 ; ++ca) for (cb = 1 ; cb <= 10 ; ++cb) printf("%d " , dfs(n , ca , cb));
    Rush solve();
}
