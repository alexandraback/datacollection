...
def game(rows, cols, diags):
    for line_type in rows, cols, diags:
        for line in line_type:
            if 'XXXX' in line:
                return 'X won'
            if 'OOOO' in line:
                return 'O won'
            if 'TXXX' == ''.join(sorted(line)):
                return 'X won'
            if 'OOOT' == ''.join(sorted(line)):
                return 'O won'


def construct_diags(rows, cols):
    diags = []
    # main diags
    main_diag = ''
    other_main = ''
...
def do_job(in_file, out_file=sys.stdout):
    ...
    for testcase in xrange(T):
        ...
        for i in xrange(4):
            ...
            assert len(row) == 4
            if not grid_not_full:
                grid_not_full = '.' in row
            rows.append(row)
            for j, char in enumerate(row):
                cols[j] += char
        diags = construct_diags(rows, cols)
        LOG.debug('rows: %s' % rows)
        LOG.debug('cols: %s' % cols)
        LOG.debug('diags: %s' % diags)
        LOG.debug('grid_not_full: %s' % grid_not_full)
        result = game(rows, cols, diags)
        if not result:
            if not grid_not_full:
                result = 'Draw'
            else:
