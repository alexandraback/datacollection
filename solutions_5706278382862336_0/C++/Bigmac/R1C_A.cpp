...
int main(int argc, char **argv)
{
	...
	for(int it_t = 1; it_t <= in_t; it_t++)
	{
		//init input
		//finish init input
		long long in_pi = readHugeInt();
		long long in_qi = readHugeIntDashed();
        
        //===========================================solve function begin================================//
		auto solve = [&]() -> long long{
            long long in_gcd = gcd(in_pi, in_qi);
            long long s_in_p = in_pi / in_gcd;
            long long s_in_q = in_qi / in_gcd;
            long long s_pow;
            if((((long long)2)<<40) % s_in_q != 0)
            {
                return -1;
            }
            else
            {
                long long delta = (((long long)2)<<40) / s_in_q;
                s_pow = s_in_p * delta;
            }
            for(int i = 1; i < 40; i++)
            {
                if(s_pow >= (((long long)2)<<(40 - i)))
                {
                    return i;
                }
            }
