...
public class Google
{
    public static boolean executeOnStringTab(String[][] input, PrintWriter output)
    {
        ...
        {
            char str[] = new char[input[1][i].length()];

            for (int j = 0; j < str.length; j ++)
                str[j] = input[1][i].charAt(j);
            ...
            for (int j = 1; j < str.length; j ++)
            {
                ...
                if (!allTheSame && j != str.length - 1 && str[j] != str[j + 1])
                    middle[str[j] - 'a'] = middle[str[j] - 'a'] + 1;
            }

            if (allTheSame)
                full[str[0] - 'a'] = full[str[0] - 'a'] + 1;
            else
            {
                if (str[0] == str[str.length - 1])
                    return outputCase(output, "0");
                begin[str[0] - 'a'] = begin[str[0] - 'a'] + 1;
                end[str[str.length - 1] - 'a'] = end[str[str.length - 1] - 'a'] + 1;
            }
        }

        double currentCombination = 1;
        int numberOfParts = 0;
        for (int i = 0; i < 26; i ++)
        {
            if (begin[i] > 1 || end[i] > 1)
                return outputCase(output, "0");
            if (middle[i] > 0 && begin[i] + end[i] > 0)
                return outputCase(output, "0");
            if (middle[i] > 1)
                return outputCase(output, "0");

            double newCombination = 1;
            for (int j = 2; j <= full[i]; j ++)
                newCombination = newCombination * j;
            currentCombination = currentCombination * newCombination;

            if ((begin[i] != 0 || full[i] != 0) && end[i] == 0)
                numberOfParts++;
        }

        double partCombination = 1;
        for (int j = 2; j <= numberOfParts; j ++)
            partCombination = partCombination * j;
        currentCombination = currentCombination * partCombination;

        currentCombination = currentCombination % 1000000007;
        long result = (long)currentCombination;
