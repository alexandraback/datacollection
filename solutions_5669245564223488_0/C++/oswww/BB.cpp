...
LL find(int u){
    ...
    if ( g[u][u]>0 ){
        ret = 1;
        for (int i=1; i<=g[u][u]; i++) ret=ret*i%mod;
    }
    vis[u] = 1;
    int i;
    for (i=0; i<256; i++){
...
int run(){
    ...
    for (int i=0; i<n; i++){
        for (int j=0; j<(int)s[i].length(); j++){
            ...
            a.insert( s[i][j] );
            tmp[ s[i][j] ].insert(i);
            if ( j>0 && j<(int)s[i].length()-1 ) mid[ s[i][j] ].insert(i);
            //if ( three[ s[i][j] ].count()>=3 ) return 0;
        }
    }
    char tt = 0;
    for (int i=0; i<n; i++){
//        for (int j=1; j<(int)s[i].length()-1; j++ ){
//            if ( mid[ s[i][j] ].size()>=2 ) return 0;
//        }
        for (int j=0; j<(int)s[i].length(); j++){
            if ( tt==0 ) {
                tt = s[i][j];
    ...
    for (int i=0; i<n; i++){
        char c1 = s[i][0], c2 = s[i][ s[i].length()-1 ];
        g[c1][c2]++;
    }
