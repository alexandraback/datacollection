...
int chainNext(char ch)
{
	...
	vis[which] = true;
	wtf.push_back(which);
	return (ll)ans * chainNext(carSet[which][carSetLen[which]-1]);
}
...
int chainPrev(char ch)
{
	...
	return (ll)ans * chainPrev(carSet[which][0]);
}
...
int main(void)
{
	...
	while(T--)
	{
		...
		{
			ans = (ll)ans * fact[pureCnt[i]];
			pureCnt[i] = 0;
			part++;
		}
		ans = (ll)ans * fact[part];
