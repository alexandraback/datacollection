...
ll rec(ll sum, ll cur)
{
    ...
    if(ret!=-1) return ret;
     ret=oo;
    if(v[cur]<sum)ret=min(ret,rec(min(sum+v[cur],1000001LL),cur+1));
    else
    {
        ret=min(ret,rec(min(sum+sum-1,1000001LL),cur)+1);
        ret=min(ret,rec(sum,cur+1)+1);
    }
    return ret;
}
