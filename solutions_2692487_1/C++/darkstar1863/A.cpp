...
int iter(const vector<int64>& V, int index, int64 curSize)
{
	if(index == V.size())
		return 0;
	...
	if(V[index] < curSize)
	{
		return iter(V, index + 1, curSize+V[index]);
	}

	int rem = V.size() - index;
	if(curSize == (int64)1)
		return rem;

	int add = 0;
	while (V[index] >= curSize)
	{
		++add;
		curSize += curSize - 1;
	}
	add += iter(V, index + 1, curSize+V[index]);
