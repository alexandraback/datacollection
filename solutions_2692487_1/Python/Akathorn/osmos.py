...
def get_floats(input_data):
    return map(lambda x: float(x), input_data.pop(0).strip().split(" "))
...
if __name__ == "__main__":
    ...
    [num_tests] = get_floats(input_data)

    for i in range(0,int(num_tests)):
        [A, N] = get_floats(input_data)
        motes  = np.array(get_floats(input_data))
        motes.sort()

        print "Case #%d: %d" % (i+1, int(solve(motes, A)))
