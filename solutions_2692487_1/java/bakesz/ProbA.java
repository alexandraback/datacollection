...
public class ProbA {
        public static void main(String[] args)
    {
        ...
        static long aktbest;
    private static long r(config start) {
        if (aktbest<start.stepnum)
            return start.stepnum;
        if (start.index>=start.motes.size())
        {
            if (aktbest>start.stepnum)
            aktbest=start.stepnum;
        ...
        else
        {
          //  System.out.println("problemo");
            ...
            long hozzaadassal=Integer.MAX_VALUE;
            if (start.size-1!=0)
            hozzaadassal=r(c2);
            c2.index=start.index+1;
            c2.motes=start.motes;
