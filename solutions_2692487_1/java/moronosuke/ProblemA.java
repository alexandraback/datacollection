...
public class ProblemA {
		...
		try {
			...
			for (int i = 0; i < count; i++) {
				...
				writer.printf("Case #%d: %s\n", i+1,
						solveIt(i+1, Integer.parseInt(parameters[0]),
	...
	private static String solveIt(int caseNumber, int a, int n, int[] motes) {
		Arrays.sort(motes);
	...
	private static int solve(int a, int[] motes, int index, int operations) {
		...
		while (index < motes.length && bestRemoveMoteResult > operations) {
			if (a > motes[index]) {
				...
				int removeMoteResult = motes.length - index + operations;
