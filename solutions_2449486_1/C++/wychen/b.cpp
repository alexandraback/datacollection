...
bool all_lower_r(vector<vector<int> > board, int i, int j, int level)
{
  int N = board.size();
  int M = board[0].size();
  int c;
  REP(c, M) {
    if(board[i][c]>level) return false;
  }
  return true;
}

bool all_lower_c(vector<vector<int> > board, int i, int j, int level)
{
  int N = board.size();
  int M = board[0].size();
  int r;
  REP(r, N) {
    if(board[r][j]>level) return false;
  }
  return true;
}
...
void solve(bool skip)
{
  ...
  REP(i, N)
    REP(j, M) {
      int level = board[i][j];
      if(!all_lower_c(board, i, j, level) && !all_lower_r(board, i, j, level)) {
        cout << "NO";
        return;
      }
    }
