...
void solve(){
    scanf("%d%d" , &n , &m);
    REP_2(i , j , n , m){
        scanf("%d" , &A[i][j]);aans[i][j] = 100;
    }
    OT(1);
    REP(i , n){
        int res = 0;
        REP(j , m) checkMax(res , A[i][j]);
        REP(j , m) checkMin(aans[i][j] , res);
    }
    REP(i , m){
        int res = 0;
        REP(j , n) checkMax(res , A[j][i]);
        REP(j , n) checkMin(aans[j][i] , res);
    }
    REP_2(i , j , n , m) if (A[i][j] != aans[i][j]){
        cout << "NO" << endl;
        return;
    }
    cout << "YES" << endl;
