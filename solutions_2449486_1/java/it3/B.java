...
public class B {
     ...
     */
    /*
     * lowest: must meet at either row or col
     * check from min h to max h 
     */
    private void run(int caseNumber) throws Exception {
        ...
        for (int r = 0; r < a.length; r++) {
            for (int c = 0; c < a[r].length; c++) {
                rowMax[r] = Math.max(rowMax[r], a[r][c]);
                colMax[c] = Math.max(colMax[c], a[r][c]);
            }
        }
        String ret = "YES";
        for (int h = 1; h <= maxH; h++) {
//            if (!ok(a,h)) {
            if (!ok(a,h,rowMax,colMax)) {
                ret="NO";
                break;
            }
        }
    ...
    private boolean ok(int[][] a, int h, int[] rowMax, int[] colMax) {
        for (int r = 0; r < a.length; r++) {
            for (int c = 0; c < a[r].length; c++) {
                if(a[r][c]==h && rowMax[r]>h && colMax[c]>h) {
                    return false;
                }
            }
        }
    ...
    private boolean ok(int[][] a, int h) {
        boolean[] badRow = new boolean[a.length];
        boolean[] badCol = new boolean[a[0].length];
        for (int r = 0; r < a.length; r++) {
            for (int c = 0; c < a[r].length; c++) {
                if (a[r][c]>h) {
                    badRow[r]=true;
