...
/* .................................................................................................................................. */
const int N = 50;
LL dp[N][2][2][2]; LL a,b,k; LL AA[N] , BB[N];

int ck(LL s , LL da , LL db , LL dk){
    if (s == 1) return 1; LL d = da&db;
    if (d < dk) return 1; if (d > dk) return -1;
    return 0;
}

LL dfs(int i, int s, bool ea , bool eb) {
    if (i==-1) return s; if (~dp[i][s][ea][eb]) return dp[i][s][ea][eb];
    LL res = 0; LL ua = ea?AA[i]:1, ub = eb?BB[i]:1;
    for (LL da = 0 ; da <= ua ; ++da)
        for (LL db = 0; db <= ub; ++db){
            int vv = ck(s , da , db , !!(k & _1(i)));
            if (vv != -1) res += dfs(i-1, vv, ea&&da==ua, eb&&db==ub);
        }
    return dp[i][s][ea][eb]=res;
}
