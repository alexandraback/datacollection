...
namespace TrainAlgorithms
{
   ...
   internal class R20131BA : Problem
   {
      private Dictionary<KeyValuePair<int, int>, int> memo;
      ...
      public override void ProcessOne()
      {
         ...
         var my = list[0];
         var N = list[1];
         var motes = this.ReadIntList();
         Array.Sort(motes);
         memo = new Dictionary<KeyValuePair<int, int>, int>();

         var min = MinSteps(motes, 0, my);
         this.WriteLine(min.ToString());
      }

      public long MinSteps(int[] motes, long current, long my)
      {
         while (current < motes.Length && motes[current] < my)
         {
            my += motes[current];
            current++;
         }

         if (current >= motes.Length)
            return 0;
         //var kv = new KeyValuePair<int, int>(current, my);
         //if (memo.ContainsKey(kv))
         //   return memo[kv];

         long remove = motes.Length - current;
         //int remove = MinSteps(motes, current + 1, my) + 1;
         long add;
         if (my - 1 > 0)
         {
            add = 0;
            long newmy = my;
            while (newmy <= motes[current])
            {
               newmy += newmy - 1;
               add++;
            }
            if (add <= remove)
            {
               //add += MinSteps(motes, current, newmy);
            }
            else
            {
               add = Int64.MaxValue;
            }
            add = MinSteps(motes, current, my + my - 1) + 1;
         }
         else
         {
            add = Int64.MaxValue;
         }
         long best = Math.Min(remove, add);
         //memo[new KeyValuePair<int, int>(current, my)] = best;

         return best;
      }
   }

   internal class R20131BB : Problem
   {
      private Dictionary<int, long> sum = new Dictionary<int, long>();
      ...
      public override void ProcessOne()
      {
         ...
         double b = -0.5 + 0.5*Math.Sqrt(1 + 8*N);
         int nr = ((int) b + 1)/2 - 1;
         int distance = Math.Abs(x) + y;
         double answer;
         if (distance > 2 * nr + 2)
            answer = 0.0;
         else if (distance <= 2*nr)
            answer = 1.0;
         else
         {
            int basesum = (1 + nr * 2 + 1)*(nr * 2 + 1)/2;
            int over = N - basesum;
            if (over == 0 || y == (nr + 1)*2)
            {
               answer = 0.0;
            ...
            else
            {
               ...
               for (int i = 0; i <= y; i++)
               {
                  prob -= ((double)BinomialCoefficient(over, i)) * Math.Pow(0.5, over);
               }
               answer = prob;   
            }
            
         }
         this.WriteLine(answer.ToString());
      }
   ...
   internal class R20131BC : Problem
   {
      public override void ProcessOne()
      {
