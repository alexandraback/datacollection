...
double solve(int ind) {
    ...
    vector<int> nways(diamonds + 1, 0);
    int totways = 0;
    for (int left = 0; left <= diamonds; ++left) {
        int right = diamonds - left;
        // see if it's possible
        if (max<int>(left, right) > curlevel * 2) {
            continue;
        }
        // get nways of this prob
        nways[left] = C[diamonds][left];
        totways += nways[left];
    }
    // and now convert nways to probs
    double res = 0.0;
    for (int left = 0; left <= diamonds; ++left) {
        if (nways[left] == 0) {
            continue;
        }
        double prob = nways[left] * 1.0 / totways;
