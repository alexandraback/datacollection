...
public class C {
	...
	static String fileName = C.class.getSimpleName() + "-small-attempt0";
	...
	public void solveCase() throws IOException {
		...
		int[] coins = nextArr(d);
		boolean[] hasCoin = new boolean[v+1];
		for (int cc: coins) hasCoin[cc] = true;
		...
		if (hasCoin.length > 2 && !hasCoin[2]) {
			hasCoin[2] = true;
			add++;
		}
		
		int[] coins2 = new int[add+d];
		int idx = 0;
		for (int i=0; i<hasCoin.length; ++i) if (hasCoin[i]) coins2[idx++] = i;
		...
		int plus = 1;
		while (true) {
			int[] coinsT = new int[coins2.length + plus];
			for (int i=0; i<coins2.length; ++i) coinsT[i] = coins2[i];
			if (gen(coinsT, v, coins2.length, 3)) {
				println(add+plus);
				break;
			}
			plus++;
	...
	boolean gen(int[] coinsT, int v, int idx, int startCheck) {
		//debug(coinsT, v, idx, startCheck);
		if (idx>=coinsT.length) {
			return can(coinsT, v);
		} else {
			for (int i=startCheck; i<=v; ++i) {
				boolean contains = false;
				for (int j=0; j<coinsT.length; ++j) {
					if (coinsT[j] == i) {
						contains = true;
						break;
					}
	...
	boolean can(int[] coins, int v) {
		int n = coins.length;
		boolean[] can = new boolean[v+1];
		for (int i = 0; i<(1<<n); ++i) {
			int sum = 0;
			for (int j=0; j<n; ++j) {
				if (hasBit(i, j)) sum+=coins[j];
			}
			if (sum<=v) can[sum]=true;
		}
		boolean ok = true;
		for (int j=1; j<=v; ++j) if (!can[j]) ok=false;
		return ok;
	}
