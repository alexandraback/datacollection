...
namespace CodeJam
{
    class Round1
    {
        ...
        static void Round1C_C()
        {
            List<string> lines = Utils.GetStringList("C-small-attempt0.in");
            //List<string> lines = Utils.GetStringList("A-large.in");
            int numofCases = Convert.ToInt32(lines[0]);
            List<string> outputs = new List<string>();

            for (int indx = 0; indx < numofCases; indx++)
            {
                string line1 = lines[indx * 2 + 1];
                string line2 = lines[indx * 2 + 2];
                string[] arr1 = line1.Split(null);
                List<string> menhgiaStr = line2.Split(null).ToList();
                List<int> menhgia = menhgiaStr.Select(int.Parse).ToList();

                int C = Int32.Parse(arr1[0]);
                int D = Int32.Parse(arr1[1]);
                int V = Int32.Parse(arr1[2]);

                int numofAdd = 0;

                for (int i = 1; i <= V; i++)
                {
                    int len = menhgia.Count;
                    int remaining = i;
                    ...
                    if (remaining > 0)
                    {
                        ...
                        numofAdd++;
                        if (menhgia.Contains(remaining))
                        {
                            menhgia.Add(i);
                        }
                        else
                        {
                            menhgia.Add(remaining);
                        }
                        menhgia.Sort();
                    }
                }
                ...
                outputs.Add("Case #" + (indx + 1) + ": " + numofAdd);
                Console.WriteLine("Case #" + (indx + 1) + ": ");
            }
            Utils.WriteFile(outputs, "round1c-c.txt");
        }
