...
bool ok(vector<int> &cs, int v)
{
    ...
    for(int c : cs)
        for(int i = v; i >= 0; i--)
            if(r[i])
                r[i+c] = 1;
    return count(r, r+v+1, 0) == 0;
}

int main()
{
    freopen("/Users/ibra/Downloads/C-small-attempt0.in", "r", stdin);
