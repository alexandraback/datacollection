...
public class GCJ_CloseMatch {
	...
	String solveKO2(String A,String B){
		...
		// maintenant cas divergent
		// on n'a pas à reflechir
		if (option==-1){
			// diverge avant qu'on ait pu changer qqchose
			log("But no option");
			sta=0;
			stb=0;
			for (int u=0;u<L;u++){
	...
	String solveOld(String A,String B){
		...
		// on tente plusieus options

		//
		// option 1: A supérieur à B
		a=A.toCharArray();
		b=B.toCharArray();
		for (int u=0;u<L;u++){
			if (a[u]=='?'){
		...
		// option 2: A inférieur à B, mais en minimisant la distance...
		a=A.toCharArray();
		b=B.toCharArray();
		for (int u=0;u<L;u++){
			if (a[u]=='?')
