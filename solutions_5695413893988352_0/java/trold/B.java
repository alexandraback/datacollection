...
public class B {
	...
	String compute(String a, String b) {
		...
		for (String s : sol) {
			String[] cw = cur.split(" ");
			int ca = Integer.parseInt(cw[0]);
			int cb = Integer.parseInt(cw[1]);
			String[] sw = s.split(" ");
			int sa = Integer.parseInt(sw[0]);
			int sb = Integer.parseInt(sw[1]);
			if ((Math.abs(ca - cb) > Math.abs(sa - sb)) ||
				Math.abs(ca - cb) == Math.abs(sa - sb) && (
					ca > sa ||
					(ca == sa && cb > sb))) {
				cur = s;
			}
