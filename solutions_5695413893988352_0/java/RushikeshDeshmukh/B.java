...
public class B 
{
	...
	private void solve()
	{
		...
		for(int i=0;i<ip1.size();i++)
		{
			...
			{
				Score score = new Score(Integer.parseInt(ip1.get(i)), ip1.get(i), Integer.parseInt(ip2.get(j)), ip2.get(j) );
				scores.add(score);
			}
		}
...
class Score implements Comparable<Score>
{
	...
	public Score(int iC, String iCStr, int iJ, String iJStr)
	{
		_cScore = iC;
		_jScore = iJ;
		_Cstr = iCStr;
	...
	{
		int diff1 = Math.abs(_cScore - _jScore);
		int diff2 = Math.abs(iScore._cScore - iScore._jScore);
