...
public class B {
	...
	public static void solve(int iCase, BufferedReaderWrapper br) throws Throwable {
		String[] l1 = br.getSplittedString();
//		String c = "0" + l1[0] + "0";
//		String j = "0" + l1[1] + "0";
		String c = l1[0];
		String j = l1[1];
		
		Vset result = dfs(0, c, j);
		U.appendCaseResult(sb, iCase+1, result.c+" "+result.j);
	}
	
	public static Vset dfs(int digit,String c, String j) {
		if (digit==c.length()) {
			return new Vset(Math.abs(Integer.valueOf(c) - Integer.valueOf(j)), c, j);
		}
		...
		for (int i=0;i<check1.length;i++) {
			for (int jj=0;jj<check2.length;jj++) {
				String replacedC = c.substring(0, digit) + check1[i]  + c.substring(digit+1, c.length());
				String replacedJ = j.substring(0, digit) + check2[jj] + j.substring(digit+1, j.length());
//				System.out.println((digit+1) + " , "+ replacedC+ "," +replacedJ);
				Vset ret = dfs(digit+1, replacedC,replacedJ);
	...
	static class Vset {
		...
		String c;
		String j;
		public Vset(int d, String c, String j) {
			diff=d;this.c=c;this.j=j;
		}
