...
public class B implements Runnable{
	...
	String mask(String s, String x) {
		if (x.length() > s.length()) return null;
		while (x.length() < s.length()) x = "0" + x;
		for (int i=0;i<s.length();i++) {
			if (s.charAt(i) == '?') continue;
			if (s.charAt(i) != x.charAt(i)) return null;
		}
		return x;
	}
	...
	// PLEASE!!!
	void solve() {
		String mask1 = readString();
		String mask2 = readString();
		...
		for (int i=0;i<1000;i++) {
			for (int j=0;j<1000;j++) {
				if (mask(mask1, ""+i) != null && mask(mask2, ""+j) != null) {
