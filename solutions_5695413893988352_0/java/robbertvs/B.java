...
public class B {
	...
	{
		Scanner sc = new Scanner(new File("input/round1b/B/B-small-attempt1.in"));
	...
	private static ans rec(int aa, String a, String b) {
		if(a.length()==0)
...
//				return min(min(rec(aa*10, a.substring(1), b.substring(1)), bigger(aa*10+1, aa*10, a.substring(1), b.substring(1))),opt);
			...
			{
				int ib = Integer.parseInt(String.valueOf(cb));
		...
		else
		{
			if(cb == '?')
			{
				int ia = Integer.parseInt(String.valueOf(ca));
	...
	private static ans bigger(int aa, int bb, String a, String b) {
		if(a.length()==0)
