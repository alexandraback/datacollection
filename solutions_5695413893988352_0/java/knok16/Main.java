...
//        System.err.println("Running time: " + (System.currentTimeMillis() - time));
    ...
    public void solve() throws NumberFormatException, IOException {
        ...
        for (int i = 1; i <= t; i++) {
            ...
            final int[] result = dfs(0, 0, a, b, 0);

            out.println(String.format(Locale.ROOT, "Case #%d: %0" + n + "d %0" + n + "d", i, result[0], result[1]));
        }
    }

    private int[] dfs(final int aResult, final int bResult, final char[] a, final char[] b, final int j) {
        final int comp = Integer.compare(aResult, bResult);
        if (j == a.length) {
            return new int[]{aResult, bResult};
        }
        if (comp < 0) {
            final int newAResult = aResult * 10 + (a[j] == '?' ? 9 : a[j] - '0');
            final int newBResult = bResult * 10 + (b[j] == '?' ? 0 : b[j] - '0');
            return dfs(newAResult, newBResult, a, b, j + 1);
        } else if (comp > 0) {
            final int newAResult = aResult * 10 + (a[j] == '?' ? 0 : a[j] - '0');
            final int newBResult = bResult * 10 + (b[j] == '?' ? 9 : b[j] - '0');
            return dfs(newAResult, newBResult, a, b, j + 1);
        } else { // comp == 0;
            final int[][] t;
            if (a[j] == '?' && b[j] == '?') {
            ...
            for (final int[] d : t) {
                ...
                final int newAResult = aResult * 10 + d[0];
                final int newBResult = bResult * 10 + d[1];
                final int[] result = dfs(newAResult, newBResult, a, b, j + 1);
