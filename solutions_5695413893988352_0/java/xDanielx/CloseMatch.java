...
//      System.err.println(possibleScores);
    ...
    Collections.sort(possibleScores, new ScoresComparator());
    Scores bestScores = possibleScores.get(0);
    return bestScores.toString(l);
  }

  private static class Scores {
    final int codersScore, jammersScore;

    Scores(int codersScore, int jammersScore) {
    ...
    private static String pad(int score, int length) {
  ...
  private static class ScoresComparator implements Comparator<Scores> {
    ...
    public int compare(Scores a, Scores b) {
      int deltaDiff = a.getDelta() - b.getDelta();
      if (deltaDiff != 0)
        return deltaDiff;

      int codersDiff = a.codersScore - b.codersScore;
      if (codersDiff != 0)
        return codersDiff;

      return a.jammersScore - b.jammersScore;
    }
  }
}
