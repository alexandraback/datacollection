...
void go(int p,string str) {
    if( p==n ) {
       ...
       // cout<<aa<< " + "<<bb<<endl;
        int nm1=atoi( aa.c_str() );
        int nm2=atoi( bb.c_str() );
        //cout<< "So, "<<nm1<< " ** "<<nm2<<endl;
        if( abs(nm1-nm2) == mn ) {
            if( cc==nm1 ) {
                if( dd>nm2 ){
                    d=bb;
...
int main() {
    ...
    freopen("FFFFFFFFFFFBBBBOutCCCC.out","w",stdout);
    S(tc);
    while(tc--) {
        cin>>a>>b;
        mn=111111111;
