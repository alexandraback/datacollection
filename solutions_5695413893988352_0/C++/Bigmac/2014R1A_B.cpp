...
int main(int argc, char **argv)
{
	...
	for(int it_t = 1; it_t <= in_t; it_t++)
	{
		//init input
		//finish init input
        string c_orig = readString();
        string j_orig = readString();
        string rc;
        string rj;
        string result_c, result_j;
        auto fill_equal = [&](int a, int b) {
            for(int i = a; i <=b; i++) {
                if(rc[i] == '?' && rj[i] == '?') {
                    rc[i] = '0';
                    rj[i] = '0';
        ...
        auto preserve_best = [&]() {
            ...
            if(result_c.length() == 0) {
                ...
                ll best_cv = atoll(result_c.c_str());
                ll rcv = atoll(rc.c_str());
                ll best_jv = atoll(result_j.c_str());
                ll rjv = atoll(rj.c_str());
                ll b = llabs(best_cv - best_jv);
                ll r = llabs(rcv - rjv);
                if(r < b) {
                    result_c = rc;
                    result_j = rj;
        ...
        for(int i = 0; i < c_orig.length(); i++) {
            if(c_orig[i] != j_orig[i] && (c_orig[i] != '?' && j_orig[i] != '?')) {
                isEqual = false;
                if(w[c_orig[i]] > w[j_orig[i]]) {
                    //>
                    rc = c_orig;
                    rj = j_orig;
                    fill_equal(0,i-1);
                    fill_cmin(i+1, c_orig.length());
                    preserve_best();
                    ...
                    fill_cmax(i+1, c_orig.length());
                    preserve_best();
                    }
                    break;
                    
                } else if(w[c_orig[i]] < w[j_orig[i]]) {
                    // >
                    rc = c_orig;
                    rj = j_orig;
                    int j = i-1;
        ...
        if(isEqual) {
            rc = c_orig;
            rj = j_orig;
            fill_equal(0,c_orig.length());
            preserve_best();
        }
        fprintf(fo, "Case #%d: %s %s\n", it_t, result_c.c_str(), result_j.c_str());
