...
class Main{
	...
	void run(){
		...
		for(int q:range(T)){
			...
			tuple<ll,string,string> res = make_tuple(INF<ll>(),"","");
			for(int k:range(n+1))for(int k2:range(n+1))for(int p:range(2)){
				...
				for(int i = n-1;i>=0;i--){
					if(at[i]=='?' && bt[i]=='?'){
						...
						for(int v1:range(2))for(int v2:range(2)){
							at[i] = v1 + '0'; bt[i] = v2+ '0';
							if(mv > make_tuple(abs(stol(at.substr(i)) - stol(bt.substr(i))),v1,v2)){
								mv =make_tuple(abs(stol(at.substr(i)) - stol(bt.substr(i))),v1,v2);
							}
						}
						at[i] = get<1>(mv)+'0';
						bt[i] = get<2>(mv)+'0';
						...
						for(int v2 = max(bt[i]-1-'0',0);v2 <= min(bt[i]+1-'0',9);v2++){
							at[i] = v2 + '0';
							if(mv > make_tuple(abs(stol(at.substr(i)) - stol(bt.substr(i))),v2)){
								mv =make_tuple(abs(stol(at.substr(i)) - stol(bt.substr(i))),v2);
							}
						}
						at[i] = get<1>(mv)+'0';
					}else if(bt[i] == '?'){
				...
				auto t = make_tuple(abs(stol(at) - stol(bt)),at,bt);
