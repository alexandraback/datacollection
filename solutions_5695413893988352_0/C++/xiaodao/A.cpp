...
void upd(){
    int fa = stoi(a), fb = stoi(b);
    int faa = stoi(aa), fbb = stoi(bb);

    //cout << abs(fa-fb) << " "<< a << " " <<b << endl;

    if (abs(fa - fb) < abs(faa - fbb) || abs(fa - fb) == abs(faa - fbb) && fa < faa ||
        abs(fa - fb) == abs(faa - fbb) && fa == faa && fb < fbb){
        aa = a, bb = b;
    }

}

void dfs(bool in_a = true, int k = 0){
    //cout << " "<< a << " " <<b << endl;
