...
string to_str(int v) {
    stringstream ss;
    ss<<setfill('0')<<setw(n)<<v;
    return ss.str();
}

bool okay(string x, string xx) {
    const int n = x.size();
    if (n != (int)xx.size())
        return false;
    for (int i = 0; i < n; ++i)
...
void process_testcase(const int testcase, const int should_run) {
    ...
    if (should_run) {
        ...
        for (int va = 0; va <= 999; ++va) {
            string sa = to_str(va);
            if (!okay(a, sa)) continue;
            for (int vb = 0; vb <= 999; ++vb) {
                string sb = to_str(vb);
                if (!okay(b, sb)) continue;
                const int delta = abs(va-vb);
                if (delta < best_delta) {
                    best_a = sa;
                    best_b = sb;
                    best_delta = delta;
        ...
        cout<<"Case #"<<testcase<<": "<<best_a<<' '<<best_b<<endl;
    }
}
//////////////////////////////////////////////////// END OF SOLUTION ////////////////////////////////////////////////////
//////////////////////////////////////////////////// END OF SOLUTION ////////////////////////////////////////////////////
