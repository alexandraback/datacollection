    ...
    for (int i = cur; i < c.length(); ++i) {
        if (c[i] == '?' && j[i] == '?') {
            if (flag == 0) {
                ...
                int a1 = abs(strToInt(a.first) - strToInt(a.second));
                int b1 = abs(strToInt(b.first) - strToInt(b.second));
                int d1 = abs(strToInt(d.first) - strToInt(d.second));
                int m = min(min(a1, b1), d1);
                if (a1 == m) {
                    c = a.first;
                    j = a.second;
                    return make_pair(c, j);
