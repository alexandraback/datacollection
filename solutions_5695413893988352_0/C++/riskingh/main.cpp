...
/*
bool ok(int a, int b) {
    string sa = getString(a), sb = getString(b);
    bool ind = true;
...
bool ok(int a, int b) {
    for (int i = s1.size(); i < nums[a].size(); ++i)
        if (nums[a][i] != '0')
            return false;
    for (int i = 0; i < s1.size(); ++i)
        if (s1[i] != '?' && s1[i] != nums[a][i])
            return false;
    for (int i = s2.size(); i < nums[b].size(); ++i)
        if (nums[b][i] != '0')
            return false;
    for (int i = 0; i < s2.size(); ++i)
        if (s2[i] != '?' && s2[i] != nums[b][i])
            return false;
    return true;
}
...
pair<int, int> best(pair<int, int> x, pair<int, int> y) {
    ...
    int vx = abs(x.first - x.second), vy = abs(y.first - y.second);
    if (vx < vy)
        return x;
    if (vy < vx)
        return y;
...
void solve() {
    ...
    for (int i = 0; i < 1000; ++i) {
        for (int j = 0; j < 1000; ++j) {
            if (ok(i, j))
                ans = best(ans, make_pair(i, j));
        }
    }
    int sz = s1.size();
    s1 = nums[ans.first].substr(0, sz);
    s2 = nums[ans.second].substr(0, sz);
    reverse(s1.begin(), s1.end());
    reverse(s2.begin(), s2.end());
    cout << s1 << " " << s2;
}
