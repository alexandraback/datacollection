...
bool check(string s, int u) {
	string t = tostr(u, (int)s.length());
	if (t.size() > s.length())
		return false;
	...
	for (int i = 0; i < t; i++) {
		...
		for (int i = 0; i <= 999; i++)
			for (int j = 0; j <= 999; j++) {
				...
				if (!check(s2, j)) continue;
				int d = abs(i - j);
				if (d < ans1 ||
					(d == ans1 && i < ans2) ||
					(d == ans1 && i == ans2 && j < ans3)) {
					ans1 = d;
					ans2 = i;
					ans3 = j;
				}
		...
		cout << "Case #" << i + 1 << ": ";
		cout << tostr(ans2, (int)s1.length()) << " " << tostr(ans3, (int)s1.length()) << endl;
	}
	return 0;
}