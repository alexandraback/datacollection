...
{
    auto match = [](string s, int k)
    {
        for (int i = 0; i < s.length(); i++)
            if (s[s.length() - i - 1] == '?' || s[s.length() - i - 1] == k % 10 + '0')
                k /= 10;
            else
                return false;
        return true;
    ...
    {
        string s = getstr(), t = getstr();
        int mi = 0, mj = maxint;
        for (int i = 0; i < pow10[s.length()]; i++)
            if (match(s, i))
                for (int j = 0; j < pow10[t.length()]; j++)
                    if (match(t, j))
        ...
        string ss = to_string(mi);
        string tt = to_string(mj);
        while (ss.length() < s.length())
            ss = '0' + ss;
        while (tt.length() < t.length())
            tt = '0' + tt;
        printf("Case #%d: %s %s\n", ti, ss.data(), tt.data());
    }
    return 0;
