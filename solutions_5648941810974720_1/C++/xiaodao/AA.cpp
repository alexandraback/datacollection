...
#define ECH(it, A) for (__typeof((A).st()) it=(A).st(); it != (A).end(); ++it)
#define rECH(it, A) for (__typeof((A).rst()) it=(A).rst(); it != (A).rend(); ++it)
...
#define ALL(A) A.st(), A.end()
#define LLA(A) A.rst(), A.rend()
#define CPY(A, B) memcpy(A, B, sizeof(A))
#define INS(A, P, B) A.insert(A.st() + P, B)
#define ERS(A, P) A.erase(A.st() + P)
#define LBD(A, x) (lower_bound(ALL(A), x) - A.st())
#define UBD(A, x) (upper_bound(ALL(A), x) - A.st())
...
template<class T> inline T& UNQQ(T &A){A.resize(unique(ALL(A))-A.st());return A;}
...
int main(){
	...
	Rush{
	    printf("Case #%d: ", ++Case); RS(s);
		init(); RST(r); REP_S(cur, s) ++r[*cur-'A'];

		VI z; dfs(0,z); string zz;
		REP(i, 10) REP(j, z[i]) zz += i + '0';
		cout << zz << endl;
	}
}
