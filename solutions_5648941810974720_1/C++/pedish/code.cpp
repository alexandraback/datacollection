...
void solve(){
	...
	vector<int> Answer;
	if( cnt['Z'] ){
		For( i, cnt['Z'] ) Answer.push_back( 0 );
		cnt['E']-= cnt['Z'];
		cnt['R']-= cnt['Z'];
		cnt['O']-= cnt['Z'];
		cnt['Z']-= cnt['Z'];
	...
	if( cnt['W'] ){
		For( i, cnt['W'] ) Answer.push_back( 2 );
		cnt['T']-= cnt['W'];
		cnt['O']-= cnt['W'];
		cnt['W']-= cnt['W'];
	}
	...
	if( cnt['X'] ){
		For( i, cnt['X'] ) Answer.push_back( 6 );
		cnt['S']-= cnt['X'];
		cnt['I']-= cnt['X'];
		cnt['X']-= cnt['X'];
	}
	...
	if( cnt['G'] ){
		For( i, cnt['G'] ) Answer.push_back( 8 );
		cnt['E']-= cnt['G'];
		cnt['I']-= cnt['G'];
		cnt['H']-= cnt['G'];
		cnt['T']-= cnt['G'];
	...
	if( cnt['H'] ){
		For( i, cnt['H'] ) Answer.push_back( 3 );
		cnt['T']-= cnt['H'];
		cnt['R']-= cnt['H'];
		cnt['E']-= cnt['H'];
		cnt['E']-= cnt['H'];
	...
	if( cnt['R'] ){
		For( i, cnt['R'] ) Answer.push_back( 4 );
		cnt['F']-= cnt['R'];
		cnt['O']-= cnt['R'];
		cnt['U']-= cnt['R'];
		cnt['R']-= cnt['R'];
	...
	if( cnt['F'] ){
		For( i, cnt['F'] ) Answer.push_back( 5 );
		cnt['I']-= cnt['F'];
		cnt['V']-= cnt['F'];
		cnt['E']-= cnt['F'];
		cnt['F']-= cnt['F'];
	...
	if( cnt['V'] ){
		For( i, cnt['V'] ) Answer.push_back( 7 );
		cnt['S']-= cnt['V'];
		cnt['E']-= cnt['V'];
		cnt['E']-= cnt['V'];
		cnt['N']-= cnt['V'];
	...
	if( cnt['O'] ){
		For( i, cnt['O'] ) Answer.push_back( 1 );
		cnt['N']-= cnt['O'];
		cnt['E']-= cnt['O'];
		cnt['O']-= cnt['O'];
	}
	...
	if( cnt['N']/2 ){
		For( i, cnt['N']/2 ) Answer.push_back( 9 );
		cnt['I']-= cnt['N']/2;
		cnt['E']-= cnt['N']/2;
		cnt['N']-= cnt['N'];
	}
	...
	For( i, 256 ){
		if( cnt[i] ) cerr<< "EEROR!!"<< " "<< (char)i<< endl;
	}
