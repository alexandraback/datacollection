...
void solve(int casenum, outboth rout) {
  ...
  counts[0] = count(All(letters), 'Z');
  counts[2] = count(All(letters), 'W');
  counts[4] = count(All(letters), 'U');
  counts[5] = count(All(letters), 'F') - counts[4];
  counts[6] = count(All(letters), 'X');
  counts[7] = count(All(letters), 'S') - counts[6];
  counts[8] = count(All(letters), 'G');
  counts[1] = (count(All(letters), 'O') - counts[0] - counts[2] - counts[4]);
  counts[9] = (count(All(letters), 'N') - counts[1] - counts[7]) / 2;
  counts[3] = count(All(letters), 'R') - counts[0] - counts[4];

  rout << "Case #" << casenum << ": ";
  Loop(i, 10) Loop(j, counts[i]) rout << i;
  rout << endl;
...
int main(int argc, char **argv) {
  ...
  if (argc > 1) {
    ...
    outfile = make_shared<ofstream>("A.out.last");
  }
...
// clang-format off
// Local variables:
// compile-command: "c++ -fmodules -std=c++11 -Wall -Wextra -g -I/opt/brew/include -L/opt/brew/lib -o A A.cc -lgmp && ./A A.test.in"
