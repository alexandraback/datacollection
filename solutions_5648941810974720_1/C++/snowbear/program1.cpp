...
string solve() {
	...
	vector<int> order = { 0 , 6, 8, 4, 2, 3, 5, 1, 7, 9};

	vector<int> cntrs(200);
	next(string, s);
	for (auto c : s) cntrs[c]++;

	vector<vector<int>> req(10, vector<int>(200));
	FOR (i, 0, 10) for (auto c : strs[i]) req[i][c]++;
	...
	for (auto d : order) {
		while (true) {
			bool ok = true;
			FOR (c, 'A', 'Z' + 1) ok = ok && req[d][c] <= cntrs[c];

			if (!ok) break;

			FOR (c, 'A', 'Z' + 1) cntrs[c] -= req[d][c];
			result.push_back('0' + d);
		}
	}

	FOR (c, 'A', 'Z' + 1) assert(cntrs[c] == 0);
