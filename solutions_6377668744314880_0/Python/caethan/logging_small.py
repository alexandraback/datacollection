...
def read_input(infile):
    ...
    N = read_int()
    trees = np.empty((N, 2), dtype=int)
    for i in range(N):
        trees[i,:] = read_ints()
    return trees

def side(ij, jk):
    return ij[0] * jk[1] - ij[1] * jk[0]

def solve_case(T):
    
    mincut = [len(T) - 1 for i in range(len(T))]
    for i in range(len(T)):
        for j in range(len(T)):
            ...
            for k in range(len(T)):
                if k == i or k == j:
                    continue
                #Is k to the left or right of i-j
                s = side(T[j] - T[i], T[k] - T[j])
                if s < 0:
                    left += 1
                elif s > 0:
                    right += 1
            #print i, j, left, right
            if min(left, right) < mincut[i]:
                mincut[i] = min(left, right)
