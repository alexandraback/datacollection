...
0";
        ...
        {
            numberOfTrees = reader.ReadInt();
            trees = CreateArray<Point>(numberOfTrees);
            for ( int i = 0; i < numberOfTrees ; i++)
            {
                Point tree = new Point();
                tree.X = reader.ReadInt();
                tree.Y = reader.ReadInt();
                trees[i] = tree;
            }
        }
        ...
        int[] GetPerimeter (bool[] treesBeingTested )
        {
            ...
            for ( int i = 0 ; i < numberOfTrees; i++)
            {
                if ( treesBeingTested [i] && trees[i].X < minx)
                {
                    minx = trees[i].X;
                    startTree = i;
                }
            ...
            Point previousPoint = new Point ( trees[startTree].X , trees[startTree].Y +1 );
            int currentTree = startTree;
            do
            {
                int nextTree = -1;
                double angle = double.MinValue;
                double distance = double.MaxValue;
                Point cTree = trees[currentTree];
                 
                for ( int i = 0; i < numberOfTrees ; i++ )
                {
                    if ( treesBeingTested[i] && i != currentTree)
                    {
                        var tree = trees[i];
                        Point v1 = new Point(previousPoint.X - cTree.X, previousPoint.Y - cTree.Y);
                        Point v2 = new Point ( tree.X - cTree.X , tree.Y - cTree.Y);
                        double a = AngleBetween(v2, v1);
                        double d = v2.X * v2.X + v2.Y * v2.Y;
                ...
                perimeter.Add(nextTree);
                previousPoint = cTree;
                currentTree = nextTree;
            }
            while (currentTree != startTree);
            return perimeter.ToArray();
            
        }
        bool PerimeterContainsTree ( int tree, bool[] treesBeingTested)
        {
            return false;
        }

        bool[][] knownPerimeters;
        int[] knownPerimetersSize;
        int MovesToPutInBoundary ( int tree)
        {
            int maxSize = 0;
            long max = (long)Math.Pow ( 2 , numberOfTrees);
            for ( int i = 1 ; i < max; i++)
            {
                if ( knownPerimeters [i][tree])
                {
                    maxSize = Max(maxSize, knownPerimetersSize[i]);
                }
            }

            return numberOfTrees - maxSize;
        }
        protected override string SolveSmall()
        {
            int[] treesToCut = CreateArray<int>(numberOfTrees);
            long l = trees.Min (x=>x.X );
            string result = "";
            long max = (long)Math.Pow(2, numberOfTrees);
            knownPerimeters = new bool[max][];
            knownPerimetersSize = new int[max];
            bool[] treesBeingTested = CreateArray<bool>(numberOfTrees);
            for ( int i = 0 ; i < max ; i++)
            {
                string r = ConvertToBase(i, 2).PadLeft(numberOfTrees, '0');
                int treesCount = 0;
                for (int j = 0; j < r.Length; j++)
                {
                    if (r[j] == '0')
                        treesBeingTested[j] = false;
                    else
                    {
                        treesCount++;
                        treesBeingTested[j] = true;
                    }
                }
                knownPerimeters[i] = CreateArray<bool>(numberOfTrees);
                knownPerimetersSize[i] = treesCount;
                if (treesCount >= 3)
                {
                    var perimeter = GetPerimeter(treesBeingTested);
                    foreach (var tree in perimeter)
                        knownPerimeters[i][tree] = true;
                }
            }

            for (int i = 0; i < numberOfTrees; i++ )
            {
                int moves = MovesToPutInBoundary(i);
