...
bool aline(const point& sp, const point& ep, const point& op){
    return (sp.x - op.x) * (ep.y - op.y)
        == (ep.x - op.x) * (sp.y - op.y);
}
...
void run(){
    ...
    for (int i=0; i<N; i++){
        ...
        for (int mask=0; mask<1<<N; mask++){
            ...
            n = graham(t, n, res);
            for (int j=0; j<n; j++){
                if ( aline(res[j], res[(j+1)%n], p[i]) ){
                    ans[i] = min(ans[i], N-count_bits(mask) );
                }
            }
        }
        assert( ans[i] <= max(N-3,0) );
        cout<<ans[i]<<endl;
    }
