...
double X_Mul(N a1,N a2,N b1,N b2)
{
    N v1 = {a2.x-a1.x,a2.y-a1.y},v2 = {b2.x-b1.x,b2.y-b1.y};
    return v1.x*v2.y - v1.y*v2.x;
}

void Cal(N p1,N p2,int n,int &in,int &notin)
{
    in = 0,notin = 0;

    for(int i = 1;i <= n; ++i)
    {
        double tmp = X_Mul(p1,p2,p1,p[i]);
        if(fabs(tmp) <= EPS)
            in++;
        else if(tmp < 0)
            notin++;
   ...
   // freopen("A-large.in","r",stdin);
    ...
    while(T--)
    {
        ...
        for(i = 1;i <= n; ++i)
        {
            for(j = i+1;j <= n; ++j)
            {
                Cal(p[i],p[j],n,in,notin);
                ans = min(notin,n-in-notin);
                anw[i] = min(anw[i],ans);
                anw[j] = min(anw[j],ans);
            }
        }
