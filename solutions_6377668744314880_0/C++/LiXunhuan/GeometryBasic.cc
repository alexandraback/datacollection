...
const TYPE INF=1e9;
TYPE square(TYPE k){//平方
    return k*k;
}
TYPE standard(TYPE k){//防止反三角函数超过定义域
    if(k>1)k=1;
    if(k<-1)k=-1;
    return k;
}
TYPE radian2degree(const TYPE radian){//弧度转角度
    return radian/PI*180;
}
TYPE degree2radian(const TYPE degree){//角度转弧度
    return degree/180*PI;
}
int signum(const TYPE x){//判断是否为0
    if(x<-EPS)return -1;
    if(x>EPS)return 1;
    return 0;
}
TYPE regular(TYPE theta){//将一个角变到[0,2*pi)
...
public:
    ...
    Point(const TYPE _x,const TYPE _y){
        x[0]=_x;
        x[1]=_y;
    }
    void out(const char*s)const{
        printf(s,x[0]+EPS,x[1]+EPS);
    }
	Point in(){
		scanf("%lf%lf",x,x+1);
		return *this;
    ...
    TYPE operator^(const Point&o)const{//点积
        TYPE ans=0.0;
        for(int i=0;i<2;++i)
            ans+=x[i]*o.x[i];
        return ans;
    ...
    static Point unit(const TYPE theta){//单位向量
        Point v;
        v.x[0]=cos(theta);
        v.x[1]=sin(theta);
        return v;
    }
    TYPE operator*(const Point&b)const{//叉积
        return x[0]*b.x[1]-x[1]*b.x[0];
    }
    Point operator*(const TYPE k)const{//数乘
        Point tmp;
        tmp.x[0]=x[0]*k;
        tmp.x[1]=x[1]*k;
        return tmp;
    ...
    TYPE to(const Point&o)const{//两点距离
        return (*this,o).len();
    }
...
public:
    ...
    Point Projection(const Point&pt)const{//点在直线上的投影
        ...
        TYPE tmp=((pt-p[0])^(p[1]-p[0]).unit());
        return (p[1]-p[0]).unit()*tmp
        +p[0];
    }
    TYPE to(const Point&pt)const{//点到直线距离
...
public:
    ...
    TYPE to(const Point&pt)const{//点到线段距离
        Point proj=Projection(pt);
...
public:
    ...
    void add(TYPE x,TYPE y){//加点,简单的写法
        add(Point(x,y));
    }
    void clear(){//清空
        n=0;
...
public:
    ...
    vector<Point> operator*(const Line&l)const{//直线与圆的交点
        ...
        TYPE dd=sqrt(square(radius)-square(pt.to(center)));
	...
	vector<Line> operator*(const Point&p)const{//过圆外一点的切线
		vector<Line> res;
		if(posToCircle(p)==-1)return res;
		if(posToCircle(p)==0){
			res.push_back(Line(p,p+(center,p).verticalVector()));
		}
		else {
			TYPE l1=p.to(center);
			TYPE l2=radius*radius/l1;
			Point v=(center,p)/l1*l2;
			Point vv=v.verticalVector()/l2*sqrt(radius*radius-l2*l2);
			res.push_back(Line(p,center+v+vv));
			res.push_back(Line(p,center+v-vv));
		}
		return res;
	}
...
class Polygon:public PointSet{//多边形,顺时针
	public:
		...
		TYPE to(const Point&pt)const{//点到多边形距离
			TYPE tmp=INF;
			for(int i=0;i<n;i++){
...
class minDisPair:public PointSet{//最近点对O(n*log(n))
	...
	minDisPair close(const TYPE X,const TYPE ds)const{
		minDisPair tmp;
		for(int i=0;i<n;i++)
	...
	TYPE solve(){//计算
		...
		TYPE tmp=min(left.solve(),right.solve());
...
class maxDisPair:public ConvexHull{//最远点对
	public:
		TYPE _solve(){//计算,注意函数名,和base的solve区分,因为要用到base的solve,所以不能重载掉
			TYPE tmp=0;
			Polygon rs=solve();
			int j=1;
...
class Rotate{//旋转
	public:
		Point rotate(const Point&o,const Point&c,const TYPE theta)const{//点o绕c逆时针旋转theta,弧度
			Point vec=(c,o);
			TYPE x=vec.x[0]*cos(theta)-vec.x[1]*sin(theta);
			TYPE y=vec.x[0]*sin(theta)+vec.x[1]*cos(theta);
			return c+Point(x,y);
		}
		Line rotate(const Line&o,const Point&c,const TYPE theta)const{//直线(线段)o绕c逆时针旋转theta,弧度
			return Line(rotate(o.p[0],c,theta),rotate(o.p[1],c,theta));
		}
		PointSet rotate(const PointSet&o,const Point&c,const TYPE theta)const{//点集(多边形,三角形等等)o绕c逆时针旋转theta,弧度
			PointSet tmp;
			for(int i=0;i<o.n;i++)
...
class Sector{//扇形
	public:
		...
		Point p[2];
		Sector(){}
		Sector(const Circle&oo,const TYPE aa){
			o=oo;
			theta=aa;
			Rotate rt;
			p[0]=Point(o.radius,0);
			p[1]=rt.rotate(p[0],o.center,theta);
		}
		...
		TYPE area()const{//扇形面积
			return square(o.radius)*theta/2;
		}
		TYPE arcuate()const{//弓形面积
			return area()-square(o.radius)*sin(theta)/2;
		}
		Point centroid()const{//重心
			TYPE S=area();
			TYPE l=sqrt(S/sin(theta));
			Point p1=(p[0],p[1]).unit()*l;
			Point p2=(p[0],p[2]).unit()*l;
			return (p1+p2)/2;
		}
};

TYPE operator*(Circle&o,Triangle&t){//圆和三角形的面积交
	...
	if(cc==0){
		...
		TYPE res=Triangle(t.p[0],a,b).area()+Sector(o,a,b).arcuate();
...
TYPE operator*(Circle&o,Polygon&p){//圆和多边形的面积交
	TYPE res=0;
	for(int i=1;i<p.n-1;i++){
		Triangle t=Triangle(p.p[0],p.p[i],p.p[i+1]);
		TYPE tmp=1;
		if((t.p[0],t.p[1])*(t.p[0],t.p[2])<0)tmp*=-1;
		tmp*=fabs(o*t);
		res+=tmp;
	}
	return res;
...
int main(){
	...
	for(int cas=1;cas<=ca;cas++){
		...
		for(int msk=1;msk<(1<<n);msk++){
			ConvexHull ps;
			for(int i=0;i<n;i++)
				if((1<<i)&msk)ps.add(pp[i]);
			Polygon res=ps.solve();
			for(int i=0;i<n;i++){
				for(int j=0;j<res.n;j++){
					if(Line(res.p[j],res.p[(j+1)%res.n]).to(pp[i])<EPS)ans[i]=min(ans[i],n-getbit(msk));
				}
			}
		}
		for(int i=0;i<n;i++)
