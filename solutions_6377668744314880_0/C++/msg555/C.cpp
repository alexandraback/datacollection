...
/* Finds the closest two points in A in N log N time. */
...
int main() {
  ...
  for (int t = 1; t <= T; t++) {
    ...
    for (int i = 0; i < N; i++) {
      poly B = A;
      B.erase(B.begin() + i);
      sort(B.begin(), B.end(), [&](point p0, point p1) {
        return radial_compare(p0 - A[i], p1 - A[i]);
      });
      int k = 0;
      int res = N - 1;
      for (int j = 0; j + 1 < N; j++) {
        k = max(j + 1, k);
        while (k < B.size() &&
               radial_compare(B[j] - A[i], B[k] - A[i], A[i] - B[j])) {
          k++;
        }
        res = min(res, k - j - 1);
        B.push_back(B[j]);
