...
// 凸包の端点だけ入れる。辺上の点を入れるなら、whileの条件式を<0ではなく、==-1にする。（ただし同一点を含まないとき）
vector<P> graham(vector<P> v) {
  ...
  for (int i = 0; i < n; ret[k++] = v[i++])
    while (k >= 2 && ccw(ret[k-2],ret[k-1],v[i]) == -1) k--;
  for (int i = n-2, t = k+1; i >= 0; ret[k++] = v[i--])
    while (k >= t && ccw(ret[k-2],ret[k-1],v[i]) == -1) k--;
  ret.resize(k-1);
  return ret;
}
...
int main() {
  ...
  REP(cs,T) {
    ...
    REP(S,1<<n) {
      vector<P> vv;
      REP(i,n) {
        if (S>>i&1) vv.push_back(v[i]);
      }
      vector<P> ret = graham(vv);
      FOR(it, ret) {
        int id = mp[*it];
        chmin(ans[id], n-__builtin_popcount(S));
      }
    }
