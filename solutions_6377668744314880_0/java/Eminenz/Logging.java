...
public class Logging {
	...
	public static void GrahamScan(Point2D[] pts) {
		hull = new Stack<>();
	...
	public static void main(String[] args) throws IOException {
		...
		try (Scanner sc = new Scanner(stream)) {
			...
			for (int c = 1; c <= t; c++) {
				...
				out.append("Case #").append(c).append(":\n");
					...
					if (hull.contains(points[i])) {
						...
						for (Point2D h : hull) {
							...
							for (int j = 0; j < points.length; j++) {
								int ccw = Point2D.ccw(h, points[i], points[j]);
								if (ccw < 0) {
									left++;
								} else if (ccw > 0) {
									right++;
	 ...
	 * <p>
	 * Note: in order to deal with the difference behavior of double and Double
	 * with respect to -0.0 and +0.0, the Point2D constructor converts any
	 * coordinates that are -0.0 to +0.0.
	 * 
	 * For additional documentation, see <a href="/algs4/12oop">Section 1.2</a>
	...
	final static class Point2D implements Comparable<Point2D> {
		...
		/**
		 * Compares two points by atan2() angle (between -pi and pi) with
		 ...
		 *             if either <tt>x</tt> or <tt>y</tt> is <tt>Double.NaN</tt>
