...
/* @JUDGE_ID: 1848XX 2015CodeJamRound1A_C Java */
...
class Main {
    ...
    /**
     * This is an implementation of Graham Scan Convex Hull
     * algorithm in Java. It takes O( NlogN ) in sorting & O(N) in
     * actual convex hull finding for N points.
     *
     * Each point passed in via the "points" array should be unique.
     * If duplicates are passed in the returned polygon might not 
     * be a convex hull.
     *
     * @param points the points
     * @return the convex hull, in counter clock-wise.
     */
...
class Point implements Comparable<Point> {
    ...
    /**
     * if p -> q -> r is left, then return > 0.
     * if p -> q -> r is right, then return < 0.
     * if p -> q -> r is a line, then = 0
     */
...
class Polygon {
     ...
     * we better to add the vertices in counter-clockwise order
     */
