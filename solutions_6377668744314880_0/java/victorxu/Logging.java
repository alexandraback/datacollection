			...
			for(int j = 0; j < n; j++){
				...
				for(int k = 0; k < n; k++){
					if(j > k) {
						if(coor[k][1] - coor[j][1] == 0 && coor[k][0] - coor[j][0] < 0) angle[k] = Math.PI;
							...
							//System.out.println(coor[k][1] - coor[j][1]);
							if((coor[k][1] - coor[j][1]) < 0) angle[k] = angle[k] + Math.PI;
						}
						//System.out.println(angle[k]);
					}
					if(j < k) {
						...
						//System.out.println("a" + (k-1) + " " + angle[k-1]);
					}
				}
				Arrays.sort(angle);
				...
				if(h < 0){
					...
					while(h < n-2){
						...
						//if(Math.abs(angle[l] - top + Math.PI) < 0.0001) {
						//	System.out.println("close");
						//	System.out.println(i + " " + j + " " + (angle[l] - top + Math.PI));
						//}
						r = h - l;
						...
						//if(h < n-2 && Math.abs(angle[h+1] - top) < 0.0001){
						//	System.out.println("close");
