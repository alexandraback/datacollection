...
namespace Googlers
{
	...
	class Program
	{
		...
		{
			IExecutor executor = new GooglerConvertor();
			executor.Execute();
		}
	}

	public class GooglerConvertor : IExecutor
	{
		private Dictionary<char, char> _googlerMap = new Dictionary<char, char>
		{
			{'a','y'},
			{'b','h'},
			{'c','e'},
		...
		public void Execute()
		{
			using (var prob = new CodeJamProblem(@"\A.small.in"))
			{
				int t = prob.ReadLineInt32();
				for (int i = 0; i < t; i++)
				{
					char[] input = prob.ReadLine().ToCharArray();
					char[] output = new char[input.Length];

					for (int j = 0; j < input.Length; j++)
					{
						output[j] = _googlerMap[input[j]];
					}
					prob.OutputCase(new string(output));
				}
			}
		}
	}
