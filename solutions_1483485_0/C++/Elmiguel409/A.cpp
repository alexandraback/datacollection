...
string A = "z y qee ejp mysljylc kd kxveddknmc re jsicpdrysi rbcpc ypc rtcsra dkh wyfrepkym veddknkmkrkcd de kr kd eoya kw aej tysr re ujdr lkgc jv";
string B = "q a zoo our language is impossible to understand there are twenty six factorial possibilities so it is okay if you want to just give up";
string line;
...
int main(){
  cin >> _T;
  map<char,char> hash;
  FOR(i,0,A.length()) hash[A[i]] = B[i];
  getline(cin,line);

  FOR(i,0,_T){
    cout << "Case #" << (i+1) << ": ";
    getline(cin,line);
    FOR(j,0,line.length()) line[j] = hash[line[j]];
    cout << line;
    cout << endl;
  }
  return 0;
