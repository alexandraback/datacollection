...
Case #5: 2 3";
        ...
        private string Iterate ( string s , string original, string allGolds)
        {
            s = s.Replace("G", "A").Replace("L", "B");
            s = s.Replace("A", allGolds).Replace("B", original);
            return s.Substring  ( 0 , Math.Min ( 1000 , s.Length ));
        }

        string[] Calculate ( int k , int c, int s)
        {
            ...
            else
            {
                ...
                for (int i = c + 1; i <= k; i++)
                    ...
                    for (int i = 0; i <= k - c; i++)
                    {
                        result.Add((aux + i).ToString());
                        last = aux + i;
                    }
                     * */
            }
            //for (int i = result.Count, j = 1; i < s; i++, j++)
            //    result.Add((last + j).ToString());
        ...
        protected override string SolveSmall()
        {
            ...
            var v = Calculate(K, C,S);
            if (v.Length > S)
                return "IMPOSSIBLE";
            List<string> v2 = v.ToList();
            v2.Sort();
            ...
            {
                l[i] = new char[K][];
                var s = ConvertToBase(i, 2, "GL").PadLeft(K, 'G');
                var r = s;
              //  l[i][0] = s.ToCharArray();
                for (int j = 1; j < C; j++)
                {
                    r = Iterate(r, s, allGolds);
                    if (j == C - 1)
                        r1.AppendLine(r);//r.Replace ( "G" , "G ").Replace ( "L" , "L "));
            //        l[i][j] = r.ToCharArray();
                }
            }
            */
