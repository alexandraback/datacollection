...
class bigint {
    ...
    bigint &trim()
    { while ( limbs.size() > 0 && limbs.back() == 0 ) limbs.pop_back(); if ( limbs.size() == 0 ) sign = 1; return *this; }
    ...
    static void binary_search_div( const bigint &a, const bigint &b, bigint &quo, bigint &rem )
    {
        rem.limbs.clear();
