...
public class Main {
    ...
    static class TaskD {
        public long[] calc(long alreadyCounted, long size, long K) {//size = K^c
            long sum = 0;
            long count = alreadyCounted;
            while (size >= K && count < K) {
                size /= K;
                sum += size * count;
                count++;
            }
            return new long[]{count, sum + 1};
        }
        ...
        public void solve(int testNumber, MyReader in, PrintWriter out) {
            ...
            if (C == 1 || K == 1) {
                ...
                long currSum = 0;
                while (checked < K) {
                    long[] rez = calc(checked, max, K);
                    checked = rez[0];
                    currSum = rez[1];
                    positions.add(currSum);
                }
                if (positions.size() > S) {
                    out.println("Case #" + testNumber + ": IMPOSSIBLE");
                } else {
                    out.print("Case #" + testNumber + ": ");
                    for (long t : positions) {
                        out.print(t + " ");
                    }
                    out.println();
                }
