...
public class Google
{
    ...
    {
        long size = Long.parseLong(input[0][0]);
        long complexity = Long.parseLong(input[0][1]);
        long students = Long.parseLong(input[0][2]);

        if (students * complexity < size)
            return outputCase(output, "IMPOSSIBLE");

        String str = "";

        long numberOfLoops = size / complexity + (size % complexity != 0 ? 1 : 0);

        for (int i = 0; i < numberOfLoops; i ++)
        {
            long position = i * complexity + 1;
            long sizeOfBunch = 1;

            for (int j = 1; (j < complexity) && (j + i * complexity < size); j ++)
            {
                sizeOfBunch *= size;
                position = position + sizeOfBunch * (i * complexity + j);
            }

            str += String.valueOf(position) + " ";
        }

        return outputCase(output, str);
    }
   ...
   /* long size = Long.parseLong(input[0][0]);
    ...
    String str = "";
    long start = 0;
    long sizePowerComplexity = complexity == 1 ? 0 : 1;
    for (int i = 1; i < complexity; i ++)
    sizePowerComplexity *= size;
    long bunchSize = sizePowerComplexity;
    ...
    {
        long position = i * complexity + 1;
        long sizeFoisComplexity = 1;

        for (int j = 1; j < complexity; j ++)
        {
            sizeFoisComplexity *= size;
            if (start + position + j * sizeFoisComplexity > sizePowerComplexity)
                break;
            position = position + j * sizeFoisComplexity;
        }

        str += String.valueOf(start + position) + " ";
