...
public class Solution {
    ...
    int[] map = new int[10_001_000];

    {
        Arrays.fill(map, Integer.MAX_VALUE);
        map[1] = 1;
        for (int i = 1; i < map.length - 1; i++) {
            map[1 + i] = Math.min(1 + map[i], map[1 + i]);
            int r = Integer.parseInt(new StringBuilder(Integer.toString(i)).reverse().toString());
            if (r >= map.length)
                continue;
            map[r] = Math.min(1 + map[i], map[r]);
        }
    }

    private long solveSmall(int n) {
        return map[n];
    }
