...
void generate(const int startPos, int pos, int last, int wrong, int v, vector < pair <pii, int > > &all)
{
	if (v == -1) return;

	word[pos] = 0;
	if (trie[v].term)
		all.pb(mp(mp(pos, last), wrong));
	...
	if (trie[v][c] != -1)
	{
		...
		generate(startPos, pos + 1, last, wrong, trie[v][c], all);
	}
	// mistake
	if (startPos + pos - last >= 5)
	{
		...
		{
			word[pos] = c;
			generate(startPos, pos + 1, startPos + pos, wrong + 1, trie[v][c - 'a'], all);
			word[pos] = 0;
		}
	}
}
...
int f(int pos, int last)
{
	...
	generate(pos, 0, last, 0, 0, all);

	for (int i = 0; i < (int)all.size(); i++)
	{
		int cres = f(pos + all[i].fi.fi, all[i].fi.se) + all[i].se;
		res = min(res, cres);
	}
	return res;	
}
