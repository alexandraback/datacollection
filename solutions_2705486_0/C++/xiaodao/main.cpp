...
namespace ACM{ // Aho¨CCorasick Automator
    const int N = 1125079, Z = 26, L = 1000009;
...
inline int new_node(){
    fail[tot] = cnt[tot] = 0, RST(trans[tot]);
    return tot++;
}
...
inline int Run(){
    RS(str);
    p = 0, q = 1; FLC(dp[0], 0x3f); dp[0][0][0] = 0;

#define ii (cc == c ? 0 : 4)

    //cout << str <<endl;

    REP(u, tot) REP(c, 26) if (!trans[u][c]) trans[u][c] = tot;


    REP_S(cur, str){
        swap(p, q); FLC(dp[p], 0x3f);

        int cc = *cur - 'a';

        //cout << cc << endl;

//#define u (cnt[uu] ? 0 : uu)

        REP(u, tot) REP(i, 5) if (dp[q][u][i] != INF){


            if (i == 0){
                REP(c, 26){
                    checkMin(dp[p][v][ii], dp[q][u][i]  + (cc != c));
                }
            }
            else{
                int c = cc;
                //cout << *cur << " " << u << " " << v << " " <<i << endl;
                checkMin(dp[p][v][i-1], dp[q][u][i]);
            }
        }

        ECH(it, Li) REP(i, 5){
            checkMin(dp[p][0][i], dp[p][*it][i]);
        }

        int res = INF; REP(i, 5) checkMin(res, dp[p][0][i]);

        /*REP(u, tot) REP(i, 5) if (res == dp[p][u][i]){
            //cout << u << " ";
            u = tot;
            break;
        }*/
