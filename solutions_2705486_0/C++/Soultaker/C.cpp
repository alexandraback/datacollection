...
static void initialize()
{
    ...
    string word;
    int words = 0;
    for (ifstream ifs("garbled_email_dictionary.txt"); getline(ifs, word); ++words)
    {
        int pos = 0;
        FORIT(it, word) {
            int ch = (*it - 'a');
            ...
            if (trie[26*pos + ch] < 0)
            {
                trie[26*pos + ch] = terminal.size();
                terminal.push_back(false);
                trie.resize(trie.size() + 26, -1);
            }
            pos = trie[26*pos + ch];
        }
        terminal[pos] = true;
...
struct Case : public AbstractCase
{
    ...
    int calc(int fix, int node, int pos);
    string S;
};
...
int Case::calc(int fix, int node, int pos)
{
    if (pos == SZ(S))
    {
        return terminal[node] ? 0 : inf;
    }

    const LL key = (node*5 + fix)*SZ(S) + pos;
    IT(memo) it = memo.find(key);
    ...
    int res = inf;
    if (trie[26*node + ch] >= 0)
    {
        res = min(res, calc(max(fix - 1, 0), trie[26*node + ch], pos + 1));
    }
    if (fix == 0)
    {
        REP(i, 26) if (trie[26*node + i] >= 0) {
            res = min(res, 1 + calc(4, trie[26*node + i], pos + 1));
        }
    }
    if (node != 0 && terminal[node])
    {
        res = min(res, calc(fix, 0, pos));
    }
    return memo[key] = res;
}
