...
public class Erdem {
	...
	static class Dictionary {
		Dictionary sub[] = new Dictionary['z'-'a'+1];
		...
		void add( String word, int i ) {
			...
			if( sub[c] == null ) {
				sub[c] = new Dictionary(word.substring(0, i+1));
			}
			sub[c].add(word, i+1);
		}
	...
	static class A extends Thread {
		...
		String result = null; // DON'T REMOVE
		
		static Dictionary theDict;
		static {
			theDict = new Dictionary("theDict");
			Scanner s = null;
			try {
				s = new Scanner(new File("src/gcj13/r1bc/garbled_email_dictionary.txt"));
			} catch (FileNotFoundException e) {
				e.printStackTrace();
			}
			while (s.hasNextLine()) {
		        theDict.add(s.nextLine(), 0);
		    }
		}
		
		private String s;
		long theMin = MAX_VALUE;
		int allMin[] = new int[4001];
		...
		public void run() {
			allMin[s.length()] = 1;
			result = ""+solve(theDict, 0, 0, -10);
		}

		private int solve(final Dictionary dic, final int i, final int change, final int lastChange) {
			if( dic == theDict && allMin[i] > 0 && i-lastChange >= 5 ) {
				return allMin[i]-1;
			}
			if( i >= s.length() ) {
				if (dic.hasWord)
			...
			final int i2 = i+1;
			if( dic.sub[c] != null ) {
				min = solve(dic.sub[c], i2, change, lastChange);
			}

			if( dic.hasWord ) {
				min = Math.min(min, solve(theDict, i, change, lastChange));
			}
			
			if( i-lastChange >= 5 ) {
				final int change2 = change+1;
				for( int j = 0; j < c; j++ ) {
					if( dic.sub[j] != null ) {
						min = Math.min(min, solve(dic.sub[j], i2, change2, i)+1);
					}
				}
	
				for( int j = c+1; j <= 'z'-'a'; j++ ) {
					if( dic.sub[j] != null ) {
						min = Math.min(min, solve(dic.sub[j], i2, change2, i)+1);
					}
				}
			...
			if( min< MAX_VALUE && min+change < theMin ) {
				theMin = min+change;
			}
