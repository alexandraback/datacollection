...
import sys, os, subprocess, time, ctypes, optparse
...
def compileIfNeeded():
    ...
    if os.path.exists(target) and os.path.getmtime(cpp) < os.path.getmtime(target):
        print '%s is uptodate' % target
        return

    print 'Compiling %s into %s...' % (cpp, target)
    if DEBUG:
        opts = '-g'
    else:
        opts = '-g -O3'
    subprocess.check_call('g++ -Wall -g %s -fPIC -std=c++0x %s -shared -o %s'
