...
public class CodeJam {
	...
	int solveItSecond(int rows, int cols, int n) {
		if (rows == 1 || cols == 1) {
		...
		int secondRow = (cols - 1) / 2 + 1;
		int firstRow = cols - secondRow;
	...
	int solveBrut(int rows, int cols, int n) {
...
//		int n = in.nextInt();
		...
		gen(0, 0, n);
		int solveI = solveIt(rows, cols, n);
		int solveItS = solveItSecond(rows, cols, n);
		solveI = Math.min(solveI, solveItS);
		if (solveI != best) {
			out.println(rows + " " + cols + " " + n + " it=" + solveI + ", best=" + best);
		}
		return best;
	}
	...
	int solveFor() {
		...
		int res2 = solveItSecond(cols, rows, n);
	...
	static private final String tests = "B3";
