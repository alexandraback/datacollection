...
int main()
{
    ...
    for (size_t test_number = 1; test_number <= TESTS; test_number++) {
        ...
        if (C < R)
            swap(R, C);
        if (R == 1 || C == 1) {
            ll worst_result = (C - (ll)1);
            worst_result = max((ll)0, worst_result - (C - N) * (ll)2);
            ...
            if (N - half == 1) {
                ...
                worst_result = (ll)4 + (N - half - (ll)2) * (ll)3;
            }
            cout << worst_result << endl;
            continue;
        } else if (R == 3 && C == 3 && N == 5) {
        ...
        ll worst_result = C * (R - (ll)1) + R * (C - (ll)1);
        ll all_rooms = R * C;

        ll rooms_with_four = ((R - (ll)2) * (C - (ll)2) + (ll) 1) / (ll)2;
        if (N < all_rooms) {
        ...
        if (N < all_rooms) {
            if (R % (ll)2 == 1 && C % (ll)2 == 1) {
                worst_result += 1;
                rooms_with_three = (ll)2 * (ll)(R / (ll)2 + C / (ll)2) - rooms_with_three - (ll)1;
                ll taken = min(rooms_with_three, all_rooms - N);
                N += taken;
                worst_result -= taken * (ll)3;
                rooms_with_two = 0;
