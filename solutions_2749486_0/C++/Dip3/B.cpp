...
int recx(int pos,int step)
{
    if(x==pos) return step;
    int sign=1;
    if(x<0) sign=-1;
    if((x-(pos+step*sign))*(x-pos)<0)
        sign*=-1;
    if(sign>0) printf("E");
    else printf("W");
    return recx(pos+step*sign,step+1);
}

int recy(int pos,int step)
{
    if(y==pos) return step;
    int sign=1;
    if(y<0) sign=-1;
    if((y-(pos+step*sign))*(y-pos)<0)
        sign*=-1;
    if(sign>0) printf("N");
    else printf("S");
    return recy(pos+step*sign,step+1);
}
