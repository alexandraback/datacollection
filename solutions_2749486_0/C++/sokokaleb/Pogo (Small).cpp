...
struct data
{
	...
	data(ii coorr,int lastt,int cntt,string anss):coor(coorr),last(lastt),cnt(cntt),ans(anss){}
};

ii start=mp(1000,1000);
int n,x,y;
bitset<205> vis[205];
char prevC[]={"NESW"};
...
int main()
{
	...
	FOR (tc,0,n)
	{
		...
		if (x<0)
		{
			FOR (i,0,abs(x)) ans+="EW";
		}
		else
		{
			FOR (i,0,abs(x)) ans+="WE";
		}
		if (y<0)
		{
			FOR (i,0,abs(y)) ans+="NS";
		}
		else
		{
			FOR (i,0,abs(y)) ans+="SN";
		}
		puts(ans.c_str());
	}
	return 0;
