...
int main()
{
	...
	REPN(cases,T)
	{
		...
		while (!q.empty())
		{
			...
			if (sudah[pos.fi + X][pos.se + X]) continue;
			sudah[pos.fi + X][pos.se + X] = 1;
			REP(i,4)
			{
				int x = pos.fi + dirx[i] * (SIZE(move) + 1);
				int y = pos.se + diry[i] * (SIZE(move) + 1);
				if (-X <= x && x <= X && -X <= y && y <= X)
				{
					if (sudah[x + X][y + X]) continue;
					q.push(mp(mp(x,y),move+dir[i]));
				}
			}
		}
		puts("-1");
