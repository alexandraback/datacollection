...
int read(bignum_t a, istream &is = cin)
{
	...
	for(i = 1; i <= a[0]; i++)
		for(a[i] = 0, j = 0; j < DIGIT; j++) a[i] = a[i] * 10 + buf[i * DIGIT - 1 - j] - '0';
	for(; !a[a[0]] && a[0] > 1; a[0]--);
	return 1;
}
...
void write(const bignum_t a, ostream &os = cout)
{
	...
	for(os << a[i = a[0]], i--; i; i--)
		for(j = DEPTH / 10; j; j /= 10) os << a[i] / j % 10;
}
...
void mul(bignum_t c, const bignum_t a, const bignum_t b)
{
	...
	for(c[0] = a[0] + b[0] - 1, i = 1; i <= a[0]; i++)
		for(j = 1; j <= b[0]; j++)
			if((c[i + j - 1] += a[i] * b[j]) >= DEPTH) c[i + j] += c[i + j - 1] / DEPTH, c[i + j - 1] %= DEPTH;
...
void mul(bignum_t b, const bignum_t a, const int c, const int d)
{
	...
	for(b[0] = a[0] + d, i = d + 1; i <= b[0]; i++)
		if((b[i] += a[i - d] * c) >= DEPTH) b[i + 1] += b[i] / DEPTH, b[i] %= DEPTH;
...
void initefile()
{
	fin = fopen("c:/googlejam/Bsmall.txt","r");
	fout = fopen("c:/googlejam/Bsmall_out.txt","w");
}
void outfile()
{
	fclose(fin);
