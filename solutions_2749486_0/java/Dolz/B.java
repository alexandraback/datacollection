...
/*
 ...
 static class havePOS {
 int x, y, move;
 String path;

 havePOS(int x, int y, int m, String p) {
 this.x = x;
 this.y = y;
 this.move = m;
 path = p;
 ...
 Queue<havePOS> q = new LinkedList<havePOS>();
 boolean[][][] v = new boolean[205][205][502];
 v[100][100][0] = true;
 q.add(new havePOS(0, 0, 0, ""));
 while (!q.isEmpty()) {
 havePOS cur = q.poll();
 if (cur.x == x && cur.y == y) {
 ...
 q.add(new havePOS(cur.x + cur.move + 1, cur.y, cur.move + 1, cur.path + "E"));
 }
 ...
 q.add(new havePOS((cur.x - (1 + cur.move)), cur.y, cur.move + 1, cur.path + "W"));
 }
 ...
 q.add(new havePOS(cur.x, cur.y + cur.move + 1, cur.move + 1, cur.path + "N"));
 }
 ...
 q.add(new havePOS(cur.x, (cur.y - (1 + cur.move)), cur.move + 1, cur.path + "S"));
 }
 }
 return "-1";
 }
	...
	public static void main(String[] args) {
		...
		while (t-- > 0) {
			...
			while (!q.isEmpty()) {
				...
				if (cur.x + cur.move + 101 <= 200
						&& cur.y + 100 <= 200
						&& cur.move <= 500
						&& !v[cur.x + cur.move + 101][cur.y + 100][cur.move + 1]) {
					v[cur.x + cur.move + 101][cur.y + 100][cur.move + 1] = true;
					q.add(new havePOS(cur.x + cur.move + 1, cur.y,
							cur.move + 1, cur.path + "E"));
				}
				if ((cur.x - (1 + cur.move)) + 100 >= 0
						&& cur.y + 100 <= 200
						&& cur.move <= 500
						&& !v[(cur.x - (1 + cur.move)) + 100][cur.y + 100][cur.move + 1]) {
					v[(cur.x - (1 + cur.move)) + 100][cur.y + 100][cur.move + 1] = true;
					q.add(new havePOS((cur.x - (1 + cur.move)), cur.y,
							cur.move + 1, cur.path + "W"));
				}
				if (cur.x + 100 <= 200
						&& cur.y + cur.move + 101 <= 200
						&& cur.move <= 500
						&& !v[cur.x + 100][cur.y + cur.move + 101][cur.move + 1]) {
					v[cur.x + 100][cur.y + cur.move + 101][cur.move + 1] = true;
					q.add(new havePOS(cur.x, cur.y + cur.move + 1,
							cur.move + 1, cur.path + "N"));
				}
				if (cur.x + 100 <= 200
						&& (cur.y - (1 + cur.move)) + 100 >= 0
						&& cur.move <= 500
						&& !v[cur.x + 100][(cur.y - (1 + cur.move)) + 100][cur.move + 1]) {
					v[cur.x + 100][(cur.y - (1 + cur.move)) + 100][cur.move + 1] = true;
					q.add(new havePOS(cur.x, (cur.y - (1 + cur.move)),
							cur.move + 1, cur.path + "S"));
				}
			}

			System.out.println("Case #" + test++ + ": " + path);
		}
	}
