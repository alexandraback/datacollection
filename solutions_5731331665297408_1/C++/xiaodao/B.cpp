...
#define v (*it)
bool ck(){
    cpt = 0; REP_1(i, n) if (!vis[i] || in[i]) Make(i), cpt++;

    if (!cpt) return 1;
    REP_1(u, n) if (!vis[u]|| in[u]){
        ECH(it, adj[u]) if (!vis[v] || in[v]) Union(u, v);
    }
...
int main(){
    ...
    dep[0] = -1;
        REP_1(i, n){
            REP_1(t, n) dp[t].clear(), vis[t] = 0, in[t] = 0;

           // cout << endl; cout << i << endl;

            vis[i] = 1; dp[i] = a[i]; in[i] = 1; dep[i] = 0;
            if (!ck()) continue;


            set<int> S; ECH(it, adj[i]) S.insert(v);
            ...
            while (1){
                ...
                int vvv = v, uu = 0;
                ECH(it, adj[vvv]) if (in[v] && dep[v] > dep[uu]) uu = v;
                REP_1(i, n) if (in[i] && dep[i] > dep[uu]) in[i] = 0;
                in[vv] = 1;

                //REP_1(i, 3) cout << in[i] << " " << vis[i] << endl; cout << ck() << endl;
                ...
                if (!vv) break;
               // cout << " " << vv;
                S.erase(vv); vis[vv] = 1; dp[i] += a[vv];

                //REP_1(i, 3) cout << in[i] << " ";

                int uu = 0;
                ECH(it, adj[vv]) if (in[v] && dep[v] > dep[uu])uu = v;
                //ECH(it, adj[vv]) cout << in[v] << " " << dep[0] << "??" << endl;
                ...
                while (!L.empty() && L.back() != uu) L.pop_back();
                dep[vv] = dep[uu] + 1;
                L.PB(vv);

                /*cout <<"Link:" << endl;
                ECH(it, L) cout <<  " " << v; cout << endl;*/

                ECH(it, adj[vv]) if (!vis[v]) S.insert(v);



                if (SZ(dp[i]) == n*5) break;
                //cout << " " << dp[i];
            }


            if (SZ(dp[i]) == n*5 && cmp(dp[i], ans)){
                    ans = dp[i];
            }
            //cout << dp[i] << endl;
        }
