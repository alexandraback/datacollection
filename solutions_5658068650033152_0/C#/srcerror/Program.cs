...
namespace BaseApp
{
    class Program
    {
        ...
        {
            string input = @"d:\Work\CodeJam\03\C-small-attempt0.in";
            string output = @"d:\Work\CodeJam\03\C-small-attempt0.out";
        ...
        private static string Task3(StreamReader sin)
        {
            ...
            if (N <= 2) return K.ToString();
            int stones = 2;
            int count = 0;
            int w = 2;

            var field = new bool[N, M];
            for (int i = 0; i < N; i++)
            {
                for (int j = 0; j < M; j++)
                {
            ...
            while (true)
            {
                ...
                if (K <= count*2) return (stones*2).ToString();

                if (K <= (count*2 + w)) return (stones*2 + 2).ToString();
            }
