...
def solve_case(case):
    ...
    if N > M: #Make sure N is the smaller number
        N, M = M, N
    
    if N <= 2:
        return K #Without a row to enclose, we have to cover each enclosed point
    
    if K <= 4:
        return K #For small numbers, we have to cover each enclosed point
    ...
    for x in range(N - 2):
        for y in range(M - 2):
            points = 2 * (x + y + 2)
            enclosed = (x + 1) * (y + 1) + points
            if enclosed >= K:
                minpts = min(minpts, points)
            elif K - enclosed <= 4:
                minpts = min(minpts, points + K - enclosed)
