...
void solveit(int x, int y) {
  ...
  int s = 0;
  while(s * (s+1) / 2 < dist) s++;
  while(((s * (s+1) / 2) & 1) != (dist & 1)) s++;
  for(int u=0; u<s; u++) {
    int di = s-u;
    int id = di-1;
    if(abs(x) > abs(y)) {
      if(x>0) steps[id] = 'E', x -= di;
      else steps[id] = 'W', x += di;
      }
    else {
      if(y>0) steps[id] = 'N', y -= di;
      else steps[id] = 'S', y += di;
      }    
    }
  steps[s] = 0;
  if(x || y) printf("ERROR!\n");
