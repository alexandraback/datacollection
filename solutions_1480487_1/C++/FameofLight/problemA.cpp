...
double score( int idx , double percent )
{
       return points[idx] + double(sum) * percent / 100.00 ;
}

bool isPossible( int idx , double percent )
{
     double conj = 100.0 - percent;
     
     double score1 = score( idx , percent );
     
     for( int i = 0; i < n; i++ )
     {
          if( i != idx )
              if( score( i , conj ) - score1 > eps )
                  return false;
     }
     return true;
}
...
{
       double mmin = 0.0, mmax = 100.0;
       
       while( fabs( mmin - mmax ) > eps )
       {
              double mmid = mmin + ( mmax - mmin ) / 2.0;
              
              if( isPossible( idx , mmid ) )
                  mmax = mmid;
              else
                  mmin = mmid;
       }
...
int main()
{
    ...
    for( int testId =1 ; testId <= t; testId++ )
    {
         ...
         double eachPlayerPoint = double( sum + sum ) / double( n );
         
         int count = 0;
         double negVal = 0;
         for( int i = 0; i < n; i++ )
         {
              double val = ( eachPlayerPoint - double( points[i] ) ) / double( sum );
