...
class Solver(ProblemSolver):
    ...
    def process(self, infile, ncase):
        ## User code here
        scores = readints(infile)[1:]
        X = sum(scores)
        numbers, scores = zip(*sorted(
                list(enumerate(scores)), key=lambda x: x[1]))
        cumulative = [sum(scores[0:i]) for i in range(len(scores))]
        for i, (S, C) in enumerate(zip(scores, cumulative)):
            if (X + C) < i * S:
                break
        else:
            i += 1
        ...
        complete = i
        level = (X + C) / complete
        for i, N in enumerate(numbers[0:complete]):
            ans[N] = (level - scores[i]) / X
        for N in numbers[complete:]:
            ans[N] = 0
        return ' '.join(str(100*m) for m in ans)
