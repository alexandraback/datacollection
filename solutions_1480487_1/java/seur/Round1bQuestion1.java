...
public class Round1bQuestion1 {
	public static void main(String[] args) throws IOException {
		...
		FileWriter fw = new FileWriter(new File("r1q1.out"));
		for (int i = 0;i < T;i++) {
			doCase(i + 1, br.readLine(), fw);
		}
		fw.close();
	}

	static void doCase(int caseNum, String ss, FileWriter fw) throws IOException {
		...
		for (int i = 0;i < N;i++) {
			fw.write("" + ret[i] * 100 + " " /*+ " (" + (X * ret[i] + ses[i]) + ") "*/);
		}
		fw.write("\n");
	}
}
