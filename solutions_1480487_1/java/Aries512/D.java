...
public class D {
	...
	public void solve() throws Exception {
		...
		nextCase: for(int testCase=1; testCase<=testCasesCnt; testCase++) {
			...
			for (int i=0; i<n; ++i) {
				double R = 100, L = 0, M = 0;
				for (int j=0; j<1000; ++j) {
					M = (L+R)/2;
					double score = s[i]+sum*(M/100);
					double totalX = 0;
					for (int k=0; k<n; ++k) if (k!=i) {
						double x = maxD(0,(score-s[k])/sum);
						totalX += x; 
//						debug(score,M,x);
					}
//					debug(totalX,L,R,M);
					if (totalX+(M/100)>1) R=M;
					else L=M;
				}
				printf("%.6f",M); print(i==n-1 ? "\n":" ");
			}
		}
	}
	...
	public static void main(String[] args) throws Exception {
		...
		if (!intoFile) {
			...
			File f = new File(System.getProperty("user.dir")+"\\src\\"+D.class.getPackage().getName()+"\\"+D.class.getSimpleName()+".out" );
