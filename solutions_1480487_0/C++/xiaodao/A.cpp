...
DB f(int i){
    ...
    while (l + EPS < r){
        ...
        REP(j, n) if (j != i && a[j] < aa){
            rm -= DB(aa - a[j]) / X;
        }

        if (sgn(rm) < 0) r = m; else l = m;
    }
    return l;
}
...
int main(){
    ...
    Rush{
        X = 0; REP_C(i, _RD(n)) X += _RD(a[i]);
        //best = 0; REP(i, n) if (a[i] < a[best]) best = i;
        //second = best == 0 ? 1 : 0; REP(i, n) if (i != best && a[i] < a[second]) second = i;
