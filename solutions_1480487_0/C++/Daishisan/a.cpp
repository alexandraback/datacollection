...
int main() {
	...
	for (int test = 1; test <= num_tests; ++test) {
		...
		for (int i = 0; i < n; ++i)
			js[i].first /= x;
		sort(js, js + n);
		assert(js[n].second == n);
		double ans[n];
		for (int i = 0; i < n; ++i) {
			if (i == 0) {
				double sum = js[1].first - js[0].first;
				ans[js[i].second] = sum;
				if (sum < 1) {
					for (int j = 2; j <= n; ++j) {
						double nsum = sum + j*(js[j].first - js[j - 1].first);
						if (nsum < 1) {
							sum = nsum;
							ans[js[i].second] += js[j].first - js[j - 1].first;
						} else {
							ans[js[i].second] += (1 - sum)/j;
							break;
						}
					}
				}
