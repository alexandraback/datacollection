    ...
    public class Program
    {
        ...
        private static void RunProgram2A(string[] testcases)
        {
            ...
            for (int i = 0; i < testcases.Length; i++)
            {
                ...
                if (audienceScorePerContestantNeeded.Values.Min() < 0)
                {
                    ...
                    double numRemoved = contestantsWithLowScores.Count();
                    for (int j = 0; j < N; j++)
                    {
                        if (!contestantsWithLowScores.Contains(j))
                        {

                            double fractionOfTotal = ((2.0 * sumOfAllJudgeSCores) - sumOfRemovedJudgeScores) / (N - contestantsWithLowScores.Count());
