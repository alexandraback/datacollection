...
partial class Solution
{
    const string name = "A-small-attempt0";
    ...
    void Solve()
    {
        ...
        int sum = inp.Sum();
        List<double> inp2 = inp.Select(x => (double)x / sum).ToList();

        double[] result = new double[inp2.Count];

        List<double> numbers = inp2.Select(x => x).ToList();
        numbers.Sort();

        double amt = 1.0;
        numbers.Add(1.0);
        foreach (double val in numbers.Skip(1))
        {
            double needed = 0;
            for (int i = 0; i < inp2.Count; i++)
            {
                if (inp2[i] < val) needed += (val - inp2[i]);
            }
            bool stop = false;
            double factor = 1.0;
            if (needed > amt)
            ...
            for (int i = 0; i < inp2.Count; i++)
            {
                if (inp2[i] < val)
                {
                    result[i] += factor * (val - inp2[i]);
                    inp2[i] += factor * (val - inp2[i]);
                }
            }
            if (stop) break;
        }
        ...
        Write("Case #{0}: ", cas);
        foreach (var x in result) Write("{0} ", x*100);
        WriteLine();
    }
