...
bignum paint(long long r, long long n)
{
	return bignum(2) * bignum(n) * bignum(r) + bignum(n) * bignum(2 * n - 1);
}
...
int main()
{
	...
	for (int n = 1; n <= T; n++)
	{
		long long r, t;
		cin >> r >> t;
		long long low = 0, upper = t;
		while (low < upper)
		{
			long long mid = upper - (upper - low) / 2;
			bignum need = paint(r, mid);
			if (need < bignum(t) || need == bignum(t))
				low = mid;
			else
				upper = mid - 1;
		}
