...
bool possible( bignum n , bignum r , bignum t ){

    bignum tot = ( r * 2 ) * n + ( n - 1 ) * ( n - 1 ) * 2 +  ( n - 1 ) * 3 + 1;
    if( tot < t || tot == t ) return true;
    return false;
}
...
int main(){
    ...
    for( q = 1 ; q <= T && scanf("%lld %lld" , &r , &t ) ; ++q ){
        printf("Case #%d: " , q  );
        bignum ans = 0, left = 0 , right = 3000000000000000000LL , mid;
        while( left < right ){
            mid = ( left + right )/2;
            if( possible( mid , r , t ) ){
                ans = mid;
