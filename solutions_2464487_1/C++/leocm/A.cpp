...
struct bigint {
  ...
  {
      return ( y.cmp() != 0 ? gcd( y, x % y ) : x );
  }
};

bool ok( bigint r, bigint t, bigint m )
{
	bigint one = bigint(1);
	bigint two = bigint(2);
	bigint a = two*m*m + (two*r-one)*m;
	return a <= t;
}
...
int main()
{
	...
	{
		cin >> sr >> st;
		bigint r(sr.c_str()), t(st.c_str());
		bigint lo("1"), hi("999999999999999999"), m, asw;
		while ( lo < hi )
		{ 
			m = lo+(hi-lo)/2;
			if ( ok( r, t, m ) ) { asw = m; lo = m + 1; }
			else hi = m;
		}
		cout << "Case #" << tt << ": " << asw << endl;
	} 
    return 0;
}
