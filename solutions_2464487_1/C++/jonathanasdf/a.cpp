...
bigint bigint::operator=(ll a)
{
len=0;
do{s[len++]=a%10;a/=10;} while(a>0);
return *this;
} bigint bigint::operator=(const char *
a)
{
len=strlen(a);
...
bigint bigint::sqrt(const bigint &
a)
{
int n=(a.len-1)/2,p;
bigint b,d;
...
int main() {
    ...
    for (int zz=1; zz <= T; zz++) {
        ll r, t; cin >> r >> t;
        bigint R(r), T(t);
        bigint lo = 1LL, hi = 3000000000LL;
        while(bigint::cmp(lo+ 1LL, hi) < 0) {
            bigint mid = (lo+hi)/2LL;
            bigint test = (R*2LL+1LL)*mid + mid*(mid-1LL)*2LL;
            if (bigint::cmp(test, T) <= 0) lo = mid;
            else hi = mid;
        }
