...
public class A {
	...
	static String filesPath = System.getProperty("user.dir")+"\\src\\"+A.class.getPackage().getName()+"\\";
	static boolean fromFile = !false;
	static String inFile = A.class.getSimpleName()+"-large.in";
	...
	public void solve() throws Exception {
		...
		nextCase: for(int testCase=1; testCase<=testCasesCnt; testCase++) {
			print("Case #"+testCase+": ");
			long r = nextLong(), t = nextLong();
			max = BigInteger.valueOf(t);
			
			long L = 0, R = 1500000000L;
			while (L+3<R) {
				long M = (L+R)/2;
				if (check(M,r,t)) L=M;
				else R=M;
			...
			for (long i=L+10;;--i) if (check(i,r,t)) {
				println(i);
				break;
			}
	...
	boolean check(long cnt, long r, long t) {
		long x = cnt*(2*cnt+1);
		BigInteger y = BigInteger.valueOf(2);
		y = y.multiply(BigInteger.valueOf(r-1));
		y = y.multiply(BigInteger.valueOf(cnt));
		y = y.add(BigInteger.valueOf(x));
		if (y.compareTo(max)>0) return false;
		else return true;
	}
	...
	public static void main(String[] args) throws Exception {
		...
		if (!toFile) {
			...
			String outPath = filesPath + A.class.getSimpleName()+".out";
