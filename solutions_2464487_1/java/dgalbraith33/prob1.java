...
public class prob1{
	...
	public static void main(String args[]){
		...
		for(int i = 0; i < n; i++){
			System.out.println("Case #"+(i+1)+": "+df.format(executeProgram()));
		}
	}
	
	public static BigDecimal executeProgram(){
		BigDecimal r, t;
		r = new BigDecimal(in.next());
		t = new BigDecimal(in.next());
		BigDecimal exp = r.multiply(new BigDecimal("2")).subtract(new BigDecimal("3")).abs();
		exp = exp.multiply(new BigDecimal("-1")).add(bigSqrt(exp.pow(2).add(t.multiply(new BigDecimal("8"))), new MathContext(50, RoundingMode.HALF_UP)));
		exp = exp.divide(new BigDecimal("4"));
		return exp;
	}
