...
//	static String BASEPATH = "/home/ec2-user/";
	...
	static String INPATH = BASEPATH + BN.class.getSimpleName().charAt(0) + (LARGE ? "-large.in" : "-small-attempt1.in");
//	static String INPATH = BASEPATH + TParallel.class.getSimpleName().charAt(0) + (LARGE ? "-large-practice.in" : "-small-practice.in");
	...
	public static void main(String[] args) throws Exception
	{
		...
		CompletionService<BN> cs = new ExecutorCompletionService<BN>(es);
		...
		for(int i = 0;i < n;i++){
			BN runner = cs.take().get(); // not ordered
			runner.out.flush();
			runner.out.close();
