...
int CodeJamMain()
{
	// interpret +/- as 0/1, LSB first, results in first answer table {0,1}
	// subsequent N+1 lengths are generated by taking the previous table, and appending a reverse version + 1
	// e.g. {0,1} -> {0,1,2,1} -> {0,1,2,1,2,3,2,1} ...
	// verifying assumption with small input precalculation; need to be able to work backwards for large
	vector<vector<int>> answers(11);
	answers[0].push_back(0);
	answers[0].push_back(1);
	for (int i = 1; i < 11; i++)
	{
		answers[i] = answers[i - 1];
		for (auto it = answers[i - 1].rbegin(); it != answers[i - 1].rend(); it++) answers[i].push_back(*it + 1);
	}
    ...
    for (int caseNum = 0; caseNum < T; caseNum++)
    {
		...
		int answer = answers[pancakes.size()][index];
