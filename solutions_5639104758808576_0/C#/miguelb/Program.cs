...
namespace CodeJam
{
    ...
    class StandingOvation :BaseGoogleJamProblem
    {

        int SolveSmall ( int[] shyness)
        {
            ...
            for ( int i =0 ; i < shyness.Length ; i++ )
            {
                if (i > peopleStanding)
                {
                    peopleRequired += i - peopleStanding;
                    peopleStanding = i;
                }
                peopleStanding += shyness[i];
            }
            return peopleRequired;
        }
        protected override void SolveProblemCase()
        {
            var maxShyness = ReadInt();
            int[] shyness = CreateArray<int>(maxShyness + 1);
            string str = ReadString();
            for (int i = 0; i <= maxShyness; i++)
                shyness[i] = int.Parse (str[i].ToString ());

            int r = SolveSmall(shyness);
            AddCaseResponse(r);
        }
    }
    ...
    class Program
    {
        ...
        {
            var problem = new StandingOvation();
            
            //problem.Solve("c:\\temp\\input.txt");
            problem.Solve("c:\\temp\\A-small-attempt0.in");
        }
    }
