...
int findCon(char name[1000000], int l, int n) {
...
int solve(char name[1000000], int l, int n) {
	int appear = 0;
	int position = 0, first = -1, current;
	long long total = 0;
	while ((current= findCon(name + position, l - position, n)) > -1) {
		position += current;
		//cout << "position " << position << endl;
		total += (position + 1) * (l - position - n + 1);
		if (appear) {
			//cout << "total_1 " << total << endl;
			total -= (first + 1) * (l - position - n + 1);
		} else {
		}
