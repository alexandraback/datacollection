...
int main()
{
  ...
  REP(TC, T) {
    ...
    uint n;
    cin >> name >> n;
    uint i = 0, j = 0, lasti = -1; // boudaries
    for (uint k = 0; k < name.length(); ++k) {
      if (any_of(vowels.begin(), vowels.end(),
                 [&](char ch){return ch == name[k];})) {
        i = k + 1;
        j = k;
      }
      else {
      ...
      if (i < name.length() && j - i + 1 == n) {
        sum += (i - lasti) * (name.length()-1 - j + 1);
        lasti = i;
        ++i;
      }
    }
