...
public class GCJ2013r1CqA {
	public static void main(String[] args) throws Exception {
		...
		File inFile = new File(FILE_DIR + "A-small-attempt1.in");
		File outFile = new File(FILE_DIR + "gcj2013R1C-qA-s1.txt");
		...
		for (int t = 1; t <= T; ++t) {
			...
			for (int i = 0; i < nameAsCharArray.length; ++i) {
				...
				if (currentStreak >= n) {
					if (lastRunEndpoint != -1) {
						nValue += ( (lastRunEndpoint - n + 2) * (i - lastRunEndpoint) );
					}
					lastRunEndpoint = i;
				}
			...
			if (lastRunEndpoint != -1) {
				nValue += ( (lastRunEndpoint - n + 2) * (nameAsCharArray.length - lastRunEndpoint) );
			}
