...
public class consonants {
	...
	public static long findCon(String s, int n)
	{
		...
		{
			range tmp = ar.get(i);
			long base =  (tmp.len-n+1)*(2*s.length()-2*tmp.end+tmp.len-n)/2;  //(tmp.len-n+1)*(s.length()-tmp.end);
			sum += base;
			long base2 = (tmp.len-n)+(s.length()-tmp.end);
			if(i > 0)
			{
				range tmp2 = ar.get(i-1);
				sum +=  (tmp.start-tmp2.end-1+n-1)*base2;  //    (tmp.start-(tmp2.end-tmp2.len+2))*base2;
			}
			else
			{
				sum += tmp.start*base2;
	...
	public static void main(String[] s) throws IOException
	{
		//BufferedReader input = new BufferedReader(new InputStreamReader(System.in));
				...
				File file = new File("1C\\A-small-attempt2.in");
				BufferedReader input = new BufferedReader(new FileReader(file));
				
				File outfile = new File("1C\\A-small2-0.out");
