...
def has_consonants(in_string, n):
    return in_string.translate(None, 'aeiou')

#def n_value(name, n, start=0, end=None, matched=0):
def n_value(name, n):
    LOG.debug('n: %d' % n)
    LOG.debug('name: %s' % name)
    found = set()
    len_name = len(name)
    for i in xrange(len_name - n + 1):
        LOG.debug('testing: %s' % name[i:i + n])
        if len(name[i:i + n].translate(None, 'aeiou')) == n:
            LOG.debug('found match at index: %d' % i)
            for j in xrange(0, i + 1):
                for k in xrange(i + n, len_name + 1):
                    LOG.debug('.' * j + name[j:k] + '.' * (len_name - k))
                    found.add('.' * j + name[j:k] + '.' * (len_name - k))
        LOG.debug(found)
    return len(found)
...
#    char = name[start]
#    if char in 'aeiou':
#        return n_value(name, n, start + 1, end, matched)
#    rest = n_value(name, n - 1, start + 1, end, 1 + matched)
#    if rest is not None:
#        return rest
...
def do_job(in_file, out_file=sys.stdout):
    ...
    for testcase in xrange(T):
        name, n = in_file.readline().split()
        n = int(n)
        result = n_value(name, n)
