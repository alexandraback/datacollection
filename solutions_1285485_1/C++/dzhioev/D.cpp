...
int main() {
    ...
    vector<int> res(testCount);
    vector<thread> threads;
    vector<Input> inputs(testCount); 
    int THREAD_COUNT = 8;
    for (auto TEST: R(1, testCount + 1)) {
        ...
        inputs[TEST - 1].D = D;
        inputs[TEST - 1].field = field;
    } 
    int beg = 0;
    for (int i: R(THREAD_COUNT)) {
        int end = beg + (testCount / THREAD_COUNT);
        if (i == THREAD_COUNT - 1) {
            end = testCount;
        }
        threads.emplace_back(
            [beg, end, &res, &inputs]() {
                for (int i: R(beg, end)) {
                    res[i] = solve(inputs[i].field, inputs[i].D);
                }
            });
        beg = end;
    }
    ...
    for (auto TEST: R(1, testCount + 1)) {
        cout << "Case #" << TEST << ": " << res[TEST - 1] << endl;
    } 
    return 0;
}
