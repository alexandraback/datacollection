...
void solve_b()
{
	...
	for (int tc = 1; tc <= TC; ++tc)
	{
		...
		for (int i = 0; i < N; ++i)
		{
			scanf("%lf %lf", carTimes + i, carPosition + i);
		}
		for (int j = 0; j < A; ++j)
		{
			scanf("%lf", accelerations + j);
		}

		printf("Case #%d:\n", tc);

		// Calculate value for all accelerations
		for (int a = 0; a < A; ++a)
		{
			double acc = accelerations[a];
			double minTime = 0.0;

			if (N == 1)
			{
				// Due to constraints, carTimes[0] = 0, carPosition[0] >= D, no cars to worry about
				// v0 = 0  ==>  D = 0.5 * acc * t^2  ==>  t = sqrt(2D / acc)
				minTime = sqrt(2 * D / acc);
			}
			else if (N == 2)
			{
				// carTimes[0] = 0, carTimes[0] = t, carPosition[0] = 
				minTime = sqrt(2 * D / acc);

				if (carPosition[0] < D)
				{
					double carSpeed = (carPosition[1] - carPosition[0]) / (carTimes[1] - carTimes[0]);
					if (((D - carPosition[0]) / carSpeed) > minTime)
						minTime = (D - carPosition[0]) / carSpeed;
				}
			}
