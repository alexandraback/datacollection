...
int main()
{
	...
	For(cc,1,ca)
	{
		...
		int n,an;
		cin>>d>>n>>an;
		vector< pair<long double ,long double> > stops;
		rep(i,n)
		{
			long double ss,tt;
			cin>>ss>>tt;
			stops.push_back( make_pair(ss,tt));
		}
		printf("Case #%d:",cc);
		rep(pp,an)
		{
			long double a,u=0,t=0,p=0;
			long double otherp=stops[0].second;
			cin>>a;

			rep(i1,stops.size()-1)
			{
				int i=i1+1;
				long double endP=stops[i].second;
				long double endT=stops[i].first;
				if(endP-d>eps)
				{
					endT=t+(endT-t)/(endP-p)*(d-p);
					endP=d;
				}
				long double pI=endP-otherp;
				otherp=endP;
				long double tI=endT-t;
				if(u>(pI/tI))
					u=pI/tI;
				long double myDis=u*tI+0.5*a*tI*tI;

				long double A=a/2, B=u, C=-(endP-p);
				long double disc=sqrt(B*B-4*A*C);
				long double temp= (-B + disc)/(2*A);
				...
				t=endT;
				/*
				long double maxa= (pI-u*tI)*2.0/(tI*tI);
				if(maxa>a)
					maxa=a;
				u=u+maxa*tI;
				long double maxU= pI/tI;
				if(maxU<u)
					u=maxU;	*/
			}
			if(p!=d)
			{
				long double A=a/2, B=u, C=-(d-p);
				long double disc=sqrt(B*B-4*A*C);
				t+= (-B + disc)/(2*A);
