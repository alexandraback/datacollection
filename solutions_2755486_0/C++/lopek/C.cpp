...
struct node {
  ...
  int l, r;
  node() {}
  node(ll w, ll e) : w(w), e(e), mins(0), ms(0), l(-1), r(-1) {}
};
...
ll inline gets(ll w, ll e) {
    ll s = w + e;
    if (s < 0) s--;
    return s / 2;
}

ll get_min(ll w, ll e, int i) {
...
void tree_max(ll w, ll e, ll str, int i) {
  //printf("w = %lld, e = %lld, i = %d, T[i].w = %lld, T[i].e = %lld\n", w, e, i, T[i].w, T[i].e);
  if (T[i].w == w && T[i].e == e) {
  ...
  //printf("\tT[%d]: w = %lld, e = %lld, mins = %lld, ms = %lld, l = %d, r = %d\n", i, T[i].w, T[i].e, T[i].mins, T[i].ms, T[i].l, T[i].r);
}
...
int main() {
  ...
  FOR(t, 1, Z) {
    ...
    while (N--) {
      int d, n, dd, dp, ds;
      ll w, e, s;
      scanf("%d %d %lld %lld %lld %d %d %d", &d, &n, &w, &e, &s, &dd, &dp, &ds);
      --e;
      while (n--) {
        A.pb(attack(d, s, w, e));
        d += dd;
