...
struct attack {
	LL day, remain, west, east, strength, delta_d, delta_p, delta_s;
	OPOVER(<, attack) {
		return day > t.day;
	}
	attack(LL d, LL n, LL w, LL e, LL s, LL dd, LL dp, LL ds) : day(d), remain(n), west(w), east(e), strength(s), delta_d(dd), delta_p(dp), delta_s(ds) {
	}
};
...
int main()
{
	...
	REP (_t, T) {
		...
		// input
		int N; cin >> N;
		map<double, LL> wall, nwall;
		priority_queue<attack> attacks;
		REP (_, N) {
			LL d, n, w, e, s, dd, dp, ds;
			cin >> d >> n >> w >> e >> s >> dd >> dp >> ds;
			attacks.push(attack(d, n, w, e, s, dd, dp, ds));
		}

		LL cday = 0;
		while (!attacks.empty()) {
			attack a = attacks.top(); attacks.pop();
			if (a.day != cday) {
				// day changed
				wall = nwall;
				cday = a.day;
			...
			// Attack!
			bool success = false;
			for (double p = a.west; p <= a.east; p += 0.5) {
				if (wall[p] < a.strength) {
					success = true;
					nwall[p] = max(nwall[p], a.strength);
				}
			}
			ans += success;
