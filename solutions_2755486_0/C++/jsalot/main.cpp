...
{ // solves a case
    ...
    for (int day=0; ; ++day)
    {
        vector<pair<int, int> > raise;
        bool haveAttacks = false;
        ...
        for (int i=0; i<N; ++i)
        {
            ...
            if (d[i] == day)
            {
                ...
                {
                    unordered_map<int, int>::iterator it = wall.find(j);
                    if (it == wall.end() || it->second < s[i])
                    {
                        won = true;
                        raise.push_back(make_pair(j, s[i]));
                    }
                }
                ...
                {
                    success += 1;
                    //cout << "tribe "<< i+1 << " day " << day << " e " << e[i] << " w " << w[i] << " s "<< s[i] << endl;
                }
            
            // if attack happens ok or not
            d[i] += delta_d[i];
            e[i] += delta_p[i];
            w[i] += delta_p[i];
            s[i] += delta_s[i];
        ...
        if (!haveAttacks) break;
        
        for (int i=0; i<raise.size(); i++)
        {
            unordered_map<int, int>::iterator it = wall.find(raise[i].first);
            if (it == wall.end())
            {
                wall[raise[i].first] = raise[i].second;
            } else if (it->second <= raise[i].second)
            {
                it->second = raise[i].second;
            }
        }
        raise.clear();
    }
