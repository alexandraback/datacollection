...
public class Erdem {
	...
	static class nt {
		long d, n, s, delta_d, delta_p, delta_s;
		int w, e;
		public nt() throws IOException {
			d = LONG();
			n = LONG();
			w = INT()+500;
			e = INT()+500;
	...
	static class A implements Runnable {
		...
		public void run() {
			...
			while( true ) {
				long firstDay = day+100000000;
				int first = -1;
				for (int i = 0; i < N; i++) {
					if( nts[i].n > 0 && nts[i].d < firstDay ) {
						firstDay = nts[i].d;
						first = i;
					}
				}
