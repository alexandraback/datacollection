...
public class C {
        ...
        for (int c = 1; c <= t; ++c) {
            ...
            for (int i = 0; i < t2; ++i) {
                ...
                for (Attack attack : tribe.createAttacks()) {
                    List<Attack> dayAttacks = attacks.get(attack.d);
                    if (dayAttacks == null) {
                        dayAttacks = new ArrayList<Attack>();
                        attacks.put(attack.d, dayAttacks);
                    }
                    dayAttacks.add(attack);
                }
            }
    ...
    private static int attack(Wall wall, List<Attack> attacks) {
        int successful = 0;
        for (Attack attack : attacks) {
//            System.out.println(String.format("attack on %d,%d on day %d with strength %d", attack.w, attack.e, attack.d, attack.s));
