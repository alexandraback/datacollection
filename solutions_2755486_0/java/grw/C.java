...
public class C {
	public static void main(String[] args) throws Exception {
		new Rename.Problem().solveAll(C.class.getName()+"-");
	}
	
	static class Attack {
		public Attack(int dd, int ww, int ee, int ss, int nn, int ddd, int dddp, int dds) {
			d=dd;
			w = ww; e = ee;
			s = ss;
			n = nn;
		...
		public Attack next() {
			if (n<=1)
			return null;
			return new Attack(d+deld, w+delp, e+delp, s+dels, n-1, deld, delp, dels);
		}
	}
	...
	static class Rename {
		static class Task extends AbstractTask<Precomputed> {
			...
			Object solve(Precomputed pd) {
				PriorityQueue<Attack> q = new PriorityQueue<>(N, new Comparator<Attack>() {
					@Override
				...
				long count = 0;
				for (int i = 0; i<N; i++)
					q.add(new Attack(d[i], w[i], e[i], S[i], n[i], delta_d[i], delta_p[i], delta_s[i]));
				HashMap<Integer, Integer> wall = new HashMap<>();
				while (!q.isEmpty()) {
					Attack a = q.poll();
					boolean success = false;
					for (int l = a.w; l<=a.e; l++) {
						Integer w = wall.get(l);
						if (w==null)
							w = 0;
						if (w.intValue()<a.s)
							success = true;
