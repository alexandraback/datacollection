...
public class L3 {
	public static void main(String[] args) {
		List<String> inputString = getInputStrings("C:\\Users\\Robert\\Documents\\Java\\input\\C-small-attempt1.in");
		...
		for (Object i: objectList){
			...
			output+=S2Result(i);
			try {
			    PrintWriter out = new PrintWriter(new BufferedWriter(new FileWriter("C:\\Users\\Robert\\Documents\\Java\\output\\C-small-attempt1.out", true)));
			    out.println(output);
			    out.close();
			} catch (IOException e) {
				e.printStackTrace();
	...
	private static String S2Result(Object i) {
		...
		for(int day: days){
			...
			for(List<Integer> a: attackList){
				if(a.get(0)==day){
					int power = a.get(1);
					int west = a.get(2)+25000;
					int east = a.get(3)+25000;
