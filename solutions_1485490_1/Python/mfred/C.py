...
while cur_line<len(lines)-1:
	...
	stB = 0
	for t in cB:
		best_prod[t_max_A][stB] = 0
		stB += t[0]
	best_prod[t_max_A][t_max_B] = 0
	
	for j in range(len(times_A)-2,-1,-1):
		best_prod[times_A[j]] = {}
		best_prod[times_A[j]][t_max_B] = 0
		for k in range(len(times_B)-2,-1,-1):
			if cA[j][1] == cB[k][1]:
				prod = min(cA[j][0],cB[k][0])
				best_prod[times_A[j]][times_B[k]] = prod + best_prod[times_A[j+1]][times_B[k+1]]
			else:
				prod = 0
				best_prod[times_A[j]][times_B[k]] = max(best_prod[times_A[j+1]][times_B[k]], best_prod[times_A[j]][times_B[k+1]])
	# print best_prod[0][0]
			
	print "Case #"+str(i)+": "+str(best_prod[0][0])
	i += 1
