...
FILE * inputFile = fopen("C:\\codejam\\data.in", "r");
FILE * outputFile  = fopen("C:\\codejam\\data.out", "w");

lint N,M;
lint *a,*A,*b,*B;
lint iijj[100][100];

lint calcToysByIterator(const int &ii, const int &jj){
	if(iijj[ii][jj] != -1){
		return iijj[ii][jj];
	}
	lint i = ii, j = jj;
	...
	return  iijj[ii][jj] = res;
}

void oneStep()
{
    fscanf(inputFile, "%I64d%I64d", &N, &M);
	memset(iijj,-1,100*100*sizeof(int));
	a = new lint[N];
	A = new lint[N];
	b = new lint[M];
	B = new lint[M];
