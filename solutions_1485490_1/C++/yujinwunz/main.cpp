...
unsigned long long cases, toys, boxes, toy[101][2], box[100][2], best[101][101];
bool good[101][101];

unsigned long long score(int t, int b, long long remaining, bool isToyRemaining){
	//returns the best way after using/not using toy t.
	//base case
	if(t==toys||b==boxes) return 0ull;
...
int main(){
	...
	for(int i = 1; i <= cases; i++){
		//init
		for(int k = 0; k < 100; k++) for(int l = 0; l < 100; l++) good[k][l] = best[k][l] = 0;
		//input;
		in>>toys>>boxes;
		...
		//process;
		unsigned long long result = score(0,0,0,0);
		//output;
		out<<"Case #"<<i<<": "<<result<<"\n";
	}
}