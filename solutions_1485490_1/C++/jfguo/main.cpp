...
map<pair<vector<pair<LL,int> >, vector<pair<LL,int> > >, LL> cache;
...
LL calc(vector<pair<LL, int> > &boxes, vector<pair<LL, int> > &toyes)
{
    ...
    pair<vector<pair<LL,int> >, vector<pair<LL,int> > > state(boxes, toyes);
    ...
    if (box.second == toy.second) {
        ...
        return cache[state] = t + calc(nboxes, ntoyes);
    }
