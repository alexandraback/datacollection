...
unsigned long long Recurse(int ind,int ind1,unsigned long long A[],int Atype[],unsigned long long B[],int Btype[],int &n,int &m)
{
    ...
    unsigned long long m1 = max(Recurse(ind+1,ind1,A,Atype,B,Btype,n,m),Recurse(ind,ind1+1,A,Atype,B,Btype,n,m)) ;
    F[ind][ind1] = true ;
    return T[ind][ind1] = m1 ;
}
