...
long long getBest(int i, int j, int k, long long left){
	...
	if(types[0][i]!=types[1][j]){
		if(k==0)
			best[i][j][k][left]=max(getBest(i+1, j, 0, -1), getBest(i, j+1, 0, left));
		else
			best[i][j][k][left]=max(getBest(i+1, j, 1, left), getBest(i, j+1, 0, -1));
	}
	else{
		long long avail[2];
		avail[0]=numbers[0][i];
		...
		if(left!=-1)
			avail[k]=left;
		if(avail[0]>avail[1])
			best[i][j][k][left]=getBest(i, j+1, 0, avail[0]-avail[1])+avail[1];
		else if(avail[0]<avail[1])
			best[i][j][k][left]=getBest(i+1, j, 1, avail[1]-avail[0])+avail[0];
		else
			best[i][j][k][left]=getBest(i+1, j+1, 0, -1)+avail[0];
	}
	return best[i][j][k][left];
}
