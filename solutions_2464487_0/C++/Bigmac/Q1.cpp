...
//sol
long long cross(long long n)
{
	return ( ((2 * in_ri) - 1) * n + (2 * n * n));
}
long long solve()
{
	long long n = 1;
	long long val_n = cross(n);
	int times = 0;
	while(val_n <= in_ti)
	{
		times++;
	...
	for(int it_times = times - 1; it_times >=0; it_times --)
	{
		long long val_np = cross(prev_val + (1 << it_times));
		if(val_np <= in_ti)
		{
			prev_val += (1 << it_times);
		}
