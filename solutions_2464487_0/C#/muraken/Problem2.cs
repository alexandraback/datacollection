    ...
    class Problem2
    {
        ...
        public IEnumerable<T> getNexts<T>(Func<String,T> f) {
            if (!this.ienum.MoveNext())
            {
                ...
                return this.ienum.Current.Split(' ').Select(str => f(str));
            }
        }
        ...
        private Problem2(string pathIn, string pathOut)
            : this() {
        ...
        /// <returns></returns>
        public static Problem2 createProblem(string[] args) {
            Problem2 ret;
            if (args.Length < 1) {
                Console.WriteLine("Error: arguments");
                return null;
            }
            ret = new Problem2(args[0], args[0] + ".out");
