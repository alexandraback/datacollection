...
public class Main {
	...
	public static void main (String[] args) {
		...
		try {
			...
			br = new BufferedReader(new FileReader("src/A-small-attempt0.in"));

			String line = br.readLine();
			
			int numberOfTestCase = Integer.parseInt(line);
			for (int i = 0; i < numberOfTestCase; i++) {
				System.out.println(numberOfTestCase);
				...
				System.out.println(radius + " " + paint);
...
//		while(true){
//			areaOfCurrLv = radius.multiply(new BigInteger("2")).add( level.multiply(new BigInteger("4")) ).add(new BigInteger("-3"));
//					
//			sum = sum.add(areaOfCurrLv);
//			if ( sum.compareTo(paint) > 0) {
//				break;
//			}
	...
	public static BigInteger findN (BigInteger a1, BigInteger min, BigInteger max) {// 1 to max
		...
		if ( min.add(new BigInteger("100")).compareTo(max) > 0 ) {
			...
			while(true) {
				...
				System.out.println(n1 + " " + n2 + " " + rhs1 + " " + rhs2);
		...
		System.out.println("return: n1: " + toReturn);
		return toReturn;
