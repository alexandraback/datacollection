...
//        double a=(2*r-1)/2.0, b=t/2.0;
//        double a=(2*r-1), b=t;
        BigInteger a=BigInteger.valueOf(2*r-1);
        BigInteger b=BigInteger.valueOf(t);
        BigInteger sq=a.pow(2).add(b.multiply(BigInteger.valueOf(8)));
        // lo:true, hi:false
        BigInteger lo = BigInteger.valueOf(0);
        BigInteger hi = BigInteger.TEN.pow(100);
        while (lo.compareTo(hi.subtract(BigInteger.ONE))<0) {
            BigInteger mid = lo.add(hi).divide(BigInteger.valueOf(2));
//            int mid = (int) (lo + ((long) hi - lo) / 2);
            if (mid.pow(2).compareTo(sq)<=0) {
                lo = mid;
            } else {
                hi = mid;
            }
...
//        long ret = (long) ((sqrt(a*a+8*b)-a)/4);
//        long ret = (long) ((sqrt(sq.doubleValue())-a.doubleValue())/4);
        long ret = lo.subtract(a).divide(BigInteger.valueOf(4)).longValue();
