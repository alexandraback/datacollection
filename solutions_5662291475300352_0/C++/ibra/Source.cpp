...
template <typename T1, typename T2> T1 safemulmod(T2 a, T2 b, T2 mod)
{
	return a*b%mod;
}
...
string inpath = "C:/Users/ibismail/Downloads/C-small-1-attempt5.in";
string outpath = "C:/Users/ibismail/Downloads/out.txt";
...
int main()
{
	...
	for (int test = 1; test <= t; test++)
	{
		...
		for (int i = 0; i < n; i++)
		{
			...
			for (int j = 0; j < c; j++)
			{
				_v.push_back({ start,         (vel + j) });
				v.push_back({ start, 360.0 / (vel + j) });
			}
		}

		int res;
		if (_v.size() < 2 || _v[0].second == _v[1].second)
		{
			res = 0;
		}
		else
		{
			int slow = 0;
			if (v[1].second < v[0].second) slow = 1;

			double dist = v[slow].first - v[1 - slow].first;
			if (dist < 0) dist += 360;

			double dv = v[1 - slow].second - v[slow].second;
			double t1 = dist / dv;
			double t2 = t1 + 360.0 / dv;

			double sSlow1 = v[slow].first + v[slow].second * t1;
			double sSlow2 = v[slow].first + v[slow].second * t2;
		
			res = 1;
			if (sSlow2 > 720 + 1e-9)
				res = 0;
			if (sSlow1 > 360.0 + 1e-9)
				res = 0;
