...
namespace CodeJam
{
    ...
    class HikingDeer : CodeJamProblem
    {
...
180 1 1";
        ...
        public override void Read(FileReader reader)
        {
            ...
            for ( int i = 0; i < n ; i++)
            {
                var startingPosition = reader.ReadInt();
                var numberOfHikers = reader.ReadInt();
                var amountofTime = reader.ReadLong();
                for ( long j = 0 ; j < numberOfHikers ; j++)
                {
                    hikers.Add(new Hiker()
                        {
                            StartPos = startingPosition,
                            AmountOfTime = j + amountofTime
                        });
                }

            }
            hikers.Sort(CompareHicker);
        }

        int CompareHicker ( Hiker hicker1, Hiker hicker2)
        {
            return hicker1.AmountOfTime.CompareTo(hicker2.AmountOfTime);
        }
        protected override string SolveSmall()
        {
            if ( hikers .Count == 2)
            {
                ...
                {
                    if (hikers[i].StartPos == 0)
                        count++;
                }
                if (hikers[0].AmountOfTime == hikers[1].AmountOfTime)
                    return count.ToString ();

                if (count == 2)
                    return "2";

                double slowTime = hikers[1].AmountOfTime * (hikers[1].StartPos / 360.0);

                double dist = hikers[0].StartPos  + 360 * slowTime / hikers[0].AmountOfTime;
                if (dist > 720)
                    return "1";
                else
                {
                    if (hikers[0].StartPos > hikers[1].StartPos)
                    {
                        return "0";
                    }
                    else
