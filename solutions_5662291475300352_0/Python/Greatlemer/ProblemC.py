...
def solve_problem(output_writer=sys.stdout, **kwargs):
    ...
    for group in group_data:
        ...
        for loop_time in range(min_loop_time, min_loop_time + hikers):
            ...
            time_to_start_once = ((360 - start_pos) * loop_time)
            time_to_start_twice = time_to_start_once + (loop_time * 360)
            hiker_info.append((time_to_start_twice, time_to_start_once))
    hiker_info.sort()
    if hiker_info[0][0] <= hiker_info[1][1]:
        answer = 1
    else:
        answer = 0
