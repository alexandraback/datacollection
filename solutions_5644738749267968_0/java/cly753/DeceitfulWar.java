...
//		bn = b.length-1;
	...
	public static void mergeSort(double[] data, int head, int tail) {
		if (head >= tail) return;
		else {
			int mid = (head + tail) / 2;
			mergeSort(data, head, mid);
			mergeSort(data, mid + 1, tail);
			merge(data, head, tail);
		}
	}
	
	public static void merge(double[] data, int head, int tail) {
		int mid = (head + tail) / 2;
		double temp[] = new double[tail - head + 1];
		int a = head;
		int b = mid + 1;
		int i = 0;
...
//		sc = new Scanner(System.in);
		sc = new Scanner(new File("E:\\GitHub\\HHH\\CodeJam\\DeceitfulWar\\D-small-attempt2.in"));
		int c = sc.nextInt();
		String sol = "";
		...
		PrintWriter pw = new PrintWriter(new File("E:\\GitHub\\HHH\\CodeJam\\DeceitfulWar\\D-small-attempt2.out"));
		pw.print(sol);
		pw.close();
	}
