...
#define DO(n) for ( int ____n= n; ____n-- ; )
...
inline LL _upd(int i){return _1(i) - 1;};
...
int main(){
    ...
    Rush{
        printf("Case #%d: ", ++Case);
        RD(n); REP(i, n) RF(A[i]); REP(i, n) RF(B[i]); sort(A, A+n), sort(B, B+n);

        int z1 = 0, z2 = 0;

        int j = 0; REP(i, n){
            while (j < n && B[j] < A[i]) ++j, ++z2; ++j;
        }
