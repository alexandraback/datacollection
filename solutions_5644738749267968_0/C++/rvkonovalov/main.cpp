...
void main(){
	...
	zSimpleFile f;
	zStringArray res;
	int T, N, n, i, j, nao_hit, ken_hit, r1, r2;
	zDoubleArray ken, nao;
	...
	if (dType == 1){
		f1 = "D-small-attempt0.in";
		f2 = "D-small-attempt0.out";
	}
	if (dType == 2){
		f1 = "D-large.in";
		f2 = "D-large.out";
	...
	for (n = 0; n < T; n++){
		...
		N = stoi(f.z[n * 3 + 1]);
		for (i = 0; i < N; i++){
			nao.Add() = stod(f.GetLex(n * 3 + 2, i));
			ken.Add() = stod(f.GetLex(n * 3 + 3, i));
		}
