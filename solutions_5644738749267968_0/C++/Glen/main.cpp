...
#define dvec(v) if (opt_debug) { copy( ALL(v), ostream_iterator<double>(cerr, " ") ); cerr << endl; }
...
int deceitful_war(VVD blocks) {
	SORT(blocks[NAOMI]);
	SORT(blocks[KEN]);
	int win = 0;
	while (blocks[NAOMI].size()) {
		if (blocks[NAOMI][0] < blocks[KEN][0]) {
			blocks[KEN].pop_back();
		} else {
			blocks[KEN].erase(blocks[KEN].begin());
			win++;
		}
		blocks[NAOMI].erase(blocks[NAOMI].begin());
	}
	return win;
}
...
int main(int argc, char** argv) {
	...
	REP (_t, T) {
		...
		printf("Case #%d: %d %d\n", _t + 1, deceitful_war(blocks), war(blocks));
	}
}
