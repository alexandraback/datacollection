...
void print_helper() {
  vpdd tot;
  re (i, N) tot.pb(mp(Naomi[i], 0));
  re (i, N) tot.pb(mp(Ken[i], 1));
  sort(all(tot));
  
  vi order;
  re (i, 2 * N) order.pb((int)tot[i].se);
...
pii solve_small() {
  ...
  // figure out normal War
  // Ken will always choose the smallest possible weight >= Naomi's
  ...
  re (i, N) {
    double told_naomi = Naomi[i];
    bool found = false;
    
    int index_ken = -1;
    re (j, N) {
      if (!used[j] && Ken[j] < told_naomi) {
        index_ken = j;
      }
    }
