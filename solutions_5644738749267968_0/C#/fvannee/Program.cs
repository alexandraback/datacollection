...
namespace TrainAlgorithms
{
   ...
   internal class R2014_C : Problem
   {
      ...
      {
         int n = this.ReadInt();
         var a2 = this.ReadDoubleList();
         var b2 = this.ReadDoubleList();
         Array.Sort(a2);
         Array.Sort(b2);
         var a = new List<double>(a2);
         var b = new List<double>(b2);
         int scoreHonest = Game(a, b);
         a = new List<double>(a2);
         b = new List<double>(b2);
         int scoreCheat = n - Game(b, a);
         this.WriteLine(scoreCheat + " " + scoreHonest);
      }

      int Game(List<double> a, List<double> b )
      {
         if (a.Count > 0)
         {
            double choice = a[0];
