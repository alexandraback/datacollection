...
#include <stdio.h>
int main(){
   int casos, i=0,N,j,k,r1,r2;
   double nao[1000], ken[1000],nao2[1000], ken2[1000], swap;
   scanf("%d\n",&casos);
   while (i++ != casos)
   {
//leyendo caso
      scanf("%d\n",&N);
      for (j = 0; j != N ; j++){scanf("%lf",&nao[j]);}
      for (j = 0; j != N ; j++){scanf("%lf",&ken[j]);}

//ordenando
      for (j = 0; j != N ; j++){
         for (k = 0 ; k != N-1; k++){
            if (nao[k] < nao[k+1]){swap=nao[k]; nao[k] = nao[k+1]; nao[k+1] = swap;}
         }
      }
      for (j = 0; j != N ; j++){
         for (k = 0 ; k != N-1; k++){
            if (ken[k] < ken[k+1]){swap=ken[k]; ken[k] = ken[k+1]; ken[k+1] = swap;}
         }
      }
      for (j = 0 ; j != N ; j++){nao2[j]=nao[j];ken2[j]=ken[j];}

//resolviendo caso Deceitful
      r1 = 0;
      for (j = 0; j != N ; j++){
         if (ken[j] < nao[0]){
            r1++;
            for(k = 0; k != N-j ; k++){if (ken[j] > nao[k]){break;}}
            for(; k != N-j ; k++){nao[k-1]=nao[k];}
         }
      }

//resolviendo caso war
      r2 = 0;
      for (j = 0; j != N ; j++){
         if (nao2[j] < ken2[0]){
            r2++;
            for(k = 0; k != N-j ; k++){if (nao2[j] > ken2[k]){break;}}
            for(; k != N-j ; k++){ken2[k-1]=ken2[k];}
         }
      }
      r2 = N - r2;
      printf("Case #%d: %d %d\n",i,r1,r2); 
