...
int program::my_program()
{
	...
	vector<const char*>& A1= argv[vi];
	unsigned int choice1= atoi(A1[0]);
	vector<const char*>& R1= argv[vi + choice1];

	vector<const char*>& A2= argv[vi +5];
	unsigned int choice2= atoi(A2[0]);
	vector<const char*>& R2= argv[vi +5 +choice2];

	vector<unsigned int> IR1= u_convert(R1);
	vector<unsigned int> IR2= u_convert(R2);
