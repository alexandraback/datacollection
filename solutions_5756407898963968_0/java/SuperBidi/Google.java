...
public class Google
{
    ...
    {
        int firstLine = Integer.parseInt(input[0][0]);
        int secondLine = Integer.parseInt(input[5][0]);
        ...
        for (int i = 0; i < 4; i ++)
        {
            for (int j = 0; j < 4; j ++)
            {
                if (input[firstLine][i].compareTo(input[5 + secondLine][j]) == 0)
                {
                    solution ++;
                    solutionString = input[firstLine][i];
                }
            }
        }
        if (solution == 0)
            return outputCase(output, "Volunteer cheated!");
        if (solution > 1)
            return outputCase(output, "Bad magician!");

        return outputCase(output, solutionString);
    }
