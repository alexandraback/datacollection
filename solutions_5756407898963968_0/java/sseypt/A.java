...
public class A
{
	...
	private static void putput() throws IOException
	{
		...
		cc: for (cc = 1; cc<=cases; cc++)
		{
			...
			for(int i=1;i<=16;i++) numbers.put(i, 0);
			// ---
			int firstrow = INT();
			for(int i=1;i<firstrow;i++) LINE();
			for(int i=1;i<=4;i++)
			{
				int n = INT();
			...
			for(int i=firstrow+1;i<=4;i++) LINE();
			// ---
			int secondrow = INT();
			for(int i=1;i<secondrow;i++) LINE();
			...
			for(int i=secondrow+1;i<=4;i++) LINE();	
			// ---
			int number = 0;
			for(int i=1;i<=16;i++)
			{
			...
			if (number == 0)	println("Case #"+cc+": "+"Volunteer cheated!");
			else				println("Case #"+cc+": "+number);
		}
	}
