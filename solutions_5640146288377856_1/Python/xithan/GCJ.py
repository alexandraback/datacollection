...
"""
    ...
    def __init__(self,  question, solver, reader=None, pre=None, path="", datatype="test", job="run"):  #"/Users/phi/Dropbox/CodeJam/Current"):
    ...
    def run(self):
        ...
        if False:
            print(text)
        self.output(text, time()-starttime, data)
...
#                                                      clean(numTodo), " ".join([clean(z) for z in workingOn])))
    ...
    def output(self, text, duration, input):
        if self.job == "run":
            ...
            if text.strip() == text2.strip():
                ...
                if self.show_input:
                    ...
                    for k in range(len(text)):
                        print("Input: %s" %  pp.pformat(input[k]) )
                        print("Result: %s" % text[k])
                ...
                for k in range(min(len(text), len(text2))):
                    ...
                    if text[k] != text2[k]:
                        print("Line %d: '%s' should be '%s'" % (k + 1, text[k], text2[k]))
