...
int main()
{
    ...
    for (int pp = 1; pp <= np; pp++)
    {
        ...
        {
            int guesses_per_row = C / W;
            guesses = guesses_per_row * R;

            // once we get out pattern, the last will be a hit, so if we say the one right after, it makes it so the ship has to be right before that.
            if (C % W == 0)
            {
                guesses += W - 1;
            }
