...
{
	ofstream out("A.data");
	out << "INSOMNIA" << endl; // n=0 seems to be the only case
	for (BigInteger n = 1; n <= 1000000; n++)
	{
		...
		{
			x = n * i;
			for (BigInteger divisor = 1; x / divisor > 0; divisor *= 10)
			{
				BigInteger digit = (x / divisor) % 10;
				seen |= (1 << digit.toInt());
			}
			i++;
		}
		out << bigIntegerToString(x) << endl;
		if ((n % 10000) == 0) cout << bigIntegerToString(n) << endl;
	}
}
...
{
	// Generate();
	vector<string> answers(1000001);
	ifstream in("A.data");
	for (auto& s : answers) in >> s;
