...
int update_seen(int x, bool seen[10], int seenCount) {
	while (x > 0) {
		int y = x % 10;
		if (!seen[y]) {
			seen[y] = 1;
...
int see_all(int x) {
	bool seen[10] = {0};
	int seenCount = update_seen(x, seen, 0);

	int y = x;
	while (seenCount != 10) {
		y += x;
		seenCount = update_seen(y, seen, seenCount);
	}
	return y;
}
...
void solve() {
	...
	if (!x) cout << "INSOMNIA" << endl;
	else cout << see_all(x) << endl;
}
