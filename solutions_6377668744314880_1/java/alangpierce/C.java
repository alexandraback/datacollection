...
public class C {
    private static final String PROBLEM = "round1a/C-large";
    ...
    private List<Integer> solve(List<Point> trees) {
        ...
        long[][] fastTrees = new long[trees.size()][2];
        for (int i = 0; i < trees.size(); i++) {
            fastTrees[i][0] = trees.get(i).x;
            fastTrees[i][1] = trees.get(i).y;
        }
        ...
        for (int i = 0; i < trees.size(); i++) {
            ...
            int minNumRemoved = Integer.MAX_VALUE;
            for (int j = 0; j < trees.size(); j++) {
                ...
                for (int k = 0; k < trees.size(); k++) {
                    if ((fastTrees[j][0] * fastTrees[k][1] - fastTrees[j][1] * fastTrees[k][0])  -
                            (fastTrees[i][0] * fastTrees[k][1] - fastTrees[i][1] * fastTrees[k][0]) +
                            (fastTrees[i][0] * fastTrees[j][1] - fastTrees[i][1] * fastTrees[j][0]) > 0) {
                        numRemoved++;
                    }
                }
                minNumRemoved = Math.min(minNumRemoved, numRemoved);
            }
            result.add(minNumRemoved);
        }
        return result;
    }

    private List<Integer> solveBackup(List<Point> trees) {
        if (trees.size() == 1) {
            return ImmutableList.of(0);
        }
