...
bool solve(){
    ...
    vector<int> on_ch(n, false);
    rep(i, n) on_ch[i] = contains(ch, ps[i]) == 1;

    rep(i, n){
        vector<pair<R, int>> events;
        int now = 0;
        rep(j, n) if(ps[i] != ps[j]){
            R a = angle(ps[j] - ps[i]);
            events.emplace_back(a, +1);
            events.emplace_back(a + 2 * PI, +1);
            events.emplace_back(a + 4 * PI, +1);
            events.emplace_back(a + 6 * PI, +1);
            events.emplace_back(a + 8 * PI, +1);
        }
        if(on_ch[i] or events.size() == 0){
            cout << 0 << endl;
            continue;
        }
        sort(all(events));
        ...
        while(r < size(events)){
            int ll = l;
            while(sgn(events[ll].fst, events[l].fst) == 0) ++ll;
            ++l;

            int rr = r;
            chmax(rr, l);
            while(sgn(events[rr+1].fst, events[ll].fst + PI) < 0) ++rr;
            chmin(res, rr - ll);
            r = rr; l = ll;
            if(events[ll].fst > 4 * PI) break;
        }
        cout << res << endl;
    }
    return true;
