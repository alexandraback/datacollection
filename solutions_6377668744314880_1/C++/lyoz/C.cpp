...
void solve()
{
	...
	rep(i,n){
		vector<Point> qs;
		rep(j,n) if(j!=i) qs.emplace_back(ps[j]-ps[i]);
		sort(all(qs),[](Point a,Point b){return atan2(a.y,a.x)<atan2(b.y,b.x);});
		
		rep(j,n-1) qs.push_back(qs[j]);
		int mn=INF;
		for(int j=0,k=0;j<n-1;j++){
			for(;k<j+(n-1);k++)
				if(CCW(Point(0,0),qs[j],qs[k])==-1)
					break;
			mn=min(mn,(n-1)-(k-j));
		}
		printf("%d\n",mn);
