...
const long double pi = acosl(-1.0l); //M_PI;
//}
/** Add On .. **/ //{
// <<= '0. Nichi Joo ., //{
...
struct point {
    ...
    point(long double x=0, long double y=0):x(x), y(y){
        if ( zero(y) ){
            ...
            long double t = atanl( fabsl(y/x) );
            if ( x>0 && y>0 ) angel = t;
            if ( x<0 && y>0 ) angel = pi - t;
            if ( x<0 && y<0 ) angel = pi + t;
            if ( x>0 && y<0 ) angel = 2*pi-t;
        }
    }
    bool operator < ( const point & t ) const {
        return angel<t.angel;
    }
//    bool operator == ( const point & t) const{
//        return x==t.x && y==t.y;
//    }
...
void run(){
    ...
    for (int i=0; i<N; i++){
        ...
        if (N<=3){
            ...
            for (int j=0; j<N; j++) {
                if ( j!=i ){
                    p[cnt++] = point( a[j].x-a[i].x, a[j].y-a[i].y );
                }
            }

            for (int j=0; j<cnt; j++){
                v.push_back( p[j].angel );
                v.push_back( p[j].angel + 2*pi );
            }
            sort(v.begin(), v.end() );
            cnt*=2;
            for (int j=0; j<N-1; j++) {
                int res = upper_bound(v.begin(), v.end(), p[j].angel+pi)
                - lower_bound(v.begin(), v.end(), p[j].angel);
                ans = min(ans, N-1-res);
                if ( i==4 ){
                }
            }
