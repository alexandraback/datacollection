...
def solve_case(cars):
    ...
    #Try and merge any singletons to the edges
    for c in single.keys():
        car = ' '
        for i, car in enumerate(others):
            if car[0] == c or car[-1] == c:
                break
        if car[0] == c:
            others[i] = single.pop(c) + others[i]
        elif car[-1] == c:
            others[i] = others[i] + single.pop(c)
    ...
    #Check for known bad arrangements
    mids = set()
    for car in cars:
        mids = mids | classify(car)[2]
    starts = [car[0] for car in cars]
    ends = [car[-1] for car in cars]
    for mid in mids:
        if mid in starts or mid in ends:
            return 0 #Can't do it!
    
    return (ways * factorial(len(cars))) % 1000000007
    ...
    print cars
    print len(cars), count, factorial(len(cars))
    return ways * count
    """
