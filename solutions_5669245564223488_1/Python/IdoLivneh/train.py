...
def merge_trains(trains):
	...
	while trains:
		...
		if poss:
			...
			count *= math.factorial(orig_trains.count(f))
			while f in res:
				res.remove(f)
			while f in trains:
				trains.remove(f)
		...
		if poss:
			...
			trains.remove(train)
			trains.remove(poss[0])
			count *= math.factorial(orig_trains.count(s))
			while s in res:
				res.remove(s)
			while s in trains:
				trains.remove(s)
...
def last_merge(trains):
	count = 1
	singles = []
	longs = []
	for train in trains:
		if len(train)==1:
			singles.append(train)
		else:
			longs.append(train)
	
	res = []
	
	for elem in set(singles):
		count *= math.factorial(singles.count(elem))
		flag = False
		for train in longs:
			if train[0] == elem or train[-1] == elem:
				flag = True
		if not flag:
			res.append(elem)
...
def solve(N, trains):
	...
	while True:
		...
		new_count, new_trains = merge_trains(copy_list(trains))
	...
	new_count, trains = last_merge(copy_list(trains))
	count *= new_count
	...
	if not is_ok:
		...
		perm_count = math.factorial(len(trains))
