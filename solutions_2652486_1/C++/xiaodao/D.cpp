...
const int RR = 109, NN = 25;

int a[NN]; int A[RR][NN];
int T, R, N, M, K;
    ...
    REP(i, R){
        ...
        REP(i, K){
            int ai; RD(ai);
                ...
                int t, tt;
            tt = 0, t = 2; while (ai % t == 0) ai /= t, tt++; checkMax(F[t], tt);

            if (tt == 1) c1 = true;
            if (tt&1) c2 = true;
            if (tt == 2) c22= true;

            tt = 0, t = 3; while (ai % t == 0) ai /= t, tt++; checkMax(F[t], tt);
            tt = 0, t = 5; while (ai % t == 0) ai /= t, tt++; checkMax(F[t], tt);


        }


        //FOR_1(i, 2, 5) cout << F[i] << " "; cout << endl;
        ...
        if (c1 || c2 && F[2] <= 2) a[t++] = 2, F[2]-=1;
        if (c22) a[t++] = 4, F[2] -= 2;

        while (F[2] >= 3) a[t++] = 8, F[2]-=2;
        while (F[2] >= 2) a[t++] = 4, F[2]-=2;
        REP(i, F[2]) a[t++] = 2;
