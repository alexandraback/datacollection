...
#undef randInt
#define randInt(maxValue) ((U64)rand()*((U64)maxValue)/((U64)(RAND_MAX+1))) // Between 0 inclusive and maxValue EXclusive
...
#undef fprintf
	...
	{
		int M = 1 + randInt(50);
		int L = (M + randInt(101 - M)) / M * M;
		fprintf(f, "%d\n", M);
		for (int k=0; k<2; k++) {
			for (int j=0; j<L; j++) {
				char *s = "abcdef????";
				fprintf(f, "%c", s[randInt(strlen(s))]);
			}
			fprintf(f, "\n");
		}
	}
...
int main(int _argc, char **_argv)
{
	...
	//doProblemDistributed("C-large.in", doC, " \t\n\r", NULL);
