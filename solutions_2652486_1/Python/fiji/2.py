...
def subsolve(f, k, n):
    ...
    six = 0
    eight = 0
    while sum(mx) + six + eight + four > n:
        if sum(mx)  + six + eight + four>n:
            if mx[0] > 1:
                mx[0] -= 2
                four += 1
        if sum(mx)  + six + eight + four>n:
    ...
    two = mx[0] + n-sum(mx)-four-six-eight
    print("%s%s%s%s%s%s%s" % (two*'2', mx[1]*'3', four*'4', mx[2]*'5', six*'6', mx[3]*'7', eight*'8'))
