...
//        PrintWriter out = new PrintWriter(System.out);
    ...
    public void solveL(int testNumber, BufferedReader in, PrintWriter out) throws IOException {
        out.println("Case #" + testNumber + ": ");
        StringTokenizer tokenizer = new StringTokenizer(in.readLine());
        int R = Integer.valueOf(tokenizer.nextToken());
        int N = Integer.valueOf(tokenizer.nextToken());
        ...
        flagR:
        for (int r = 0; r < R; ++r) {
            long[] prods = new long[K];
            tokenizer = new StringTokenizer(in.readLine());
            for (int i = 0; i < prods.length; ++i) prods[i] = Long.valueOf(tokenizer.nextToken());
            int[] D = new int[N];
            Random random = new Random();
            int MaxT = 50, T;
            for (T = 0; T < MaxT; ++T) {
                for (int i = 0; i < N; ++i) D[i] = random.nextInt(M - 2 + 1) + 2;
                boolean vis[] = new boolean[K];
                for (int mask = 0; mask < (1L << N); ++mask) {
                    long product = 1L;
                    for (int i = 0; i < N; ++i) if ((mask & (1 << i)) != 0) product *= (long)D[i];
                    for (int i = 0; i < K; ++i) if (prods[i] == product) vis[i] = true;
                }
                boolean find = true;
                for (boolean e : vis) if (!e) find = false;
