...
void myCode() {
  getint();
  int ttt=getint(), n=getint(), m=getint(), k=getint();
  m = 5;
  printf("Case #1:\n");
  fo(tt,ttt){
    vi a;
    ...
    int max2=0, max3=0, max5=0;
      ...
      chmax(max2, invPow(2, a[i]));
      chmax(max3, invPow(3, a[i]));
      chmax(max5, invPow(5, a[i]));
    }
    vi ret;
    fi(max3)
      ret.pb(3);
