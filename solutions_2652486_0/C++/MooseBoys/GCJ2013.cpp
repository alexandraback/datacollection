...
int CodeJamMain()
{
    ...
    for(int caseNum=0;caseNum<T;caseNum++)
    {
        ...
        for(int r=0;r<R;r++)
        {
            ...
            vector<int> numbers(N, 2);
            vector<int> bestNumbers = numbers;
            int bestMatches = 0;
            bool done = false;
            while(!done)
            {
                ...
                {
                    int product = 1;
                    if(sel&1) product *= numbers[0];
                    if(sel&2) product *= numbers[1];
                    if(sel&4) product *= numbers[2];
                    if(find(products.begin(),products.end(),product) != products.end()) matches++;
                }
            ...
            for(auto v : bestNumbers) output<<v;
        output<<endl;
        }
