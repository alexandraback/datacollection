...
int main(int argc, char **argv)
{
	...
	for(int it_r = 1; it_r <= in_r; it_r++)
	{
		...
		REP(sol_v_i, 10)
		{
			REP(it_c, sol_v[sol_v_i])
			{
				fprintf(fo, "%d", sol_v_i);
			}
		}
		fprintf(fo, "\n");
	}
...
void solve()
{
	...
	REP(it_k, in_k)
	{
		int sol_v_2 = get_pow(in_ki[it_k], 2);
		int sol_v_3 = get_pow(in_ki[it_k], 3);
		int sol_v_5 = get_pow(in_ki[it_k], 5);
		int sol_v_7 = get_pow(in_ki[it_k], 7);
		p_v[2] += sol_v_2;
		p_v[3] += sol_v_3;
		p_v[5] += sol_v_5;
		if(sol_v_2 > 2)
		...
		sol_v[2] = MAX(sol_v[2], sol_v_2);
		sol_v[3] = MAX(sol_v[3], sol_v_3);
		sol_v[5] = MAX(sol_v[5], sol_v_5);
		sol_v[7] = MAX(sol_v[7], sol_v_7);
	}
	REP(it_i,10)
	{
		if(p_v[it_i] <= 1)
	...
	if(in_m >= 4)
	{
		int sum_sol = sol_v[2] + sol_v[3] + sol_v[4] + sol_v[5] + sol_v[6] + sol_v[7] + sol_v[8];
		if(sum_sol > in_n)
		{
			sol_v[2] -= (sum_sol - in_n) * 2;
			sol_v[4] += (sum_sol - in_n);
		}
		else if(sum_sol < in_n)
		{
			int sum_p_v = 0;
			FOR(it_i,2,8)
			{
			...
			FOR(it_i,2,8)
			{
				des_p_sol[it_i] = p_v[it_i] / sum_p_v;
			}
			while(sum_sol < in_n)
			{
				int i_dp_sol_max = 2;
				...
				FOR(it_i,2,8)
				{
					p_sol[it_i] = (double)sol_v[it_i] / (double) sum_sol;
					if(des_p_sol[it_i] > p_sol[it_i] && (des_p_sol[it_i] - p_sol[it_i]) > dp_sol_max)
					{
						dp_sol_max = (des_p_sol[it_i] - p_sol[it_i]);
						i_dp_sol_max = it_i;
					}
				}
				sol_v[i_dp_sol_max]++;
				sum_sol = sol_v[2] + sol_v[3] + sol_v[4] + sol_v[5] + sol_v[6] + sol_v[7] + sol_v[8];
			}
		}
	}
}