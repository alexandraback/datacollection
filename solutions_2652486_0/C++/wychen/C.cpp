...
vector<vector<int> > allin;
map<vector<int>, set<int> > pp;

void getpp(const vector<int> &in, int ans, int level)
{
  if(level==N) {
    pp[in].insert(ans);
    if(D) {
  ...
  for(int i=0;i<2;i++) {
    if(i==0)
      getpp(in, ans, level+1);
    else
      getpp(in, ans*in[level], level+1);
  }
}

void genallin(vector<int> in, int level)
{
  if(level==N) {
    allin.push_back(in);
    if(D) {
  ...
  for(int i=2;i<=M;i++) {
    if(level>0 && in[level-1] > i) continue;
    in[level] = i;
    genallin(in, level+1);
  }
}

void guess(set<int> &p)
{
  set<vector<int> > pool;
  FOREACH(i, pp) {
    pool.insert(i->first);
  }
  //vector<bool> couldbe(allin.size(), true);
  FOREACH(i, p) {
    set<vector<int> > npool;
    //out(*i);
    FOREACH(j, pool) {
      int product = *i;
      if(pp[*j].find(product)!=pp[*j].end()) {
        npool.insert(*j);
        if(D) {
          REP(k,N) out((*j)[k]);
          cerr << endl;
  ...
  FOREACH(i, pool) vp.push_back(*i);
  out(vp.size());
  int index = rand() % vp.size();
  out(index);
  vector<int> ans = vp[index];
  REP(i, N) {
    cout << ans[i];
  }
  cout << endl;
