...
int main(){
    ...
    forall(t, 1, T+1){
        ...
        sort(sen, sen+N);
        int diff = sen[N-1].F - sen[N-2].F;
        printf("Case #%d: ",t);
        while(diff != 0){
            if(diff >= 2){
                printf("%c%c ",sen[N-1].S, sen[N-1].S);
                diff -= 2;
            }else{
                printf("%c ",sen[N-1].S);
                diff -= 1;
        ...
        forall(i, 0, N-2){
            while(sen[i].F >= 2){
                printf("%c%c ",sen[i].S,sen[i].S);
                sen[i].F-=2;
            }
            if(sen[i].F == 1 && i+1 < N-2){
                printf("%c%c ",sen[i].S,sen[i+1].S);
                sen[i].F--;
                sen[i+1].F--;
            }else if(sen[i].F == 1){
                printf("%c ",sen[i].S);
        ...
        while(sen[N-1].F != 0){
            printf("%c%c ",sen[N-1].S,sen[N-2].S);
            sen[N-1].F--;
            sen[N-2].F--;
        }
        printf("\n");
