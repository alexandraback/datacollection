 ...
 */
public class GCJ16_R1C_A implements Runnable {
...
//    static String inputFilename = "A-small-attempt0.in";
    ...
    public static class Data{
        ...
        public Data(char party, int cnt){
            this.party = party;
            this.cnt = cnt;
        }
    }
    ...
    public static class Solver extends  SolverAbstract{
        ...
        public void readInput() throws IOException {
            int N = scanner.nextInt(); // 3, or 26
            int[] P = scanner.loadIntArray(N); //10^3
            ...
            for (int i = 0; i < N; i++) {
                queue.add(new Data((char)('A'+i), P[i]));
            }
            while(!queue.isEmpty()){
                removeCarefully();

                if (!queue.isEmpty()) {
                    Data next = queue.peek();
                    if (next.cnt > sum / 2) {
                        removeCarefully();
                    }
                }
        ...
        public void removeCarefully(){
            Data first = queue.poll();
            first.cnt--;
            sum--;
            sb.append(first.party);
            if (first.cnt>0) {
                queue.add(first);
            }
        }
...
//    }
    ...
    public static void main(String[] args) {
        GCJ16_R1C_A program = new GCJ16_R1C_A();
        /*
        for(String arg:args){
