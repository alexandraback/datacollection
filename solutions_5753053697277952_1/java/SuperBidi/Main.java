...
public class Main
{
    ...
    {
        int parties[] = new int [26];

        int biggestPartyValue = 0;
        int biggestParty = 0;
        int secondBiggestPartyValue = 0;
        int secondBiggestParty = 0;

        String result = "";
        for (int i = 0; i < input[1].length; i ++)
        {
            parties[i] = Integer.parseInt(input[1][i]);
            if (parties[i] >= biggestPartyValue)
            {
                secondBiggestPartyValue = biggestPartyValue;
                secondBiggestParty = biggestParty;
                biggestPartyValue = parties[i];
                biggestParty = i;
            }
            else if (parties[i] > secondBiggestPartyValue)
            {
                secondBiggestPartyValue = parties[i];
                secondBiggestParty = i;
            }
        }

        while (parties[biggestParty] > parties[secondBiggestParty] + 1)
        {
            char a = (char) ('A' + biggestParty);
            result += a + "" + a + " ";
            parties[biggestParty] -= 2;
        }
        if (parties[biggestParty] > parties[secondBiggestParty])
        {
            char a = (char) ('A' + biggestParty);
            result += a + " ";
            parties[biggestParty] -= 1;
        }

        for (int i = 0; i < input[1].length; i ++)
        {
            if (i != biggestParty && i != secondBiggestParty)
            {
                while (parties[i] > 0)
                {
                    if (parties[i] > 1)
        ...
        for (int i = 0; i < parties[biggestParty]; i ++)
        {
            char a = (char) ('A' + biggestParty);
            char b = (char) ('A' + secondBiggestParty);
            result += a + "" + b + " ";
        }

        return outputCase(output, result.substring(0, result.length() - 1));
    }
