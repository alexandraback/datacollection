...
int main(int argc, char** argv) {
	...
	REP (_t, T) {
		...
		while (true) {
			// not buy
			double time_not_buying = X / productivity;
			// buy
			double time_buying = C / productivity + X / (productivity + F);

			dump(time_not_buying);
			dump(time_buying);
			if (time_not_buying < time_buying) {
				time += time_not_buying;
				break;
			} else {
				time += C / productivity;
				productivity += F;
