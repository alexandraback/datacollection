...
public class D {
	...
	static String filesPath = System.getProperty("user.dir") + "/src/" + D.class.getPackage().getName() + "/";
	static String fileName = D.class.getSimpleName() + "-small-attempt0";
	...
	public void solve() throws Exception {
		...
		nextCase: for(int testCase=1; testCase<=testCasesCnt; testCase++) {
			...
			String can = "GABRIEL", cant = "RICHARD";
			...
			if (x<=2) {
				...
				println(r<=1 ? cant: can);
			} else if (x==4) {
				println(r<=2 ? cant: can);
			} else if (x==5) {
				if (r<=2) println(cant);
				else if (r==3) println(c<10 ? cant : can);
				else println(can);
			} else if (x==6) {
				println(r<=3 ? cant: can);
			} else {
				println(cant);
			}
	...
	public static void main(String[] args) throws Exception {
		...
		if (!toFile) {
			...
			String outPath = filesPath + outFile;
