...
bool run(){
    ...
    if(X>=7) return false;
    ///spillover
    int lupper = min(R+1, C+1)*2 - 1;
    if ( X>=lupper ) return false;
    //if ( X>= min(R, C)*2 ) return false;
    ...
    if ( left>=R ){
        ...
        for (int i=0; i<=left; i++){
            int flag = false;
            for (int j=0; j<C; j++){
                int a=R*j-i, b=R*(C-j-1)-(left-i);
                if ( a<0 || b<0 ) continue;
                if ( a%X==0 && b%X==0 ) flag = true;
            }
            can &= flag;
        }
        if (!can) {
