...
bool track2(int baixo, int dir, int a, int b, int extra){
    mat[a][b]=1;
    if (baixo==1 && dir==1){
        for (int j=0; j<M; j++){
            for (int i=N-1; i>=0; i--){
                if (mat[i][j]==0){
                    ...
                    bool verdade=true;
                    dfs(&cont, i, j);
                    for (int e=0; e<=extra && cont-e>=0; e++){
                        verdade=true;
                        for (int k=0; k<=N-n2; k++){
                            if ((j==0 && (cont-e+N*k)%X==0) || (j>0 && (cont-e-N*k)%X==0)){
                                verdade=false;
                            }
                        }
                        clean();
    ...
    else if (baixo>1 && track2(baixo-1, dir, a+1, b, extra)) return true;
    else if (dir>1 && track2(baixo, dir-1, a, b+1, extra)) return true;
    mat[a][b]=0;
    return false;
}
    ...
    for (int caso=1; caso<=T; caso++){
        ...
        if ((R*C)%X!=0) vitoria=true;
            ...
            if (M>X){
                ...
                else vitoria = track(N, n2+n1-N, 0, 0);
            }
