    ...
    class D
    {
        static void Main(string[] args)
        {
            ...
            /*for (var x = 1; x < 20; x++)
                for (var r = 1; r < 20; r++)
                    for (var c = 1; c < 20; c++)
                        Console.WriteLine("X={0}, R={1}, C={2} - {3}", x, r, c, DTests.RichardCanWin(x, r, c) ? "RICHARD" : "GABRIEL");
            */

            var input = @"C:\Users\kaspir\Downloads\D-small-attempt3.in";
        ...
        public class Solver
        {
            public static IEnumerable<string> Solve(IEnumerable<string> input)
            {
                ...
                for (var i = 0; ; i++)
                {
                    ...
                    var r = parts[1];
                    var c = parts[2];
                    yield return string.Format("Case #{0}: {1}", i + 1, RichardCanWin(x, r, c) ? "RICHARD" : "GABRIEL");
                }
            }
