...
public class ProblemA {
    ...
    int minimize(List<Integer> values)
    {
        ...
        for (Integer candidate : values)
        {
            ...
            for (Integer test : copy)
            {
                val += Math.abs(candidate - test);
            }
            score.add(val);
        }
    ...
    int stepThrough(List<List<Pair>>input)
    {
        ...
        {
            char testChar = input.get(0).get(k).aChar;
            List<Integer> values = new ArrayList<Integer>();
            for (List<Pair> in : input)
            {

                if (in.get(k).aChar != testChar)
                {
                    return -1;
                }
                values.add(in.get(k).count);
            }
            moves += minimize(values);
