...
public class AL {
	...
	void run() {
		...
		for (int k = 1; k <= testCase; k++) {
			...
			int[][] action = new int[n][idstr.length()];
			for (int i = 0; i < n; i++) {
			...
			if (possible) {
				...
				for (int j = 0; j < idstr.length(); j++) {
					...
					int max = 0;
					for (int i = 0; i < n; i++) {
						tmpcount += Math.abs(action[i][j]);
						max = Math.max(max, action[i][j]);
					}
					...
					for (int x = 1; x <= max; x++) {
						int tmpcount2 = 0;
						for (int i = 0; i < n; i++) {
							action[i][j] = action[i][j] - 1;
							tmpcount2 += Math.abs(action[i][j]);
						}
						min = Math.min(min, tmpcount2);
					}
