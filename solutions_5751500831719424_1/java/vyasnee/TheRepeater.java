	...
	public static void main(String[] args) {
		...
		try {
			...
			for(int i=0;i< numTestCase ; i++) {
				...
				if(ans!=0){
					List<List<String>> commonStringAr = cehckValid(words);
					if(commonStringAr!=null){
					String commonString = commonStringAr.get(commonStringAr.size()-1).get(0);
					
					
						ans = findNumOfMoves(commonString, commonStringAr, words);
					}
				}
				sbfResult.append("Case #"+(i+1)+ ": " +  (ans==-1?"Fegla Won" : ans));
				sbfResult.append("\n");
