...
//    private static final String IMPOSSIBLE = "IMPOSSIBLE";
    private static final String NO = "Fegla Won";
    ...
    private void run(int caseNumber) throws Exception {
        ...
        int ret=0;
        int[] idx = new int[N];
        for (int chIdx = 0; chIdx < trim[0].length(); chIdx++) {
            char c = trim[0].charAt(chIdx);
            int[] cnt = new int[N];
            for (int i = 0; i < N; i++) {
            ...
            sort(cnt);
            int v=INF;
            for (int to = cnt[0]; to <= cnt[cnt.length-1]; to++) {
                int s=0;
...
//        int ret = abs(a[0].length()-a[1].length());
//        int ret = INF;
