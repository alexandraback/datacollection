...
public class Task1 {
    ...
    public static String getResult() {
        ...
        for (int j = 0; j < D; j++) {
            int max = 0;
            for (int i = 1; i < N; i++) {
                if (nums[i][j] > nums[max][j]) max = i;
            }
            int[] occ = new int[nums[max][j]+1];
            for (int i = 0; i < N; i++) {
                occ[nums[i][j]]++;
            }
            max = 0;
            ...
            for (int i = 0; i < occ.length; i++) {
                if (occ[i] > occ[max]) max = i;
                sum += occ[i];
            }
            double avg = sum / occ.length;
            int bestMode = -1;
            for (int i = 0; i < occ.length; i++) {
                if (occ[i] != occ[max]) continue;
                if (bestMode == -1) bestMode = i;
                else if (Math.abs(i - avg) < Math.abs(bestMode - avg)) bestMode = i;
            }
            for (int i = 0; i < N; i++) {
                result += Math.abs(nums[i][j] - bestMode);
            }
        }
        return "" + result;
    }
