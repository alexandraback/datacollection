...
namespace CodeJam
{
    ...
    public class CodeJamHelper : IDisposable
    {
        ...
        {
            using (var helper = new CodeJamHelper('A', ProblemType.Large, 0))
            {
                TheRepeaterLarge.Run(helper);
            }
        }
        ...
        public CodeJamHelper(char problem, ProblemType problemType)
            : this(Path.Combine(filesDirectory, char.ToUpper(problem).ToString() + "-" + GetProblemTypeName(problemType) + "-practice" + ".in"))
        {
        }
