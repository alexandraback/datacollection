...
string make_solution(int res) {
  if (res == -1) return "Fegla Won";
  else {
    SS ss; ss << res; return ss.str();
  }
}
...
int solve() {
  ...
  re (i, N) {
    //debug("S ", i, ":", S[i]);
    re (j, (int)S[i].length()) {
      if (j == (int)S[i].length() - 1 || S[i][j] != S[i][j+1]) {
        //debug(i, j, S[j]);
        s[i] += S[i][j];
        
        int cur = j, nr = 0;
        while (cur >= 0 && S[i][cur] == S[i][j]) {
          --cur;
          ++nr;
        }
  ...
  re (i, N) re (j, i) if (s[i] != s[j]) return -1;
  
  re (i, N) {
    assert(size(occ[i]) == size(occ[0]));
  }
  ...
  re (j, size(occ[0])) {
    ...
    fo (k, 1, 100) {
      int cur_cost = 0;
      re (i, N) cur_cost += abs(occ[i][j] - k);
      remin(bst_cost, cur_cost);
    }
    res += bst_cost;
  }
