...
void fill_chs(const string& s, vector<char>& chs) {
    chs.clear();
    chs.push_back(s.front());
    range(valr c : s) {
        if(c != chs.back()) chs.push_back(c);
    }
}
...
void solver_t::solve() {
    ...
    vector<char>            chs;
    fill_chs(in.ss.front(), chs);
    val M = chs.size();

    vector<vector<uint>>    cts;
            ...
            if(c != pc) {
                {
                    val j = cs.size();
                    // cerr << j << endl;
                    if(j >= chs.size()) return;
                    if(c != chs.at(j)) return;
                }
                cs.push_back(1);
                pc = c;
            } else {
    ...
    range(j,M) {
        ...
        range(k, 1, 102) {
            ...
            range(i,N) {
                // cerr << i << " " << j << endl;
                su += abs(int(k) - int(cts.at(i).at(j)));
            }
            mi = min(mi, su);
        }
        //cerr << "Mi: " << mi << endl;
