...
const int N = 1009; vector<PII> a[N];
...
int main(){
    ...
    Rush{
        ...
        REP_C(i, RD(n)){
            ...
            //cout << b << endl;
            a[i].PB(MP(b[0] , 1));
            FOR(j, 1, b.size()) if (b[j] == a[i].back().fi) a[i].back().se++;
            else a[i].PB(MP(b[j] , 1));
        }

        bool fg = 0;

        FOR(i, 1, n) if (a[i].size() != a[0].size()){
            puts("Fegla Won"); fg = 1;
            break;
        }
        if (fg) continue;

        FOR(i, 1, n) REP(j, SZ(a[0])) if (a[i][j].fi != a[0][j].fi){
            puts("Fegla Won"); fg = 1;
            break;
        }
        if (fg) continue;
        ...
        int ans = 0; VI c;
        REP(i, SZ(a[0])){
            CLR(c); REP(j, n) c.PB(a[j][i].se); SRT(c);
            REP(j, n) ans += abs(c[SZ(c) / 2] - c[j]);
        }
        OT(ans);
    }
}
