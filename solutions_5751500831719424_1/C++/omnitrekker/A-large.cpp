...
void solve(int i) {
	...
	for (int s=0;s<in_c[i].size();s++) {
		vector<char> tmp;
		vector<int> ntmp;
		int n=1;
		tmp.push_back(in_c[i][s][0]);
		for (int a=1;a<in_c[i][s].size();a++) {
			if (in_c[i][s][a-1]!=in_c[i][s][a]) {
				tmp.push_back(in_c[i][s][a]);
				ntmp.push_back(n);
				n=1;
			} else {
				n++;
	...
	for (int x=0;x<seq.size();x++) {
		if (seq[x]!=seq[0]) possible=false;
	}
	if (possible) {
		int moves=0;
		for (int y=0;y<num[0].size();y++) {
			vector<int> nums;
			for (int x=0;x<seq.size();x++) {
				nums.push_back(num[x][y]);
			}
