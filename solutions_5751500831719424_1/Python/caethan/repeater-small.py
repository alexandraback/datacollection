...
def solve_case(strings):
    ...
    #Check that the character ordering is identical for each string
    chars = np.array([output[0] for output in char_count(strings[0])])
    for s in strings:
        test = np.array([output[0] for output in char_count(s)])
        if not len(test) == len(chars):
            return "Fegla Won"
        if not (chars == test).all():
            return "Fegla Won"
            
    counts = np.empty((len(strings), len(chars)), dtype=int)
    for i, s in enumerate(strings):
        counts[i,:] = np.array([output[1] for output in char_count(s)])
   
    #Shift each count to the median for that character    
    median = np.median(counts, axis=0).astype(int)
    
    changes = median - counts
    
    return abs(changes).sum()
