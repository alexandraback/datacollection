...
char s[1010];
bool atBegin[1010];
bool notAtBegin[1010];
int N;

string schstr(char c)
{
	string s;
	s += c;
	return s;
...
int main()
{
	...
	jk(T)
	{
		...
		ji(N)
			bucket[s[i]].pb(i);
		ji(N) atBegin[i] = notAtBegin[i] = false;
		...
		foreach(q, bucket)
		{
			foreach(idx, q)
			{
				...
				{
					atBegin[idx] = notAtBegin[idx] = true;
				}
			}
			for (int i = 1; i < N; i++)
				notAtBegin[i] = notAtBegin[i] || notAtBegin[i-1];
		}
		string ret;
		ji(N) if (atBegin[i]) ret = schstr(s[i]) + ret; else ret += s[i];
		printf("%s\n", ret.c_str());
	}
}
