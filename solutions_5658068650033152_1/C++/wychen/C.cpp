...
#define out(x) {if(D) cerr << #x"=" << x << endl;}
#define outv(x) {if(D){cerr << #x"={";FOREACH(i,x){cerr << *i << " ";}cerr << "}" << endl;}}
#define outm(x) {if(D) {cerr << #x"={";FOREACH(i,x){cerr << i->first << ": " << i->second << ", ";}cerr << "}" << endl;}}
...
/*
 ...
 template<class T> void stov(string s,vector<T> &vi){vi.clear();istringstream sin(s);for(T v;sin>>v;vi.push_back(v));}//NOTES:stov(
...
int inside(int N, int M, int K, int offx, int offy, double r) {
  
  double xc = (N+offx-1)*0.5;
  double yc = (M+offy-1)*0.5;
  
  vector<pair<double, pii> > dots;
  
  REP(x, N) {
    REP(y, M) {
      double dist = (x-xc)*(x-xc) + (y-yc)*(y-yc);
      dist += (1.0*rand())/RAND_MAX;
      if(x!=xc && y!=yc) {
        double angle = (atan2(x-xc, y-yc) / 2 / M_PI + 0.5);
        assert(angle >= 0);
        assert(angle <= 1);
        dist += fmod(angle + r, 1.0);
      }
      dots.push_back(make_pair(dist, pii(x, y)));
    }
  }
  sort(dots.begin(), dots.end());
  vector<vi> board(N, vi(M));
  
  double bdist = dots[K-1].first;
  //out(bdist);
  //for(auto i: dots) {
  int bi = 0;
  for(int i=0; i<K; i++) {
    double dist = dots[i].first;
    //out(dist);
    if (fabs(dist-bdist)<1e-5) {
      bi = i;
      out(bi);
      //break;
    }
    int x = dots[i].second.first;
    int y = dots[i].second.second;
    board[x][y] = 1;
    //out(dist);
    //out(x);
    //out(y);
  ...
  FOR(x, 0, N) {
    FOR(y, 0, M) {
      ...
      pq.push(make_pair(score, pii(x, y)));
    }
  }
  ...
  while(pq.size() && rest) {
    ...
    FOR(i, -1, 2) {
      ...
      FOR(j, -1, 2) {
        ...
        int score = 0;
        FOR(a, -1, 2) {
          if(x+i+a <0 || x+i+a>=N) continue;
          FOR(b, -1, 2) {
            if(y+j+b <0 || y+j+b>=M) continue;
            if (board[x+i+a][y+j+b])
              score++;
          }
        }
        pq.push(make_pair(score, pii(x+i, y+j)));
      }
    }
  }
...
void solve(bool skip)
{
  ...
  REP(rnd, 500) {
    ...
    FOR(i, -sr, 1)
      FOR(j, -sr, 1) {
        best = max(best, inside(N, M, K, i, j, rand()*1.0 / RAND_MAX));
      }
    if(obest != best && rnd>0) {
      out(best);
      out(rnd);
