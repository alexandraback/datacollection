...
public class Enclosure {
    ...
    public static void main(String[] args) {
        try {
            s = new Scanner(new BufferedReader(new FileReader("C-large.in")));
            w = new BufferedWriter(new FileWriter("C-large.out"));
    ...
    private static int handle() {
        ...
        for (int n = 1; n < N - 1; ++n) {
            boolean maxenclosed = false;
            for (int m = 1; !maxenclosed && m < M - 1; ++m) {
                int k = n * m + m * 2 + n * 2;
                int nec = m * 2 + n * 2;
                if (k >= K) {
                    if (nec < best)
