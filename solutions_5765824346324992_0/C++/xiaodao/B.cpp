...
LL get(LL x){
    priority_queue<pair<LL, LL> > Q; CLR(Q); m -= cut(x);

    REP(i, n){
        LL t = x % A[i]; //if (t == 0)  t = A[i];
        Q.push(MP(-t, -i));
    ...
    while (--m){
        LL tt = Q.top().fi, t = Q.top().se;
        Q.pop(); Q.push(MP(tt - A[-t], t));
        //cout <<  "  " << t << endl;
    }
...
int main(){
    ...
    Rush{
        printf("Case #%d: ", ++Case);
         RD(n, m); REP(i, n) RD(A[i]);

        LL l = 0, r = 100000LL * 1000000000LL;
        while (l < r){
            LL m = (l + r + 1) / 2;
                //cout << l << " " << r << endl;
            if (cut(m) >= ::m) r = m - 1;
