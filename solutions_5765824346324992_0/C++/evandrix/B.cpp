...
inline LL f(LL mid)
{
	...
	FOR(i,0,B)
	{
		double md = (double) mid;
		double mi = (double) M[i];
		total += (LL) ceil(md/mi);
	}
	return total;
}
inline LL lb(LL lo, LL hi, LL tgt)
{
	while(lo <= hi)
	{
		LL mid = lo + (hi-lo)/2;
...
int main()
{
	...
	FOREQ(t,1,T)
	{
		scanf("%lld%lld", &B,&N); //N=1-idx
		FOR(i,0,B)
		{
			scanf("%lld", &M[i]); //mi=0-idx
		}
		if (N<=B)
		{
			printf("Case #%d: %lld\n", t, N);
		...
		else
		{
			...
			//	fprintf(stderr, "NmodX = %lld\n", N%X);
				int final = -1;
				LL ans = lb(0,L+10,N%X);
			//	fprintf(stderr, "ans = %lld; f(ans) = %lld\n", ans, f(ans));
				LL J = (N%X)-f(ans-1);
