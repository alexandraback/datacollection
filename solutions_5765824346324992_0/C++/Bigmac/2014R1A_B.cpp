...
int main(int argc, char **argv)
{
	...
	for(int it_t = 1; it_t <= in_t; it_t++)
	{
        ...
        {
            in_m[i] = readInt();
            max_m = MAX(max_m, in_m[i]);
        }
        int selected = -1;
        long long total_cut_num = 0;
        auto judge = [&](long long tim) -> int{
            long long min_minute = INT_MAX;
            long zero_cnt = 0;
            int in_m_zero[2000];
            total_cut_num = 0;
            for(int i= 0; i<in_b; i++)
            {
                long long cut_num = (tim / in_m[i]) + 1;
                min_minute = min(min_minute,(tim - (cut_num-1) * in_m[i]));
                total_cut_num += cut_num;
            }
            if(total_cut_num >=in_n && total_cut_num <(in_n + in_b))
            {
                FOR(i,0,in_b)
                {
                    long long cut_num = (tim / in_m[i]) + 1;
                    long long delta = (tim - (cut_num-1) * in_m[i]);
                    if(delta == min_minute)
                    {
                        in_m_zero[zero_cnt] = i;
                        zero_cnt++;
                    }
                }
                if(total_cut_num >=in_n && total_cut_num <(in_n + zero_cnt))
                {
                    selected = in_m_zero[zero_cnt - (total_cut_num - in_n) - 1];
                    printf("[%d - %d] = %d\n", total_cut_num, in_n, selected);
                    return 0;
                }
            }
            if(total_cut_num < in_n)
            {
                return -1;
            }
            if(total_cut_num > in_n)
		...
		auto solve = [&]() -> int{
            judge(max_m);
            long long low_time = (in_n / total_cut_num) * max_m;
            long long high_time = (in_n / (total_cut_num - in_b)) * max_m;
            int judge_low_time;
            int judge_high_time;
            printf("Low Time = %lld, m=%d\n", low_time, max_m);
            printf("Start Low\n");
            do
            {
                judge_low_time = judge(low_time);
                if(judge_low_time == 0)
                {
                    return 0;
                }
            ...
            while(judge_low_time > 0);
            printf("Finish Low\n");
            do
            {
                judge_high_time = judge(high_time);
                if(judge_high_time == 0)
                {
                    return 0;
                }
                if(judge_high_time < 0)
            ...
            while(judge_high_time < 0);
            printf("Finish High\n");
            while(low_time < high_time)
            {
                printf("It: %d, %d\n", low_time, high_time);
                long long mid_time = (high_time + low_time)/2;
                int judge_mid_time = judge(mid_time);
                if(judge_mid_time == 0)
                {
                    return 0;
                }
        ...
        int sol = solve();
        printf("Sol: %d Done\n", it_t);
		fprintf(fo, "Case #%d: %d", it_t, selected+1);
		fprintf(fo, "\n");
	}
	closeIOFile();
}