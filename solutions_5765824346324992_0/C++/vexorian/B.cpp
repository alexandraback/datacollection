...
struct solver
{
    ...
    int solve()
    {
        // I always forget how to use priority_queue so std::set it is.
        set< pair<int,int> > q;
        for (int i = 0; i < B; i++) {
            q.insert( {0, i} );
        }
        ...
        for (int i = 1; i < T; i++) {
            // get event
            auto it = q.begin();
            int j = it->second; //Barber j
            int t = it->first;
            q.erase(it);
            
            seq.push_back(j);
            //cout << "time "<<t<<" barber "<<j<<endl;
            q.insert( {t + M[j], j} );
            //cout << j << " ";
        }
        //cout << endl;
        int m = -1;
        for (int x = TX/2; x <= TX; x++) {
            bool good = true;
            for (int j = 0; good && (j < x); j++) {
                good &= (seq[j + x] == seq[j]);
                good &= (seq[j + 2*x] == seq[j + x]);
                good &= (seq[j + 3*x] == seq[j + 2*x]);
            }
            if (good) {
                m = x;
                break;
