...
def solve_time(t, M):
    remain, finished = np.array([[t % m, t // m] for m in M]).transpose()
    N = finished.sum() + (remain > 0).sum()
    
    out = []
    l = None
    for i in range(len(M)):
        if remain[i] == min(remain) and remain[i] > 0:
            l = i
    if l is not None:
        out.append([N, l + 1])
...
def binary_search(t_hi, k, M):
    t_lo = 0
    while t_lo <= t_hi:
        t_mid = (t_lo + t_hi) // 2
        solve = solve_time(t_mid, M)
        for N, b in solve:
            if N == k:
                return b
            elif solve[0][0] > k:
