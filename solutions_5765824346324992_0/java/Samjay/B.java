...
//	private static final String islarge = "-large";
	private static final String fileName = "results/con2015/con2015R1a/"+B.class.getSimpleName().toLowerCase();//+islarge;
	...
	private void solve() {
		...
		if(B == 1){
			...
			long tot = calc(bs,best);
			System.err.println(">> "+tot+" "+best);
			for(int i=B-1;i>=0;i--){
				if((best%bs[i]) == 0){
					if(tot == N){
						out.printLine(i+1);
	...
	public static long calc(int[]bs, long time){
		long total = 0;
