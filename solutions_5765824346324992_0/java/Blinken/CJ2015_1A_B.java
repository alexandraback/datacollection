...
public class CJ2015_1A_B {
	...
	static void doProblem(){
		String sDirectory = "R:\\dev\\codejam\\20151A\\B\\";
//		String sFileName = "B-example";
		String sFileName = "B-small-attempt0";
//		String sFileName = "B-small-attempt1";
//		String sFileName = "B-large";
		...
		for( int xTestCase = 1; xTestCase <= ctTestCases; xTestCase++ ){
			int[] barbers_place = splitInteger( listLines.get( ++xLine ) );
			int[] minutes = splitInteger( listLines.get( ++xLine ) );
			int ctBarbers = barbers_place[1];
//			System.out.println( "tc " + xTestCase + " " + minutes[0] + " " + ctBarbers );
			int xPlace = barbers_place[2];
			int iAnswer = 0;
			if( xPlace <= ctBarbers ){
				iAnswer = xPlace;
			} else {
				long nLow = 0;
				long nHigh = 1;
				while( true ){ // find high value
					long served = computeServed( ctBarbers, minutes, nHigh ) + ctBarbers;
//			System.out.println( "served up " + nHigh + " " + served );
					if( served > xPlace ) break;
					nLow = nHigh;
					nHigh *= 2;
				}
				long nFloor = find_floor( ctBarbers, minutes, nHigh, nLow, xPlace );
				long nServedAtFloor = computeServed( ctBarbers, minutes, nFloor ) + ctBarbers;
				long nLeft = xPlace - nServedAtFloor;
				int xBarber = 1;
				for( ; xBarber <= ctBarbers; xBarber++ ){
					if( ( nFloor + 1 ) % minutes[ xBarber ] == 0 ) nLeft--;
					if( nLeft == 0 ) break;
				}
				iAnswer = xBarber;
			}
...
//			if( xTestCase % 100 == 0 ) System.out.println( "Case #" + xTestCase + ": " + ctFairAndSquareNumbers );
			sbAnswer.append( "Case #" + xTestCase + ": " + iAnswer + "\n" );
		}
		fileSave( sAbsolutePath_output, sbAnswer.toString(), sbError );
		System.out.println( "wrote " + ctTestCases + " cases" );
	}

	static long find_floor( int ctBarbers, int[] minutes, long nHigh, long nLow, long servedTarget ){
//		System.out.println( " floor " + nHigh + " " + nLow );
		if( nHigh - nLow == 1 ) return nLow;
		long nMid = ( nHigh - nLow ) / 2 + nLow;
		long servedMid = computeServed( ctBarbers, minutes, nMid ) +  ctBarbers;
		if( servedMid < servedTarget ){
			nLow = nMid;
		} else {
			nHigh = nMid;
		}
		return find_floor( ctBarbers, minutes, nHigh, nLow, servedTarget );
	}

	static long computeServed( int ctBarbers, int[] minutes, long minute ){
		long n = 0;
		for( int x = 1; x <= ctBarbers; x++ ){
			long servedForBarber = minute / minutes[x];
			n += servedForBarber;
		}
		return n;
	}
