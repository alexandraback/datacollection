...
public:
	...
	BigInteger bruteSolver(int index, string &c, string &j, BigInteger curDif)
	{
		if (index >= C.size())
			return curDif;
		...
		if (C[index] == '?' && J[index] == '?')
		{
			BigInteger min = BigInteger(LONG_MAX)*BigInteger(LONG_MAX);
			BigInteger absmin = min;
			string cc = c;
			string cj = j;
			FOR(ii, 0, 10)
			{
				FOR(ji, 0, 10)
				{
					cc[index] = '0' + ii;
					cj[index] = '0' + ji;
					BigInteger d = bruteSolver(index + 1, cc, cj, curDif * 10 + ii - ji);
					if (absBigInt(d) < absmin)
					{
						min = d;
						absmin = absBigInt(d);
						c = cc;
		...
		else if (C[index] == '?')
		{
			...
			{
				cc[index] = '0' + ii;
				BigInteger d = bruteSolver(index + 1, cc, cj, curDif * 10 + cc[index] - cj[index]);
				if (absBigInt(d) < absmin)
				{
					min = d;
					absmin = absBigInt(d);
...
int main(int argc, char* argv[])
{
	...
	std::string inputFile("C:\\Users\\David\\Downloads\\B-large (1).in");
