...
SS res;
void update(int i,string s3, string s4,int com) {
	if (com!=0)
	For(j,i+1,n) 
	{
		if (s3[j]=='?') if (com==2) s3[j] = '9'; else s3[j] = '0';
		if (s4[j]=='?') if (com==2) s4[j] = '0'; else s4[j] = '9';
	}
	// cout<<s3<<" "<<s4<<endl;
	// cout<<convert(s3)<<endl;
	if (abs(convert(s3)-convert(s4))<ans) {
		ans = abs(convert(s3)-convert(s4));
		res = SS(s3,s4);
	} else if (abs(convert(s3)-convert(s4))==ans) res = min(res,SS(s3,s4));
	// cout<<ans<<endl;
}
int process() {
	int com = 0;;
