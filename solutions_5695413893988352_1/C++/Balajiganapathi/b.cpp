...
pair<string, string> solve(string s1, string s2) {
    int n = si(s1);
    pair<string, string> ans(string(n, '?'), string(n, '?'));

    ll dbest = 0;
    pair<ll, ll> nnbest, nbest, nmx, nmn, nnmx, nnmn, cnbest;
    ll dmx, dmn;
    dmx = dmn = 0;
    nbest = nmx = nmn = mp(0, 0);
    ...
    for(int i = n - 1; i >= 0; --i) {
        ...
        rep(d1, f1, t1) rep(d2, f2, t2) {
            ...
            if(d1 > d2) {
                cdbest = m * (d1 - d2) + dmn;
                cnbest = mp(m * d1 + nmn.fi, m * d2 + nmn.se);
            } else if(d1 == d2) {
                cdbest = m * (d1 - d2) + dbest;
                cnbest = mp(m * d1 + nbest.fi, m * d2 + nbest.se);
            } else {
                cdbest = m * (d1 - d2) + dmx;
                cnbest = mp(m * d1 + nmx.fi, m * d2 + nmx.se);
            }

            if(m * (d1 - d2) + dmn < ndmn) {
                ndmn = m * (d1 - d2) + dmn;
                nnmn = mp(m * d1 + nmn.fi, m * d2 + nmn.se);
            }
            if(m * (d1 - d2) + dmx > ndmx) {
                ndmx = m * (d1 - d2) + dmx;
                nnmx = mp(m * d1 + nmx.fi, m * d2 + nmx.se);
            }
            if(abs(cdbest) < abs(ndbest)) {
                ndbest = cdbest;
                nnbest = cnbest;
            }
        ...
        //trace(i, dbest, nbest, dmn, nmn, dmx, nmx);
    }

    assert(nbest.fi >= 0 && nbest.se >= 0);
    ans = mp(to(nbest.fi, n), to(nbest.se, n));
    return ans;
}
...
int main() {
    ...
    //freopen("new_small1.txt", "w", stdout);
    ...
    for(int kase = 1; kase <= kases; ++kase) {
        ...
        pair<string, string> ans = solve(s1, s2);


        //if(bans != ans) {
            //trace(s1, s2, bans, ans);
        //}

        printf("Case #%d: %s %s\n", kase, ans.fi.c_str(), ans.se.c_str());
