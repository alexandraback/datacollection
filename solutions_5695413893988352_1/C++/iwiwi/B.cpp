...
using T = tuple<ll, string, string>;
T INF = make_tuple(1LL << 62, string(), string());

T doit(string A, string B, int i) {
  rep (j, i) if (A[j] != B[j]) return INF;
  if (i < N && A[i] >= B[i]) return INF;
  for (int j = i; j < N; ++j) if (A[j] == '?') A[j] = '9';
  for (int j = i; j < N; ++j) if (B[j] == '?') B[j] = '0';
  return make_tuple(stoll(B) - stoll(A), A, B);
}

// A <= B
T hoge(string A, string B) {
  T ans = INF;
  rep (i, N) {
    if (A[i] == '?' && B[i] == '?') {
      tie(A[i], B[i]) = mp('0', '1');
      ans = min(ans, doit(A, B, i));
      tie(A[i], B[i]) = mp('0', '0');
    } else if (A[i] == '?' && B[i] != '?') {
      A[i] = max(char('0'), char(B[i] - 1));
      ans = min(ans, doit(A, B, i));
      A[i] = B[i];
    } else if (A[i] != '?' && B[i] == '?') {
      B[i] = min(char('9'), char(A[i] + 1));
      ans = min(ans, doit(A, B, i));
      B[i] = A[i];
    } else if (A[i] != '?' && B[i] != '?') {
      ans = min(ans, doit(A, B, i));
...
void solve() {
  ...
  T t1 = hoge(C, J);
  T t2 = hoge(J, C);
  swap(get<1>(t2), get<2>(t2));
  T ans = min(t1, t2);
  // cerr << get<0>(ans) << " " << get<1>(ans) << " " << get<2>(ans) << endl;
  assert(get<0>(ans) >= 0);
  printf("%s %s\n", get<1>(ans).c_str(), get<2>(ans).c_str());
}
