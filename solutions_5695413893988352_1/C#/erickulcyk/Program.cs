...
namespace GCJ2016
{
    class Program
    {
        ...
        private static BigInteger GetValue(int j, char[] a, char[] b, int comp)
        {
            if (j == a.Length)
            {
                return BigInteger.Abs(BigInteger.Subtract(BigInteger.Parse(new string(a)), BigInteger.Parse(new string(b))));
            }
            ...
            if (a[j] == '?')
            {
                if (b[j] == '?')
                {
                    if (comp == 0)
                    {
                        a[j] = '0';
                        b[j] = '0';
                        BigInteger val = GetValue(j + 1, (char[])a.Clone(), (char[])b.Clone(), comp);
                        a[j] = '1';
                        b[j] = '0';
                        BigInteger val1 = GetValue(j + 1, (char[])a.Clone(), (char[])b.Clone(), 1);
                        a[j] = '0';
                        b[j] = '1';
                        BigInteger val2 = GetValue(j + 1, (char[])a.Clone(), (char[])b.Clone(), -1);
                        comp = -1;
