...
for i in range(Cases):
 ...
 #               print dev, One, Two
                if Two[dev] not in ['0', '?']:
                    newOne = MIN(minO[:dev]) + str(int(Two[dev]) - 1) + MAX(One[dev+1:]);
                    ANS.append( [newOne, minT] );
                    ANS.append( [newOne, maxT] );
                if Two[dev] not in ['9', '?']:
                    newOne = MIN(minO[:dev]) + str(int(Two[dev]) + 1) + MIN(One[dev+1:]);
                    ANS.append( [newOne, minT] );
                    ANS.append( [newOne, maxT] );

            if (Two[dev] == '?'):
                if One[dev] not in ['0', '?']:
                    newTwo = MIN(minT[:dev]) + str(int(One[dev]) - 1) + MAX(Two[dev+1:]);
                    ANS.append( [minO, newTwo] );
                    ANS.append( [maxO, newTwo] );
                if One[dev] not in ['9', '?']:
                    newTwo = MIN(minT[:dev]) + str(int(One[dev]) + 1) + MIN(Two[dev+1:]);
                    ANS.append( [minO, newTwo] );
                    ANS.append( [maxO, newTwo] );
