...
//	static String BASEPATH = "/home/ec2-user/";
	...
	public BSmall(int cas, Scanner in)
	{
		this.cas = cas;
		this.in = in;
	...
	public static void main(String[] args) throws Exception
	{
		...
		CompletionService<BSmall> cs = new ExecutorCompletionService<BSmall>(es);
		...
		preprocess();
		for(int i = 0;i < n;i++){
			BSmall runner = new BSmall(i+1, in);
			runner.read();
		...
		for(int i = 0;i < n;i++){
			BSmall runner = cs.take().get(); // not ordered
			runner.out.flush();
			runner.out.close();
