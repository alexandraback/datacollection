...
namespace BaseApp
{
    class Program
    {
        ...
        {
            string input = @"d:\Work\CodeJam\02\B-small-attempt0.in";
            string output = @"d:\Work\CodeJam\02\B-small-attempt0.out";
        ...
        {
            var s = sin.ReadLine().Split(' ').Select(t => BigInteger.Parse(t)).ToList();
            ...
            moves += K * (A - K);
            // left
            for (BigInteger i = (B-1); i >= K; i--)
            {
                for (BigInteger j = (A-1); j >= K; j--)
                {
                    if ((i & j) < K) moves++;
                }
            }
