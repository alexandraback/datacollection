...
void foreachTest(uns testNum) {
    ...
    double curr;
    probs[0] = 1.0;
    for (uns aa=0U; aa < A; ++aa) {
        curr = getDouble();
        cerr << aa << " : " << curr << endl;
        probs[aa+1] =  probs[aa] * curr;
    }
    ...
    //u64 mtries = (1<<A);
    //u64 tries;
    //cerr << " mt " << mtries << " b " << best << endl;
    cerr << " mt " << " b " << best << endl;
    for (uns aa = 0; aa <= A; ++aa) {
        suc = aa + 1 + (B-(A-aa));
        fail = suc + B + 1;
        //tries = 1 << aa;
        //cerr << " succ " << suc << " fail " << fail << " nt " << tries << endl;
        //trial = (suc)*tries*probs[A-aa] + (fail)*(mtries - tries)*(1.0-probs[A-aa]);
        cerr << " succ " << suc << " fail " << fail << " nt " << endl;
        trial = (suc)*probs[A-aa] + (fail)*(1.0-probs[A-aa]);
        best = min(best, trial);
        cerr << " tr " << trial << " b " << best << endl;
    }
