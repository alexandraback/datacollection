...
public class B {
	...
	private static final String fileName = "results/con2015/con2015R1c/"+B.class.getSimpleName().toLowerCase()+islarge;
	...
	private void solve() {
		...
		double[]pTotal=new double[L+1]; // chance to be at pos x of L
		pTotal[0]=1D; // 100% chance of being at the start of the word in the beginning
		int[][]dfa = KMP(lword,MAX_CHARS);
		int start=1;
		while(start <= L && !lword.substring(start).equals(lword.substring(0, L-start))){
//			System.err.println(lword.substring(start)+".equals("+lword.substring(0, L-start)+")");
			start++;
		}
//		System.err.println(lword.substring(start)+".equals("+lword.substring(0, L-start)+")");
		int failure = L-start;
...
//		System.err.println("-- "+failure+" "+expected);
		for(int t=0;t<S;t++){
			...
			for(int key=0;key<MAX_CHARS;key++){ // pressing key at time t
				if(pKey[key]==0)continue; // ignore if non existant
				for(int l=0;l<L;l++){ // at place L in previous
					if(pTotal[l]==0)continue;
					pNext[dfa[key][l]] += pTotal[l]*pKey[key]; // pnext[x] may be reachable from multiple paths
				}
			}
			pTotal = pNext;
			res += pTotal[L];	// add chance to finish word
			// prepare next iteration
			pTotal[failure]+=pTotal[L]; // start over when finished
			pTotal[L] = 0; // just for the record
		}
	...
	private static int conv(char c){
	 ...
	 public static int[][] KMP(String pattern, int R) {
        ...
        for (int X = 0, j = 1; j < M; j++) {
            ...
            dfa[conv(pattern.charAt(j))][j] = j+1;      // Set match case. 
            X = dfa[conv(pattern.charAt(j))][X];        // Update restart state. 
        }
        return dfa;
    } 
