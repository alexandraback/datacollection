...
double fun(LL pos,LL m)
{
    ...
    if(visit[pos][m] == CS)return dp[pos][m];

    double ans = 0.0;
    for(LL i=0;i<k;i++)
    {
        LL val = 0;
        if(key[i] == str[m])val = m+1;
        LL ret = 0;
        if(val == l)
        {
            ret++;
        ...
        ans += ((double)ret + fun(pos+1,val));
    }

    visit[pos][m] = CS;
    return dp[pos][m] = ans/(double)k;
}
