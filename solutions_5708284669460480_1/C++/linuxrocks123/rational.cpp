...
// set the numerator
void Rational::SetNumerator(long numer) {
...
long Rational::Compare(const Rational& r) const
{
	const long& a=Numerator();
	const long& b=Denominator();
	const long& c=r.Numerator();
	const long& d=r.Denominator();
	if (a*d == b*c)
		return 0;
	else if (a*d < b*c)
		return -1;
...
{
     long integer_component = Numerator()/Denominator();
     long fractional_component = Numerator()%Denominator();
