          ...
          */
          ...
          for(int i=0; i<target.length(); i++)
               if(probabilities.count(target[i]))
                    word_probability*=((double)probabilities[target[i]])/keyboard.length();
               else
                    word_probability = 0;
          ...
          if(word_probability!=0)
               bananas_to_bring = ceil(possible_locs/overlap_pos);

          Rational expected_bananas_consumed = word_probability*possible_locs;
