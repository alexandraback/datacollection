...
//    cout<<pos<<" "<<numWord<<" "<<upto<<": "<<dp[pos][numWord][upto]<<endl;
    ...
    for(int i=0; i<26; i++){
        ...
        int j = upto;
        while(true){
            double prob = 1.0*cnt[i]/k;
            if( K == word[j] ){
                if(j==l-1){
                    dp[pos+1][numWord+1][F[l]] += (prob)*dp[pos][numWord][upto];
                }
                else{
                    dp[pos+1][numWord][j+1] += (prob)*dp[pos][numWord][upto];
                }
                break;
            }
            if(j==0){
                dp[pos+1][numWord][0] += (prob)*dp[pos][numWord][upto];
                break;
            }
            j = F[j];
        }
