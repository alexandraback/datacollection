...
void testcase_for_multithread(int t, int K, int L, int S, string letters, string target)
{
    g_results[t].second = testcase_itr(K, L, S, letters, target);
...
void testcase_multi(istream &in)
{
    ...
    for(int t=0;t<T;t++){
        ...
        threads.push_back(thread(testcase_for_multithread, t, K, L, S, letters, target ));
