...
int main(int argc, char **argv)
{
	...
	for(int it_t = 1; it_t <= in_t; it_t++)
	{
        ...
        auto solve = [&]() -> int{
            int t = 0;
            for(t = 0; t < myints.size(); t++) {
                if (myints[t] == in_n) break;
                if((t > 0 && f[myints[t]] == myints[t-1]+1) || (t < in_n && f[myints[t]] == myints[t+1]+1)
                   || (myints[t+1] == in_n && f[myints[t]] == myints[0]+1))
                {
                    continue;
                }
                else {
