...
//BFF[v[v.size()-1]] == v[v.size()-2] && BFF[v[v.size()-2]] == v[v.size()-1] || 
				if(v.size() >= 2 && BFF[v[v.size()-1]] == v[v.size()-2]) { // reciprocal
					for(int k = 0; k < N; k++){
						vector<int>::iterator itr;
						itr = find(v.begin(),v.end(),k);
						if(itr == v.end()){ // not yet appeared
							vector<int> v2 = v;
							v2.PB(k);
							q.PB(v2);
				...
				//	xdump(v);
					...
					vector<int> :: iterator itr = find(v.begin(),v.end(),BFF[v.back()]);
					if(itr == v.end()){
						vector<int> v2 = v;
						v2.PB(BFF[v.back()]);
						q.PB(v2);
