...
int main()
{
	...
	for(int cas=1;cas<=T;++cas)
	{
		...
		//cerr<<cas<<endl;
		scanf("%d",&n);
		if(cas==18)cerr <<n<<endl;
		for(int i=1;i<=n;++i)mp[i].clear();
		for(int i=1;i<=n;++i)
		{
			scanf("%d",&go[i]);
			if(cas==18)cerr <<go[i]<<endl;
			++ind[go[i]];
			mp[go[i]].push_back(i);
		}
		...
		for(int i=1;i<=n;++i)
			if(!vis[i])
			{
				...
				if(tmp==2)
				{
					...
					for(deque<int>::iterator it=q.begin();it!=q.end();++it)dist+=dis[*it];
		...
		if(cas==18)cerr <<res<<" "<<num*2+rd<<endl;
		printf("Case #%d: %d\n",cas,max(res,num*2+rd));
	}
	return 0;
}
