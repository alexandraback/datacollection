...
public class Google
{
    public static boolean executeOnStringTab(String[][] input, PrintWriter output)
    {
        ...
        for (int i = 0; i < input[1].length; i ++)
            BFFs[i + 1] = Integer.parseInt(input[1][i]);
        boolean[] sited = new boolean[1001];
        ...
        for (int i = 0; i < input[1].length; i ++)
        {
            int numberSited = order(i + 1, BFFs, size, sited);
            if (numberSited > result)
                result = numberSited;
        }
    ...
    public static int order(int originalOne, int[] BFFs, int size, boolean[] oldSited)
    {
        boolean[] sited = new boolean[1001];
        for (int i = 0; i < 1001; i ++)
            sited[i] = oldSited[i];
        ...
        int number = 1;
        sited[originalOne] = true;
        int currentBFF = BFFs[originalOne];
        int previousBFF = originalOne;

        while (!sited[currentBFF])
        {
            number++;
            sited[currentBFF] = true;
            previousBFF = currentBFF;
            currentBFF = BFFs[currentBFF];
        }

        if (currentBFF == originalOne)
        {
            int secondNumber = 0;
            for (int i = 1; i <= size; i ++)
            {
                if (!sited[i])
                {
                    int thirdOrder = order(i, BFFs, size, sited);
                    if (thirdOrder > secondNumber)
                        secondNumber = thirdOrder;
                }
            }
            return number + secondNumber;
        }

        if (BFFs[currentBFF] == previousBFF)
        {
            int secondNumber = 0;
            for (int i = 1; i <= size; i ++)
            {
                if (!sited[i])
                {
                    int secondOrderResult = secondOrder(previousBFF, i, BFFs, sited, size);
                    if (secondOrderResult > secondNumber)
                        secondNumber = secondOrderResult;
                }
            }
    ...
    public static int secondOrder(int finalOne, int originalOne, int[] BFFs, boolean[] oldSited, int size)
    {
        boolean[] sited = new boolean[1001];
        for (int i = 0; i < 1001; i ++)
            sited[i] = oldSited[i];
        ...
        if (currentBFF == finalOne)
        {
            for (int i = 1; i <= size; i ++)
            {
                ...
                {
                    int thirdOrder = order(i, BFFs, size, sited);
                    if (thirdOrder > thirdResult)
                        thirdResult = thirdOrder;
                }
            }
            return number + thirdResult;
        }

        return 0;
    }

    public static double calculateTimeToFinish(double startPosition, double time)
    {
        return (360 - startPosition) * time / 360;
    }
