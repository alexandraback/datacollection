...
//        String questionName = "B-small-attempt0";
        ...
        for(int x = 0; x < count; x++) {
            ...
            for(int a = 0; a < notFound.size(); a++) {
                answer += "" + notFound.get(a);
                if( a < notFound.size() - 1) {
                    answer += " ";
                }
            }
    ...
    static int findMax(int count, int firstId, Child cur, Child prev, Set<Child> left, Map<Integer, Child> map) {
        Child target = map.get(cur.bff);
        int max = 0;
        if(cur.bff == firstId ) {
            max = count;
        }

        if(prev != null && cur.bff == prev.id) {
            max = Math.max(max, count);
            for(Child c : map.values()) {
                if(left.contains(c)) {
                    if(firstId == c.bff || left.contains(map.get(c.bff)) || cur.id == c.bff) {
                        left.remove(c);
                        max = Math.max(max, findMax(count + 1, firstId, c, cur, left, map));
                        left.add(c);
                    }
                }
            }
        ...
        if(target != null) {
            if (left.contains(target)) {
                left.remove(target);
                max = Math.max(max, findMax(count + 1, firstId, target, cur, left, map));
                left.add(target);
            }
        }
        return max;
...
//        String questionName = "C-small-attempt1";
//        String questionName = "C-small-attempt2";
//        String questionName = "C-small-attempt3";
//        String questionName = "C-small-attempt4";
        String questionName = "C-small-attempt5";
//        String questionName = "C-large";
        ...
        for(int x = 0; x < count; x++) {
            ...
            int N = readNumber(r);
            int [] F = readNumbers(r);
            Set<Child> left = new HashSet<Child>();
            HashMap<Integer, Child> map = new HashMap<Integer, Child>();
            for(int a = 0; a < N; a++) {
                Child c = new Child(a + 1, F[a]);
                left.add(c);
                map.put(c.id, c);
            }
            int max = 0;
            for(Child c : map.values()) {
                left.remove(c);
                max = Math.max(max, findMax(1, c.id, c, null, left, map));
                left.add(c);
            }
            if(max == 1) {
                max = 0;
