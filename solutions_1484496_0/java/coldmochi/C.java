...
//    20 120 266 858 1243 1657 1771 2328 2490 2665 2894 3117 4210 4454 4943 5690 6170 7048 7125 9512 9600
    private void solve() {
        ...
        List<Double> original = new LinkedList<Double>();
        // Contains the next set of sums mapped to the index to which they should start next in the original list
        Map<Double, Integer> sums = new LinkedHashMap<Double, Integer>();
        // Contains the sum mapped to the indices in the original list that add to that sum
        Map<Double, List<Integer>> sumToIndices = new HashMap<Double, List<Integer>>();
        
        for (int i = 0; i < n; i++) {
        	original.add(sc.nextDouble());
        }
        ...
        while(!done) {
        	...
        	Map<Double, Integer> newSums = new LinkedHashMap<Double, Integer>();
        	for (Map.Entry<Double, Integer> e : sums.entrySet()) {
        		double currentSum = e.getKey();
        		int nextIndex = e.getValue();
        		for (int j = nextIndex; j < original.size(); j++) {
        			double sum = currentSum + original.get(j);
        			newSums.put(sum, j+1);
        			...
        			if (sumToIndices.containsKey(sum)) {
        				...
        				for (int k : newIndices) {
        					out.print(original.get(k).intValue() + " ");
        				}
        				out.println();
