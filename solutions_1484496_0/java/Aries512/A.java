...
public class A {
	...
	public void solve() throws Exception {
		...
		nextCase: for(int testCase=1; testCase<=testCasesCnt; testCase++) {
			...
			for (int m=1; m<1<<n; ++m) {
				int sum = 0;
				for (int i=0; i<20; ++i) if (hasBit(m, i)) sum+=s[i];
				if (map.containsKey(sum)) {
					int mask = map.get(sum);
					String str = "";
					for (int j=0; j<20; ++j) if (hasBit(mask,j)) str+=s[j]+" ";
					println(str.trim());
					str = "";
					for (int j=0; j<20; ++j) if (hasBit(m,j)) str+=s[j]+" ";
					println(str.trim());
					continue nextCase;
				}
				map.put(sum, m);
	...
	public static void main(String[] args) throws Exception {
		...
		if (!fromFile) {
			...
			inputReader = new InputReader(new FileInputStream(new File(filePath)), 1<<16);
		}
		if (!intoFile) {
			outputWriter = new BufferedWriter(new OutputStreamWriter(System.out));
		}
		else {
			File f = new File(System.getProperty("user.dir")+"\\src\\"+A.class.getPackage().getName()+"\\"+A.class.getSimpleName()+".out" );
