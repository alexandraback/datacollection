...
struct SubSet{
    int sum;
    VI list;
    SubSet(int s, VI l):sum(s), list(l){}
...
void dfs(int k = 0, int s = 0, vector<int> ll = _){
...
bool found(){
    REP(i, SZ(L)){
        FOR(j, i+1, SZ(L)){
            if (L[j].sum != L[i].sum) break;

            bool flag = true;

            REP(ii, SZ(L[i].list)) REP(jj, SZ(L[j].list)){
                if (L[i].list[ii] == L[j].list[jj]){
                    flag = false;
                }
            }

            if (flag){
                REP(ii, SZ(L[i].list)) cout << L[i].list[ii] << " "; cout << endl;
                REP(jj, SZ(L[j].list)) cout << L[j].list[jj] << " "; cout << endl;
                return true;
            }
