...
struct RealCase : public Case
{
    ...
    void dfs(int n, int mask, int sum);
    string subset_string(int mask);
...
static map<int, vector<int> > subsets;

void RealCase::dfs(int n, int mask, int sum)
{
    if (n == N)
    {
        subsets[sum].push_back(mask);
        return;
    }
    dfs(n + 1, mask, sum);
    dfs(n + 1, mask|(1<<n), sum + S[n]);
}

string RealCase::subset_string(int mask)
{
    ostringstream oss;
    bool first = true;
    REP(n, N) if (mask & (1<<n)) {
...
void RealCase::solve(Result &result)
{
    dfs(0, 0, 0);
    FORIT(it, subsets) {
        const VI &masks = it->second;
        REP(j, SZ(masks)) REP(i, j) if ((masks[i] & masks[j]) == 0) {
            result.add(subset_string(masks[i]));
            result.add(subset_string(masks[j]));
            return;
        }
    }
