...
bool tryOpen(int Chest[202], int &K, int Key[]){
	if(Key[Chest[0]]>0){
		Key[Chest[0]]--;
		K--;
		for(int i=1;i<201;i++){
...
void tryClose(int Chest[202], int &K, int Key[]){
	Key[Chest[0]]++;
	K++;
	for(int i=1;i<201;i++){
		Key[i]-=Chest[1+i];
...
void permutate(int K, int Key[], int Chest[][202], int O){
	for(int i=0; i<N; i++){
		if (K==0)
			return;
		if(isNotOpened(i,O)){
...
int main() {
	...
	fora(T,qq){
		flag=false;
		int buf,K,Key[201],K1[201],K2[201];
		fora(i,201){
			Key[i]=0;
			K1[i]=0;
			K2[i]=0;
