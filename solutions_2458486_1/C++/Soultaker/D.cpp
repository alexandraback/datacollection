/* This is my Facebook Hacker Cup C++ solution template.
 *
 ...
 * platform. I use GCC 4.6.3 on Linux with an x86-64 processor.
 *
 ...
 *
 * Maks Verver  [Soultaker]  <maksverver@geocities.com> 
 * July 2008, September 2009, January 2011, February 2013
 */

/* FBHC template part 01 -- Common headers */
#include <algorithm>
#include <cassert>
#include <cmath>
#include <cstdio>
...
/* FBHC template part 02 -- Common macro definitions */
...
/* FBHC template part 03 -- Common type definitions */
...
/* FBHC template part 04 -- Utility function definitions */
...
/* FBHC template part 05 -- Execution framework */
...
/* FBHC template part 06 -- Solution implementation */
...
struct Case : public AbstractCase
{
    ...
    bool dfs(int i, int goal);
    bool possible(vector<int> start);

    vector<Chest> C;  /* list of all chests */
};
...
Case::Case()
{
    ...
    C = vector<Chest>(N);
    C[0].type = 0;
    C[0].keys = vector<Key>(K);
    REP(k, K) cin >> C[0].keys[k].type;
    FOR(n, 1, N) {
        int type = 0, L = 0;
        cin >> type >> L;
        C[n].type = type;
        C[n].keys = vector<Key>(L);
        REP(l, L) cin >> C[n].keys[l].type;
    }
}

static vector<char> visited;
static vector<vector<int> > adj;  /* adjacency list; i->j if i has a key of j's type */

bool Case::dfs(int i, int goal)  // this takes O(E) time
{
    visited[i] = true;
    FORIT(it, adj[i]) {
        int j = *it;
        if (j == goal || (!visited[j] && dfs(j, goal))) {
            assert(!C[i].open && C[j].open);
            C[i].open = true;
            bool key_found = false;
            FORIT(it, C[i].keys) if (it->type == C[j].type) {
                assert(!it->used);
                it->used = j;
                key_found = true;
                break;
...
bool Case::possible(vector<int> start)  // takes O(E*E) time
{
    ...
    REP(n, N) {
        C[n].open = (n == 0);
        FORIT(it, C[n].keys) {
            if (it->type < 0) it->type = -it->type;
            it->used = 0;
        }
    }

    // Open initial sequence of boxes:
    FORIT(it, start) {
        int j = *it;
        if (C[j].open) return false;
        REP(i, N) if (C[i].open) {
            FORIT(it, C[i].keys) if (it->type == C[j].type && it->used == 0) {
                C[j].open = true;
                it->used = j;
                it->type = -it->type;  // HACK: lock this key!
                goto opened_it;
            }
        }
        return false;
...
repeat:
    int opened = 0;
    REP(i, N) REP(j, N) if (C[i].open && !C[j].open) {
        FORIT(it, C[i].keys) if (it->type == C[j].type && it->used == 0) {
            //cerr << "key of chest " << i << " opens chest " << j << endl;
            C[j].open = true;
            it->used = j;
            ++opened;
            break;
    ...
    if (opened > 0) goto repeat;
    REP(i, N) REP(j, N) if (C[i].open && !C[j].open) {
        FORIT(it, C[i].keys) if (it->type == C[j].type && it->used > 0) {
            assert(C[it->used].open);
            visited.resize(N);
            REP(n, N) visited[n] = C[n].open;
            if (dfs(j, it->used)) {
                //cerr << "found improvement from " << i << " to " << j << endl;
                it->used = j;
                goto repeat;
            }
        }
    }
    REP(i, N) if (!C[i].open) return false;
    return true;
}
...
void Case::solve(Result &result)
{
    ...
    // Find lexicographically minimum sequence of boxes:
    ostringstream oss;
    while ((int)start.size() < N - 1)
    {
        if (!start.empty()) oss << ' ';
        start.push_back(1);
