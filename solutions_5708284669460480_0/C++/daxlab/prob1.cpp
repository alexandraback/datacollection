...
pii solve(int i, string str)
{
    ...
    f(j,0,k)
    {
        pii t = solve(i+1, str + key[j]);
        ret.ss = max(ret.ss,t.ss);
        ret.ff += t.ff;
    }
    return ret;
}
...
int main()
{
    ...
    f(i,0,t)
    {
        ...
        cin >> key >> tar;
        pii p = solve(0,"");
        double ans = (double) p.ss - (double) p.ff/ (double)power(k,s);
        cout << "Case #" << i+1 <<": "<<setprecision(8) << ans << endl;
    }
    return 0;
}
