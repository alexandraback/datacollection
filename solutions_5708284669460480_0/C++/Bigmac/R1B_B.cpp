...
int main(int argc, char **argv)
{
	...
	for(int it_t = 1; it_t <= in_t; it_t++)
	{
		//init input
		//finish init input
		long long in_k = readHugeInt();
		long long in_l = readHugeInt();
        long long in_s = readHugeInt();
        long long k_t[256] = {0};
        long long l_t[256] = {0};
        long long t_k = 0;
        long double k_tf[256] = {0.0f};
        for(long long it_i = 0; it_i < in_k;it_i++)
        {
            char c = readChar();
            k_t[c - 'A']++;
            t_k++;
        }
        for(long long it_i = 0; it_i < 100; it_i++)
        {
            k_tf[it_i] = k_t[it_i] / (long double)t_k;
        }
        for(long long it_i = 0; it_i < in_l;it_i++)
        {
            char c = readChar();
            l_t[it_i] = c - 'A';
        }
        ...
        long long max_pos = 0;
        long long b_cnt = 0;
        auto placable = [&](long long pos)->bool{
            if(cur_pos + in_l > in_s)
            {
                return false;
            }
            for(long long it_i = 0; it_i < in_l; it_i++)
            ...
            for(long long it_i = 0; it_i < in_l; it_i++)
            {
                if(cur_pos + it_i<max_pos)
                {
                    if(b_c[cur_pos + it_i] != l_t[it_i])
                    {
                        ret = false;
                        break;
                    }
            ...
            if(ret)
            {
                for(long long it_i = 0; it_i < in_l; it_i++)
                {
                    b_c[cur_pos + it_i] = l_t[it_i];
                }
                max_pos = cur_pos + in_l;
            }
            return ret;
        };
        while(cur_pos < in_s)
