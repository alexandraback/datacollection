...
void solveB(){
	...
	while(t<=T){
		...
		if(can && (S>=L)){
			...
			double expEat = prob*(S-L+1);
			int match = L;
			for(int i=1;i<L;i++){
				string suffix = target.substr(i,L - i);
				string prefix = target.substr(0,L - i);
				if(suffix == prefix){
					match = i;
					break;
				}
			}
			int maxB = 1 + (S-L)/match;
			double dmb = maxB;
			ans = dmb - expEat;


		}
		cout<<std::fixed<<std::setprecision(9)<<"Case #"<<t<<": "<<ans<<endl;
		t++;
	}
}
