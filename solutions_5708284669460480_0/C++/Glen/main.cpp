...
int n_overlap;
string keys, target;
int n_required_bananas;

int count_bananas(string s) {
	int target_len = target.length();
	int len = s.length();
	int count = 0;
	for (int i = 0; len - i >= target_len; ++i) {
		string st(s, i, target_len);
		if (st == target) {
			count++;
		}
	}
...
double expectation(string s) {
	if (s.length() == S) {
		return n_required_bananas - count_bananas(s);
	}

	double e = 0;
	for (int i = 0; i < K; ++i) {
		e += expectation(s + keys[i]) * 1.0 / K;
	}
	return e;
}
...
int main(int argc, char** argv) {
	...
	REP (_t, T) {
		...
		int target_len = target.length();
		n_overlap = 0;
		while (n_overlap < target_len - 1) {
			int t = n_overlap + 1;
			string s1(target, target_len - t, t);
			string s2(target, 0, t);
			dump(s1);
			dump(s2);
			if (s1 == s2) {
				n_overlap++;
		...
		string best_result;
		while (best_result.length() < S) {
			dump(best_result);
			best_result += target;
			best_result.erase(best_result.length() - n_overlap, n_overlap);
		}
		best_result.erase(S, best_result.length() - S);
		dump(best_result);
		n_required_bananas = count_bananas(best_result);

		// when it's impossible to create target
		for (int i = 0; i < target.length(); ++i) {
			if (keys.find(target[i]) == string::npos) {
				n_required_bananas = 0;
			}
		}

		dump(n_required_bananas);

		double ans = expectation("");
		printf("Case #%d: %.12lf\n", _t + 1, ans);
	}
