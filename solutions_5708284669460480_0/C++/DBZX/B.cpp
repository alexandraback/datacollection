...
II caseCount = 0;
II foundCount = 0;
II Count(const string typed)
{
	II res = 0;
	REP(i,typed.size()-word.size()+1)
	{
		if(memcmp(word.c_str(),typed.c_str()+i,word.size()) == 0)
			res++;
	}
	return res;

}
void Test(vector<int> &indexes)
{
	string typed;
	FORA(i,indexes)
	{
		typed.push_back(key[indexes[i]]);
//		cout << typed ;
	}
	II found = Count(typed);
	checkmax(maxVal ,found);
	foundCount  += Count(typed);
	++caseCount;
//	cout << endl;
}

void AllNum(int pos,vector<int> &indexes)
{
	if(pos == indexes.size())
	{
		Test(indexes);
	...
	{
		indexes[pos] = i;
		AllNum(pos+1,indexes);
	}

}

void AllNum(int total)
{
	vector<int> indexes(total,0);
	AllNum(0,indexes);
}
...
void Solve()
{
	...
	foundCount = 0;
	AllNum(S);
	double res = (double)((caseCount*maxVal)-foundCount)/(double)caseCount;
	out << res;
	//	out << n;
}
