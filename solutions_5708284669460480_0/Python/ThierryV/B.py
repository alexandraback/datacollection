...
for tc in xrange(1, int(raw_input())+1):
	...
	#K NB KEYBOARDS
	#L LENGHT OF THE TARGET WORDS
	#S LENGTH OF THE WORDS TYPED BY THE MONKEY

	keyboards = raw_input().strip()
	target = raw_input().strip()

	#print 'keyboards, target', keyboards, target

	###CHECK ALL LETTERS OF TARGETS IN KEYBOARDS
	allLetterInKeyboards = True
	for t in target:
		if t not in keyboards:
			print "0.0"
			allLetterInKeyboards = False
			break

	if allLetterInKeyboards == True:
		possibleTyped = []
		firstInsert = True
		for s in range(S):
			if firstInsert == True:
				for k in keyboards:
					firstInsert =  False
					possibleTyped.append(k)
				#print 'possibleTyped', possibleTyped

			else:
				possibleTypedC = []

				for p in possibleTyped:
					for k1 in keyboards:
						possibleTypedC.append(k1+p)
				possibleTyped = possibleTypedC
		...
		counterManyTimes = 0
		maxManyTime = 0
		for p1 in possibleTyped:
			#print len(p1)-L+1
			maxManyTime2 = 0
			for pos in range(len(p1)-L+1):
				if p1[pos:pos+L ] == target:
					#print "p1, target OK", p1
					counterManyTimes +=1
					maxManyTime2 += 1
			maxManyTime = max(maxManyTime2, maxManyTime)


		#print 'counterManyTimes', counterManyTimes
		#print 'maxManyTime', maxManyTime
		result = (maxManyTime* len(possibleTyped) -counterManyTimes )/float(len(possibleTyped))
