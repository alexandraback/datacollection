...
public class Solution {
    ...
    private void solve() throws IOException {
        int T = io.nI();
        for (int t = 1; t <= T; t++) {
            int k = io.nI(), l = io.nI(), s = io.nI();
            char[] keyb = io.nS().toCharArray();
            char[] targ = io.nS().toCharArray();
...
//        boolean[] mask = new boolean[100 + 1];
//        final int M = 1;
//
//        for (int i1 = 0; i1 <= M; i1++)
//            for (int i5 = 0; i5 <= M; i5++)
//                for (int i10 = 0; i10 <= M; i10++)
//                    for (int i25 = 0; i25 <= M; i25++)
//                        for (int i50 = 0; i50 <= M; i50++)
//                            for (int i100 = 0; i100 <= M; i100++)
//
//                                for (int i2 = 0; i2 <= M; i2++)
//                                    for (int i4 = 0; i4 <= M; i4++)
//                                        for (int i23 = 0; i23 <= M; i23++)
//
//                                        {
//                                            int i = 1 * i1 + 5 * i5 + 10 * i10 + 25 * i25 + 50 * i50 + 100 * i100;
//
//                                            i += 2 * i2;
//                                            i += 4 * i4;
//                                            i += 23 * i23;
...
//                io.wln(i);
    ...
    private double solve(char[] keyb, char[] targ, int textLen) {
        int s = 0;
        int occ = 0;
        int max = 0;

        for (char[] text : texts(keyb, textLen)) {
            int localMax = 0;
            f:
            for (int start = 0; start <= text.length - targ.length; start++) {
                for (int off = 0; off < targ.length; off++)
                    if (text[start + off] != targ[off])
                        continue f;
                occ++;
                localMax++;
            }
        ...
        double exp = 1.0d * occ / s;


        return max - exp;
    }

    private List<char[]> texts(char[] alph, int s) {
        List<char[]> res = new ArrayList<>();
        texts(res, alph, new char[s], 0);
        return res;
    }

    private void texts(List<char[]> list, char[] alph, char[] currword, int pos) {
        if (pos == currword.length) {
            list.add(currword.clone());
            return;
        }
        for (char a : alph) {
            currword[pos] = a;
            texts(list, alph, currword, 1 + pos);
        }
    }
