...
public class Main implements Runnable{
	...
	HashSet vovels; 
	ArrayList<Long> FoundPosition(char[] work,int n){
	...
	void Solve(){
		...
		int n = NextInt();
		ArrayList<Long> position = FoundPosition(work,n);
		long pred = 0;
		long length = work.length;
		long result = 0;
		for(long item : position){
			if (pred == item){
				result += length - n - item+1;
				pred++;
			}else {
				long countLeft = item - pred+1;
				long countRight = length - item - n+1;
