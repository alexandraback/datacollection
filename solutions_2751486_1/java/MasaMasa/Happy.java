...
public class Happy {
    ...
    public void haha() {
        ...
        try{
            ...
            //in = new BufferedReader(new FileReader("A-small-attempt0.in")); out = new BufferedWriter(new FileWriter("A-small-attempt0_test.out"));
            ...
            for(int i=0;i<T;i++){
                ...
                if (checkV(name.charAt(L-1))==false) ss[L-1] = 1;
                for (int j=L-2; j>=0; j--) {
                    if (checkV(name.charAt(j))==false) ss[j] = ss[j+1]+1;
                }

                Arrays.fill(next, -1);
                for (int j=L-1, last=-1; j>=0; j--) {
                    if (ss[j]>=n) last = j;
                    next[j] = last;
                }
