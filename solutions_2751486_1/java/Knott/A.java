...
//	String filePrefix = "src/codejam1c/A";
	String filePrefix = "src/codejam1c/A-large";
...
//			int count = input.length()-1-c;
			long count = input.length()-c-n+1;
//			System.out.println("c="+c+" first count = "+count);
			result += count;
			
			for (int j=0; j<c-startIndex; j++)
				result += (input.length()-c-n+1);//*(c-startIndex);
