...
int main()
{
    ...
    int test;
    cin >> test;
    for(int t_cnt = 1; t_cnt <= test; t_cnt++)
    {
        cout << "Case #" << t_cnt << ": ";
        ...
        while(l < len)
        {
            while(l < len && !is_c(s[l])) l++;
            r = l;

            while(r < len && is_c(s[r])) ++r;
        ...
        forn(i, cur.size())
        {
            ...
            if (cur[i].y - cur[i].x + 1 < n) continue;

            int d = cur[i].y - cur[i].x + 1 - n;

            ans += (cur[i].x - pos + 1) * (len - cur[i].y + d);
            For(j, 1, d)
                ans += len - cur[i].y + d - j;
            pos = cur[i].x + d + 1;
        }
