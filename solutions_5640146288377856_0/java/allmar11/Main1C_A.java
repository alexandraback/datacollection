...
public abstract class Main1C_A {
	...
	private static final String IN_NAME = "A-small-attempt0";
	...
	public static void main(String[] args) throws Exception {
		...
		for (int caseIdx = 1; caseIdx <= numCases; caseIdx++) {
			...
			int result = solve(height, width, boatWidth);
	...
	private static int solve(int height, int width, int boatWidth) {
		int result = 0;
		
		int toRemoveRow = width / boatWidth;
		
		// Narrow down possibilities to a single row.
		result += (height - 1) * toRemoveRow;
