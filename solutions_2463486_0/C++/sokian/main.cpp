...
void gen(int len, string s = "", int d = 0, int sum = 0)
{
    if (d == len)
    {
        pref.pb(s);
    ...
    forn(i, 3)
    {
        ...
        if (sum + i * i < 5)
        {
            gen(len, s + char(i + '0'), d+1, sum + i*i);
        }
    }
}
...
inline string sq(const string &s)
{
    VI a;
...
int main()
{
    ...
    for(int t = 1; t <= test; t++)
    {
        string n,m;
        cin >> n >> m;
        int ret = upper_bound(all(ans), m, cmp) - lower_bound(all(ans), n, cmp);
        cout << "Case #" << t << ": ";
        cout << ret << endl;
    }
    return 0;
