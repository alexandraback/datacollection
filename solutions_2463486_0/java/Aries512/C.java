...
public class C {
	...
	static String filesPath = "D:/AAA/google_jam/";
	static boolean fromFile = true;
	static String inFile = C.class.getSimpleName()+"-small-attempt0.in";
	static boolean toFile = true;
	static String outFile = C.class.getSimpleName()+".out";
	
	public void solve() throws Exception {
		
		ArrayList<Long> list = new ArrayList<Long>();
		for (long i=0; i<10000010; ++i) if (isPal(i) && isPal(i*i)) {
			list.add(i*i);
		}
		...
		nextCase: for(int testCase=1; testCase<=testCasesCnt; testCase++) {
			...
			int low = Collections.binarySearch(list, a);
			if (low<0) low = -low-1;
			int hi = Collections.binarySearch(list, b);
			if (hi<0) hi = -hi-1;
			else hi++;
			println(hi-low);
