...
#define dvec(v) if (opt_debug) { copy( ALL(v), ostream_iterator<P>(cerr, " ") ); cerr << endl; }
...
void print_field(string field) {
	REP (i, R) {
		REP (j, C) {
			dprt("%c", field[AT(P(i, j))]);
		}
		dprt("\n");
	}
}

string solve(string field, int revealed, vector<P> candidates) {
	if (EXIST(done, field)) {
		return "";
	}
	done.insert(field);
	...
	REP (i, candidates.size()) {
		P pos = candidates[i];
		field[AT(pos)] = '0';
		int nrevealed = revealed;
		string nfield = field;
		vector<P> ncandidates = candidates;
		ncandidates.erase(ncandidates.begin() + i);
		REP (d, 8) {
			P npos = pos + dydx8[d];
			if (!in_field(npos)) {
				continue;
			}
			switch (field[AT(npos)]) {
				case '0':
					break;
				case '1':
					break;
        ...
        string ans = solve(nfield, nrevealed, ncandidates);
		if (ans != "") {
			return ans;
		}
		field[AT(pos)] = '1';
...
int main(int argc, char** argv) {
	...
	REP (_t, T) {
		...
		cin >> R >> C >> M;
		g_height = R; g_width = C;
		string field(R * C, '*');
		vector<P> candidates(1, P(0, 0));
		string answer = solve(field, 1, candidates);
