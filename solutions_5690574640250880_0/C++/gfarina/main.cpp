...
typedef pair<ii, int> stato;
//map<stato, vector<matrice> > mappa;
map<stato, matrice> mappa;

vector<int> DR{-1,0,1};
vector<int> DC{-1,0,1};

int vaga(matrice& M, int r, int c) {
    bool visitato[MAXR][MAXC];
    int ans = 0;
    ...
    while (!celle.empty()) {
        r = celle.front().first, c = celle.front().second;
        celle.pop();
        
        if (visitato[r][c]) continue;
        ++ans;
        visitato[r][c] = true;
        
        if (M.campo[r][c] != 0) continue;
        
        for (int dr: DR) {
            for (int dc: DC) {
                if (r+dr < 0 || r+dr >= M.R || c+dc < 0 || c+dc >= M.C) continue;
                
                if (M.campo[r+dr][c+dc] != '*')
                    celle.push(ii(r+dr, c+dc));
            }
        }
    }
...
void init() {
    for (int R: range(1,MAXR+1)) {
        for (int C: range(1,MAXC+1)) {
            matrice M;
            M.R = R, M.C = C;
            
            for (int i: range(1<<(R*C))) {
                int mine = 0;
                memset(M.campo, 0, sizeof M.campo);
                ...
                if (valida) {
                    mappa[make_pair(ii(R,C), mine)] = M; //.push_back(M);
                }
            }
        }
    }
...
void testcase(int testnum) {
    ...
    cin >> R >> C >> M;
    cout << "Case #" << testnum << ":\n";
    if (mappa.count(make_pair(ii(R,C), M)))
        mappa[make_pair(ii(R,C), M)].stampa();
    else
        cout << "Impossible\n";
}
