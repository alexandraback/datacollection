...
void solve(){
    ...
    if (r == 1 || c == 1){
        if (r * c - 2 >= n){
            if (r == 1){
                printf("c.");
                for (int i = 0 ; i < c - n - 2 ; ++i) putchar('.');
                for (int i = 0 ; i < n ; ++i) putchar('*');
                puts("");
            }
            else{
                printf("c\n.\n");
                for (int i = 0 ; i < r - n - 2 ; ++i) puts(".");
    ...
    n = r * c - n;
    for (int i = 2 ; i <= c && i * 2 <= n; ++i)
        dp[1][i][2 * i] = i;
    for (int i = 1 ; i < r - 1; ++i){
        for (int j = c; j > 2 ; --j)
            for (int has =  0 ; has <= n ; ++has) if (dp[i][j][has])
                dp[i][j - 1][has] = dp[i][j][has];
        for (int j = 2 ; j <= c; ++j)
            for (int has = 0 ; j + has <= n ; ++has) if (dp[i][j][has])
                dp[i + 1][j][has + j] = j;
    }
    for (int i = 0 ; i < r ; ++i)
        for (int j = 1 ; j <= c ; ++j)
