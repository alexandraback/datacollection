...
void print(char g[101][101],int n, int m, bool rev){
    if (!rev){
        for (int i=0; i<n; i++){
            for (int j=0; j<m; j++){
                cout<<g[i][j];
        ...
        for (int ay=1; ay<m; ay++){
            for (int bx=1; bx<n; bx++){
                for (int by=1; by<m; by++){
                    int area = (ax+1)*(ay+1) + (bx+1)*(by+1);
                    area -= (min(ax,bx)+1)*(min(ay,by)+1);
                    //printf("%d %d %d %d\n", ax, ay, bx, by);
                    if (area==h){
                        FLC(g,'*');
                        for (int i=0; i<=ax; i++){
