...
class App {
	...
	static bool Build(int col, int rem, int r, int c) {
		if (col == c) return rem == 0;
		for (heights[col] = 0 ; heights[col] <= heights[col - 1] && heights[col] <= rem ; heights[col]++) {
			if (col == 1 && heights[0] == 1 && heights[col] > 0) return false;
			if (heights[col] < heights[0] && col == 1 && heights[0] != 1) continue;
			if (heights[col] == 1) continue;
			if (Build(col + 1, rem - heights[col], r, c)) return true;
		}
		return false;
	}
