...
public class Google
{
    public static int blou = 0;
    public static boolean executeOnStringTab(String[][] input, PrintWriter output)
    {
        blou++;
        trace(blou);
        int x = Integer.parseInt(input[0][0]);
        int y = Integer.parseInt(input[0][1]);
        int mines = Integer.parseInt(input[0][2]);

        int holes = x * y - mines;
        if (holes != 1 && x > 1 && y > 1 && ((holes == 2 || holes == 3 || holes == 5 || holes == 7) ||
                ((x == 2 || y == 2) && holes % 2 == 1)))
            return outputCase(output, "\nImpossible");

        String solution[][] = new String [x][y];
        for (int i = 0; i < x; i ++)
        {
            for (int j = 0; j < y; j ++)
                solution[i][j] = "*";
        ...
        holes--;
        if (holes == 0)
            return outputTab(output, solution);
        if (x > 1)
        {
            solution[1][0] = ".";
            holes --;
