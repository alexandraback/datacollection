...
def rec_serach_good_brd(brd, mines, trg_mines, steps, tabu_set, move_order):
    ...
    bad_pos = can_unfold(brd)

    if mines == trg_mines and not bad_pos:
...
def topfill(rs, cs, m):
    ...
    if rs >= 4 and cs*rs-m in [2,3]:
        raise NoSolution()

    if rs >= 5 and cs*rs-m in [5, 7]:
        raise NoSolution()
...
solvec_file('/tmp/C-small-attempt4.in')
