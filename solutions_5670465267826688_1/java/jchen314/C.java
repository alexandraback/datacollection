...
public class C {
    ...
    private static boolean isPossible(int L, long X, int[] arr) {
        int[] left = new int[L];
        left[0] = arr[0];
        for (int t = 1; t < L; t++) {
        ...
        long lastJIndex = getLastKIndex(L, X, right);
        if (lastJIndex == -1) {
            return false;
        }
    ...
    private static long getLastKIndex(int L, long X, int[] right) {
        int rightProd = 0;
        for (int i = 0; i < 4; i++) {
            for (int j = 0; j < L; j++) {
