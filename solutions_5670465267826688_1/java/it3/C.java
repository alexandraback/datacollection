...
public class C {
    ...
    static int IJ=mul(I,J), IJK=mul(IJ,K);
...
//            System.out.println("----- skip case "+caseNumber+" -----");
        ...
        String ret = solve(L, X, a);
        out.printf("Case #%d: %s\n", caseNumber, ret);
    }

    String solve(int L, long X, int[] a) {
//        if (true) return bf(a, X);
                
        if (X*L<3) {
            return NO;
        }

//        int p=stream(a).reduce(1, (x,y) -> mul(x,y)); // XXX reduce is for parallel, only work for commutative op
        int p=1;
        for (int v : a) {
            p=mul(p,v);
        }
        ...
        String ret = scanOne(X, a, p);
//        String ret = scanFull(X, a);
        
//        if (X*a.length<=10000 && !bf(a, X).equals(ret)) {
...
//        }
     ...
     * otherwise p^2 = -1
     * 
     * only possible to get -v from v  using p^k 
     */
    String scanOne(long X, int[] a, int p) {
        int n = a.length;
        ...
        for (int i = 0; i < a.length; i++) {
            cur=mul(cur,a[i]);
            // v*p^j
            for (int j=0, idx=i, v=cur; j <= 4; j++, idx+=n, v=mul(v,p)) {
                if (v==I) {
                    mi = Math.min(mi, idx);
                    break;
                }
        ...
        for (int i = 0; i < a.length; i++) {
            ...
            // p^j * v
            long idx=end-i;
            for (int j=0, v=cur; j <= 4; j++, idx-=n, v=mul(p,v)) {
                if (v==K) {
                    mk = Math.max(mk, idx);
                    break;
                }
...
wrong : 5 lines, lines start from 1: [18, 37, 65, 72, 88]

Time taken: 36.667 sec
...
//    static int[] debugTestCases = {18,37,65,72,88}; // small 0, should be YES
//    static int[] debugTestCases = {51,62,67}; // small 1, should be YES
...
//                out.printf("Case #%d: ", i);
    ...
    private static void myTest() throws Exception {
        ...
        for (int i = 1; i <= num; ++i) {
//            int L=Rn.i(1,10000);
////            long X=Rn.l(1,min(1000000000000L, 10000000000000000L/L));
//            long X=Rn.l(1,10000/L);
//            System.out.println(i+" : "+new C().solve(L,X,Rn.a(L,2,4)));
        }
    }
