...
public class C {
	...
	private static final String fileName = "results/con2015/con2015Q/"+C.class.getSimpleName().toLowerCase()+islarge;
	...
	static boolean isPossibleI(int T, long L){
		for(int i=0;i < T;i++){
			if(ltr[i]==2){ // i == i
				if(isPossibleJ(T, L, i, 0))
					return true;
			}
			if(convert(full,ltr[i])==2){ // [full]i == i
				if(isPossibleJ(T, L, i, 1))
					return true;
			}
			if(convert(full2, ltr[i])==2){ // [full][full]i == i
				if(isPossibleJ(T, L, i, 2))
					return true;
			}
			if(convert(full3, ltr[i])==2){ // [full][full][full]i == i
	...
	static boolean isPossibleJ(int T, long L, int left, int fulls){
		left++;
		if(left < T){
	...
	static boolean isPossibleJgo(int T, long L, int from, int fulls, int sub){
		if(fulls > L)return false;
		for(int i=from;i < T;i++){
			sub = convert(sub, chars[i]);
			if(sub==3 && isPossibleK(T, L, i, fulls)){ // j
				return true;
			}
		}
		return false;
	...
	static boolean isPossibleK(int T, long L, int left, int fulls){
		left++;
		if(left == T){
			fulls++;
			left = 0;
		...
		int num = rtl[left];
		long fullsTodo = L - (fulls+1);
		if(fullsTodo < 0)return false;
		if(fullsTodo > 0){
			if(rtl[0]!=1){
				if((fullsTodo&1) == 1){
					num = convert(num,rtl[0]);
