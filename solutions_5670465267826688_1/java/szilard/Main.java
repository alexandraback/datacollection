...
class TaskC {
    ...
    private boolean possible(String s, long n, long x) {
        int period;
        int curr = 1;
        int[] a = new int[(int)n];
    ...
    private Pair find(int[] a, long x, Pair p, int target, int period) {
    ...
    private boolean isGoodMult(int period, long x) {
        int powSol = pow(period, x);

        return powSol == -1;
    }

    private int pow(int x, long exp) {
        if (exp ==0){
            return 1;
        }
