...
public class C
{
	...
	public static void main(String[] klein)
	{
		...
		{
			final int L = in.getInt(); final long X = in.getLong();
			final int len = (int)(L*Math.min(10,X));
			...
			int a = 1, ai = 0;
			for(; ai<4*L && ai<len && a!='i'; ai++) a = mul(a,s[ai]);
			int b = 1, bi = ai;
			for(; bi<len && bi<9*L && b!='j'; bi++) b = mul(b,s[bi]);
			int c = 1, ci = bi;
			for(; ci<len; ci++) c = mul(c,s[ci]);
			int sym = 1;
			for(int i = 0; i<L; i++) sym = mul(sym,s[i]);
			if(len<(long)L*X) c = mul(c,pow(sym,X-10));
	...
	private static int pow(final int sym, long exp)
	{
		int ans = 1;
