...
class Solver {
    ...
    String solveTest(){
        ...
        for (int i = 0; i < rep && builder.length() < 1e6; i++) {
            //s += sub;
            builder.append(sub);
        }
        s = builder.toString();
        ...
        long cntLastMin = minus[sub.length()-1]==0? 0 : rep%2;
        long lastVal = -1; 
        if( vl[sub.length()-1] == 0 ) lastVal = 0;
        else{
            if( rep%4 == 0 ){ lastVal= 0; }
            if( rep%4 == 1 ){ lastVal=-1; }
            if( rep%4 == 2 ){ lastVal= 0; cntLastMin=(cntLastMin+1)%2; }
            if( rep%4 == 3 ){ lastVal=-1; cntLastMin=(cntLastMin+1)%2; }
        }

        boolean ans = getI && getJ && 
                //vl[s.length()-1]==0 && minus[s.length()-1]==1;
                lastVal==0 && cntLastMin==1;
