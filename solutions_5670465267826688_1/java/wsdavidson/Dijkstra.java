...
public class Dijkstra
{
	...
	private static void start() throws NumberFormatException, IOException
	{
		...
		for (int i = 0; i < testCases; ++i)
		{
			...
			long patternCount = Long.parseLong(unparsed[1]);
	...
	private static boolean doTestCase(long patternCount, int[] pattern)
	{
		if (pattern.length < 2) return false;
		
		// 20 to be sure we cover the biggest area,cut,area,cut,area possible
		if (patternCount > 20)
		{
			patternCount = 20 + (patternCount % 4); // chop off extraneous duplicate patterns
		}
		
		int[] longPattern = new int[pattern.length*(int)patternCount];
