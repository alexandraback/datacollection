...
bool solve() {
	...
	// cerr << absPrefixPos << "; " << absPostfixPos << " = " << printable(totalMul) << endl;
	...
	// cerr << "pl = " << printable(pl) << ", pr = " << printable(pr) << endl;
	assert(repTotal % nums.size() == 0);
	// cerr << "blocks to fill: " << repTotal / nums.size() << endl;

	switch ((repTotal / nums.size()) % 4) {
		case 0: totalMul = 1; break;
		case 1: totalMul = totalMul; break;
		case 2: totalMul = multiply(totalMul, totalMul); break;
		case 3: totalMul = multiply(totalMul, multiply(totalMul, totalMul)); break;
	}
