...
quad power(quad x, long long int expo){
	if (x==1) return 1;
...
string hasAWay(long long int L, long long int X, string const& S){
	...
	long long int frontPos[9];
	long long int backPos[9];
	...
	for (long long int i=0; i<4; i++){
		for (long long int j=0; j<4; j++){
			if (i+j<X){	
				for (long long int x =-Qk; x<=Qk; x++){
					if (frontPos[x+4]!=-1 && multiply(power(package,i),x) == Qi){
						for (long long int y=-Qk; y<=Qk; y++){
