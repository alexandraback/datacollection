...
void main(){
	...
	for (n = 0; n < T; n++){
		__int64 L, X;
		L = f.GetInt64Lex(row, 0);
		X = f.GetInt64Lex(row++, 1);
		string s = f.z[row++];
		...
		// => max length of substring reduces to any letter = 4*L
		...
		int i, sign = 1;		
		char last = '1', substr_type = 'i';
		for (; k >= 1; k--){
			for (i = 0; i < L; i++){
				if (last == '1')last = s[i];
				else if (last == s[i]){ last = '1'; sign *= -1; }
				else if ((last == 'i') && (s[i] == 'j'))  last = 'k';
				else if ((last == 'i') && (s[i] == 'k')){ last = 'j'; sign *= -1; }
				else if ((last == 'j') && (s[i] == 'i')){ last = 'k'; sign *= -1; }
				else if ((last == 'j') && (s[i] == 'k'))  last = 'i';
				else if ((last == 'k') && (s[i] == 'i'))  last = 'j';
				else if ((last == 'k') && (s[i] == 'j')){ last = 'i';  sign *= -1; }

				if ((last == substr_type) && (sign == 1)){
					if (substr_type == 'j'){
						substr_type = 'k';
						last = '1';						
					}
		...
		if ((substr_type == 'k') && (last == 'k') && (sign == 1))res[-1] += "YES";
		else res[-1] += "NO";
		std::cout << (n + 1) << endl;		
	}
