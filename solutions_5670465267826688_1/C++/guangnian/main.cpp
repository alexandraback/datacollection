...
bool valid(int x, int y, int z, int same, LL tot)
{
    int cnt = x + y + z + 2;
    if (same == 1)
        cnt--;
            ...
            for (int y = 0; y < 4; y++) {
                for (int j = 1; j <= l; j++) {
                    for (int same = 0; same < 2; same++) {
                        ...
                        if (y == 0) {
                            //in the two different pieces
                            if (same == 0) {
                                if (cal(v[i+1][l], v[1][j-1]) != 3)
                                    ok = false;
                            }
                        //i, j in the same piece
                            if (same == 1) {
                                if (j - 1 < i + 1 || v[i+1][j-1] != 3)
                                    ok = false;
                            }
                        }
                        ...
                        for (int z = 0; z < 4; z++) {
                            ...
                            /*if (t == 4) {
                             ...
                             cout << "i= " << i << " j= " << j << " same=" << same << endl;
                             cout << valid(x, y, z, same, tot) << endl;
                             }*/
                            if (t == 4 && valid(x, y, z, same, tot)) {
                                //cout << x << " " << y << " " <<  z << " " << i << " " << j << endl;
                                return "YES";
                            }
                        }
