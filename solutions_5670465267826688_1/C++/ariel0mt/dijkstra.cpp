...
bool simplificak(long  x,long  lon,long  z,string s){
    quaternion resu;
    resu.signo=1;
    resu.q=l;
    for(long  i=z;i<lon;i++){
      resu=producto(resu,asigna(s.c_str()[i]));
    }
    x--;
    for(long  i=x;i>0;i--){
        for(long  j=0;j<lon;j++){
...
bool simplificajk(long  x,long  lon,long  z,string s){
    quaternion resu;
    resu.signo=1;
    resu.q=l;
    for(long  i=z; i<lon; i++){
...
bool simplifica(long  x, long  lon, string s){
    x=min(12+(x%4),x);
    quaternion resu;
    resu.signo=1;
    resu.q=l;
...
int main(){
  ...
  freopen("C-large-ariel0.out","w",stdout);
  int casos;
  cin>>casos;
