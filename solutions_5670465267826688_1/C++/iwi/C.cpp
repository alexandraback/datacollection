...
void solve() {
  ...
  rep (i, B.length()) {
    ...
    set<int> nxt[3];
    rep (j, 3) {
      for (int x : crr[j]) nxt[j].insert(mul(x, y));
      if (j == 0 && nxt[j].count(I)) nxt[j + 1].insert(1);
      if (j == 1 && nxt[j].count(J)) nxt[j + 1].insert(1);
    }

    rep (j, 3) crr[j].swap(nxt[j]);
  }

  puts(crr[2].count(K) ? "YES" : "NO");
}
