...
int main() {
//    input("test.txt");
    input("DijkstraBig.in");
    output("DijkstraBig.out");
    precompute();
...
pair<char,int> calculate(string str) {
    pair<char,int> ans = mp('1',1);
//    cout << str.size() << endl;
    f(i,0,str.size()-1) {
        multiply(ans,str[i]);
    }
...
bool ok(pair<char,int> p, int power) {
    if (p.first == '1' && p.second == -1) {
        return (power % 2);
    }
//    cout << p.first << " " << p.second << " " << power << endl;
    return (1 - (power % 2));
}
