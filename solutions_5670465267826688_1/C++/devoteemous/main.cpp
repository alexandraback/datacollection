...
int main(int argc, char** argv) {
  ...
  for (int i = 0; i < cn; i++) {
    ...
    for (_int64 j = 0; j < X[i]; j++) {
      ...
      //
      // if (old_let == let && act == false) break;      
      if (act == false) {
        pas++;
      } else {
        pas = 0;
      ...
      if (pas == 4) {
        ...
        rem %= 4;
        int let1 = 1;
        for (int l = 0; l < L[i]; l++) let1 = quat(let1, str[i][l]);
        for (_int64 l = 0; l < rem; l++) let = quat(let, let1);
        break;
      }      
    }
