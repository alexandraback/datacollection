...
if __name__ == '__main__':
    ...
    def setup_fa(fa):
        ...
        fa.new_edge(_1, _i, set('i'))
        fa.new_edge(_1, _j, set('j'))
        fa.new_edge(_1, _k, set('k'))
        fa.new_edge(_i, _m1, set('i'))
        fa.new_edge(_i, _k, set('j'))
        fa.new_edge(_i, _mj, set('k'))
        fa.new_edge(_j, _mk, set('i'))
        fa.new_edge(_j, _m1, set('j'))
        fa.new_edge(_j, _i, set('k'))
        fa.new_edge(_k, _j, set('i'))
        fa.new_edge(_k, _mi, set('j'))
        fa.new_edge(_k, _m1, set('k'))

        fa.new_edge(_m1, _mi, set('i'))
        fa.new_edge(_m1, _mj, set('j'))
        fa.new_edge(_m1, _mk, set('k'))
        fa.new_edge(_mi, _1, set('i'))
        fa.new_edge(_mi, _mk, set('j'))
        fa.new_edge(_mi, _j, set('k'))
        fa.new_edge(_mj, _k, set('i'))
        fa.new_edge(_mj, _1, set('j'))
        fa.new_edge(_mj, _mi, set('k'))
        fa.new_edge(_mk, _mj, set('i'))
        fa.new_edge(_mk, _i, set('j'))
        fa.new_edge(_mk, _1, set('k'))
    ...
    accepts_j = Fa()
    accepts_k = Fa()
    _i1, _i, _, _ = setup_fa(accepts_i)
    _j1, _, _j, _ = setup_fa(accepts_j)
    _k1, _, _, _k = setup_fa(accepts_k)

    fa = accepts_i
    fa.initial = set([_i1])
    fa.add_fa(accepts_j)
    fa.add_fa(accepts_k)
    fa.new_edge(_i, _j1, None)
    fa.new_edge(_j, _k1, None)
    fa.accept_labels = { _k: True }

    fa = minimize_enfa(fa)
    init = next(iter(fa.initial))
    ...
    for t in xrange(pop_int()):
        ...
        while True:
            for c in S:
                for outedge in cur.outedges:
                    if c in outedge.label:
                        cur = outedge.target
                        break
                else:
                    raise RuntimeError('no edge: {}'.format(c))
        ...
        inv_ann = [None]*loop_end
        for k, v in ann.items():
            inv_ann[v] = k

        if X >= loop_end:
            X = (X - loop_start) % (loop_end - loop_start) + loop_start

        if fa.accept_labels.get(inv_ann[X]):
