...
namespace CodeJam
{
    class Program
    {
        ...
        {
            List<string> lines = Utils.GetStringList("C-large_1.in");
            //List<string> lines = Utils.GetStringList("input.txt");
            int numofCases = Convert.ToInt32(lines[0]);
            ...
            for (int i = 0; i < numofCases; i++)
            {
                ...
                {
                    char element = pattern[k];
                    oneCircle = Merge(oneCircle, element, ref sign);
                }
            ...
            Utils.WriteFile(outputs, "c-output-new.txt");
        }
    }
}
