...
int main(int argc, char **argv)
{
	...
	for(int it_t = 1; it_t <= in_t; it_t++)
	{
		//init input
		//finish init input
		unsigned long long Smax = readHugeInt();
		int cnt = 0;
		bool c[10] = {false};
		unsigned long long v_final = 0;
		if(Smax != 0) {
		FOR(a,1,INT32_MAX)
		{
			unsigned long long v = a * Smax;
			//get all digits
			while(v > 0) {
				int d = v % 10;
				cnt += (!c[d]);
 ...
 //		fprintf(fo, "Case #%d: %lld", it_t, sol);
		...
		if(Smax == 0) {
			fprintf(fo, "Case #%d: INSOMNIA", it_t);
		}
		else {
			fprintf(fo, "Case #%d: %lld", it_t, v_final);
		}
		fprintf(fo, "\n");
	}
	closeIOFile();
