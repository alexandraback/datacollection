...
namespace googleshit {
    class Program {
        static void Main(string[] args) {
            ...
            bool first=true;
            foreach (String s in File.ReadLines(@"C:\Users\Daniel\Downloads\A-small-attempt2.in")) {
                if(!first)
            ...
            for (int i = 0; i< output.Count;i++) {
                File.AppendAllText(@"C:\Users\Daniel\Downloads\A-small-attempt2.out","Case #"+(i+1) + ": " + ((output[i] == -1)? "INSOMNIA" : output[i].ToString())+ "\n");
            }
        }
