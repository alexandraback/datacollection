...
public class Google
{
    ...
    {
        long number = Long.parseLong(input[0][0]);
        boolean validated[] = new boolean[10];
        for (int i = 0; i < 10; i ++)
            validated[i] = false;

        if (number == 0)
            return outputCase(output, "INSOMNIA");

        long growingNumber = 0;

        do
        {
            growingNumber += number;
        } while (!parseInput(validated, growingNumber));

        return outputCase(output, String.valueOf(growingNumber));
    }

    public static boolean parseInput(boolean validated[], long number)
    {
        String str = String.valueOf(number);
        for (int i = 0; i < str.length(); i ++)
        {
            validated[Integer.parseInt(str.substring(i, i + 1))] = true;
        }
