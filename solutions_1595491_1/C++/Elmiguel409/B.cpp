...
bool canDo(bool isShock, int total){
   if(p == 0) return true; if(p == 1) return total > 0;

   int trip = p * 3;
   if(trip - 2 <= total) return true;
   return isShock && trip - 4 <= total;
}

int solve(int pos, int shocks){
   if(pos == N) return (shocks)?-INF:0;
   int &ret = dp[pos][shocks]; if(ret != -1) return ret; ret = -INF;
   
   if(shocks && nums[pos] >= 2 && nums[pos] <= 28){
        if(canDo(1,nums[pos])) ret = max(ret,1 + solve(pos+1,shocks-1));
	else		       ret = max(ret,solve(pos+1,shocks-1));
   }

   if(canDo(0,nums[pos])) ret = max(ret,1 + solve(pos+1,shocks));
   else                   ret = max(ret,solve(pos+1,shocks));
