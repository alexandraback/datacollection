...
class Matrix {
    ...
    private class Node {
        int v;
        Node up, down, left, right, sUp, sLeft;
    ...
    public Matrix(Scanner in) {
        ...
        for(Node i = h.down; i!=h; i = i.down)
            for(Node j = h.right; j!=h; j = j.right) {
                ...
                if(node.v==node.up.v)
                    node.sUp = node.up.sUp;
                else
                    node.sUp = node;
                if(node.v==node.left.v)
                    node.sLeft = node.left.sLeft;
                else
                    node.sLeft = node;
            }
    }
    ...
    public void removeColumn(int y) {
        ...
        for(Node j = i.down; j!=i; j = j.down) {
            ...
            j.right.left = j.left;
            if(j.right.v==j.left.v)
                j.right.sLeft = j.left.sLeft;
            else
                j.right.sLeft = j.right;
        }
        n--;
        if(n==0) {
            m = 0;
