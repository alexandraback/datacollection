...
namespace RAVE
{
	class TestCase
	{
		...
		{
			// Return either the word "YES" if it's possible to get the x-th pattern using the lawnmower,
			// or "NO", if it's impossible (quotes for clarity only).
			var shortColumns = A.GetColumns().Select(c => c.All(h => h == 1)).ToArray();
			var shortRows = A.GetRows().Select(c => c.All(h => h == 1)).ToArray();
			return Enumerable.Range(0, N).All(i => Enumerable.Range(0, M).All(j => A[i, j] != 1 || shortRows[i] || shortColumns[j]))
				? "YES"
				: "NO";
		}
	}
