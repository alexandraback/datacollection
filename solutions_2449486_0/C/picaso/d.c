...
int main(){
    int t,u=1;
    FILE *f,*r;
    f=fopen("/Users/akshaykhanna/Downloads/B-small-attempt3.in.txt", "r");
    ...
    while (u<=t) {
        ...
        /*row[0]=a[0][0];
         func(n, m, a, 0, 0,row,col,1);
         for (i=0; i<n; i++) {
         for (j=0; j<m; j++) {
         if(col[j]!=1000 && row[i]!=1000 && a[i][j]!=min(col[j],row[i]))
         fl=0;
         }
         }
         if(fl==0){
         ...
         for (i=1; i<n && fl; i++) {
         for (j=0; j<m && fl; j++) {
         if(a[i][j]>max(row[i],col[j]))
         fl=0;
         }
         }*/
        //printf("%d %d\n",n,m);
        if(fl==1 || (n==1 || m==1)){
            fprintf(r,"Case #%d: YES\n",u);
            printf("Case #%d: YES\n",u);
        }
        else if(fl==0 ){
            fprintf(r,"Case #%d: NO\n",u);
