...
struct Instance {
   ...
   #define N 107
   int rowmax[N], colmax[N], a[N][N], n, m;
   string res;
   
   void readInput() { // should read input; will run sequentially
      cin >> n >> m;
      FORI(i,n) FORI(j,m) cin >> a[i][j];
   }
   
   void compute() { // should produce output and store it, not use IO; will run in parallel
      FORI(i,n) {
         rowmax[i] = 0;
         FORI(j,m) REMAX(rowmax[i], a[i][j]);
      }
      FORI(j,m) {
         colmax[j] = 0;
         FORI(i,n) REMAX(colmax[j], a[i][j]);
      }
      res = "YES";
      FORI(i,n) FORI(j,m) if (a[i][j] != min(rowmax[i], colmax[j])) res = "NO";
   }
