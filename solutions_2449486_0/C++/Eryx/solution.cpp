...
int X, Y;
  err=scanf("%d%d",&Y,&X);
   FOR(y,0,Y) FOR(x,0,X) err=scanf("%d", &g[y][x]),v[y][x]=22222;

  FOR(y,0,Y) {
    int m=-672;
    FOR(x,0,X) m=max(m,g[y][x]);
//printf("%d ", m);
    FOR(x,0,X) v[y][x]=m;
    }
...
;
  FOR(y,0,X) {
    int m=-672;
    FOR(x,0,Y) m=max(m,g[x][y]);// printf("%d ", m);
    FOR(x,0,Y) if(g[x][y]!=min(v[x][y],m)) ok=false;
    }
;
