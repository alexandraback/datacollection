...
bool isedge(const int &i, const int &j, const int &N, const int &M) {
    return (i == 0 && j == 0) || (i == 0 && j == M-1)
...
/*
    for (int i = 0; i < N; ++i) {
        for (int j = 0; j < M; ++j) {
            if (i == 0 || i == N-1) {
                if ((j != 0 && j != M-1) && lawn[i][j] > cols[j]) return "NO";
            } else if (j == 0 || j == M-1) {
                if ((i != 0 && i != N-1) && lawn[i][j] > rows[i]) return "NO";
            } else {
                if (lawn[i][j] > rows[i] && lawn[i][j] > cols[j]) return "NO";
            }
        }
