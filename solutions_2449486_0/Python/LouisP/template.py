...
def cut_grass(cur_lawn, target_lawn, heights, N, M):
    while heights:
        # heights are sorted
        max_height = heights.pop()
        LOG.debug('trying cut at height: %d' % max_height)
        cut = False
        for i, row in enumerate(target_lawn):
            if max(row) <= max_height:
                #cut_row(cur_lawn, i, max_height)
                cut = True
                for j in xrange(M):
                    cur_lawn[i][j] = max_height
        for j in xrange(M):
            col = [target_lawn[i][j] for i in xrange(N)]
            if max(col) <= max_height:
                #cut col
                cut = True
                for i in xrange(N):
                    cur_lawn[i][j] = max_height
        if not cut:
            LOG.debug('could not cut at height %d' % max_height)
            return False
    return is_equal(cur_lawn, target_lawn)
...
def do_job(in_file, out_file=sys.stdout):
    ...
    for testcase in xrange(T):
        ...
        initial_lawn = [[100] * M for _ in range(N)]
        heights = set()
        target_lawn = []
        for _ in xrange(N):
            ai = map(int, in_file.readline().split())
            heights.update(ai)
            assert len(ai) == M
            target_lawn.append(ai)
        result = cut_grass(initial_lawn, target_lawn, sorted(heights), N, M)
        LOG.debug(initial_lawn)
        if result:
            result = 'YES'
        else:
            result = 'NO'
