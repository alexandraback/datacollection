...
map<int, list<int> > inheritance;
int N;

bool solve(const int i)
{
  //printf("solve %d \n", i);
  list<int> inheritanceList = inheritance[i];
  set<int> parents;
  bool bingo = false;
  while(inheritanceList.size() > 0)
  {
    int C = inheritanceList.front();
    //printf("check %d\n", C);
    inheritanceList.pop_front();
    if(parents.find(C) != parents.end()) {
      bingo = true;
      break;
    } 
    parents.insert(C);
    list<int>::iterator it    = inheritance[C].begin();
    list<int>::iterator itEnd = inheritance[C].end();
    for(;it != itEnd;++it)
    {
      //printf("append %d\n", *it);
...
int main()
{
  ...
  for(int tc = 1; tc <= T; ++tc)
  {
    ...
    {
      int M;
      list<int> inheritanceList;
      scanf("%d", &M);
      for(int j = 0; j < M; ++j)
      {
        int C;
        scanf("%d", &C);
        inheritanceList.push_back(C);
      }
      inheritance[i] = inheritanceList;
    }
    printf("Case #%d: ", tc);
    solveAll();
  }
