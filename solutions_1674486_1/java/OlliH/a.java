...
public class a {
	...
	private static Boolean addReachables(Hashtable[] reachableFrom, int[][] erbt,
		...
		if (reachableFrom[i]!=null && reachableFrom[i].containsKey("done"))
		{
			return false;
		}
		if(anzErb[i]>0)
		{
			...
			{
				if (reachableFrom[i].containsKey(erbt[i][j]))
				{
					return true;
				}
				...
				while(e.hasMoreElements())
				{
					...
					if (x!="done")
					{
						if (reachableFrom[i].containsKey(x))
						{
							return true;
						}
						else
