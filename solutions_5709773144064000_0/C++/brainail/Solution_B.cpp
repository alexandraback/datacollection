...
int main() {
    ...
    FORN(testIndex, 1, T) {
        ...
        while (currentTime <= resultTime) {
            LD timeCookie = X / extraCookie;

            if (currentTime + timeCookie < resultTime) {
                resultTime = currentTime + timeCookie;
            }

            currentTime += C / extraCookie;
            extraCookie += F;
        }

        cout << fixed << setprecision(7) << "Case #" << testIndex << ": " << resultTime << endl;
    }
