...
long double farm_cost, freq, must_have;

long double stupid() {
	long double ans = must_have / 2, cur = 0;
	for(int i = 1; i <= 1E6; i++) {
		cur += farm_cost / (freq * (i - 1) + 2);
		ans = min(ans, cur + must_have / (i * freq + 2));
	}
	return ans;
}
