...
void main(){
	...
	if (dType == 1){
		f1 = "B-small-attempt0.in";
		f2 = "B-small-attempt0.out";
	}
	if (dType == 2){
		f1 = "B-large-practice.in";
		f2 = "B-large-practice.out";
	}
	...
	T = stoi(f.z[0]);
	for (n = 0; n < T; n++){
		C = stod(f.GetLex(n + 1, 0));
		F = stod(f.GetLex(n + 1, 1));
		X = stod(f.GetLex(n + 1, 2));
		...
		if (X <= C)t = X / 2;
			...
			// iteration count = floor(X/C - 1 - 2/F)			
			while (((X - C) / v) > (C / F)){
				v += F;
				t += C / v;
			}
