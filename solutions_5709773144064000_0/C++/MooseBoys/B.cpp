...
int CodeJamMain()
{
    ...
    {
        double C; // cost of factory
        double F; // production of factory
        double X; // total goal
        input >> C >> F >> X;

        double rate = 2.0;
        double bestTargetTime = X / rate;
        double currentTime = 0.0;

        // TODO - check accumulation error? large input might break it; could use rational type

        while (true)
        {
            double timeToNextFactory = C / rate;
            double newRate = rate + F;
            double newTargetTime = currentTime + timeToNextFactory + X / newRate;
            if (newTargetTime < bestTargetTime)
            {
                bestTargetTime = newTargetTime;
                currentTime += timeToNextFactory;
                rate = newRate;
            }
            else
            {
