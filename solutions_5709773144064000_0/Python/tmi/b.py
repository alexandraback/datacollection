...
def getProblemAnswer(S):
    ...
    gen = float(2)
    while True:
        time_to_end = goal / gen
        time_to_farm = cost / gen
        time_to_end_with_farm = (time_to_farm + goal / (gen + inc))
        if time_to_end < time_to_end_with_farm:
            return "{0:.7f}".format(time + time_to_end)
        else:
            time += time_to_farm
            gen += inc
