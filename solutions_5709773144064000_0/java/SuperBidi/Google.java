...
public class Google
{
    ...
    {
        double farm = Double.parseDouble(input[0][0]);
        double rate = 2;
        double farmRate = Double.parseDouble(input[0][1]);
        double goal = Double.parseDouble(input[0][2]);

        double solution = goal / rate;
        double farmNumber = 0;
        double farmCost = 0;

        while (true)
        {
            farmCost = farmCost + farm / (rate + farmRate * farmNumber);
            farmNumber++;
            if (solution > farmCost + goal / (rate + farmRate * farmNumber))
                solution = farmCost + goal / (rate + farmRate * farmNumber);
            else return outputCase(output, String.valueOf(Math.round(solution * 10000000.0 ) / 10000000.0));
        }
