...
public class B {
    ...
    public class Solver implements Callable<String> {
        ...
        public String call() throws Exception {
            ...
            while (true) {
                double timeTilDone = X / (2 + N*F);
                double timeTilNextUpgrade = C / (2 + N*F);
                double timeTilDoneIfNextUpgrade = timeTilNextUpgrade + (X / (2 + (N+1)*F));
                if (timeTilDone < timeTilDoneIfNextUpgrade) {
                    t += timeTilDone;
                    break;
                } else {
                    t += timeTilNextUpgrade;
                    N++;
                }
            }

            return String.format(Locale.US, "%.7f", t);
        }
    }
}
