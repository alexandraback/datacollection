...
public class B implements Runnable {
	...
	final long INF = 1000L * 1000L * 1000L * 1000L * 1000L;
	...
	void solveTest() throws IOException {
		...
		for (int i = 0; i < n; i++) {
			for (int j = 0; j < m; j++) {
				down[i][j] = readLong() * 10;
			}
		}
		long[][][] waitTime = new long[n][m][4];
		for (int i = 0; i < n; i++) {
			for (int j = 0; j < m; j++) {
		...
		while (!pq.isEmpty()) {
			...
			for (int dir = 0; dir < 4; dir++) {
				...
				int ny = y + dy[dir];
				if (good(nx, ny, n, m)) {
					long thisTimeForMoving = max(best[x][y], waitTime[x][y][dir]);
					if (thisTimeForMoving < INF) {
						long level = h - thisTimeForMoving;
		...
		long result = best[n-1][m-1];
		out.printf("%.12f\n", result / 100.0);
	}

	private void tryUpdate(PriorityQueue<Vertex> pq, int nx, int ny, long[][] best, long arriveTime) {
	...
	private long getWaitTimeForMoving(long upFrom, long downFrom, long upTo, long downTo, long h) {
		long left = 0, right = h, ans = INF;
		while (left <= right) {
			long t = (left + right) / 2;
			long level = h - t;
	...
	private boolean canGo(long upFrom, long downFrom, long upTo, long downTo, long level) {
