...
//                      if (t % 1000 == 0) printf("t=%d : %d\n", t, h);
                        if (vis[t%20000][n-1][m-1] == ca) { printf("%d.%d\n", t/10, t%10); break; }

                        int hd = 0, tl = 0;

                        FOR(i, 0, n) FOR(j, 0, m) if (vis[t%20000][i][j] == ca) {
                                FOR(k, 0, 4) {
                                        int x = i + dx[k], y = j + dy[k];
                                        if (!in(x, y)) continue;
                                        // kayak!
                                        ...
                                        if (c[x][y] >= a[i][j] + 50
                                         ...
                                         && c[x][y] >= a[x][y] + 50)
                                                if (a[x][y] <= c[i][j] - 50) {
                                                        vis[(t+dt)%20000][x][y] = ca;
                                                }
                                }
                        }

                        FOR(i, 0, n) FOR(j, 0, m) {
                                if (vis[t%20000][i][j] == ca)
                                        vis[(t+1)%20000][i][j] = ca;
                        }
//                        if (t >= 16000) {
                              FOR(i, 0, n) FOR(j, 0, m) {
                                     vis[(t+16000)%20000][i][j] = 0;
                              }
                }
        }
        return 0;
