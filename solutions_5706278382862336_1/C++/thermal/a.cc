...
template<class T>inline T Mod(T a,T b){a%=b;return a<0?a+b:a;}///¼ÆËãa%b,Ö§³Öa<=0
template<class T>inline T MulMod(T a,T b,T c)/**¶þ·Ö¼ÆËãa*b mod c ¿ÉÒÔ´¦Àíc+c²»Òç³öµÄÇé¿ö**/{if(c==1)return 0;if(c<=0)return -1;T ret=0,tmp;tmp=a=Mod(a,c);b=Mod(b,c);while(b){if(b&0x1)if((ret+=tmp)>=c)ret-=c;if((tmp<<=1)>=c)tmp-=c;b>>=1;}return ret;}
template<class T,class Tb> inline T PowMod(Tb a,T b,T c)/**a^b mod c ÒªÇóc*c²»Òç³ö**/{if(c==1)return 0;if(c<=0)return -1;a=Mod(a,(Tb)c);Tb ret(1L%c);while(b){if(b&0x1)ret=ret*a%c;a=a*a%c;b>>=1;}return (T)ret;}
template<class T> inline T HPowMod(T a,T b,T c)/**¶þ·Ö¼ÆËãa^b mod c ÒªÇóc+c²»Òç³ö**/{if(c==1)return 0;if(c<=0)return -1;a=Mod(a,c);T ret(1L%c);while(b){if(b&0x1)ret=MulMod(ret,a,c);a=MulMod(a,a,c);b>>=1;}return ret;}
...
template <class T>inline T Inv(T a,T n)/**·µ»ØÒ»¸öÊýa¶ÔÓÚnµÄ³Ë·¨ÄæÔª**/{if(n<=0)return -1;T d,x,y;d=EXT_GCD(a,n,x,y);if(d!=1)return -1;return Mod(x,n);}
...
inline int leap(int year)/**ÅÐÈòÄê**/{return (year%4==0&&year%100!=0)||year%400==0;}
inline int legal(date a)/**ÅÐºÏ·¨ÐÔ**/{if (a.month<0||a.month>12) return 0;if (a.month==2)return a.day>0&&a.day<=28+leap(a.year);return a.day>0&&a.day<=days[a.month-1];}
inline int datecmp(date a,date b)/**±È½ÏÈÕÆÚ´óÐ¡**/{if (a.year!=b.year)return a.year-b.year;if (a.month!=b.month)return a.month-b.month;return a.day-b.day;}
int weekday(date a)/**·µ»ØÖ¸¶¨ÈÕÆÚÊÇÐÇÆÚ¼¸**/{int tm=a.month>=3?(a.month-2):(a.month+10);int ty=a.month>=3?a.year:(a.year-1);return (ty+ty/4-ty/100+ty/400+(int)(2.6*tm-0.2)+a.day)%7;}
int date2int(date a)/**ÈÕÆÚ×ªÌìÊýÆ«ÒÆ**/{int ret=a.year*365+(a.year-1)/4-(a.year-1)/100+(a.year-1)/400,i;days[1]+=leap(a.year);for (i=0;i<a.month-1;ret+=days[i++]);days[1]=28;return ret+a.day;}
date int2date(int a)/**ÌìÊýÆ«ÒÆ×ªÈÕÆÚ**/{date ret;ret.year=a/146097*400;for (a%=146097;a>=365+leap(ret.year);a-=365+leap(ret.year),ret.year++);days[1]+=leap(ret.year);for (ret.month=1;a>=days[ret.month-1];a-=days[ret.month-1],ret.month++);days[1]=28;ret.day=a+1;return ret;}
