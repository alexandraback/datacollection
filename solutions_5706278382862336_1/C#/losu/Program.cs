  ...
  class Program
  {
    ...
    static void Main(string[] args)
    {
      ...
      for (int iTestcase = 0; iTestcase < Helper.NofTestcases; iTestcase++)
      {
        ...
        string[] quot = line.Split(new char[] { '/' });
        long p = long.Parse(quot[0]);
        long q = long.Parse(quot[1]);

        long ggt = GetGcd(p, q);
        p /= ggt;
        q /= ggt;
        
        int generation = qs.IndexOf(q);
        ...
        if (generation >= 0)
          Helper.WriteOutput(String.Format("Case #{0}: {1}", iTestcase + 1, generation));
        else
          Helper.WriteOutput(String.Format("Case #{0}: {1}", iTestcase + 1, "impossible"));
      }

      Helper.WriteOutputFile();
    }

    internal static long GetGcd(long a, long b)
    {
      while (true)
      {
        long c = a % b;
