...
public class Mainer {
	...
	public static void main(String[] args) throws IOException {
		...
		FastReader in = new FastReader(new BufferedReader(new FileReader(
				"input.txt")));
		...
		for (int test = 0; test < tests; ++test) {
			long n = in.nextLong();
			long d = in.nextLong();
			long gcd = BigInteger.valueOf(n).gcd(BigInteger.valueOf(d)).longValue();;
			n = n / gcd;
			d = d / gcd;
			if ((d & (d - 1)) != 0){
...
//				out.println("Case #" + (test + 1) + ": impossible");
			...
			i /= 2;
			long  l = (long) (Math.log((double) (0.0 + d )/i)/Math.log(2.0) + 0.001);
