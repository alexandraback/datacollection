...
//	private static final String islarge = "-large";
	private static final String fileName = "results/con2015/con2015R1a/"+A.class.getSimpleName().toLowerCase();//+islarge;
	...
	private void solve() {
		...
		for(int i=1;i<N;i++){
			if(ns[i] < ns[i-1]){
				A += ns[i-1]-ns[i];
				rate = Math.max(rate, ns[i-1]-ns[i]);
			}
		}
		// CALC B
		for(int i=0;i+1<N;i++){
			B += Math.min(ns[i],rate);
		}
		out.printLine(A+" "+B);
	}
