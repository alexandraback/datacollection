...
int main() {
  ...
  FOR(test, 1, numTests + 1) {
    in >> n;
    int ret1 = 0, ret2 = 0, s = 0;
    REP(i, n) { in >> m [i]; if (i > 0) { ret1 += MAX(0, m [i - 1] - m [i]); s = MAX(s, m [i - 1] - m [i]); } }
    FOR(i, 1, n) { if (m [i - 1] >= s) ret2 += s; else ret2 += m [i - 1]; }

    cout << test << ":" << ret1 << " " << ret2 << endl;
    out << "Case #" << test << ": " << ret1 << " " << ret2 << endl;
  }
