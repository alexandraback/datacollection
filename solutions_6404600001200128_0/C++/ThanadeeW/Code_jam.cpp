...
void firstMethod(int number, int const* mushrooms){
	int lastMushroom = mushrooms[0];
	int sum = 0;
	for (int i = 1; i < number; ++i){
		auto currentMushroom = mushrooms[i];
		if (currentMushroom < lastMushroom) {
			sum += lastMushroom - currentMushroom;
		}
		lastMushroom = currentMushroom;
	}
	cout << sum;
}

void secondMethod(int number, int const* mushrooms){
	int maxLate = 0;
	int lastMushroom = mushrooms[0];
	for (int i = 1; i < number; ++i){
		auto currentMushroom = mushrooms[i];
		if (lastMushroom - currentMushroom > maxLate) {
			maxLate = lastMushroom - currentMushroom;
		}
		lastMushroom = currentMushroom;
	}
	int sum = 0;
...
void MushroomMonster(){
	int number;
	cin >> number;
	auto mushrooms = new int[number];
	for (int i = 0; i < number; ++i){
		cin >> mushrooms[i];
	}
	firstMethod(number, mushrooms);
	cout << " ";
	secondMethod(number, mushrooms);
	delete[] mushrooms;
}
