...
{
    int i,j,k,met_1,met_2,max_step;
    ...
    CASES
    {
        ...
        F(i,0,sch_count)
        {
            S(schrooms[i]);
            if (i != 0){
                if (schrooms[i] < schrooms[i-1]){
                    met_1 += schrooms[i-1] - schrooms[i];
                    REMAX(max_step,schrooms[i-1] - schrooms[i]);
                }
            }
        }
        ...
        F(i,0,sch_count-1)
        {
            DBG("met_2: max: %d eat %d\n",max_step,MIN(max_step,schrooms[i]));
            met_2 += MIN(max_step,schrooms[i]);
        }
