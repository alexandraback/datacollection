...
int score[2][31]; // score, 0 -> not spc, 1 -> spc
int T,n,s,p,z;

int main()
{
	RESET(score,-1);
	for (int i=3;i<30;i+=3) score[1][i]=(score[0][i]=i/3)+1;
	...
	{
		int A=0,B=0,C=0;
		getint(n);getint(s);getint(p);
		FOR (i,0,n)
		{
			getint(z);
			if (score[0][z]>=p && score[1][z]>=p) ++A;
			else if (score[0][z]>=p) ++B;
			else if (score[1][z]>=p) ++C;
		}
		int ans=0;
		if (s<C) ans+=s;
		else ans+=C;
