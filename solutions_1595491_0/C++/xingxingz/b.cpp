...
pint Possibilities(int s,int lower) {
  pint res(0, 0);
  for (int a = max(0, (s-3) / 3); a * 3 <= s ; ++a)
  for (int b = a; b <= a +2 && s-a-b >= b; ++b) {
    int c = s -a - b;
    if (c - a <= 1 && c>=lower) {
      res.first = 1;
    }
...
int main()
{
    ...
    for (int i = 0; i< n; ++i) {
      cin>>scores[i];
      all[i] = Possibilities(scores[i], p);
      ans += all[i].first;
      debug(all[i]);
      all[i].first = all[i].second -all[i].first;
    }
    sort(All(all));
    reverse(All(all));
    for (int i = 0; i < s; ++i) {
