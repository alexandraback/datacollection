...
int pList[5000000],tot=0;
const ll maxPrime=50000000;
...
ll minNo(int a,int b)
{
	int l=static_cast<int>(pow(10.0,digs(a))+0.5);
	for(ll i=0;;i++)
	{
		ll t=i*l+a;
		if(t%b==0)
...
ll rads(ll n)
{
	vector< pair<ll,int> > l;//=factorize(n);
	ll ret=1;
	rep(i,l.size())
		ret*=l[i].first;
	return ret;
...
struct facstate
{
	...
	int facCount;
	int valDone;
	facstate(int a,int b,int c): facInd(a), facCount(b), valDone(c){}
};
...
int arr[N][N];//={-2,5,3,2,9,-6,5,1,3,2,7,3,-1,8,-4,8};
ll getVal(int k)
{
	ll val=0;
	if(k<=55)
	{
		val=(300007*k)%1000000;
		val=(val*k)%1000000;
		val=(val*k)%1000000;
		val=val+100003-200003L*k;
		if(val<0) 
			val+= (-val/1000000+1)*1000000;
		val%=1000000;

		val-=500000;
	}
	else
		val=(*(arr[0]+(k-25)) + *(arr[0]+(k-56))+ 1000000 )% 1000000 -500000;
...
int main()
{
	...
	rep(i,3)
	{
		for(int j=0;j<str[i].size();j++)
			arr[str[i][j]]=str[i+3][j];
	}
	int ca;
	cin>>ca;
	For(cc,1,ca)
	{
		int s,p,n,always=0,some=0;
		cin>>n>>s>>p;
		rep(i,n)
		{
			int x;
		...
		fout<<"Case #"<<cc<<": "<<always+min(some,s)<<endl;
