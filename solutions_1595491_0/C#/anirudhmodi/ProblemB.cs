...
public class CodeJamProblem : IDisposable
{
    ...
    static class ProblemB
    {
        ...
        static void Main(string[] args)
        {
            ...
            using (var prob = new CodeJamProblem('B', ProblemType.Small, 0))
            {
                var t = prob.ReadLineInt32();
                foreach (var cur in prob.ReadInt16Arrays(t))
                {
                    short num = cur[0];
                    short surprises = cur[1];
                    short minScore = cur[2];
                    List<short> scores = new List<short>(cur);
                    scores.RemoveRange(0, 3);

                        Debug.Assert(scores.Count == num);

                    short max = 0;
                    short maxWithSurprise = 0;

                    foreach (var score in scores)
                    {
                        if (CanHaveMinScore(score, minScore, false))
                        {
                            ++max;
                        }
                        else if (CanHaveMinScore(score, minScore, true))
                        {
                            ++maxWithSurprise;
                        }
                    }

                    max += Math.Min(maxWithSurprise, surprises);
        ...
        private static bool CanHaveMinScore(int score, int minScore, bool surprise)
        {
            int maxScore = surprise ? (score + 4) / 3 : (score + 2) / 3;
            maxScore = Math.Min(maxScore, score);
            maxScore = Math.Min(maxScore, 10);
