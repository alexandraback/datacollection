...
public class Jam2012 {
	...
	public static void main(String[] args) throws NumberFormatException, IOException {		
		...
		for (int i = 0; i < T; i++) {
			...
			int p = sc.nextInt();
			for (int j = 0; j< n; j++) {
				googlers.add(sc.nextInt());
			}

			output = solve(n,s,p,googlers);
	...
	private static String solve(int n, int s, int p, ArrayList<Integer> googlers) {
		...
		for (int j = 0; j< n; j++) {
			/*if ((googlers.get(j)+2)/3 >= p) {
				max++;
			} else if (((googlers.get(j)+4)/3 >= p) && (s > 0)) {
				s--;
				max++;
			}*/
			if (p == 0) {
	...
	static String filename = "D:\\gcj\\2012\\Q\\B-small-attempt1";
}