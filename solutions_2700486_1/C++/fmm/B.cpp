...
double func(int H, int have, int h, int L, int R) {
	if(L>h) return 1.0;
	if(have<=0) return 0;
	if(L+1>=H) return func(H,have-1,h,L,R+1);
	if(R+1>=H) return func(H,have-1,h,L+1,R);
	assert(have<M&&L<M);
	double &res = p[have][L];
	if(res==res) return res;
	else return res=(func(H,have-1,h,L+1,R) + func(H,have-1,h,L,R+1))/2.0;
}
