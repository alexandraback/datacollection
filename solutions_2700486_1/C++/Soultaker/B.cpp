...
static long double memo[4000][4000];

// TODO: memoize!
static long double prob(int left, int right)
{
    assert(left  >= 0 && left  < 4000);
    assert(right >= 0 && right < 4000);
    long double &m = memo[left][right];
    if (m == m) return m;
    if (left  == 0 && right == 0) return m = 1.0;
    if (left  == 0) return m = 0.5*prob(left, right - 1);
    if (right == 0) return m = 0.5*prob(left - 1, right);
    return m = 0.5*prob(left - 1, right) + 0.5*prob(left, right - 1);
}

double calc(int N, int X, int Y)
{
    for (int wave = 0; N > 0; ++wave)
    {
        int size = 1 + 4*wave;
        if (N >= size)
        {
        ...
        {
            if (wave < (X + Y)/2 || Y == 2*wave) return 0.0;

            long double p = 0.0;
            for (int left = 0; left <= N; ++left)
            {
                int right = N - left;
                if (right + max(left - 2*wave, 0) > Y) p += prob(left, right);
            }
            return p;
        }
    }
