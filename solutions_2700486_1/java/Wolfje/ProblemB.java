...
//	BigDecimal[][] bin;
	...
	public double nchoosek(int n, int k) {
		...
		if (bin[n][k] >= 0)
			...
			bin[n][k] = nchoosek(n-1, k-1) + nchoosek(n-1,k);
...
//			bin[n][k] = sum;
	...
	public ProblemB() throws Exception {
		...
		int MAXBIN = 2500;
//		bin = new BigDecimal[MAXBIN][];
		bin = new double[MAXBIN][];
		for ( int i = 0; i < MAXBIN; i++) bin[i] = new double[i+2];
//		int a = 0;
//		for ( int b = 0; b < 800; b++) {
//			a += 4*b +1;
...
//				BigDecimal sum = new BigDecimal(0);
				...
				for ( int left = 0; left <= remaining; left++) {
					...
					if ( right >= y +1 ) {
						sum += nchoosek(remaining, left);
//						sum = sum.add(nchoosek(remaining, left), ctx);
					}
				}
				result = sum/Math.pow(2, remaining);
