...
namespace codejam2013
{
    public class B
    {
        ...
        public double Result(int n, int x,int y)
        {
            ...
            return calcola(0,nuovi,z,y+1);
        }

        private double calcola(int deep,int tentativi, int errori, int validi)
        {
            if (deep > 20) return 0;
            double res = 0;
            ...
            {
                div++;
                res += calcola(deep+1,tentativi - 1, errori - 1, validi);
            }
            if (validi > 0)
            {
                div++;
                res += calcola(deep+1,tentativi - 1, errori, validi-1);
            }
