...
public class Main
{
    ...
    {
        int buildings = Integer.parseInt(input[0][0]);
        long ways = Long.parseLong(input[0][1]);

        long waysThroughBuilding[] = new long[buildings];
        for (int i = 0; i < buildings; i ++)
            waysThroughBuilding[i] = 0;

        String result[] = new String[buildings + 1];
        result[0] = "0";
        ...
        ways --;
        waysThroughBuilding[0] = 1;

        for (int i = 1; i < buildings - 1; i ++)
        {
            ways = computeThisBuilding(ways, buildings, waysThroughBuilding, i, result);
        }

        if (ways > 0)
            return outputCase(output, "IMPOSSIBLE");

        outputCase(output, "POSSIBLE");
        for (int i = 0; i < buildings; i ++)
    ...
    public static long computeThisBuilding(long ways, int buildings, long waysThroughBuilding[], int building, String result[])
    {
        result[building] = "";
        for (int i = buildings - 2; i > 0; i --)
        {
            if (waysThroughBuilding[i] == 0)
                result[building] = "0" + result[building];
            else if (waysThroughBuilding[i] > ways)
                result[building] = "0" + result[building];
            else
            {
                ways -= waysThroughBuilding[i];
                result[building] = "1" + result[building];
                waysThroughBuilding[building] += waysThroughBuilding[i];
            }
        }
        ...
        if (ways > 0)
        {
            result[building] += "1";
            ways --;
            waysThroughBuilding[building]++;
        }
        else result[building] += "0";
