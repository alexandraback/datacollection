...
public class C_1 {
	...
	static int[] a;
	static boolean[] done;
	static FasterScanner input;
	static PrintWriter output;

	public static void main(String[] args) {
		input = new FasterScanner();
		output = new PrintWriter(System.out);
		int testecases = input.nextInt();
		int test = 0;
		while (testecases-- > 0) {
			test++;
			function(test);
...
//		System.out.println("test "+test);
		...
		if(go(a, 0, 0)){
			output.println("Case #"+te+": POSSIBLE");
			for(int i =0 ;i < n ; i++){
				for(int j = 0 ;j < n; j++)
					output.print(a[i][j]);
				output.println();
			...
			output.println("Case #"+te+": IMPOSSIBLE");
		}
	}
