...
public class q2{
	...
	static int[] a;
	static boolean[] done;
	static FasterScanner in;
	static PrintWriter pw;

	public static void main(String[] args) throws Exception {
		in = new FasterScanner();
		pw = new PrintWriter("bsmallout.txt");
	...
	static void maximize(int test) {
		...
		if(promethod(a, 0, 0)){
			...
			pw.println("Case #"+test+": IMPOSSIBLE");
		}
	}

	static boolean promethod(int[][] a, int i ,int j){
//		System.out.println(i+" "+j);
	...
	public static int[][] matrixpowersum(int[][] a, int p) {
		int n = a.length;
		if (p == 0) {
			return new int[n][n];
		}
		if (p % 2 == 0) {
			return matrixMul(matrixpowersum(a, p / 2), matrixAdd(matrixUnit(n), matrixPow(a, p / 2)));
		} else {
			return matrixAdd(a, matrixMul(matrixpowersum(a, p - 1), a));
		}
	}
