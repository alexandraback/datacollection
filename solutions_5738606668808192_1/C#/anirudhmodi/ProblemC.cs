...
public class CodeJamProblem : IDisposable
{
    ...
    static class ProblemC
    {
        ...
        {
            var num = GetNumForBase(arr, baseK);
            return IsPrime(num, out divisor);
        }

        private static BigInteger GetNumForBase(bool[] arr, int baseK)
        {
            BigInteger num = 0;
            BigInteger mul = 1;
            for (int i = 0; i < arr.Length; i++)
        ...
        private static bool IsPrime(BigInteger num, out int divisor)
        {
            divisor = -1;
            if (num <= 1)
            {
