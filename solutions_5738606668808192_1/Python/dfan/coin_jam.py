    ...
    for n in xrange(2**(N-1) + 1, 2**N, 2):
        ...
        for b in xrange(2, 10 + 1):
            ...
            if is_prime(interpretation):
                break
            divisor = get_divisor(interpretation, max_tries = 1000, can_fail = True)
            # If we failed to get a divisor, give up and go on to the next one
            if not divisor:
                break
...
def get_divisor(n, max_tries = None, can_fail = False):
    """
