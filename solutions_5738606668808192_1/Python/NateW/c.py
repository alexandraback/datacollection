...
def factor(n, tests=None):
    primes = PRIMES if PRIMES else xrange(2, int(math.sqrt(n)) + 2)
    for i in primes:
        if i != n and n % i == 0:
            return i
    return None
...
def main():
    ...
    primes = [2]
    for i in xrange(2, int(math.sqrt(2 ** 33)) + 2):
        if factor(i) is None:
            primes.append(i)
    global PRIMES
    PRIMES = primes
            ...
            for basei, base in enumerate(range(2, 11)):
                if int(jamcoin, base) % proof[basei] != 0:
                    assert False, ("Result %s converted to base %d is %d, "
                        "divisor %d, doesn't work!") % (jamcoin, base,
