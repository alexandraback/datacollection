...
//   			 System.out.println("base: " + base);
	   		 BigInteger value = new BigInteger(currentStr, base);
...
//   		 System.out.println(Arrays.toString(currentDivisors));
	...
	public static int factorOf(BigInteger n) {
		for (int prime : primes) {
			if (n.mod(new BigInteger(Integer.toString(prime))).intValue() == 0) return prime;
		}
		...
		search: while (nextPrime < 30) {//Math.sqrt(n)) {
			...
			if (n.mod(new BigInteger(Integer.toString(nextPrime))).intValue() == 0) return nextPrime;
		}
		return 0;
	}
