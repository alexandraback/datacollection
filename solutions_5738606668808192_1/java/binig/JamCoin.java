...
public class JamCoin {
    ...
    //        System.out.println(toBase(5,2)+ " " + toBase(5,10)+ " " +  Integer.parseInt("101",2));
        ...
        public long getDivisor(long bin, int base) {
            BigInteger n = new BigInteger(Long.toBinaryString(bin),base);
            for (int i:this.primes) {
                BigInteger pr = BigInteger.valueOf(i);
                if (pr.compareTo(n)>0) break;
                if (n.mod(pr).equals(BigInteger.ZERO)) {
                    return i;
                }
            }
            return -1;
    ...
    public static void main(String[] args) throws FileNotFoundException {
        Scanner scanner = new Scanner(Thread.currentThread().getContextClassLoader().getResourceAsStream("jamcoin/C-large.in"));
        PrintStream printStream =  new PrintStream("C:\\Users\\benoi_000\\workspace\\codeJam2016\\process\\jamcoin\\C-large.text" );
