...
public class Main {
    ...
    static class TaskC {
        public void solve(int testNumber, InputReader in, OutputWriter out) {
            ...
            BigInteger[] bigPrimes = new BigInteger[primes.length];
...
//        int cnt = 500;
            ...
            for (long mask = 1; mask < (1L << (len - 1)); mask += 2) {
                ...
                for (int base = 2; base <= 10; base++) {
                    BigInteger x = new BigInteger(str, base);
                    for (int i = 0; i < primes.length; i++) {
                        if (bigPrimes[i].compareTo(x) >= 0) {
                            break;
                        }
                        if (x.remainder(bigPrimes[i]).equals(BigInteger.ZERO)) {
                            counter[base] = primes[i];
                        }
                    }
                }
                boolean good = true;
