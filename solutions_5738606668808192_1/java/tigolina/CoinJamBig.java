	...
	private static class Coin {
		...
		private void validate() {
			...
			for (int base = 2; base <= 10; base++) {
				...
				BigInteger valueByBase = new BigInteger(binarString, base);
				for (long div = 2; div < 1000; div++) {
					BigInteger bigDiv = new BigInteger(String.valueOf(div));
					if (valueByBase.remainder(bigDiv).intValue() == 0) {
						divisorList.add(div);
						isPrime = false;
						break;
					}
