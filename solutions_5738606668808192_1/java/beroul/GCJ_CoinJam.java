...
public class GCJ_CoinJam {
	...
	long findSomeDivisor(BigInteger x,int MX){
		long y=2;
		while (!(x.mod(new BigInteger(""+y)).equals(BigInteger.ZERO)) && y<MX)
			y++;
		if (y==MX)
			return -1;
		return y;
	...
	String[] solve(int N,int J){
		...
		while (x<max && cnt<J){
			...
			loop:for (int u=2;u<=10;u++){
				BigInteger w=new BigInteger(s,u);
				if (w.isProbablePrime(32)){
					ok=false;
					break loop;
				} else {
					long y=findSomeDivisor(w,10000);
					if (y>0) {
						res+=" "+y;
						boolean check=w.mod(new BigInteger(""+y)).equals(BigInteger.ZERO);
