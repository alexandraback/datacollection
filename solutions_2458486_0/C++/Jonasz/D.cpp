...
void remove_set(vector<Chest>&chests, set<int>&to_remove, vector<int>&keys){
...
void remove_set(vector<vector<Chest > >&chests, set<int>&to_remove, vector<int>&keys){
    for(int i=0; i<chests.size(); i++) remove_set(chests[i], to_remove, keys);
}

bool possible(
        map<int,Chest>&chests_map,
        vector<int>keys, vector<Chest>chests_,
        int first){
    ...
    set<int>types_used;
    //types_used.insert(chests_map[first].type);
    ...
    while(true){
        ...
        for(int i=0; i<chests.size(); i++) if(types_used.count(i)>0) {
            ...
            remove_set(chests, to_remove, keys);
...
void solve(){
    ...
    for(int i=0; i<n; i++){
        ...
        for(id=1; id<=n; id++) {
            ...
            if(possible(chests, K, C, id)) break;
        }
        if(id==n+1){
            printf("IMPOSSIBLE\n");
            return;
