...
void main(){
	...
	if (dType == 1){
		f1 = "A-small-attempt1.in";
		f2 = "A-small-attempt1.out";
	}
	if (dType == 2){
		f1 = "A-large.in";
		f2 = "A-large.out";
	...
	for (n = 0; n < T; n++){
		...
		for (i = 1; (i < z.Len()) && b; i++){
			...
			for (j = 0; j < z[0].Len(); j++)if (z[0][j].c != z[i][j].c){
				b = false;
				break;
			}		
		}
		...
		if (b){
			...
			for (j = 0; j < z[0].Len(); j++){
				for (k = 0; k <= 100; k++){
					cost = 0;
					for (i = 0; i < z.Len(); i++)cost += abs(k - z[i][j].num);
					if (k == 0)min_cost = cost;
					else if (min_cost > cost)min_cost = cost;
				}			
				r += min_cost;
			}
		}
		...
		res[-1] += to_string(n + 1);
		res[-1] += ": ";
		if (b)res[-1] += to_string(r);
		else res[-1] += "Fegla Won";
		cout << (n + 1) << endl;
	}	
	f.SaveFile(f2, res);
	cout << "OK ";
