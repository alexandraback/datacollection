...
{
    vector<string> characters = lines;
    FORIT(it, characters) it->erase(unique(it->begin(), it->end()), it->end());
    FOR(n, 1, N) if (characters[n - 1] != characters[n]) {
        result.set("Fegla Won");
        return;
    }

    int cost = 0;
    vector<int> pos(N, 0);
    while (pos[0] < SZ(lines[0]))
    {
        vector<int> lengths(N);
        REP(n, N) {
            int start = pos[n];
            while (pos[n] < SZ(lines[n]) && lines[n][pos[n]] == lines[n][start]) ++pos[n];
            lengths[n] = pos[n] - start;
        }
        SORT(lengths);
        REP(n, N) cost += abs(lengths[n] - lengths[N/2]);
    }
    result.set(STR(cost));
}
