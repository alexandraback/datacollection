...
int main() {
    ...
    FORN(testIndex, 1, nTest) {
        ...
        _FORN(i, 0, N) {
            ...
            int index = 0;
            while (index < LENGTH(curWord)) {
                int curIndex = index;
                while (curIndex < LENGTH(curWord) && curWord [curIndex] == curWord [index]) ++ curIndex;
                words [i].PB(MP(curWord [index], curIndex - index));
                index = curIndex;
            }
        }

        bool okData = true;
        vector<int> maxLetters(SIZE(words [0]), 0);

        _FORN(i, 1, N) {
            if (SIZE(words [i]) != SIZE(words [0])) {
                okData = false;
                break;
            } 

            _FORN(j, 0, SIZE(words [0]))
                if (words [0][j].first != words [i][j].first) {
                    okData = false;
                    break;
                } else {
                    if (words [0][j].second > maxLetters [j]) maxLetters [j] = words [0][j].second;
                    if (words [i][j].second > maxLetters [j]) maxLetters [j] = words [i][j].second;
                }
        ...
        _FORN(letterIndex, 0, SIZE(maxLetters)) {
            int bestDiff = INF;
            FORN(nLetter, 1, maxLetters [letterIndex]) {
                int resultLetterDiff = 0;
                _FORN(i, 0, N) resultLetterDiff += abs(nLetter - words [i][letterIndex].second);
                FIX_MIN(resultLetterDiff, bestDiff);
            }
            result += bestDiff;
        }
