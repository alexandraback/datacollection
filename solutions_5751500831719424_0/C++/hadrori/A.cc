...
void solve(){
    ...
    rep(i,N){
        int cur = 0, prev = 0;
        rep(j,k){
            while(cur < s[i].size()-1 and s[i][cur] == s[i][cur+1]) cur++;
            cur++;
            len[j][i] = cur - prev;
            prev = cur;
        }
    }
    rep(j,k) sort(len[j], len[j]+N);
    int ans = 0;
    rep(j,k){
        if(N%2) rep(i,N/2) ans += len[j][N-i-1]-len[j][i];
        else {
            rep(i,N/2-1) ans += len[j][N-i-1]-len[j][i];
            ans += len[j][N/2]-len[j][N/2-1];
        }
    }
    cout << ans << endl;
}
