...
int solve() {
    for(int i=0;i<n;i++) {
        occ[i].clear();
        strcpy(u[i],s[i]);
        int lu=unique(u[i],u[i]+strlen(u[i]))-u[i];
        u[i][lu]='\0';
    }
    for(int i=0;i<n;i++)
        if(strcmp(u[0],u[i])) return -1;
    for(int i=0;i<n;i++) {
        for(int j=0;s[i][j];j++) {
            if(!j||s[i][j]!=s[i][j-1]) occ[i].PB(1);
            else occ[i].back()++;
        }
    }
    int sol=0;
    for(int t=0;t<occ[0].size();t++) {
        vector<int> arr;
        for(int i=0;i<n;i++)
            arr.PB(occ[i][t]);
        int me=arr.size()/2;
        nth_element(arr.begin(),arr.begin()+me,arr.end());
        int ma=arr[me];
        for(int i=0;i<n;i++)
            sol+=abs(arr[i]-ma);
    }
