...
//            outputStream = new ConsoleOutputStream();
    ...
    private void solve(int testNum) throws IOException {
        ...
        StringBuilder sb = new StringBuilder().append(a[0][0]);
        ...
        int[][] d = new int[t.length][n];
        for (int i = 0; i < n; i++) {
            int idx = 0;
            for (int j = 0; j < a[i].length; ) {
                if (idx < t.length && a[i][j] == t[idx]) {
                    int cnt = 0;
                    while (j < a[i].length && a[i][j] == t[idx]) {
                        j++;
                        cnt++;
                    }
                    d[idx][i] = cnt;
        ...
        out.println(res < 0 ? "Fegla Won" : res);
    }
