    ...
    int main()
    {
        ...
        while((T--)>0){
            ...
            for(i=0;i<N;i++){
                ...
                }else if(k+3==p && remain!=0 && Surp>0 && k+1>=0){//remain==0的时候不存在下面的p=k+3的情况
                        Surp--;
                        count++;
                }
            }
...
namespace NS_RecycledNumbers
{
    ...
    */
    int main()
    {
        ...
        ReopenStdinStdout();
        int T,A,B,i,j,k,n,m,newm,q,count=0,nCase,AFirst,BFirst;//B的首位
        scanf("%d",&T);
        nCase=1;
        list<int> L;
        list<int>::iterator iter;
        ...
        while((T--)>0){
            ...
            //bug:A ≤ n < m ≤ B? 所以这里n不能取道B，
            //IM:1111 2222我的答案是Case #4: 288,bug出在:
