...
int mult;
int lowestInRange(int cur, int low, int high){
   int orig = cur, lowest = cur;
   do{
      cur = mult * (cur % 10) + cur/10;
      if(cur >= low) lowest = min(lowest,cur);
   }while(cur != orig);
   return lowest;
}
...
int solve(int low, int high){
   map<int,int> cycles;
   int tmp = low; mult = 1; while(tmp >= 10) mult *= 10, tmp /= 10;

   FOR(i,low,high+1) cycles[lowestInRange(i,low,high)]++;

   int ret = 0;
   ITER(it,cycles) if(it->second > 1) ret += it->second * (it->second - 1) / 2;
   return ret;
}
