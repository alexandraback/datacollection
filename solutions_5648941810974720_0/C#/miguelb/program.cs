...
Case #4: 3";
        ...
        int NumberOfChars ( string s , string ch)
        {
            var aux = s.Replace(ch, "");
            return s.Length - aux.Length;
        }
        ...
        string RemoveChars( string s , string chars)
        {
            foreach( char ch in chars)
            {
                ...
                s = string.Format ("{0}{1}" , s.Substring ( 0, pos) , s.Substring ( pos+1));
            }
            return s;
        }

        List<int> Process ( ref string s , int value , string name , string ch)
        {
            List<int> result = new List<int>();
            int values = NumberOfChars(s, ch);
            for (int i = 0; i < values; i++)
            {
                result.Add(value);
                s = RemoveChars(s, name);
            }
            return result;
        ...
        protected override string SolveSmall()
        {
            ...
            result.AddRange(Process(ref s, 0, "ZERO", "Z"));
            result.AddRange(Process(ref s, 2, "TWO", "W"));
            result.AddRange(Process(ref s, 4, "FOUR", "U"));
            result.AddRange(Process(ref s, 5, "FIVE", "F"));
            result.AddRange(Process(ref s, 6, "SIX", "X"));
            result.AddRange(Process(ref s, 7, "SEVEN", "S"));
            result.AddRange(Process(ref s, 1, "ONE", "O"));
            result.AddRange(Process(ref s, 3, "THREE", "R"));
            result.AddRange(Process(ref s, 8, "EIGHT", "H"));
            result.AddRange(Process(ref s, 9, "NINE", "I"));
