...
string s1[10] = {"ZERO","TWO","EIGHT","SIX","SEVEN","FIVE","NINE","ONE","FOUR","THREE"};
int a[10] = {0,2,8,6,7,5,9,1,4,3};
int test;
int n,f[200];
char s[2001];
vector <int> ans;
int main()
{
	...
	For(Test,1,test) {
		...
		For(i,0,9) {
			int t = 1000000;
			For(j,0,s1[i].length()-1) {
				t = min(t,f[s1[i][j]]);
			}
			For(j,1,t) ans.push_back(a[i]);
			For(j,0,s1[i].length()-1) {
				f[s1[i][j]] -= t;
			}
		}
		sort(All(ans));
		printf("Case #%d: ",Test);
		For(i,0,(int) ans.size()-1) cout<<ans[i];
		cout<<endl;
	}
	return 0;
}
