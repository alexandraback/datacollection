...
class Level implements Comparable
{
	public final int A;
	public final int B;
	public boolean used1 = false;
	public boolean used2 = false;

	public Level(int a, int b) {A = a; B = b;}
	public int compareTo(Object o) {Level l = (Level)o; return (0 != (B - l.B)) ? (B - l.B) : (A - l.A);}
	...
	public static void main(String[] args)
	{
		try
		{
			...
			for (int testcase = 1; testcase <= testcases; testcase++)
			{
				...
				{
					line = input.readLine();
					level[i] = new Level(getInt(line, 0), getInt(line, 1));
				}
				Arrays.sort(level);
				for (int i = 0; i < N; i++) {debug(level[i].toString());}
				...
				while (true)
				{
					if ((2 * N) == star) {result += count; break;}

					boolean star2 = false;
					boolean star1 = false;

					for (int i = 0; i < N; i++) if (!level[i].used2 && star >= level[i].B)
					{
						star += (level[i].used1) ? 1 : 2;
						level[i].used2 = level[i].used1 = true;
						star2 = star1 = true;
						count++; debug("i2", i);
						break;
					}

					if (!star2) for (int i = N - 1; i >= 0; i--) if (!level[i].used1 && star >= level[i].A)
					{
						star += 1;
						level[i].used1= true;
						star1 = true;
