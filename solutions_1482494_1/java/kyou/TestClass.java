...
public class TestClass {
	...
	public static void main (String[] args) throws IOException {
		...
		for(int i=0; i<caseNum; i++) {
			...
			for(int j=0; j<N; j++) {
				...
				numbers = line.split(" ");
				levels.add(testClass.new Level(Integer.parseInt(numbers[0]), Integer.parseInt(numbers[1])));
			}

			int an = testClass.getB(levels);

			writer.write("Case #" + (i+1) + ": " + (an == -1 ? "Too Bad" : an) + "\n");
			writer.flush();
			levels.clear();
		}
	}
	...
	private static Comparable<Number> max(Comparable... numbers) {
	...
	@SuppressWarnings("unchecked")
	private static Comparable<Number> min(Comparable... numbers) {
