...
for i in range(cases):
	...
	while done < num_levels:
		...
		try:
			candidate = next(l[0] for l in levels if completed[l[0]] == 0 and l[2] <= stars)	# Fresh 2's
		...
		try:
			candidate = next(l[0] for l in levels if completed[l[0]] == 1 and l[2] <= stars)	# Bump 2's
			completed[candidate] = 2
			stars += 1
			done += 1
			continue
