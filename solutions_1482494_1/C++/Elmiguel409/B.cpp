...
long long solve(){
   ...
   while(cur < 2*N){
     ...
     FOR(i,0,N){
	if(state[i] != 2 && two[i] <= cur){ cur += 2-state[i]; state[i] = 2; ret++; did2 = true;}
	if(state[i] == 0 && one[i] <= cur && (best1 == -1 || two[best1] < two[i])) best1 = i; 
     }

     if(!did2){
	if(best1 == -1) break;
	cur++; ret++; state[best1] = 1;
     }
   }
   return cur==2*N?ret:INF;
}
...
int main(){
  ...
  FOR(i,0,_T){
    cout << "Case #" << (i+1) << ": ";
    cin >> N; FOR(j,0,N) cin >> one[j] >> two[j];
    long long ret = solve();
    if(ret >= INF) cout << "Too Bad";
    else cout << ret;
    cout << endl;
  }
  return 0;
