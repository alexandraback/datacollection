...
namespace CodeJam
{
    ...
    class GCJ
    {
        ...
        {
            init("2016r1c", "A");
            IProblemSolver solution = new _2016r1c.ProblemA();
            ...
            for (int i = 0; i < T; i++)
            {
                ...
                if (solution.UsedBruteForce)
                {
                    ...
                    string sol = solution.Solve(i + 1);
                    if (bf != sol) Console.WriteLine("Solution missmatch on Case #{0}: BF:{1} - SOL:{2}", i + 1, bf, sol);
                }
                else
                {
                    solution.Solve(i + 1);
