...
public class SenateEvacuation {
	public static void main(String[] args) {
		...
		try {
			BufferedReader reader = new BufferedReader(new FileReader("Round1C/A-small-attempt0.in"));

			Scanner in = new Scanner(reader);
			int T = in.nextInt();

			try {
				BufferedWriter writer = new BufferedWriter(new OutputStreamWriter(new FileOutputStream("Round1C/A-small-attempt0.out")));
				for (int i = 0; i < T; i++) {
					int N = in.nextInt();
					int[] sizes = new int[N];
					for (int j = 0; j < N; j++) {
