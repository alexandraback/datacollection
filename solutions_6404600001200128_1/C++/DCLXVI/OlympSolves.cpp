...
{
	ll result = 0;
	foriv if (i) result += max(v[i-1] - v[i], 0);
	return result;
}
...
{
	ll speed = 0;
	foriv if (i) speed = max(ll(v[i-1] - v[i]), speed);
	ll result = 0;
	foriv if (i + 1 < sz(v)) result += min(speed, ll(v[i]));
	return result;
}
...
//	ofstream cout("output.txt");
	...
	fort
	{
		...
		vi v(n);
		forin read v[i];
		cout << "Case #" << gett << ": " << solve1(v) << ' ' << solve2(v) << endl;
	}
