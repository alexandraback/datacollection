...
class Main{
	...
	void run(){
		...
		for(int q:range(T)){
			int N;cin >> N;
			vector<ll> ms(N);cin >> ms;

			ll res1=0;
			for(int i:range(1,N))res1+=max(0LL,ms[i-1]-ms[i]);

			ll eM=0;
			for(int i:range(1,N))eM=max(eM,max(0LL,ms[i-1]-ms[i]));

			ll res2=0;
			for(int i:range(1,N))res2+=min(eM,ms[i-1]);

			cout << make_tuple("Case","#"+to_string(q+1)+":",res1,res2)<<endl;
		}
