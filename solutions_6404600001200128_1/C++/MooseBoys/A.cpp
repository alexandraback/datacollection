...
int CodeJamMain()
{
    ...
    for (int caseNum = 0; caseNum < T; caseNum++)
    {
        ...
        // method 2:
        int minRateRequired = 0; // per 10 seconds
        for (int i = 1; i < N; i++)
        {
            int decrease = m[i - 1] - m[i];
            minRateRequired = max(minRateRequired, decrease);
        }
        int count2 = 0;
        for (int i = 1; i < N; i++)
        {
            int expected = max(0, m[i - 1] - minRateRequired);
            int eaten = m[i - 1] - expected;
            count2 += eaten;
        }
