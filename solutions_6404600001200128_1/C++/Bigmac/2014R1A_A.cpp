	...
	for(int it_t = 1; it_t <= in_t; it_t++)
	{
        ...
        REP(i,in_n)
        {
            ...
            if(i>0)
            {
                ...
                {
                    //Down Delta
                    sum_delta += in_m[i-1] - in_m[i];
                    //Max Delta
                    max_delta = MAX(max_delta,(in_m[i-1] - in_m[i]));
                }
            }
        }
        long long sum_max_delta = 0;
        FOR(i,0,in_n-1)
        {
            sum_max_delta += MIN(in_m[i], max_delta);
        }
        
        //===========================================solve function  end ================================//
            fprintf(fo, "Case #%d: %lld %lld", it_t, sum_delta, sum_max_delta);
		fprintf(fo, "\n");
	}
	closeIOFile();
}