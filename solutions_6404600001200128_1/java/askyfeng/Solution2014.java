...
//			dataRow = content.readLine();
			...
			String num = MushroomMonster( N, dataRow );
	...
	public static String MushroomMonster(int N, String str){
		String[] s = str.split(" ");
		int[] nums = new int[N];
		...
		for(int i=1;i<N;i++){
			...
			num1 += x>0?x:0;
			maxGap = Math.max(maxGap, x);
		}
		for(int i=0;i<N-1;i++){
			int x = nums[i];
			num2 += x<maxGap?x:maxGap;
		}
	...
	public static String Haircut(int B, int N, String str){
		...
		long scm = commonMultiple(Mk[0], Mk[1]);
		for(int i=2;i<B;i++){
	...
	public static long commonMultiple(long n,int m){
		return n*((long)m)/commonDivisor((int)n,m);
	}
