...
def read_input(infile):
    ...
    N = read_int()
    mushrooms = read_ints()
    assert N == len(mushrooms)
    
    return mushrooms

def solve_case(M):
    #Method 1 - loop through, and any time M[i] < M[i-1], add the different to our time
    method_1 = 0
    maxeat = 0
    ...
    #Method 2 - find the most mushrooms she *has* to eat at any time multiply by the number of
    #segments
    method_2 = 0
    for i in range(len(M) - 1):
        method_2 += min(maxeat, M[i])
    
    return '%i %i' % (method_1, method_2)
