...
namespace CodeJam
{
    ...
    class MushroomMonster : CodeJamProblem
    {
...
Case #4: 181 244";
        ...
        {
            M = reader.ReadInt();
            mushrooms = reader.ReadSpaceSeparatedLongs();
        }
        ...
        protected override string SolveSmall()
        {
            ...
            for (int i = 0; i < M-1; i++ )
            {
                if (mushrooms[i + 1] < mushrooms[i])
                    firstMethod += mushrooms[i] - mushrooms[i + 1];
            }
            ...
            for (int i = 0; i < M - 1; i++ )
            {
                minRate = Max(minRate, (mushrooms[i] - mushrooms[i + 1]) );

            }
            for (int i = 0; i < M - 1; i++)
            {
                
                if (  mushrooms[i] - minRate < 0)
