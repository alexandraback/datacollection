...
namespace GCJ
{
    ...
    public class InfinitePancakes : GCJBase
    {
        ...
        private void minTimeDP()
        {
            // Min time to break down stack of size X into stacks of size Y
            for (int x = 2; x < N+1; x++)
            {
                for (int y = 1; y < x; y++)
                {
                    ...
                    for (int z = 1; z+z <= x; z++)
                    {
                        min[x, y] = Math.Min(min[x, y], min[x - z, y] + min[z, y] + 1);
                    }
                }
            }
        }

        private long UseDP(int[] pancakesArray)
        {
            long best = 1000000;
