  ...
  class Program
  {
    ...
    static void Main(string[] args)
    {
      ...
      for (int iTestcase = 0; iTestcase < Helper.NofTestcases; iTestcase++)
      {
        ...
        int nofInitialDiners = Helper.GetInteger(Helper.GetLine(), 0);
        int[] pancakeDistribution = Helper.GetIntArray(Helper.GetLine());
        Debug.Assert(nofInitialDiners == pancakeDistribution.Length);

        int maxP = pancakeDistribution.Max();
        int minTime = maxP;

        for (int maxPanCakeToEat = 2; maxPanCakeToEat <= maxP; maxPanCakeToEat++)
        {
          int nofSpecialMinutes = 0;
          for (int diner = 0; diner < nofInitialDiners; diner++)
          {
            int pancakesOnPlate = pancakeDistribution[diner];
            int nofMoves = (pancakesOnPlate + maxPanCakeToEat - 1) / maxPanCakeToEat - 1;
            nofSpecialMinutes += nofMoves;
          }

          int timeRequired = nofSpecialMinutes + maxPanCakeToEat;
          if (timeRequired < minTime)
            minTime = timeRequired;
        }

        Helper.WriteOutput(String.Format("Case #{0}: {1}", iTestcase + 1, minTime));
      }
