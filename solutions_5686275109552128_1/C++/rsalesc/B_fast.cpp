...
void solve(int tn){
	...
	for(int high = 1; high < max_p; high++){
		int special = 0;
		for(int i = high+1; i <= max_p; i++){
			if(h[i] == 0) continue;
			int l = 2, r = i;
			// cerr << "INF" << endl;
			while(l < r){
			...
			//cerr << i << " divided by " << l << endl;
			special += (l-1)*h[i];
		}
		
		//cerr << "Special = " << special << " when high = " << high << endl;
		ans = min(ans, special + high);
	}
