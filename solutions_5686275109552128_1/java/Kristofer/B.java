    ...
    public class Solver {
        ...
        public String solve() throws Exception {
            ...
            for (int i = 0; i < D; i++) {
                int p = getInt();
                plates[i] = p;
                worstCase = Math.max(worstCase, p);
            }
            int best = worstCase;
            for (int maxPerPlate = 1; maxPerPlate < worstCase; maxPerPlate++) {
                best = Math.min(best, solve(maxPerPlate));
            }
            return "" + best;
        }

        private int solve(int maxPerPlate) {
            int splits = 0;
            for (int plate : plates) {
                int splitsNeeded = (plate - 1) / maxPerPlate;
                splits += splitsNeeded;
            }
            return splits + maxPerPlate;
        }
    }
}
