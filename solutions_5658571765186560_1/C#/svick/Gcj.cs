...
namespace GCJ_base
{
    public abstract class Gcj
    {
        ...
        public static int[] ReadIntLine(TextReader reader)
        {
            return reader.ReadLine().Split().Select(int.Parse).ToArray();
        }

        public static long[] ReadLongLine(TextReader reader)
        {
            return reader.ReadLine().Split().Select(long.Parse).ToArray();
        }
    }
}