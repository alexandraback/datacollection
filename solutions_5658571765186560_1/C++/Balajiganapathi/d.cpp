...
bool brute2(int cnt) {
    ...
    fo(i, cnt) {
        fo(d, 4) {
            int ni = cur_xm[i].fi + di[d], nj = cur_xm[i].se + dj[d];
            if(ni >= 0 && ni < r && nj >= 0 && nj < c && b[ni][nj] == 0) {
                b[ni][nj] = 1;
                cur_xm[cnt] = mp(ni, nj);
                if(brute2(cnt + 1)) return true;
                b[ni][nj] = 0;
            }
        }
    }
...
int possible() {
    ...
    trace2(si(pos), si(xminos));
    return si(pos) == si(xminos);
}

bool wild(int x, int r, int c) {
    if(c < r) swap(r, c);
