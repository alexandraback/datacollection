...
int main() {
  ...
  FOR(test, 1, numTests + 1) {
    in >> X >> R >> C;
    int minS = MIN(R, C), maxS = MAX(R, C);
    bool ret = true;
    if (X > 6 || (C * R) % X != 0 || maxS < X || (X + 1) / 2 > minS) ret = false;
    else {
      if (X == 4 && minS <= 2) ret = false;
      if (X == 5 && minS == 3 && maxS == 5) ret = false;
      if (X == 6 && minS <= 3) ret = false;
    }

    out << "Case #" << test << ": " << (ret ? "GABRIEL" : "RICHARD") << endl ;
  }
