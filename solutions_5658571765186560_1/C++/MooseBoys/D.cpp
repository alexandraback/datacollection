...
int CodeJamMain()
{
    ...
    for (int caseNum = 0; caseNum < T; caseNum++)
    {
        ...
        // X = 4 => (R*C)%4==0 && R>2 && C>2 ? GABRIEL : RICHARD (T blocks 4x2)
        // X = 5 => (R*C)%5==0 && R>3 && C>3 ? GABRIEL : RICHARD (W blocks 5x3)
        // X = 6 => (R*C)%6==0 && R>3 && C>3 ? GABRIEL : RICHARD (many ways to block 6x3)
        // X >= 7 => RICHARD (use the piece with the hole)
        ...
        if (X == 5)
        {
            answer = ((R*C) % 5 == 0 && R > 3 && C > 3) ? gab : ric;
        }
        if (X == 6)
        {
            answer = ((R*C) % 6 == 0 && R > 3 && C > 3) ? gab : ric;
        }
        if (X >= 7)
        {
            answer = ric;
