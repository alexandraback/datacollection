...
bool CheckConnectedComponents(const vector< vector<bool> > &board, const int size) {
    vector< vector<bool> > used = board;
    for (int x = 0; x < int(board.size()); ++x)
        for (int y = 0; y < int(board[0].size()); ++y)
            used[x][y] = !used[x][y];
    for (int x = 0; x < int(board.size()); ++x)
        for (int y = 0; y < int(board[0].size()); ++y)
            if (used[x][y] && DFS(used, x, y) % size != 0)
                return false;
    return true;
}
        ...
        for (int x = 0; x < rows; ++x) {
            for (int y = 0; y < columns; ++y) {
                ...
                if (valid) {
                    ...
                    if (CheckConnectedComponents(board, int(p.size())) && CoverBoard(board, coveredArea + int(p.size()), polyominoes))
                        return true;
                    for (const auto &c: p)
