...
public class PD implements Runnable {
    ...
    private boolean subsolve(int x, int r, int c) {
        ...
        if ((r * c) % x != 0) {  // number of cells are not divisible by x
            return true;
        }
        ...
        if (x == 4) {  // 4-omino
            ...
            // 3 x 4 can be covered
            return false;
        }

        if (x == 5) {
            if (minrc == 3 && maxrc < 10) {   // W is messing with parity for maxrc < 10
                return true;
            }
            // 3 x 10 can be covered, 4 x 5 too
            return false;
        }

        if (x == 6) {
            if (minrc == 3) {  // another T is messing with parity
                return true;
            }
            // 4 x 6 can be covered
            return false;
        }
