            ...
            if (x == 2 * min - 1) {
                ...
                for (int left = 0; left <= min - 1; ++left) {
                    ...
                    if (test(x, Math.max(r, c), Math.min(r, c), left, min - left, (min - 1) * left))
                        win = true;
                    if (left > 0 && min > 1 && test(x, Math.max(r, c), Math.min(r, c), left, min - 1 - left, (min - 1) * left - 1))
                        win = true;
                    if (left < min - 1 && min > 1 && test(x, Math.max(r, c), Math.min(r, c), left, min - 1 - left, (min - 1) * left))
                        win = true;
                }
            }
