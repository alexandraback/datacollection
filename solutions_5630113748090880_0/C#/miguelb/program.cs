...
namespace CodeJam
{
    ...
    public class Engine
    {
        ...
        private bool Solve<T>(Type type)
        {
            ...
            File.Copy ( string.Format ( "{0}\\program.cs" , Path.GetDirectoryName( Path.GetDirectoryName( Path.GetDirectoryName( GetType().Assembly.Location)))),
                string.Format ( "{0}\\program.cs" , Folder)
                ,true);
...
1 2 3";
        ...
        public override void Read(FileReader r)
        {
            ...
            for (int i = 0; i < 2 * N - 1; i++)
                lists.Add(r.ReadSpaceSeparatedIntegers());

        }

        protected override string SolveSmall()
        {
            int[] count = CreateArray<int>(2500, 0);
