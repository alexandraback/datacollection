...
public class Main {
    ...
    private static String SolveProblemB(int caseNumber, Scanner in)
    {
        ...
        if(in.hasNext())
        {
            ...
            //Solve
            println("2.Solve:");
            final int maxH = 4000 * 2;
            int[] counts = new int[maxH];
            for (ArrayList<Integer> row : form)
                for (int height : row)
                    counts[height]++;
    ...
    Boolean check(Vector<Integer> bff, Vector<Integer> circle)
    {
        for (int i = 0; i < (int)circle.size(); i++)
        {
            int prev = (i + circle.size() - 1) % circle.size();
            int next = (i + 1) % circle.size();
            if (bff.get(circle.get(i)) != circle.get(next) &&
                    bff.get(circle.get(i)) != circle.get(prev))
            {
                return false;
            }
        }
