...
void bfs(){
    ...
    for (int i = 0; i < n; i++)
        for (int j = 0; j < m; j++)
            if (used[i][j])
                dist[h][i][j] = 0, q.pb(mp(h, mp(i, j)));
    while (!q.empty()){
        int t = q.front().fs, x = q.front().sc.fs, y = q.front().sc.sc;
        q.pop_front();
        for (int i = 0; i < 4; i++){
            int nx = x + cx[i], ny = y + cy[i];
            if (nx < 0 || nx >= n || ny < 0 || ny >= m)
            ...
            if (cango(x, y, nx, ny, t)){
                ...
                if (t >= f[x][y] + 20)
                    ...
                    cost = 10;
                if (dist[max(0, t-cost*10)][nx][ny] > dist[t][x][y] + cost){
                    dist[max(0, t-cost*10)][nx][ny] = dist[t][x][y] + cost;
                    q.push_back(mp(max(0, t-cost*10), mp(nx, ny)));
                }
            }
        }
        for (int i = t-1; i >= 0; i--)
            if (dist[i][x][y] > dist[t][x][y] + 1.0*(t-i)/10)
                dist[i][x][y] = dist[t][x][y] + 1.0*(t-i)/10, q.pb(mp(i, mp(x, y)));
    }
}
