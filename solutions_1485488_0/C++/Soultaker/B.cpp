 ...
 * platform. I use GCC 4.4.1 on Linux 2.6 with an x86-64 processor.
 *
 ...
 * July 2008, September 2009, June 2011
 */
...
static Job *create_job(int id, Case *test_case)
{
    /* Open output file */
...
RealCase::RealCase()
{
    ...
    HC = VVI(R, VI(C));
    HF = VVI(R, VI(C));
    REP(r, R) REP(c, C) cin >> HC[r][c];
    REP(r, R) REP(c, C) cin >> HF[r][c];
}
...
{
    if (s.t != t.t) return s.t > t.t;
    if (s.r != t.r) return s.r > t.r;
    if (s.c != t.c) return s.c > t.c;
    return false;
}

void RealCase::solve(Result &result)
{
    // Note: all times are in tenths of seconds, so that the water level falls
    // with exactly 1 centimeter per unit of time.

    // Dijkstra's algorithm.
    priority_queue<State, vector<State>, greater<State> > Q;
    VVI min_time(R, VI(C, inf));
    State initial = { 0, 0, 0 };
    min_time[0][0] = 0;
    Q.push(initial);
    while (!Q.empty())
    {
        State s = Q.top();
        Q.pop();
        if (s.t != min_time[s.r][s.c]) continue;

        if (s.r == R - 1 && s.c == C - 1)
        {
            result.set(STR(s.t/10) + '.' + STR(s.t%10));
            return;
        }
        ...
        {
            int nr = s.r + DR[dir], nc = s.c + DC[dir];

            if ( nr < 0 || nr >= R ||
                 nc < 0 || nc >= C ) continue;  // out of grid

            // The floor height of your current square must be at least 50 cm
            // lower than the ceiling height of the adjacent square.
            if (HC[nr][nc]   - HF[s.r][s.c] < 50) continue;

            // The floor height of the adjacent square must be at least 50 cm
            // lower than the ceiling height of the adjacent square.
            if (HC[nr][nc]   - HF[nr][nc]   < 50) continue;

            // The floor height of the adjacent square must be at least 50
            // centimeters below the ceiling height of your current square.
            if (HC[s.r][s.c] - HF[nr][nc]   < 50) continue;

            int t;  // time at destination in tenths of seconds!
            if (s.t == 0 && HC[nr][nc] - H >= 50)
            {
                t = 0;   // can move here before the tide starts to fall
            }
            else
            {
                // time at which destination first becomes accessible
                t = max(s.t, H + 50 - HC[nr][nc]);
                if (H - t - HF[s.r][s.c] >= 20)
                    t += 10;  // move by kayak -- 1 second == 10 tenths
                else
                    t += 100;  // move by foot -- 10 seconds = 100 tenths.
            }
