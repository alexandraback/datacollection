...
double canMove(int x, int y, int * c, int * f, int w, int h, int H, double ct, int dir)
{
	int cc = c[x+y*w];
	int cf = f[x+y*w];
	int nc = c[x+y*w+dir];
	...
	if(nc < cf+50 || nc < nf+50 || cc < nf+50) return 1e100;
	if(nc >= ch + 50)
	{
		if(ct == 0) return 0;
		else if(ch - cf >= 20) return ct+1;
		return ct+10;
	}
	ct += (ch+50-nc)/10.;
	ch = H-ct*10;
	if(ch - cf >= 20) return ct+1;
	return ct+10;
//
...
void move0(int x, int y, double * t, int * c, int * f, int w, int h, int H)
{
	if(x < w-1 && t[x+y*w+1] > 0)
	{
		double ct = canMove(x,y,c,f,w,h,H,0,1);
		if(ct == 0)
		{
			t[x+y*w+1] = 0;
	...
	{
		double ct = canMove(x,y,c,f,w,h,H,0,w);
		if(ct == 0)
		{
			t[x+y*w+w] = 0;
...
double nextTime(double * t, int w, int h, double ct)
{
	double nt = 1e100;
...
void moveT(double * t, int * c, int * f, int w, int h, int H, double ct)
{
	for(int x = 0; x<w; x++)
	for(int y = 0; y<h; y++)
	{ 
		if(t[x+y*w] == ct)
		{
			...
			{
				double nt = canMove(x,y,c,f,w,h,H,ct,1);
			...
			{
				double nt = canMove(x,y,c,f,w,h,H,ct,w);
