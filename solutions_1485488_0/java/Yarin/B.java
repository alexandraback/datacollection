...
//		io = new Kattio(new FileInputStream("src/round1/B-large.in"), new FileOutputStream("src/round1/B-large.out"));
...
//				int lo = water - earliest[ct.coord.y][ct.coord.x];
//				while (!canGo(ct.coord.x, ct.coord.y, nx, ny, lo)) {
//					lo--;
//				}
...
//		int minCeil = Math.min(ceil[y][x], ceil[ny][nx]);
//		int maxFloor = Math.max(Math.max(floor[y][x], floor[ny][nx]), water);
