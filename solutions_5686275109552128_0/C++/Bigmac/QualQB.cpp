...
int main(int argc, char **argv)
{
	...
	for(int it_t = 1; it_t <= in_t; it_t++)
	{
		...
		FOR(k,0,vald)
		{
			int newplate = readInt();
			maxval = max(maxval, newplate);
			v[newplate]++;
		}
		orig_maxval = maxval;
		int min_move = maxval;
		...
		{
			int overhead = 0;
			for(int m = maxval; m > 0; m--)
			{
				overhead += (ceil(m / (float)k) - 1) * v[m];
			}
			int mv_cnt = overhead + k;
			min_move = min(mv_cnt, min_move);
		}
		
        //===========================================solve function  end ================================//
        fprintf(fo, "Case #%d: %d", it_t, min_move);
		fprintf(fo, "\n");
	}
	closeIOFile();
}