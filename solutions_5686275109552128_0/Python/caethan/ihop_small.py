...
def read_input(infile):
    ...
    D = read_int()
    pancakes = read_ints().tolist()
    assert D == len(pancakes)
    return pancakes

@memoize    
def split_count_from_maxval(num, maxval):
    return int(np.ceil((num - maxval) / maxval))
    
def eat_all(pancakes, maxval):
    #print pancakes
    if not pancakes:
        return 0
        
    maximum_stack = max(pancakes)
    
    splits = 0
    for stack in pancakes:
        splits += split_count_from_maxval(stack, maxval)
    return splits + maxval
...
def solve_case(pancakes):
    ...
    best = max(pancakes)
    for maxval in range(max(pancakes) - 1, 0, -1):
        new = eat_all(pancakes, maxval)
        if new < best:
            print new, best, maxval
            best = new
    return best
