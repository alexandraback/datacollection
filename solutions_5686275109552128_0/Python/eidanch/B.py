...
def handle_case(case):
    ...
    """
    special_minutes = 0
    result = minutes_left(lst)
    for i in xrange(max(lst)):
        newlst, delta = try_improve(lst)
        special_minutes += delta
        lst = newlst
        if special_minutes + minutes_left(lst) < result:
            result = special_minutes + minutes_left(lst)
    """
    return backtrack0(dic)
