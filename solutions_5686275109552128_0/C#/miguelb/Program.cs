...
namespace CodeJam
{
    ...
    class InfiniteHouseOfPancakes: BaseGoogleJamProblem
    {
        Dictionary<string, int> cache = new Dictionary<string, int>();
        int Solve(List<int> pancakes)
        {
            int min = pancakes.Max();
            if (min == 0)
                return min;
            string key = "";
            foreach (var pancacke in pancakes)
                key += pancacke.ToString();
            int result;
            ...
            for (int i = 0; i < pancakes.Count; i++)
            {
                if (pancakes[i] > 3)
                {
                    List<int> newPancakes = new List<int>(pancakes);
                    if (pancakes[i] == 9)
                    {
                        newPancakes[i] -= 3;
                        newPancakes.Add(3);
                    ...
                    {
                        int leftOver = newPancakes[i] / 2;
                        newPancakes[i] -= leftOver;
                        newPancakes.Add(leftOver);
                    }
                    newPancakes.Sort(Compare);
                    min = Min(min, 1 + Solve(newPancakes));
                }
            }
        ...
        int SolveSmall ( int[] pancakes)
        {
            var list = new List<int>(pancakes);
            list.Sort(Compare);
            return Solve(list);
        }
        protected override void SolveProblemCase()
        {
            var diners = ReadLineAsInt();
            var pancakes = ReadSpaceSeparatedIntegers();
            int r = SolveSmall(pancakes);
            AddCaseResponse(r);
        }
    }
    ...
    class Program
    {
        ...
        {
            var problem = new InfiniteHouseOfPancakes();
            
            //problem.Solve("c:\\temp\\input.txt");
            problem.Solve("c:\\temp\\B-small-attempt1.in");
        }
    }
