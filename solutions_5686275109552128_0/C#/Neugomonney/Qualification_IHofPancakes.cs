...
{
    internal class Qualification_IHofPancakes
    {
        private const string ModuleName = "Qualification_IHofPancakes";
        ...
        private static string ProcessCase(int id, List<string> parameters)
        {
            ...
            var plates = parameters[1].Split(new[] { ' ' }).Select(int.Parse).ToList();
            var res = Solution(int.Parse(parameters[0]), plates);

            return String.Format("Case #{0}: {1}", id, res);
        }

        private static int Solution(int people, List<int> plates)
        {
            var worst = plates.Max();
            var result = GetBest(plates);

            if (result > worst) Console.WriteLine("===> Solution ({0}) is worse than worst case ({1})", result, worst);
            
            return result;
        }

        private static int GetBest(List<int> plates)
        {
            var max = plates.Max();
            if (max <= 3) return max;
            
            var reduced = plates.Select(p => p - 1).ToList();
            var best = 1 + GetBest(reduced);

            var half = max / 2;
            var maxCount = plates.Count(p => p == max);
            if (maxCount >= half) return best;

            var halved = new List<int>(plates);
            var i = halved.FindIndex(p => p == max);
            halved.Add(0);
            
            for (var h = 1; h <= half; h++)
            {
                halved[i] = h;
                halved[halved.Count - 1] = max - h;
                var interrupted = 1 + GetBest(halved);
                if (interrupted < best) best = interrupted;
            }
            return best;
        }
