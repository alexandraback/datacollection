...
public class B {
	...
	static String fileName = B.class.getSimpleName() + "-small-attempt0";
	...
	public void solve() throws Exception {
		...
		for (int i=2; i<dp.length; ++i) for (int j=1; j<i; ++j) {
			int best = 1000;
			for (int k=1; k<i; ++k) best = min(best, dp[i-k][j]+dp[k][j]);
			dp[i][j] = 1+best;			
		}
		...
		nextCase: for(int testCase=1; testCase<=testCasesCnt; testCase++) {
			...
			for (int i=1; i<best; ++i) {
				int overhead = 0;
				for (int j=0; j<n; ++j) overhead += dp[p[j]][i];
				best = min(best, i+overhead);
			}
			println(best);
