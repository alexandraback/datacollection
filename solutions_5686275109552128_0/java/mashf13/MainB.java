...
public class MainB {
  ...
  static int process() {
    ...
    for(int c = 1; c <= cases; c++) {
      ...
      PriorityQueue<Integer> pq = new PriorityQueue<Integer>(2 * sz, new Comparator<Integer>() {
      ...
      int stops = 0, tm = pq.peek();
      while(pq.size() > 0) {
        Integer mx = pq.peek();
        if(mx.intValue() > 3) {
          int a = mx.intValue() / 2, b = mx.intValue() - (mx/2);
          pq.remove();
          pq.add(a);
          pq.add(b);
          stops++;
      ...
      System.out.println(String.format("Case #%d: %d", c, tm));
    }
    return 0;
  }
