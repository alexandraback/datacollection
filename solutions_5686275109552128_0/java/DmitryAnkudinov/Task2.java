...
public class Task2 {
	private static int Splits[][] = null;
	
	private static void GenerateSplits()
	{
		Splits = new int[1000][];
		for (int i = 0; i < 1000; ++i)
		{
			Splits[i] = new int[i + 1];
			for (int j = 0; j < i + 1; ++j)
			{
				Splits[i][j] = (i + 1) / (j + 1);
				if ((i + 1) % (j + 1) != 0) ++Splits[i][j];
			}
		}
	}
	
	private static int calculateOptimalTime(ArrayList<Integer> P, int unprocessedIndex, int timeSpentRedistributing, int maxAmountToEat)
	{
		int currentResult = timeSpentRedistributing + maxAmountToEat; 
		if (unprocessedIndex == -1)
			return currentResult;
		int currentValue = P.get(unprocessedIndex);
		if (maxAmountToEat > currentValue)
			return currentResult;
		int[] currentSplit = Splits[currentValue - 1];
		int localMinimumTime = 1000;
		
		for (int i = 0; i < currentSplit.length; ++i)
		{
			int optimalTimeForPreviousIndices = calculateOptimalTime(P, unprocessedIndex - 1, timeSpentRedistributing + i, 
																	 Math.max(currentSplit[i], maxAmountToEat));
			if (optimalTimeForPreviousIndices < localMinimumTime)
				localMinimumTime = optimalTimeForPreviousIndices;
		}
	...
	public static void main(String[] args) throws FileNotFoundException {
		GenerateSplits();
		File file = new File("./data/B-small-attempt3.in");
		PrintStream out = new PrintStream(new FileOutputStream("./data/B-small-attempt3.out"));
		System.setOut(out);
		...
		for (int i = 0; i < T; ++i)
		{
			...
			result[i] = calculateOptimalTime(P, P.size() - 1, 0, 0);
		}	
...
1
3
Problem Infinite House of Pancakes
