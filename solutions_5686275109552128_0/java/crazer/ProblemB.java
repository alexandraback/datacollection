...
public class ProblemB {
	...
	public static void main(String[] args) throws IOException {
		...
		for (int t = 1; t <= T; t++) {
			...
			count = new int[1001];
			MaxPQ<Integer> plates = new MaxPQ<>();
			...
			noOfMinutes = minTime(plates);
			
			answer = "Case #" + t + ": " + noOfMinutes;
	...
	static private int minTime(MaxPQ<Integer> plates) {
		if (plates.size() == 0) {
			return 0;
		}

		int maxItemsPlate = plates.delMax();
		int countOfMaxItems = count[maxItemsPlate];
		count[maxItemsPlate] = 0;
		...
		/*
		for (int i = 0; i < maxItemsPlate; i++) {
			
		}
		*/
		
		int subPlate1 = (int) Math.sqrt(maxItemsPlate);
		int parts = maxItemsPlate / subPlate1 + (maxItemsPlate % subPlate1 == 0 ? 0 : 1);
		
		
		int subPlate2 = maxItemsPlate % subPlate1;
		
		if (count[subPlate1] == 0) {
			plates.insert(subPlate1);
		}
		count[subPlate1] = parts;
		
		
		
		if (subPlate2 != 0) {
			if (count[subPlate2] == 0) {
				plates.insert(subPlate2);
			}
			count[subPlate2]++;
		}
		
		
		int alternativeMin = countOfMaxItems * (parts - 1) + minTime(plates);
		
		
		return maxItemsPlate < alternativeMin ? maxItemsPlate : alternativeMin;
	}
