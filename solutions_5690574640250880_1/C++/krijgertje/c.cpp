...
bool been[50][50];
char z[50][50];
int qx[2500],qy[2500],qh,qt;

void check() {
	int cnt=0; REP(i,h) REP(j,w) if(g[i][j]=='*') ++cnt; assert(cnt==mines);
	int sx=-1,sy=-1; REP(i,h) REP(j,w) if(g[i][j]=='c') { assert(sx==-1&&sy==-1); sx=i; sy=j; }
	REP(i,h) REP(j,w) z[i][j]=g[i][j];
	REP(i,h) REP(j,w) if(z[i][j]!='*') { int q=0; FORE(dx,-1,+1) FORE(dy,-1,+1) { int nx=i+dx,ny=j+dy; if(0<=nx&&nx<h&&0<=ny&&ny<w&&g[nx][ny]=='*') ++q; } z[i][j]='0'+q; }
	memset(been,false,sizeof(been));
	qh=qt=0;
	qx[qh]=sx; qy[qh]=sy; been[sx][sy]=true; ++qh;
	while(qt<qh) {
		int x=qx[qt],y=qy[qt]; ++qt;
		if(z[x][y]!='0') continue;
		FORE(dx,-1,+1) FORE(dy,-1,+1) {
			int nx=x+dx,ny=y+dy; if(0<=nx&&nx<h&&0<=ny&&ny<w&&g[nx][ny]!='*'&&!been[nx][ny]) {
				qx[qh]=nx; qy[qh]=ny; been[nx][ny]=true; ++qh;
			}
		}
	}
	REP(i,h) REP(j,w) assert(been[i][j]==(g[i][j]!='*'));
}
