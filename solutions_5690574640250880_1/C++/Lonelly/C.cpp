...
void Check(int i, int j, vector<vector<char> >& map, vector<vector<bool> >& checked)
{
	if (i < 0 || j < 0 || i >= map.size() || j >= map[0].size() || checked[i][j])
	{
		return;
	}
	if (map[i][j] == '*')
	...
	if (NoMineNearby(i, j, map))
	{
		...
		Mark(i - 1, j, map);
		Mark(i, j - 1, map);
		Check(i, j - 1, map, checked);
		Check(i - 1, j, map, checked);
		Check(i - 1, j - 1, map, checked);
	}
}
