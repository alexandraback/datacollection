...
bool Next2(s32 i, s32 j, s32 m) {
    if (m==0) {
        //Print();
        if (Check()) return true;
        return false;
    }
    if (C*(R-i) < m) return false;
    if (i==R) return false;
    bool b;
    for (s32 k=min(m,j); k>=0; k--) {
        FOR (l,0,k) f[i][l] = 1;
        FOR (l,k,C) f[i][l] = 0;
        b = Next2(i+1, k, m-k);
        if (b) return true;
...
void Smart()
{
    memset(f,0,sizeof(s32)*50*50);
    if (!Next2(0,C,M)) cout << "Impossible";
}
