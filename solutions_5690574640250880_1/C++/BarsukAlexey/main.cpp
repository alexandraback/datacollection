...
bool was_solution(int rows, int cols, int mines, int was = 0) {

    if (rows < 1 || cols < 1) return false;
    if ((rows == 1 || cols == 1) && was) return false;
    if (was > 3) return false;
    ...
    if (was_unopen) {
        if (need_mines - cols > 0 && was_solution(rows - 1, cols, need_mines - cols, was++)) {
        ...
        if (need_mines - rows > 0 && was_solution(rows, cols - 1, need_mines - rows, was++)) {
            for (int i = rows; i >= 1; i--) {
                for (int j = cols; j > 1; j--) {
...
int main()
{
    ...
    for (int i = 0; i < test; i++) {
        ...
        cout << "Case #" << i + 1 << ":" << endl;
        if (!was_solution(rows, cols, mines, 0)) {
            swap(rows, cols);
            if (!was_solution(rows, cols, mines, 0)) {
                cout << "Impossible" << endl;
                cout.flush();
