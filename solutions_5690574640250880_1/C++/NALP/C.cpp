...
int n, m, c, d[MAXN][MAXN][MAXN * MAXN];
...
bool solve(int y, int p, int have) {
	if (have == 1 && y != 0) return false;
	...
	ans = 0;
	for(int i = 1; i <= min(p, have); i++) {
		if (y == 1 && i != p) continue;
		if (solve(y + 1, i, have - i)) {
			ans = 1;
		}
	}
...
void restore(int y, int p, int have, vector<int> &res) {
	if (have == 1 && y != 0) throw;
	if (have == 0) return;
	if (y == m) throw;

	bool ans = solve(y, p, have);
	for(int i = 1; i <= min(p, have); i++) {
		if (y == 1 && i != p) continue;
		if (solve(y + 1, i, have - i)) {
			res.pb(i);
			return restore(y + 1, i, have - i, res);
		}
	}
