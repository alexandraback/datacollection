...
// Vector typedefs
typedef vector<int> intv; typedef vector<llint> llintv; typedef vector<double> doublev; typedef vector<char> charv;
...
void run(istream & fin, ostream & fout)
{
	...
	for(int t:Range(cases)) {
		int w, h, m;
		fin >> h >> w >> m;
		vector<charv> grid(h,charv(w,'.'));
		int cw = w, ch = h;
		while( m >= min(cw,ch) ) {
			int rows_used = 0;
			if( ch >= cw && ch > 2 && m >= cw ) {
				rows_used = 1;
				if( ch-2 == rows_used && cw > 2 && cw != ch && ((m - rows_used * cw) % 2 == 1) )
					rows_used--;
				for(int y:Range(ch-rows_used,ch))
					for(int x:Range(cw))
						grid[y][x] = '*';
				ch -= rows_used;
				m -= rows_used * cw;
			...
			if( cw >= ch && cw > 2 && m >= ch ) {
				cols_used = 1;
				if( cw-2 == cols_used && ch > 2 && cw != ch && ((m - cols_used * ch) % 2 == 1) )
					cols_used--;
				for(int y:Range(0,ch))
					for(int x:Range(cw-cols_used,cw))
						grid[y][x] = '*';
				cw -= cols_used;
				m -= cols_used * ch;
			}
...
//			fout << grid[y] << "\n";
		...
		if( m != 0 ) {
			...
			else if( cw == 2 && ch == 2 && m == 3 ) {
				grid[1][1] = grid[1][0] = grid[0][1] = '*';
				m = 0;
			} else if( cw > 2 && ch > 2 ) {
				int row=ch-1, col=cw-1;
				while( m > 0 && row > 1 ) {
					if( col == 1 ) {
						col = cw-1;
						row--;
