...
def solve(N, M, boxes, toys):
    ...
    def rec(i, j):
        ...
        if box.oid == toy.oid:
            debug("========Fast Forward ", boxi, " ", toyi)
            while boxi != -1 or toyi != -1:
                debug(boxsum, " ", toysum, " ", boxi, " ", toyi)
                if toyi == -1 or (boxsum <= toysum and not boxi == -1):
                    tbox = boxes[boxi]
                    debug(tbox)
                    if tbox.oid == oid:
                        boxsum += tbox.length
                        debug("rec ", boxi, " ", toyi, " ", min(toysum, boxsum), " boxi--")
                    ...
                    if ttoy.oid == oid:
                        toysum += ttoy.length
                        debug("rec ", boxi, " ", toyi, " ", min(toysum, boxsum), " toyi--")
                        if min(toysum, boxsum) != 0:
