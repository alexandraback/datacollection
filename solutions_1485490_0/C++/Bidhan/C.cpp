...
i64 call(int i,int j){
    ...
    if(bb.xx!=tt.xx) ret=max(ret, max(call(i+1,j),call(i,j+1)) );
        ...
        bb=box[i1], tt=toy[i2];
        while(true){
            //if(!i && !j) out<<bb.yy<<endl;
            if(bb.yy==tt.yy) {  tot+=bb.yy; ret=max(ret,call(i1+1,i2+1)+tot); break; }
            else if(bb.yy<tt.yy) {
                //if(!i && !j) cout<<"2"<<endl;
                tot+=bb.yy;
                tt.yy-=bb.yy;
                //if(!i && !j) cout<<"from "<<i<<" "<<j<<" going to "<<i1+1<<" "<<i2+1<<endl;
                ret=max(ret,call(i1+1,i2+1)+tot);
                //if(!i && !j) cout<<"returned to "<<i<<" "<<j<<" now ret="<<ret<<endl;
                i1++;
                while(i1<n && box[i1].xx!=tt.xx) i1++;
                if(i1<n) bb=box[i1];
                //if(!i && !j) cout<<"new "<<i1<<"-- "<<i2<<endl;
            }
            else{
                //if(!i && !j) cout<<"3"<<endl;
                tot+=tt.yy;
