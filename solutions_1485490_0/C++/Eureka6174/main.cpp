...
i64 gao(int n, int m)
{
    ...
    if(ta[n] != tb[m])
    {
        dp[n][m] = max(gao(n-1, m), gao(n, m-1));
    }
    else
    {
        i64 t = ta[n];
        dp[n][m] = 0;
        checkmax(dp[n][m], gao(n-1, m));
        checkmax(dp[n][m], gao(n, m-1));
        i64 suma = 0, sumb = 0;
        for(int i=n; i>0; --i)
        {
            if(ta[i] != t) continue;
            ...
            for(int j=m; j>0; --j)
            {
                if(tb[j] != t) continue;
                sumb += nb[j];
                checkmax(dp[n][m], gao(i-1, j-1) + min(suma, sumb));
            }
        }
    }
    return dp[n][m];
