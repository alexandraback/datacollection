...
int check(const int type, int const idxM, int* N3, int* M3)
{
  int i;
  for(i = idxM; i < M; ++i)
  {
...
int solve(const int idxN, const int idxM, int* N3, int* M3)
{
  if(idxN >= N)
    return 0;
  if(idxM >= M)
  ...
  // choose idxN
  int idxM2 = check(N2[idxN], idxM, N4, M4);
  int get = 0;
  if(idxM2 != M) {
    if(N4[idxN] > M4[idxM2]) {
      get += M4[idxM2];
      N4[idxN] -= M4[idxM2];
      M4[idxM2] = 0;
      get += solve(idxN, idxM2 + 1, N4, M4);
    } else {
      get += N4[idxN];
      N4[idxN] = 0;
      M4[idxM2] -= N4[idxN];
      get += solve(idxN + 1, idxM2, N4, M4);
    }
  }
  // don't choose idxN
  int get2 = solve(idxN + 1, idxM, N4, M4);
  if(get > get2) {
    return get;
  } else {
    return get2;
