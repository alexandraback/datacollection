...
int N, M;
uint64 a[100]; int A[100];
uint64 b[100]; int B[100];
			...
			//for (int k=j; k<=M; ++k) {
				...
				if (N>1)
				{
					...
					for (int ii=i; ii<j; ++ii) {
						if (B[ii] == A[1]) cnt0 += b[ii];
					}

					if (j>i) ans += min(a[1], cnt0);
				...
				if (N>2)
				{
					...
					if (A[1] == A[2] && j != i) cnt0 = r1;

					for (int ii=j; ii<M; ++ii) {
						if (B[ii] == A[2]) cnt0 += b[ii];
					}
