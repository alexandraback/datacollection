...
#define repr(i,b,e) for (long long i=b; i<=e; i++)
...
pair<long long,long long> boxes[101], toys[101];
#define num first
#define tipo second

struct Stato {
    long long d1, q1, d2, q2;
    Stato(long long d1, long long q1, long long d2, long long q2) : d1(d1), q1(q1), d2(d2), q2(q2) {}
...
long long f(long long d1, long long q1, long long d2, long long q2) {
    //cout << d1 << " " << q1 << " " << d2 << " " << q2 << "\n";
    if (d1 >= n || d2 >= m) return 0;
    assert(q1 > 0 && q2 > 0);
    Stato s = Stato(d1, q1, d2, q2);
    if (dp.count(s)) return dp[s];
    long long t1 = boxes[d1].tipo;
    long long t2 = toys[d2].tipo;
    long long r = 0;
    if (t1 != t2) {
