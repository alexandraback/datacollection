...
int sorta(const void *x, const void *y) {
  return (a[*(int*)x] - a[*(int*)y]);
}

int sortb(const void *x, const void *y) {
...
{	
	...
	memset(f_a,0,sizeof(int)*N);
	//memset(f_b,0,sizeof(int)*N);

	qsort(i_a, N, sizeof(int), sorta);
	qsort(i_b, N, sizeof(int), sortb);
...
int main()
{
	//freopen("..\\B.in","r",stdin);freopen("..\\B-small.out","w",stdout);
	freopen("..\\B-small-attempt1.in","r",stdin);freopen("..\\B-small.out","w",stdout);
