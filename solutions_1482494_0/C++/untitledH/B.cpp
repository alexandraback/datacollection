	...
	fora(i,qq){
		...
		while(flag){
			...
			fora(j,ca){
				...
				if(star>=b[j] && b[j]!=12345 && a[j]!=12345){
					star+=2;
			...
			flag2=true;
			fora(k,ca){
				if(b[k]!=12345){flag2=false;break;}
			}
			if(buf==star){break;}
			buf=star;
		}

		if(flag2){GCJ(i,ans);}else{printf("Case #%d: Too Bad\n",i+1);}
