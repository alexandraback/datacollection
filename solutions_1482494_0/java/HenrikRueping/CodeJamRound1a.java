...
public class CodeJamRound1a {
    ...
    public String testCase2() throws IOException{
        ...
        String[] read;
       java.util.PriorityQueue PQ = new PriorityQueue();
        for(int i=0;i<N;i++){
            read = BR.readLine().split(" ");
            PQ.add(new Pair(Integer.parseInt(read[0]),Integer.parseInt(read[1])));
        }
        int i=0;
        while (!PQ.isEmpty()){
            Pair P=(Pair) PQ.poll();
        ...
        int[] stars =  new int[N];
        int curstars =0,steps=0;
        while(true)
        {
            //System.out.println(curstars);
            // Schaffe  alle b-machbaren level
            i=0;
            while(b[i]<=curstars){
                if(stars[i]!=2){
                    curstars+=2-stars[i];
                    stars[i]=2;
                    steps++;
                    if(curstars> 2*N) System.out.println("ERROR");
                    if(curstars>=2*N) return ""+steps;
                }
                i++;               
            }
            i=N-1;
            while((i>=0)&&((stars[i]>=1)||(a[i]>curstars)))i--;
            if(i==-1) return "Too Bad";
            steps++;
            curstars+=1;
            stars[i]=1;
        }
