...
/* @JUDGE_ID: 1848XX 2016CodeJamRound1A_C Java */
...
class Main {
	...
	for (int head=0; head<N; head++) {
	    ...
	    while (true) {
		int current = list1.size()-1;
		int prev = (current+list1.size()-1)%(list1.size());
		int next = (current+1)%(list1.size());
		if (table[list1.get(current)] == list1.get(next)
		    || table[list1.get(current)] == list1.get(prev)) {
		    validFlag=true;
		    break;
		}
		int needed = table[list1.get(current)];
		if (list1.contains(new Integer(needed))) {
		    validFlag=false;
		    break;
		}
		list1.add(new Integer(needed));
	    }
	    if (!validFlag) {
		continue;
	    }
		...
		if (logger != null) {
			...
			sb1.append(" ");
			sb1.append(list1.get(j).intValue()+1);
		    }
		    logInfo("Update Max: %1$s",sb1.toString());
		}
		maxN = list1.size();
	    }
	    ...
	    while (true) {
		boolean flag2=false;
		int lastone = list2.size()-1;
		int prevlastone = lastone-1;
		if (table[list2.get(lastone)] != list2.get(prevlastone)) {
		    break;
		}
		for (int newhead = 0; newhead < N; newhead++) {
		    if (list2.contains(new Integer(newhead))) {
			continue;
		    }
		    if (table[newhead] == list2.get(list2.size()-1)) {
			list2.add(newhead);
			flag2=true;
			...
			while (continueFlag) {
				...
				if (list2.contains(new Integer(k1))) {
				    continue;
				}
				if (table[k1] == list2.get(list2.size()-1)) {
				    list2.add(k1);
				    continueFlag=true;
				}
			    }
		...
		for (int newhead = 0; newhead < N; newhead++) {
		    ...
		    if (list2.contains(new Integer(table[newhead]))) {
			continue;
		    }
		    if (table[table[newhead]] == newhead) {
			list2.add(newhead);
			list2.add(table[newhead]);
			flag1=true;
		    }
		}
		if (!flag1) {
    ...
    private void solve_small(int[] table) {
	int N = table.length;
	int maxN = -1;
...
/**
 * Graph = (V,E) class.
 * VertexType can be any type.
 * WeightType should be only Long or Double. If you want to use Double,
 * please replace all "Long" to "Double" and "long" to "double".
 */
...
class Graph <VertexType, WeightType extends java.lang.Number> {
    ...
    /**
     * Add a vertex. Add existed vertex will be ignored.
     *
    ...
    /**
     * get the vertex index as Integer.
     *
    ...
    /**
     * Get the list of all the vertexs
     *
    ...
    /**
     * Add an edge. Adding an existed edge will overwrite the old weight.
     *
     ...
     * @from the from vertex
     * @to the to vertex
     * @return the weight of edge. null if the edge is not existed.
     */
    ...
    /**
     * Get an edge by vertex index.
     *
     * @i the from vertex int index.
     * @j the to vertex Integer index.
     * @return the weight of edge. null if the edge is not existed.
     */
    ...
    /**
     * Get a Set of edges. The Set contains the all of the index of vertexs
     * which v is connected to.
     *
     * @v the vertex.
	...
	long dFlow=0;
	for (flow=0; true; ) {
	    /* BFS to find a path. If no, break loop */
	    java.util.Arrays.fill(visited, false);
