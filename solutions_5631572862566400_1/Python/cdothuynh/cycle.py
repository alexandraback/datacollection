...
def find_loop2(cur_list, cur_set, graph):
    while graph[cur_list[-1]] not in cur_set:
        cur_set.add(graph[cur_list[-1]])
        cur_list.append(graph[cur_list[-1]])
    if cur_list[0] == graph[cur_list[-1]]:
        return [], cur_list
    else:
...
for t in xrange(T):
    ...
    for i in xrange(N):
        if (i+1) in in_loops or (i+1) in is_dangling:
            continue
        dangle, loop = find_loop2([i+1], set([i+1]), graph)
