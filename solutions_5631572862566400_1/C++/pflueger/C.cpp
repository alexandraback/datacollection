...
int N;
int F[1001];
int G[1001]; // This is the first "cyclic kid" in the path starting from kid k. 0 means not yet found.
int max_spur[1001]; //Longest path on non-cyclic kids leading to this point

bool seen[1001];
int findG(int k) {
	for (int kk=1; kk<=N; kk++) seen[kk] = false;
	int l = k;
	while (!seen[l]) {
		seen[l] = true;
		l = F[l];
...
void doCase(int case_number) {
	...
	for (int k=1; k<=N; k++) {
		...
		max_spur[G[k]] = max(max_spur[G[k]],cand);
	}
	
	int max_cycle = -1;
	int disjoint = 0; //Sum of sizes of disjoint pieces that can be stuck together

	//for (int k=1; k<=N; k++) err("max spur of %d is %d.\n",k,max_spur[k]);
	for (int k=1; k<=N; k++) {
		if (G[k] != k) {
			continue;
		}
		...
		if (length == 2 && k < F[k]) { //Second condition prevents double-counting
			disjoint += (2 + max_spur[k] + max_spur[F[k]]);
		}
		//err("Cycle length of %d is %d.\n",k,length);
	}
	int res = max(max_cycle,disjoint);
