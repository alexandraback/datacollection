...
int bff[2000];
bool visited[2000];
int findBigLoop(int st,int now,int cnt) {
    if(st == now) return cnt;
    if(visited[now]) return -1;
    visited[now] = true;
    return findBigLoop(st,bff[now],cnt+1);
}
int btSelfCom(vector<int> *rev,int now,int bf) {
    int toRet = 0;
    for(int i = 0;i < rev[now].size();i++) {
        if(rev[now][i] == bf) continue;
        toRet = max(toRet,btSelfCom(rev,rev[now][i],bf));
    }
    //printf("%d ret %d\n",now,toRet+1);
    return toRet+1;
}
...
{
    int n = getInt();
    vector<int> *rev = carray<vi>(n+1);
    for(int i = 1;i <= n;i++) {
        bff[i] = getInt();
        rev[bff[i]].push_back(i);
    }
    int ans = 0;
    // first , big loop
    for(int i = 1;i <= n;i++) {
        setArray(visited,n+1,false);
        visited[i] = true;
        ans = max(ans,findBigLoop(i,bff[i],1));
        //printf("%d = %d\n",i,ans);
    }
    // self-completed branch
    setArray(visited,n+1,false);
    ...
    for(int i = 1;i <= n;i++) {
        ...
        if(i == bff[bff[i]]) {
            // self-completed , backtrack !
            //printf("[%d]",i);
            sCom += btSelfCom(rev,i,bff[i])+btSelfCom(rev,bff[i],i);
            visited[i] = visited[bff[i]] = true;
        }
    }
    ans = max(ans,sCom);
    printf("%d\n",ans);
