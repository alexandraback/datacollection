...
int solve()
{
	...
	for (int i = 0; i < n; ++i)
	{
		...
		ans = std::max(ans, size1 + twos);
		fucked[x] = fucked[bff[x]] = std::max(fucked[x], size1);
		...
		for (int k = 0; k < n; ++k)
		{
			...
			//printf("  k = %d, size 2 = %d\n", k + 1, size2);
			if (size2 < 0) continue;
			ans = std::max(ans, size1 + size2 + twos);
			fucked[x] = fucked[bff[x]] = std::max(fucked[x], size1 + size2);
		}
	}
	int sum = 0;
	for (int i = 0; i < n; ++i) sum += fucked[i];
	sum = sum / 2 + twos;
	ans = std::max(sum, ans);
	return ans;
}
