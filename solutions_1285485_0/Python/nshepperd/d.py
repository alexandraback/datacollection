    ...
    while True:
        ...
        for (o, xy) in mirrors:
            ...
            if o == 'horiz' and (uxy - ux)*dx > 0:
                dt = min(dt, (uxy - ux)//dx)
                # features.add((uxy - ux)//dx)
            elif o == 'vert' and (uxy - uy)*dy > 0:
                dt = min(dt, (uxy - uy)//dy)
                # features.add((uxy - uy)//dy)
        if (uvx - ux)*dx > 0:
            dt = min(dt, (uvx - ux)//dx)
            # features.add((uvx - ux)//dx)
        if (uvy - uy)*dy > 0:
            dt = min(dt, (uvy - uy)//dy)
            # features.add((uvy - uy)//dy)
        # dt = min(features)
...
data = open('D-small-attempt2.in', 'r')
T = int(data.readline().strip())
for case in range(1, T+1):
    skip = False
    (H, W, D) = tuple(map(int, data.readline().split()))
    # doubling the scale to keep everything integer
    D = 2 * D
