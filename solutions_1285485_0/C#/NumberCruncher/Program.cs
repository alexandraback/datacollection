...
namespace Mirrors
{
    class Program
    {
        ...
        {
            StreamReader sr = new StreamReader("c:\\codejam\\2012\\qual\\D-small-attempt1.in");
            StreamWriter sw = new StreamWriter("c:\\codejam\\2012\\qual\\D-small-attempt1..out");
            ...
            foreach (int caseN in Enumerable.Range(1, T))
            {
                ...
                //* everything by 2
                //for each possible ray direction

                int reflections = 0;

                HashSet<Tuple<int, int>> vectors = new HashSet<Tuple<int, int>>();

                //int max = Math.Max(15 * H, 15 * W);
                int max = 100;
                for (int sry = -max; sry <= max; sry++)
                {
                    for (int srx = -max; srx <= max; srx++)
                    {
                        ...
                        //current length^2 of path we have taken
                        int cly = 0;
                        int clx = 0;
                        ...
                        while ((clx / mx ) * (clx / mx) + (cly / my) * (cly / my) <= D * D)
                        {
                            ...
                            //int newy = (ay / (my / 2) + 1 * ry) * (my / 2);
                            //int newx = (ax / (mx / 2) + 1 * rx) * (mx / 2);
