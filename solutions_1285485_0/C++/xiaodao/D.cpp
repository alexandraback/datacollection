...
struct ray{
    Po p; DB alpha;
    ray(){}
    ray(Po _p, DB _alpha):p(_p), alpha(_alpha){}
    Po p_(){
        return Po(p.x + 1e4*cos(alpha), p.y + 1e4*sin(alpha));
    }
} cur;

vector<pair<Po, bool> > L;
DB _d; Po O;
...
void init(){
    ...
    RST(Map); CLR(mirror); RD(n, m, d); d <<= 1; REP_2(i, j, n, m){
        RC(Map[i][j]); if (Map[i][j] == 'X') X0 = i, Y0 = j;
    }
...
int main(){
    ...
    Rush{
        ...
        REP(a, MAX){
            ...
            cur = ray(O, (DB) 2 * PI / MAX * a), _d = 0;

            int cnt = 0;

            while (dist(cur.p, O) + _d <= d){

                Line l = Line(cur.p, cur.p_());

                CLR(L); REP(i, SZ(mirror)) if (!isOnseg(cur.p, mirror[i])){
                    Po p = intersect(l, mirror[i]);
                    if (isOnseg(p, mirror[i]) && isOnseg(p, l)){
                        L.PB(MP(p, !sgn(mirror[i].a.x, mirror[i].b.x)));
                    }
                }

                //cout << cur.p << " " << int(cur.alpha / PI * 180 + EPS) << " " << SZ(L) << " " << L[0] << endl;

                if (cur.p != O && isOnseg(O, Seg(cur.p, cur.p_()))){
                    //cout << a << endl;
                    ++res; break;
                }

                sort(ALL(L), comp), _d += dist(cur.p, L[0].first);

                //cout << cur.p << " " << int(cur.alpha / PI * 180 + EPS) << " " << SZ(L) << " " << L[0].first << endl;

                cur.p = L[0].first;
                if (SZ(L) >= 2 && !sgn(dist(cur.p, L[0].first) , dist(cur.p, L[1].first))){
                    //cout << "?" << endl;
                    if (0 < cur.alpha && cur.alpha < PI/2){
                        cur.alpha = (0.5 * PI - cur.alpha) + PI;
                    }
                    else if (PI/2 < cur.alpha && cur.alpha < PI){
                        cur.alpha = (1.5 * PI - cur.alpha) + PI;

                    }
                    else if (PI < cur.alpha && cur.alpha < 1.5 * PI){
                        cur.alpha = (2.5 * PI - cur.alpha) + PI;
                    }
                    else {
                        cur.alpha = (3.5 * PI - cur.alpha) + PI;
                    }
                }
                else {
                    if (L[0].second) cur.alpha = PI - cur.alpha ;
                    else {
                        //cout << "!" << endl;
                        cur.alpha = - cur.alpha ;
                    }
