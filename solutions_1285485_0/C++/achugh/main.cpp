		...
		For(i,-200,200)
		{
			...
			For(j,-200,200)
			{
				...
				if(j>0)
					...
					yd=0;
				if((ll)xd*xd+(ll)yd*yd>(ll)d*d) continue;
				int hc=hcf(xd,yd);
			
				if( found.find(make_pair(xd/hc,yd/hc)) == found.end() )
				{
					found.insert( make_pair(xd/hc,yd/hc) );
					c++;
				}
			}
		}
		fout<<"Case #"<<cc<<": "<<c-1<<endl;
	}
