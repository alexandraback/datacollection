...
void solve() {
    ...
    const int rem = tot-pos;
    if (cur_size > ans_size)
        ans = cur;
    if (cur_size + tot <= ans_size)
        return;
    if (pos == tot)
        return;
    pos += 1;
    {
        // with pos
        tuple<int, int, int> val = dir[pos];
        const int a = get<0>(val); // j
        const int b = get<1>(val); // p
        const int c = get<2>(val); // s
        if (jp[a][b] >= k || js[a][c] >= k || ps[b][c] >= k) {
...
void process_testcase(const int testcase, const int should_run) {
    ...
    if (should_run) {
        tot = j*p*s;
        for (int cont=0, a = 1; a <= j; ++a)
            for (int b = 1; b <= p; ++b)
                for (int c = 1; c <= s; ++c)
                    dir[cont++] = tuple<int, int, int>(a, b, c);
        pos = 0;
        solve();
        printf("Case #%d: %d\n", testcase, (int) ans.size());
