...
template<typename T> constexpr bool odd(T a)
{ return bool(a % 2); }

template<typename T> constexpr bool even(T a)
{ return !bool(a % 2); }
...
void force(int j, int s, int k, int ti, int p)
{
    vector<tuple<int, int, int> > vt;
    for (int i1 = 0; i1 < j; i1++)
        for (int i2 = 0; i2 < p; i2++)
            for (int i3 = 0; i3 < s; i3++)
                vt.emplace_back(i1, i2, i3);
    int c = 0;
    int ii = 0;
    for (int i = 0; i < 1 << vt.size(); i++)
    {
        ...
        for (int k = 0; k < vt.size(); k++)
            if (i & 1 << k)
                ab[pair<int, int>(get<0>(vt[k]), get<1>(vt[k]))]++,
                bc[pair<int, int>(get<1>(vt[k]), get<2>(vt[k]))]++,
                ac[pair<int, int>(get<0>(vt[k]), get<2>(vt[k]))]++;
        bool ok = true;
        for (auto pp : ab)
            if (pp.second > k)
                ok = false;
    ...
    for (int i = 0; i < vt.size(); i++)
        if (ii & 1 << i)
            writeln(get<0>(vt[i]), SP, get<1>(vt[i]), SP, get<2>(vt[i]));
}

void greedy(int j, int k, int p, int ti, int s)
{
    vector<tuple<int, int, int> > vt;
    map<pair<int, int>, int> ab, ac, bc;
    int c = 1;
    for (int a = 1; a <= j; a++)
        for (int b = 1; b <= p; b++)
            ...
            {
                if (ab[pair<int, int>(a, b)] >= k
                    || bc[pair<int, int>(b, c)] >= k
                    || ac[pair<int, int>(a, c)] >= k)
                    continue;
                ab[pair<int, int>(a, b)]++;
                bc[pair<int, int>(b, c)]++;
                ac[pair<int, int>(a, c)]++;
    ...
    writeln(vt.size());
    for (auto tp : vt)
        writeln(get<0>(tp), SP, get<1>(tp), SP, get<2>(tp));
}
...
int main(int argc, char *argv[])
{
    ...
    {
        int j = getint(), p = getint(), s = getint(), k = getint();
        if (j <= k && p <= k && s <= k || j == p && p == s)
            greedy(j, k, p, ti, s);
        else
            force(j, s, k, ti, p);
    }
