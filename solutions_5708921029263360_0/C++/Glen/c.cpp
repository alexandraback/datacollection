...
int main(int argc, char** argv) {
	...
	REP (_t, T) {
		...
		REP (i, combs.size()) {
			VULL comb = combs[i];
			PII JP = MP(comb[0], comb[1]);
			PII JS = MP(comb[0], comb[2]);
			PII PS = MP(comb[1], comb[2]);
			if (!EXIST(JPs, JP)) {
				JPs[JP] = 0;
			}
			if (!EXIST(JSs, JS)) {
			...
			if (JPs[JP] < K && JSs[JS] < K && PSs[PS] < K) {
				outfits.PB(comb);
				JPs[JP]++; JSs[JS]++; PSs[PS]++;
			}
		}

		printf("Case #%llu: %lu\n", _t + 1, outfits.size());
		REP (i, outfits.size()) {
			cout << outfits[i][0] << ' ' << outfits[i][1] << ' ' << outfits[i][2] << endl;
		}
	}
}
