...
void solve(){
	...
	vector<bool> calculated(1<<n, false);
	...
	for(int i=1; i<(1<<n); i++){
		...
		vector<pair<int,pair<int,int>>> v_;

		map<pair<int,int>,int> c_JP, c_PS, c_SJ;

		for(int j=0; j<J && ok; j++)
		for(int p=0; p<P && ok; p++)
		for(int s=0; s<S && ok; s++, idx++){
			if( (i>>idx)&1 ){
				c_JP[ {j,p} ] += 1;
				c_PS[ {p,s} ] += 1;
				c_SJ[ {j,p} ] += 1;
				...
				if(c_JP[ {j,p} ] > K) ok = false;
				if(c_PS[ {p,s} ] > K) ok = false;
				if(c_SJ[ {s,j} ] > K) ok = false;
			}
		}
	...
	for(int i=0; i<v.size(); i++){
		print(v[i].first+1, v[i].second.first+1, v[i].second.second+1);
		if(i+1!=v.size()) println("");
	}
