...
int jp[256*16], ps[256*16], sj[256*16];
void solve(){
	int F[3], K;
	cin>> F[0]>> F[1]>> F[2]>> K;
	...
	For( j, F[0] ){
		For( kk, F[1] ){
			...
			For( k, F[2] ){
				...
				if( jp[( j << 4 ) | p] >= K ) continue;
				if( ps[( p << 4 ) | s] >= K ) continue;
				if( sj[( s << 4 ) | j] >= K ) continue;
	...
	*/
	...
	For( p, F[1] ){
		For( k, min( K, F[2] ) ){
			...
			last= s;
			ansCnt++;
			answer.push_back( (p<<4)|s );
		}
		s= last;
	}
	
	vector<int> answeranswer;
	ForItr( itr, answer ){
		const int p= *itr >> 4;
		const int s= *itr & 0xf;
		For( j, min( F[0], K ) ){
			...
			answeranswer.push_back( (j<<8) | *itr );
		}
	}

	cout<< answeranswer.size();
	ForItr( itr, answeranswer ){
		cout<< endl<< ( *itr >> 8 ) +1<< " "<< ( ( *itr >> 4 ) & 0xf ) +1<< " "<< ( *itr & 0xf ) +1;
	}
}
