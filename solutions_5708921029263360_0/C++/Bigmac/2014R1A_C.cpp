...
//  R1A_C
//
//  Created by Bigmac on 2016/4/11.
...
vector<string> readLineString()
{
    ...
    in_b[0] = 0x00;
    vector<string> ret;
    while(in_b[0] == ' ' || in_b[0] == 0x0d || in_b[0] == 0x0a || in_b[0] == 0x00)
    {
        if(fgets(in_b, 60000, fi) == NULL){out_stop("Failed Reading Input");}
    }
    char *in_t = strtok(in_b, " \n\t\r");
    while(in_t) {
        ret.push_back(in_t);
        in_t = strtok(NULL, " \n\t\r");
    }
...
struct lessstr : binary_function <string,string,bool> {
...
int main(int argc, char **argv)
{
	...
	for(int it_t = 1; it_t <= in_t; it_t++)
	{
        ...
        vector<int> sol_b;
        vector<int> sol_c;
        for(int it_j = 1; it_j <= j; it_j++) {
            for(int it_p = 1; it_p <= p; it_p++) {
                for(int it_s = 1; it_s <= s; it_s++) {
                    if(j_p[it_j * 11 + it_p] >=k) {
                        continue;
                    }
                    if(p_s[it_p * 11 + it_s] >=k) {
                        continue;
                    }
                    if(j_s[it_j * 11 + it_s] >=k) {
                        continue;
                    }
                    j_p[it_j * 11 + it_p]++;
                    p_s[it_p * 11 + it_s]++;
                    j_s[it_j * 11 + it_s]++;
                    sol_a.push_back(it_j);
                    sol_b.push_back(it_p);
                    sol_c.push_back(it_s);
                }
            }
        }
        int r_j_p = 0;
        ...
        for(int it_j = 1; it_j <= j; it_j++) {
                for(int it_s = 1; it_s <= s; it_s++) {
                    if(j_s[it_j * 11 + it_s] <k) {
                        r_j_s++;
                        //printf("Remain: j_s(%d %d)\n", it_j, it_s);
                    }
                }
        }
        if(r_j_p > 0 && r_p_s > 0 && r_j_s > 0) {
            //printf("ERORR in case %d!!\n", it_t);
        }
        fprintf(fo, "Case #%d: %d\n", it_t, sol_a.size());
        for(int i = 0; i < sol_a.size(); i++) {
            fprintf(fo, "%d %d %d\n", sol_a[i], sol_b[i], sol_c[i]);
        }
	}
	closeIOFile();
}