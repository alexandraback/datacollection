...
public class Main
{
    ...
    {
        int J = Integer.parseInt(input[0][0]);
        int P = Integer.parseInt(input[0][1]);
        int S = Integer.parseInt(input[0][2]);
        int K = Integer.parseInt(input[0][3]);

        ArrayList<Integer> combinaisons = new ArrayList<Integer>();
        ...
        for (int i = 1; i <= 64; i ++)
        {
            ...
            if (jacket > J || pants > P || shirt > S || jacket == 0 || pants == 0 || shirt == 0)
                continue;

            int jacketPants = jacket * 16 + pants * 4;
            int jacketShirt = jacket * 16 + shirt;
            int pantsShirt = pants * 4 + shirt;
            int incompatiblejacketPants = 0;
            int incompatiblejacketShirt = 0;
            int incompatiblepantsShirt = 0;

            for (int j = 0; j < combinaisons.size(); j ++)
            {
                if (!compatibleOutfits(jacketPants, combinaisons.get(j)))
                    incompatiblejacketPants ++;
                if (!compatibleOutfits(jacketShirt, combinaisons.get(j)))
                    incompatiblejacketShirt ++;
                if (!compatibleOutfits(pantsShirt, combinaisons.get(j)))
                    incompatiblepantsShirt ++;
            }
            if (incompatiblejacketPants < K && incompatiblejacketShirt < K && incompatiblepantsShirt < K)
                combinaisons.add(i);
        }

        outputCase(output, String.valueOf(combinaisons.size()));

        for (int i = 0; i < combinaisons.size(); i ++)
            output.println(getFromNumber(combinaisons.get(i)));

        return true;
    }

    public static boolean compatibleOutfits(int outfit1, int outfit2)
    {
        int jacket = outfit1 / 16;
        int pants = (outfit1 - jacket * 16) / 4;
        int shirt = outfit1 - jacket * 16 - pants * 4;

        int jacket2 = outfit2 / 16;
        int pants2 = (outfit2 - jacket2 * 16) / 4;
        int shirt2 = outfit2 - jacket2 * 16 - pants2 * 4;

        if (shirt + pants * 4 == shirt2 + pants2 * 4 || pants * 4 + jacket * 16 == pants2 * 4 + jacket2 * 16 || jacket * 16 + shirt == jacket2 * 16 + shirt2)
            return false;
        return true;
    }
    ...
    {
        int shirt = (number - 1) % 4 + 1;
        int pants = (number / 4 - 1) % 4 + 1;
        int jacket = number / 16;

        return String.valueOf(jacket) + " " + String.valueOf(pants) + " " + String.valueOf(shirt);
    }
