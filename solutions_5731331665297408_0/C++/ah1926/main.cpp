...
string DFS(const vector< vector<int> > &tree, const int x) {
    string code = Codes[x];
    vector<string> sonCodes;
    for (const auto &y: tree[x])
        sonCodes.push_back(DFS(tree, y));
    sort(sonCodes.begin(), sonCodes.end());
    for (const auto &c: sonCodes)
        code += c;
    return code;
}
