...
multiset<int> go;
void gao(int u , SI & A){
    if (A.find(u) != A.end()) return;
    A.insert(u);
    ECH(it , e[u]) if (!vis[*it]) gao(*it , A);
}
void dfs(int u){
    cnt++;
    vis[u] = 1;
    now += c[u].ZIP;
    if (go.find(u) != go.end()) go.erase(u);
    set<int> nowcango;
    ECH(it , go) gao(*it , nowcango);
    int hastogo = 0;
    ECH(it , e[u]) if (!vis[*it]){
        go.insert(*it);
        if (nowcango.find(*it) == nowcango.end()) hastogo = *it;
    }
...
//    }
    ECH(it , e[u]) if (!vis[*it]){
        d[*it]--;
        if (*it <= hastogo || *it == *go.begin()) dfs(*it);
    }
    ECH(it , e[u]) if (!vis[*it]){
        multiset<int> :: iterator iter = go.find(*it);
        go.erase(iter);
    }
}
void solve(){
