...
struct Case : public AbstractCase
{
    ...
    bool possible(const vector<int> &path);
    void mark_reachable(int i, vector<char> &visited);
...
Case::Case()
{
    cin >> N >> M;
    code = vector<int>(N);
    edge = vector<vector<char> >(N, vector<char>(N));
    REP(n, N) cin >> code[n];
    REP(m, M) {
        int i = 0, j  = 0;
        cin >> i >> j;
...
void Case::mark_reachable(int i, vector<char> &visited)
{
    REP(j, N) if (edge[i][j] && !visited[j]) {
        visited[j] = true;
        mark_reachable(j, visited);
    }
}

bool Case::possible(const vector<int> &path)
{
    assert(!path.empty());
    ...
    FORIT(it, stack) mark_reachable(*it, visited);
    REP(n, N) if (!visited[n]) return false;
...
{
    vector<pair<int,int> > code_index(N);
    REP(n, N) code_index[n] = make_pair(code[n], n);
    SORT(code_index);
