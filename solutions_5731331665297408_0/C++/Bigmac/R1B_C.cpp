...
void visit(int t, vector<int> candidate)
{
    char buf[100];
    sprintf(buf, "%d", zip[t]);
    out_str.append(buf);
    ...
    for(auto it = e[t].begin(); it != e[t].end(); it++)
    {
        if(color[*it] == 0)
        {
            candi_new.push_back(*it);
        }
    }
    FOR(i, 0, candi_new.size())
    {
        if(color[candi_new[i]] == 0)
        {
            clear_odr(candi_new[i]);
        }
    }
    FOR(i, 0, candi_new.size())
    {
    ...
    while(touched)
    {
        ...
        if(is_back)
        {
            ...
            {
                if(zip[candidate[i]] < min_back && color[candidate[i]] == 0)
                {
                    min_back = zip[candidate[i]];
                }
            }
        }
        int candi = -1;
        int candi_zip = 100000;
        for(auto it = e[t].begin(); it != e[t].end(); it++)
        {
            if(color[*it] == 0)
            {
                if(!is_back || zip[*it] < min_back)
                {
                    if(zip[*it] < candi_zip)
                    {
                        candi_zip = zip[*it];
        ...
        if(candi >0)
        {
            visit(candi, candi_new);
            touched = true;
        }
        else
        {
            printf("Choose to back %d\n", min_back);
            FOR(i,1,in_n)
            {
                printf("color[%d] = %d\n", zip[i], color[i]);
            }
...
int main(int argc, char **argv)
{
	...
	for(int it_t = 1; it_t <= in_t; it_t++)
	{
        ...
        REP(i, in_m)
        {
            int s = readInt();
            int t = readInt();
            if(e[s].find(t) == e[s].end())
            {
                e[s].insert(t);
            }
            if(e[t].find(s) == e[t].end())
            {
                e[t].insert(s);
            }
        }
        ...
        calc_order(s);
        visit(s, vector<int>());
		fprintf(fo, "Case #%d: %s", it_t, out_str.c_str() );
		fprintf(fo, "\n");
	}
	closeIOFile();
}