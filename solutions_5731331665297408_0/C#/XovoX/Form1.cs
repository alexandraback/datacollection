...
namespace gcj_rotate
{
    public partial class Form1 : Form
    {
      ...
      var sr = new StreamReader("A-large.in ");
        ...
        List<Point> getsubtree(List<Point> edges,int j,int i)
        {
                    var edges2=new List<Point>(edges);
                    edges2.Remove(new Point(i, j));
                    edges2.Remove(new Point(j, i));
               /*     bool added=true;
                    ...
                    while(added)
                    {
                        ...
                        {
                            if(edges[k].X!=i && edges[k].Y!=i)
                            {
                                if(vert.Contains(edges[k].X) && !vert.Contains(edges[k].Y))
                                {
                                    vert.Add(edges[k].Y);
                                    edges2.Add(edges[k]);
                                    added=true;
                                } else
                                if(vert.Contains(edges[k].Y) && !vert.Contains(edges[k].X))
                                {
                                    vert.Add(edges[k].X);
                                    edges2.Add(edges[k]);
                                    added=true;
        ...
        List<Point> alledges=new List<Point>();
        List<int>[] neigh = null;
        int solve(int except,int i)
        {
            ...
            for (int j = 0; j < neigh[i].Count; j++)
            {
                if (neigh[i][j] != except)
                {
                    results.Add(solve(i, neigh[i][j]));
                }
            }
            if (results.Count >= 2)
            {
                int max1=results.Max();
                results.Remove(max1);
                return (1 + max1 + results.Max());
            }
        ...
        {
            var sr = new StreamReader("B-small-practice.in ");
            var sw = new StreamWriter("B-small-practice.out");
            int T = Convert.ToInt32(sr.ReadLine());
            for (int t = 0; t < T; t++)
            {
                int N = Convert.ToInt32(sr.ReadLine());
                neigh = new List<int>[N+1];
                alledges.Clear();
                for (int i = 1; i <= N; i++) neigh[i] = new List<int>();
                for (int i = 0; i < N-1; i++)
                {
                    var vals = sr.ReadLine().Split(' ');
                    alledges.Add(new Point(Convert.ToInt32(vals[0]), Convert.ToInt32(vals[1])));
                    neigh[Convert.ToInt32(vals[0])].Add(Convert.ToInt32(vals[1]));
                    neigh[Convert.ToInt32(vals[1])].Add(Convert.ToInt32(vals[0]));
                }
                ...
                if (max == -1) max = N-1;
                    sw.WriteLine("Case #" + (t + 1).ToString() +": "+ (N-max).ToString());
        ...
        private void bProperShuffle_Click(object sender, EventArgs e)
        {
            var sr = new StreamReader("C-small-practice.in ");
            var sw = new StreamWriter("C-small-practice.out");
            int T = Convert.ToInt32(sr.ReadLine());
            for (int t = 0; t < T; t++)
            {
                int N = Convert.ToInt32(sr.ReadLine());
                ...
                for (int i = 0; i < N/2; i++)
                {
                    int cur=Convert.ToInt32(vals[i]);
                    if (cur <= i) bad++;
                    isbad=true;
                    for (int j = i; j < N; j++)
                        if (a[j] == cur) { a[j] = a[i]; a[i] = cur; isbad = false; }
                    if (isbad) break;
                }
                Console.WriteLine(bad);
                var rand=new Random();
                if (bad>=120/*isbad /*|| rand.Next(2) == 1*/) sw.WriteLine("Case #" + (t + 1).ToString() + ": BAD"); else sw.WriteLine("Case #" + (t + 1).ToString() + ": GOOD");
        ...
        private void bTheRepeater_Click(object sender, EventArgs e)
        {
                var sr = new StreamReader("A-large.in ");
                var sw = new StreamWriter("A-large.out");
            int T = Convert.ToInt32(sr.ReadLine());
            for (int t = 0; t < T; t++)
            {
                    ...
                    while (true)
                    {
                        ...
                        if (num1 > num2)
                        {
                            ...
                            if (sym1 == last)
                            {
                                for (int i = 0; i < N; i++)
                                {
                                    if (strings[i][j] == sym2) strings[i] = strings[i].Substring(0, j) + last + strings[i].Substring(j);
                                }
                            } else
                                if (sym2 == last)
                                {
                                    for (int i = 0; i < N; i++)
                                    {
                                        if (strings[i][j] == sym2) strings[i] = strings[i].Substring(0, j) + strings[i].Substring(j + 1);
                                    }
                                }
                                else
                                {
                        ...
                        } else
                            {
                                ...
                                if (sym2 == last)
                                {
                                    for (int i = 0; i < N; i++)
                                    {
                                        if (strings[i][j] == sym1) strings[i] = strings[i].Substring(0, j) + last + strings[i].Substring(j);
                                    }
                                }
                                else
                                    if (sym1 == last)
                                    {
                                        for (int i = 0; i < N; i++)
                                        {
                                            if (strings[i][j] == sym1) strings[i] = strings[i].Substring(0, j) + strings[i].Substring(j + 1);
                                        }
                                    }
                                    else
                                    {
                ...
                if (stop) sw.WriteLine("Case #" + (t + 1).ToString() + ": Fegla Won"); else sw.WriteLine("Case #" + (t + 1).ToString() + ": "+res.ToString());
        ...
        private void bNewLotteryGame_Click(object sender, EventArgs e)
        {
            ...
            {
                var vals = sr.ReadLine().Split(' ');
                Decimal A = Convert.ToDecimal(vals[0]);
                Decimal B = Convert.ToDecimal(vals[1]);
                Decimal K = Convert.ToDecimal(vals[2]);
                decimal res=0;
                for (decimal a = 0; a < A; a++)
                    for (decimal b = 0; b < B; b++)
                    {
            ...
            //            if ((a & b) < K) res++;
             ...
             sw.WriteLine("Case #" + (t + 1).ToString() + ": " + res.ToString());
        ...
        private void bTheBored_Click(object sender, EventArgs e)
        {
            ...
            for (int t = 0; t < T; t++)
            {
                ...
                zips.Add(int.MaxValue);
                for(int i=0;i<N;i++) zips.Add(Convert.ToInt32(sr.ReadLine()));
                List<int>[] fl = new List<int>[N+1];
                for (int i = 1; i <= N; i++) fl[i] = new List<int>();
                for (int i = 0; i < M; i++)
                {
                    var vals2 = sr.ReadLine().Split(' ');
                    int v1 = Convert.ToInt32(vals2[0]);
                    int v2 = Convert.ToInt32(vals2[1]);
                    fl[v1].Add(v2);
                    fl[v2].Add(v1);
                }
                int minzip=zips.Min();
                string res = minzip.ToString();
                List<int> path = new List<int>();
                int cur = zips.IndexOf(minzip);
                int l = 1;
                int prev=-1;
                while (l < N)
                {
                    ...
                    minzip = int.MaxValue;
                    int minv = -1;
                    for (int i = 0; i < fl[cur].Count; i++)
                    {
                        if (!path.Contains(fl[cur][i]))
                        {
                            if (minv == -1 || zips[fl[cur][i]] < minzip)
                            {
                                minv = fl[cur][i];
                                minzip = zips[fl[cur][i]];
                            }
                        }
                    }
                    if (minv == -1)
                ...
                sw.WriteLine("Case #" + (t + 1).ToString() + ": " + res);
